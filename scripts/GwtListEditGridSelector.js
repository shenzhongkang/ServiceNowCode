...
var GwtListEditGridSelector = Class.create(GwtCellSelector, {
  ...
  selectVertical: function (e, direction, row, col) {
    ...
    if (direction == 'up') {
      if (this.selectCount > 0) this._cleanCellSelection(selectedRow, col);
      this.selectCount--;
    } else {
      if (this.selectCount < 0) this._cleanCellSelection(selectedRow, col);
      this.selectCount++;
  ...
  handleOnSelect: function (selectedCells) {
    ...
    for (var key in selectedCells) {
      if (!this.originalCell) this.originalCell = this._parseKey(key);
  ...
  handleBeforeSelect: function (e) {
    ...
    if (!e.shiftKey && !e.ctrlKey && !e.metaKey) {
      this.selectorCol = 0;
      if (!this.selectedCells[selectedCellKey]) this._resetCellSelection();
      return false;
    }
    ...
    if (e.shiftKey && this.selectedCells[selectedCellKey]) return;
    this._clearCellSelection();
    return true;
  },
  ...
  _addToSelection: function () {
    if (this.rowFrom <= 0 || this.rowTo <= 0) return;
  ...
  getSelectedRows: function () {
    ...
    for (var key in this.selectedCells) {
      var recPos =
        window.g_detail_row && window.g_detail_row_present
          ? this._parseKey(key).row * 2 - 1
          : this._parseKey(key).row;
      if (result.indexOf(recPos) < 0) result.push(recPos);
    }
    return result;
  },
