...
(function ($, undefined) {
  ...
  function syncCssClasses(dest, src, adapter) {
    ...
    if (replacements.indexOf('hidden-select2') > -1)
      dest.attr('aria-hidden', 'true');
  }
  ...
  AbstractSelect2 = clazz(Object, {
    ...
    positionDropdown: function () {
      ...
      if (above) {
        ...
        var searchBarTopOffset = 5;
        if (css.top < searchBarTopOffset) {
          var $dropdownList = $dropdown.find('.select2-results');
          var newDropdownListHeight =
            $dropdownList.height() + css.top - searchBarTopOffset;
          $dropdownList.css('max-height', newDropdownListHeight + 'px');
          css.top = searchBarTopOffset;
        }
        css.bottom = 'auto';
  ...
  SingleSelect2 = clazz(AbstractSelect2, {
    createContainer: function () {
      var labelAriaLabel = this.opts.element.attr('aria-label');
      var container = $(document.createElement('div'))
        .attr({ class: 'select2-container' })
        .html(
          [
            "<input class='select2-focusser select2-offscreen' type='text' role='combobox' aria-expanded='false' />",
            "<a href='javascript:void(0)' class='select2-choice' tabindex='-1' aria-hidden='true' aria-expanded='false' role='button'>",
            "   <span class='select2-chosen'>&#160;</span><abbr class='select2-search-choice-close' tabindex='0' " +
              (this.opts.clearAriaLabel
                ? "aria-label='" + this.opts.clearAriaLabel + "'"
                : '') +
              '></abbr>',
            ...
            "       <label for='' class='select2-offscreen'",
            labelAriaLabel ? "aria-label='" + labelAriaLabel + "'" : '',
            '></label>',
    ...
    opening: function () {
      ...
      this.container.find('a')[0].ariaExpanded = true;
      this.container.find('input')[0].ariaExpanded = true;
    },
    close: function () {
      if (!this.opened()) return;
      ...
      this.container.find('a')[0].ariaExpanded = false;
      this.container.find('input')[0].ariaExpanded = false;
    },
    focus: function () {
      if (this.opened()) {
        this.close();
    ...
    initContainer: function () {
      ...
      selection
        .on('mousedown touchstart', 'abbr', clearValue)
        .on('keydown', 'abbr', function (e) {
          switch (e.which) {
            case KEY.ENTER:
            case KEY.SPACE:
              clearValue(e);
