...
angular
  ...
  .directive('snUserAvatar', function (getTemplateUrl) {
    'use strict';
    return {
      restrict: 'E',
      ...
      link: function (scope, element) {
        ...
        scope.directConversationButton =
          scope.directConversationButton !== 'false' &&
          scope.directConversationButton !== false;
        scope.template =
          '<sn-user-profile tabindex="-1" id="sn-bootstrap-popover" profile="evaluatedProfile" show-direct-message-prompt="::directConversationButton" class="avatar-popover avatar-popover-padding"></sn-user-profile>';
        scope.ariaRole = scope.disablePopover ? 'presentation' : 'button';
      },
      controller: function ($scope) {
        $scope.getBackgroundStyle = function () {
          if (
            ($scope.avatarUrl && $scope.avatarUrl !== '') ||
            ($scope.evaluatedProfile && $scope.evaluatedProfile.avatar !== '')
          )
            return {
              'background-image':
                'url(' +
                ($scope.avatarUrl || $scope.evaluatedProfile.avatar) +
                ')',
            };
