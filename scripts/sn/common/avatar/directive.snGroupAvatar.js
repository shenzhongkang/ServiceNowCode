...
angular
  ...
  .directive(
    'snGroupAvatar',
    function ($http, $compile, getTemplateUrl, avatarProfilePersister) {
      'use strict';
      return {
        restrict: 'E',
        ...
        controller: function ($scope, liveProfileID) {
          ...
          $scope.avatarType = function () {
            ...
            if ($scope.users.length === 2) result.push('sn-avatar_duo');
            if ($scope.users.length === 3) result.push('sn-avatar_trio');
            if ($scope.users.length >= 4) result.push('sn-avatar_quad');
            return result;
          };
