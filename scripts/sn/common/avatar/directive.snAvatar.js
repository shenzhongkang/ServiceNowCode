            ...
            return function (scope, element, attrs, controller, transcludeFn) {
              ...
              if (scope.enableContextMenu !== false) {
                ...
                if (
                  ...
                  scope.users.length === 1 &&
                  scope.users[0] &&
                  (scope.users[0].userID || scope.users[0].sys_id)
                ) {
                  scope.contextOptions = [
                    [
                      "Open user's profile",
                      function () {
                        if (scope.users && scope.users.length > 0) {
                          window.open(
                            '/nav_to.do?uri=' +
                              encodeURIComponent(
                                'sys_user.do?sys_id=' + scope.users[0].userID
                              ),
                            '_blank'
                          );
                        }
                  ...
                  if (
                    (gUser &&
                      ...
                      scope.users[0].userID !== gUser.userID) ||
                    (scope.liveProfileID &&
                      scope.users[0] &&
                      scope.users[0].sysID !== scope.liveProfileID)
                  ) {
          ...
          controller: function ($scope, liveProfileID) {
            ...
            $scope.$watch('primary', function (newValue, oldValue) {
              if ($scope.primary && newValue !== oldValue) {
                if (!firstBuildAvatar) buildAvatar();
            ...
            $scope.$watch('members', function () {
              if ($scope.members && !firstBuildAvatar) buildAvatar();
            });
            ...
            function buildAvatar() {
              if (firstBuildAvatar) firstBuildAvatar = false;
              ...
              $scope.isDocument =
                ...
                $scope.primary.table !== 'chat_queue_entry';
            ...
            function buildCompositeAvatar(members) {
              var currentUser = window.NOW.user
                ...
                : window.NOW.user_id;
              var users = angular.isArray(members)
                ? members.slice()
                : [members];
