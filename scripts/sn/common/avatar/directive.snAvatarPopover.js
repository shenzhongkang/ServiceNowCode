      ...
      return {
        ...
        compile: function (tElement) {
          ...
          return function (scope, element, attrs, controller, transcludeFn) {
            if (scope.directconversation) {
              ...
              else scope.directconversation = false;
              scope.showdirectconversation = !scope.directconversation;
            } else {
              scope.showdirectconversation = true;
            }
            if (
              $injector.has('inSupportClient') &&
              $injector.get('inSupportClient')
            )
              scope.showdirectconversation = false;
            if (scope.primaryNonAssign) {
              scope.primary = angular.extend(
                {},
                scope.primary,
                scope.primaryNonAssign
              );
              if (scope.users && scope.users[0]) scope.users[0] = scope.primary;
            }
            function recompile() {
              if (scope.primaryNonAssign) {
                scope.primary = angular.extend(
              ...
              if (scope.enableTooltip) {
                element
                  ...
                  .attr('data-original-title', scope.users[0].name)
                  .tooltip('fixTitle');
                if (element.hideFix) element.hideFix();
              }
            }
        ...
        controller: function (
          ...
          $scope.$watch('members', function (newVal, oldVal) {
            ...
            if ($scope.members) buildAvatar();
          });
          $scope.noPopover = function () {
            $scope.popoverCursor =
              $scope.nopopover || ($scope.members && $scope.members.length > 2)
                ? 'default'
                : 'pointer';
            return (
              $scope.nopopover || ($scope.members && $scope.members.length > 2)
          ...
          $scope.avatarType = function () {
            var result = [];
            if ($scope.groupAvatar || !$scope.users) return result;
            if ($scope.users.length > 1) result.push('group');
            if ($scope.users.length === 2) result.push('avatar-duo');
            if ($scope.users.length === 3) result.push('avatar-trio');
            if ($scope.users.length >= 4) result.push('avatar-quad');
            return result;
          };
          $scope.getBackgroundStyle = function (user) {
            var avatar = user ? user.avatar : '';
            if ($scope.groupAvatar) avatar = $scope.groupAvatar;
            if (avatar && avatar !== '')
          ...
          function buildAvatar() {
            if (typeof $scope.primary === 'string') {
              $http
                .get('/api/now/live/profiles/sys_user.' + $scope.primary)
            ...
            if (
              $scope.primary &&
              (!$scope.members || $scope.members.length <= 0) &&
              ($scope.primary.avatar || $scope.primary.initials) &&
              $scope.isDocument
            ) {
              $scope.users = [$scope.primary];
            ...
            $scope.presenceEnabled =
              $scope.showPresence &&
              !$scope.isGroup &&
              $scope.users.length === 1;
          }
          function buildCompositeAvatar(members) {
            var currentUser = window.NOW.user
              ? window.NOW.user.userID
              : window.NOW.user_id;
            var users = angular.isArray(members) ? members.slice() : [members];
            users = users.sort(function (a, b) {
              var aID = a.userID || a.document;
              var bID = b.userID || b.document;
              if (a.table === 'chat_queue_entry') return 1;
              if (aID === currentUser) return 1;
              else if (bID === currentUser) return -1;
              return 0;
            });
          ...
          $scope.loadFullProfile = function () {
            if (
              $scope.primary &&
              !$scope.primary.sys_id &&
              !avatarProfilePersister.getAvatar($scope.primary.userID)
            ) {
              $http
                .get('/api/now/live/profiles/' + $scope.primary.userID)
                .then(function (response) {
                  try {
                    angular.extend($scope.primary, response.data.result);
                    avatarProfilePersister.setAvatar(
                      $scope.primary.userID,
                      $scope.primary
                    );
                  } catch (e) {}
                });
