...
angular
  ...
  .factory('snNotificationWrapper', function ($window, $timeout) {
    'use strict';
    ...
    return function NotificationWrapper(title, options) {
      ...
      options.onClick = function () {
        if (angular.isFunction($window.focus)) $window.focus();
        if (typeof optionsOnClick === 'function') optionsOnClick();
      };
      var result = {};
      options = angular.extend(defaults, options);
      var previousOnClose = options.onClose;
      options.onClose = function () {
        if (angular.isFunction(result.onclose)) result.onclose();
        if (angular.isFunction(previousOnClose)) previousOnClose();
      };
