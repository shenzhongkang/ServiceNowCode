...
angular
  ...
  .factory('snNotifier', function ($window, snNotificationWrapper) {
    'use strict';
      ...
      function notify(title, options) {
        ...
        for (var i = 0, len = settings.notifyMethods.length; i < len; i++)
          if (typeof settings.notifyMethods[i] == 'string') {
            if (methods[settings.notifyMethods[i]](title, options)) break;
          } else {
            if (settings.notifyMethods[i](title, options)) break;
          }
      }
