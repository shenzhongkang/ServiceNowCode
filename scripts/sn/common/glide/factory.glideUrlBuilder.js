...
angular.module('sn.common.glide').factory('glideUrlBuilder', [
  '$window',
  function ($window) {
    'use strict';
      ...
      var objDef = {
        ...
        setFromString: function (href) {
          ...
          while (i--) {
            ...
            this.params[hashes[i].substring(0, pos)] = hashes[i].substring(
              ++pos
            );
          }
        },
        ...
        addEncodedString: function (s) {
          if (!s) return;
          if (s.substr(0, 1) != '&') this.encodedString += '&';
        ...
        _getParamsForURL: function (params) {
          ...
          for (var n in params) {
            var p = params[n] || '';
            url +=
              '&' + n + '=' + (this.encode ? encodeURIComponent(p + '') : p);
          }
          return url;
        },
