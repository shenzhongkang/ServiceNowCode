      ...
      Stream.prototype = {
        ...
        tap: function () {
          if (!this.inflightRequest) {
            this._stopPolling();
            this._startPolling();
          } else this.requestImmediateUpdate = true;
        },
        ...
        insertEntries: function (entries, table, sys_id, mentionIDMap) {
          ...
          for (var i = 0; i < entries.length; i++) {
            ...
            entries[i].text = this.addCarriageReturns(
              this.expandMentions(entryText, mentionIDMap)
            );
        ...
        _startPolling: function () {
          ...
          function establishNextRequest(intervalOverride) {
            ...
            intervalOverride =
              parseFloat(intervalOverride) >= 0 ? intervalOverride : interval;
        ...
        _successCallback: function (response) {
          ...
          if (response.entries && response.entries.length) {
            ...
            if (response.entries.length > 0) {
              ...
              this.lastTimestamp =
                response.sys_timestamp || response.entries[0].sys_timestamp;
            }
          }
          ...
          for (var i = 0; i < entries.length; i++) {
            if (entries[i].sys_timestamp == this.lastTimestamp) {
              if (this.lastEntry) {
                if (
                  !angular.equals(
                    this._makeComparable(entries[i]),
                    this._makeComparable(this.lastEntry)
                  )
                )
                  continue;
        ...
        _makeComparable: function (entry) {
          ...
          if (copy.attachment) delete copy.attachment.state;
          return copy;
        },
        _getAction: function () {
        ...
        _getInterval: function () {
          ...
          else return nowStreamTimerInterval;
        },
      };
      return {
