...
angular
  ...
  .directive('contenteditable', function ($timeout, $sanitize) {
    return {
      require: 'ngModel',
      link: function (scope, elem, attrs, ctrl) {
        ...
        if (!scope.usenewline)
          elem.keypress(function (event) {
            if (event.which == '13') {
              if (scope.entercallback) scope.entercallback(elem);
        ...
        function processNodes(nodes) {
          ...
          for (var i = 0; i < nodes.length; i++) {
            ...
            switch (node.nodeName) {
              ...
              case nodeDIV:
                ...
                if (
                  ...
                  node.childNodes[0].nodeName == nodeBR
                )
                  follow = false;
                break;
              case nodeBR:
            ...
            if (follow) val += processNodes(node.childNodes);
          }
          return val;
        }
        function readHtml() {
        ...
        function writeHtml() {
          ...
          val = val.replace(/  /gi, ' &nbsp;');
          try {
            if (attrs.contenteditableEscapeHtml == 'true') val = $sanitize(val);
          } catch (err) {
            var replacement = {
              '&': '&amp;',
              '<': '&lt;',
        ...
        elem.bind('focus', function () {
          if (
            scope[attrs.tracker] &&
            scope[attrs.tracker]['isDefault_' + attrs.trackeratt]
          )
            $timeout(function () {
              selectText(elem[0]);
            });
        ...
        elem.bind('blur', function () {
          scope.$apply(function () {
            ...
            if (elem.original != ctrl.$viewValue && changehandler) {
              if (
                scope[attrs.tracker] &&
                typeof scope[attrs.tracker]['isDefault_' + attrs.trackeratt] !=
                  'undefined'
              )
        ...
        scope.$watch(
          ...
          function (newValue, oldValue) {
            if (newValue.val != oldValue.val) processPlaceholder();
          },
          true
        );
        writeHtml();
