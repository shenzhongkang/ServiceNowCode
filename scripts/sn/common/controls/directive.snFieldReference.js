...
angular
  ...
  .directive('snFieldReference', function ($timeout, $http, nowServer) {
    'use strict';
    return {
      restrict: 'E',
      replace: true,
      ...
      template:
        '<select ng-disabled="snDisabled" name="{{field.name}}" style="min-width: 150px;" ng-model="fieldValue" ng-model-options="{getterSetter: true}" ng-options="choice.name as choice.label for choice in field.choices"></select>',
