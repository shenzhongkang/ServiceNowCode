        ...
        link: function (scope, element) {
          function isValidImage(file) {
            ...
            if (file.type.indexOf('tiff') > 0) {
              $alert(
                i18n.getMessage(
                  'Please select a common image format such as gif, jpeg or png'
                )
              );
              return false;
            }
          ...
          scope.onFileSelect = function ($files) {
            ...
            snAttachmentHandler
              .create(scope.tableName, scope.sysId)
              .uploadAttachment(file, uploadParams)
              .then(function (response) {
                $timeout(function () {
                  scope.uploading = false;
                });
                if (scope.onUpload) scope.onUpload(response);
                $rootScope.$broadcast(
                  'snImageUploader:complete',
                  scope.sysId,
                  response
                );
              });
          ...
          scope.activateUpload = function ($event) {
            if (scope.readOnly !== 'true') scope.openFileSelector($event);
            else scope.showUpload = !scope.showUpload;
          };
          scope.deleteAttachment = function () {
            var sys_id = scope.src.split('.')[0];
            snAttachmentHandler.deleteAttachment(sys_id).then(function () {
              scope.src = null;
              if (scope.onDelete) scope.onDelete();
              $rootScope.$broadcast(
                'snImageUploader:delete',
                scope.sysId,
                sys_id
              );
            });
