  ...
  .directive(
    ...
    function (
      ...
      return {
        ...
        template:
          '<input type="text" name="{{field.name}}" ng-disabled="snDisabled" style="min-width: 150px;" ng-model="field.displayValue" ' +
        ...
        link: function (scope, element, attrs, ctrl) {
          ...
          var fieldAttributes = {};
          if (
            angular.isDefined(scope.field) &&
            angular.isDefined(scope.field.attributes) &&
            typeof scope.ed.attributes == 'undefined'
          )
            if (Array.isArray(scope.field.attributes))
              fieldAttributes = scope.field.attributes;
            else fieldAttributes = parseAttributes(scope.field.attributes);
          else fieldAttributes = parseAttributes(scope.ed.attributes);
          if (
            !angular.isDefined(scope.additionalDisplayColumns) &&
          ...
          if (
            scope.ed.dependent_field &&
            scope.ed.dependent_value &&
            scope.ed.dependent_table &&
            !(
              scope.ed.reference == 'sys_user' &&
              scope.ed.dependent_field.indexOf('group') != -1
            )
          ) {
            if (atfQualifier) atfQualifier += '^';
            atfQualifier +=
              scope.ed.dependent_field + '=' + scope.ed.dependent_value;
          }
          ...
          var config = {
            ...
            clearAriaLabel: scope.snOptions
              ? scope.snOptions.clearAriaLabel
              : '',
            id: function (item) {
              return item.sys_id;
            },
            ...
            ajax: {
              ...
              data: function (filterText, page) {
                ...
                var params = {
                  ...
                  sysparm_target_field:
                    scope.ed.dependent_field || scope.ed.name,
                  ...
                  p:
                    scope.ed.reference + ';q:' + q + ';r:' + scope.ed.qualifier,
                };
                if (scope.domain) {
                ...
                if (
                  angular.isDefined(scope.field) &&
                  scope.field['_cat_variable'] === true
                ) {
          ...
          function parseAttributes(strAttributes) {
            ...
            for (var i = 0; i < attributeArray.length; i++) {
              if (attributeArray[i].length > 0) {
                ...
                attributeObj[attribute[0]] =
                  attribute.length > 1 ? attribute[1] : '';
              }
            }
            return attributeObj;
          }
          ...
          function render() {
            $timeout(function () {
              ...
              select2.bind('select2-open', function () {
                scope.$apply(function () {
                  if (scope.snOnOpen) scope.snOnOpen();
                });
              });
          ...
          function select2Change(e) {
            ...
            function triggerSnOnChange() {
              if (scope.snOnChange) scope.snOnChange(e);
            }
          }
          ...
          function getDisplayValues(selectedItem) {
            ...
            if (selectedItem && selectedItem.sys_id) {
              ...
              else if (selectedItem.name) current = selectedItem.name;
              else if (selectedItem.title) current = selectedItem.title;
          ...
          function setValueATF(e) {
            ...
            if (newValue) {
              ...
              if (params.qualifier) params.qualifier += '^';
              else params.qualifier = '';
        ...
        controller: function ($scope, $rootScope) {
          $scope.pageSize = 20;
          if ($scope.snPageSize) $scope.pageSize = parseInt($scope.snPageSize);
