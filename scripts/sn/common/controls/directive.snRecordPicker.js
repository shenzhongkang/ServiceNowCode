    ...
    function (
      ...
      function cleanLabel(val) {
        if (typeof val == 'object')
          return typeof val.display_value == 'string'
            ? val.display_value.trim()
            : '';
      ...
      return {
        ...
        template:
          '<input type="text" ng-disabled="snDisabled" style="min-width: 150px;" name="{{field.name}}" ng-model="field.value"' +
        ...
        controller: function ($scope) {
          if (!angular.isNumber($scope.pageSize)) $scope.pageSize = 20;
          ...
          this.filterResults = function (data, page) {
            return {
              ...
              more:
                page * $scope.pageSize <
                parseInt(data.headers('x-total-count'), 10),
            };
          };
        },
        link: function (scope, element, attrs, ctrl) {
          ...
          var select2Helpers = {
              ...
              if (displayFields.length > 1) {
                ...
                for (var i = 1; i < displayFields.length; i++)
                  markup +=
                    "<div style='width: " +
                    width +
                    "%;' class='select2-additional-display-field'>" +
                    escapeHtml(cleanLabel(displayFields[i])) +
                    '</div>';
                markup += '</div>';
                return markup;
              }
            ...
            search: function (queryParams) {
              var url =
                ...
                scope.table +
                '?' +
                urlTools.encodeURIParameters(queryParams.data);
          ...
          var config = {
            ...
            formatSearching: '',
            allowClear:
              scope.options && typeof scope.options.allowClear !== 'undefined'
                ? scope.options.allowClear
                : true,
            id: function (item) {
            ...
            ajax: {
              ...
              data: function (filterText, page) {
                ...
                if (scope.displayField) fields.push(scope.displayField);
                if (scope.displayFields) fields.push(scope.displayFields);
                var params = {
          ...
          function buildQuery(filterText, searchFields, defaultQuery) {
            ...
            if (scope.startswith) operator = 'STARTSWITH';
            if (filterText.startsWith('*')) {
              filterText = filterText.substring(1);
              operator = 'CONTAINS';
            }
            if (defaultQuery) queryParts.push(defaultQuery);
          ...
          if (scope.field.displayValue && scope.multiple)
            scope.field.displayValue = scope.field.displayValue.replace(
              /,/g,
              TOKENIZER
            );
          ...
          function onSelecting(e) {
            ...
            if (scope.multiple && selectedItem[scope.valueField] != '') {
              var values = !scope.field.value
                ? []
                : scope.field.value.split(',');
              var displayValues = !scope.field.displayValue
                ? []
                : scope.field.displayValue.split(TOKENIZER);
          ...
          function getDisplayValue(selectedItem) {
            ...
            if (selectedItem) {
              if (
                scope.displayField &&
                angular.isDefined(selectedItem[scope.displayField])
              )
          ...
          scope.$watch('field.value', function (newValue) {
            ...
            if (angular.isDefined(newValue) && $select) {
              ...
              else $select.select2('val', newValue).select2('close');
            }
          });
