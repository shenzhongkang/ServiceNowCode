...
angular
  .module('sn.common.datetime')
  .factory('timeAgoTimer', function ($interval, $rootScope, DATE_GRANULARITY) {
    'use strict';
    var digestInterval;
    return function (displayGranularityType) {
      displayGranularityType =
        typeof displayGranularityType !== 'undefined'
          ? displayGranularityType
          : DATE_GRANULARITY.DATETIME;
      if (
        !digestInterval &&
        displayGranularityType == DATE_GRANULARITY.DATETIME
      )
...
angular
  .module('sn.common.datetime')
  .factory('timeAgo', function (timeAgoSettings, DATE_GRANULARITY) {
    var service = {
      ...
      toWords: function toWords(distanceMillis, messageGranularity) {
        ...
        var ago = $l.ago;
        if (
          (seconds < 45 && messageGranularity == DATE_GRANULARITY.DATETIME) ||
        ...
        var words =
          (distanceMillis <= 0 && wantDateTime && substitute($l.justNow, 0)) ||
          (distanceMillis <= 0 && wantDate && substitute($l.today, 0)) ||
          (seconds < 45 &&
            (distanceMillis >= 0 || !service.settings.allowFuture) &&
            wantDateTime &&
            substitute($l.justNow, Math.round(seconds))) ||
          (seconds < 45 &&
            wantDateTime &&
      ...
      parse: function (iso8601) {
        if (angular.isNumber(iso8601)) return new Date(parseInt(iso8601, 10));
        var s = iso8601.trim();
        s = s.replace(/\.\d+/, '');
        s = s.replace(/-/, '/').replace(/-/, '/');
        ...
        link: function (scope) {
          ...
          function setTitleTime(date) {
            ...
            if (
              ...
              g_user_date_time_format.indexOf('z') == -1
            )
              scope.titleTime = dateUtils.formatDate(
                new Date(t),
                g_user_date_time_format
              );
            else scope.titleTime = new Date(t).toLocaleString();
          }
        },
      };
    }
