...
angular
  ...
  .service(
    'snTabActivity',
    function ($window, $timeout, $rootElement, $document) {
      'use strict';
      var activeEvents = [
          'keydown',
          'DOMMouseScroll',
      ...
      function createGUID(l) {
        ...
        while (strResult.length < l)
          strResult += (
            ((1 + Math.random() + new Date().getTime()) * 0x10000) |
            0
          )
            .toString(16)
            .substring(1);
      ...
      if (detectedApi)
        $document.on(detectedApi.eventName, function () {
          if (!$document[0][detectedApi.propertyName]) {
            ...
            if (!idleTimeout && !idleTime) waitForIdle(0);
            isVisible = false;
          }
        });
      angular.element($window).on({
        mouseleave: function (e) {
          var destination = angular.isUndefined(e.toElement)
            ? e.relatedTarget
            : e.toElement;
        ...
        storage: function (e) {
          if (e.originalEvent.key !== storageKey) return;
      ...
      function waitForIdle(index, delayOffset) {
        ...
        if (index >= numCallbacks) return;
        if (idleTimeout) $timeout.cancel(idleTimeout);
        idleTimeout = $timeout(
          function () {
      ...
      function setActive() {
        ...
        activeCallbacks.some(function (callback) {
          if (callback.delay <= idleTime) callback.cb();
          else return true;
        });
        idleTime = 0;
        makePrimary();
      ...
      function pageIdleHandler() {
        ...
        if (hasActed) {
          hasActed = false;
          if (pageIdleTimeout) $timeout.cancel(pageIdleTimeout);
          pageIdleTimeout = $timeout(pageIdleHandler, minDelay, false);
          listenForActivity();
          return;
        }
      ...
      function makePrimary(initial) {
        ...
        if (idleTime && $document[0].hasFocus()) setActive();
      }
      ...
      function registerCallback(event, callback, scope) {
        var cbObject = angular.isObject(callback)
          ? callback
          : { delay: defaultIdle, cb: callback };
        if (callbacks[event]) {
        ...
        function destroyCallback() {
          if (callbacks[event]) {
            ...
            if (pos !== -1) callbacks[event].splice(pos, 1);
          }
        }
        if (scope)
      ...
      function registerIdleCallback(options, onIdle, onReturn, scope) {
        ...
        function destroyAll() {
          if (angular.isFunction(onIdleDestroy)) onIdleDestroy();
          if (angular.isFunction(onReturnDestroy)) onReturnDestroy();
        }
        if (scope)
          scope.$on('$destroy', function () {
            destroyAll();
