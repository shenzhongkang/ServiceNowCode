...
angular
  ...
  .constant('angularProcessorUrl', 'angular.do?sysparm_type=');
angular
  .module('sn.common.util')
  .factory('urlTools', function (getTemplateUrl, angularProcessorUrl) {
    'use strict';
    ...
    function parseQueryString(qs) {
      ...
      if (a && a[0].indexOf('http') != -1) a[0] = a[0].split('?')[1];
      var b = {};
    ...
    function isUrlExternal(url) {
      ...
      if (
        matchResult &&
        matchResult[hostnameGroupIndex] &&
        matchResult[hostnameGroupIndex].length > 0
      )
        return true;
      return false;
    }
