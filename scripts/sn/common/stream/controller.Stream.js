...
angular
  ...
  .controller(
    'Stream',
    function ($rootScope, $scope, snRecordWatcher, $timeout) {
      ...
      $scope.controls = {
        ...
        openRecord: function () {
          ...
          if (NOW.linkTarget == 'form_pane') {
            ...
            window.parent.CustomEvent.fireTop('glide:nav_open_url', {
              url: url,
              openInForm: true,
            });
            return;
      ...
      $scope.showListStream = function () {
        ...
        angular
          .element('div.list-stream-record')
          .velocity('snTransition.streamSlideRight', {
            duration: 400,
          });
        angular
          .element('[streamType="list"]')
          .velocity('snTransition.slideIn', {
            duration: 400,
        ...
        function (newValue, oldValue) {
          if (newValue == 1) {
            angular
              .element('div.list-stream-record')
              .delay(100)
              .velocity('snTransition.streamSlideLeft', {
                begin: function (element) {
                  angular.element(element).css({ visibility: 'visible' });
                  angular
                    .element('.list-stream-record-header')
