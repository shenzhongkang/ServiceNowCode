...
angular
  ...
  .directive('snStream', function (getTemplateUrl, $http, $sce, $sanitize) {
    'use strict';
    return {
      restrict: 'E',
      replace: true,
      ...
      link: function (scope, element) {
        element.on('click', '.at-mention', function (evt) {
          var userID = angular
            .element(evt.target)
            .attr('class')
            .substring('at-mention at-mention-user-'.length);
          $http({
            ...
            function () {
              $http({
                ...
                tempProfile.userID = tempProfile.sys_id =
                  response.data.result.document;
                scope.mentionPopoverProfile = tempProfile;
                scope.mentionPopoverProfile.sysID =
