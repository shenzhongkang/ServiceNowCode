      ...
      return {
        ...
        link: function (scope, element) {
            ...
            if (scope.showDirectMessagePrompt) {
              ...
              return !(
                ...
                (scope.profile.document &&
                  activeUserID === scope.profile.document)
              );
            } else {
              return false;
            }
          ...
          $timeout(
            function () {
              element
                .find('#direct-message-popover-trigger')
                .on('click', scope.openDirectMessageConversation);
            },
            0,
            false
          );
        ...
        controller: function ($scope, snConnectService) {
          if (
            ...
            avatarProfilePersister.getAvatar($scope.profile.userID)
          ) {
            $scope.profile = avatarProfilePersister.getAvatar(
              $scope.profile.userID
            );
            $scope.$emit('sn-user-profile.ready');
          } else {
            $http
              .get('/api/now/live/profiles/sys_user.' + $scope.profile.userID)
          ...
          $http
            .get(
              '/api/now/live/profiles/' +
                $scope.profile.userID +
                '/has_connect_roles'
            )
            .then(function (response) {
              $scope.profile.hasConnectRoles =
                response.data.result.hasConnectRoles;
            });
