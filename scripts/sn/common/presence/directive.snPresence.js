...
angular
  ...
  .directive('snPresence', function (snPresence, $rootScope, $timeout, i18n) {
    'use strict';
    ...
    return {
      ...
      link: function (scope, element) {
        if (scope.profile) {
          ...
          if (scope.profile.isAccessible) scope.profile.tabIndex = 0;
        }
        if (!element.hasClass('presence')) element.addClass('presence');
        function updatePresence() {
          ...
          if (presences[id]) {
            ...
            if (status === 'maybe offline' || status === 'probably offline') {
              element.removeClass(
                'presence-online presence-offline presence-away'
              );
              element.addClass('presence-away');
            } else if (
              status == 'offline' &&
              !element.hasClass('presence-offline')
            ) {
              ...
              (status == 'online' ||
                status == 'entered' ||
                status == 'viewing') &&
              !element.hasClass('presence-online')
            ) {
            ...
            if (scope.profile)
              angular
                .element('div[user-avatar-id="' + id + '"]')
                .attr(
                  'aria-label',
                  scope.profile.userName + ' ' + presenceStatus[status]
                );
            else
              angular
                .element('div[user-avatar-id="' + id + '"]')
                .attr(
                  'aria-label',
                  scope.displayName + ' ' + presenceStatus[status]
                );
          } else {
