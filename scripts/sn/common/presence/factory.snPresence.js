    ...
    function (
        ...
        if (!initialized) {
          ...
          if (!primary) {
            ...
            presenceArray = getLocalPresence(
              $window.localStorage.getItem('snPresence')
            );
            if (presenceArray) $timeout(schedulePresence, 100);
            else updatePresence();
          }
        }
      ...
      function initRootScopes() {
        if ($window.NOW.presence_scopes) {
          ...
          if (ps.indexOf($rootScope) == -1) ps.push($rootScope);
        } else {
      ...
      function setPresence(data, st) {
        ...
        if (rt > 500) console.log('snPresence response time ' + rt + 'ms');
      ...
      function updatePresence(numAttempts) {
        ...
        st = new Date().getTime();
        $http
          .post(REST.PRESENCE + '?sysparm_auto_request=true&cd=' + st, p)
      ...
      function determineStatus(presenceArray) {
        if (!presenceArray || !presenceArray.forEach) return;
        ...
        presenceArray.forEach(function (p) {
          ...
          var y = t - x;
          p.status = 'online';
          if (y > 5 * RETRY_INTERVAL) p.status = 'offline';
          else if (y > 3 * RETRY_INTERVAL) p.status = 'probably offline';
          else if (y > 2.5 * RETRY_INTERVAL) p.status = 'maybe offline';
        });
      }
      ...
      function getLocalPresence(p) {
        ...
        if (!p.presenceArray) return null;
        var now = new Date().getTime();
        if (now - p.saved >= RETRY_INTERVAL) return null;
        return p.presenceArray;
      }
      function getDecayingRetryInterval(numAttempts) {
        return Math.min(
          RETRY_INTERVAL * Math.pow(2, numAttempts),
          MAX_RETRY_DELAY
        );
      }
      return {
