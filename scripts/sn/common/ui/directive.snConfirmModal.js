...
angular
  .module('sn.common.ui')
  .directive('snConfirmModal', function (getTemplateUrl) {
    return {
      ...
      controller: function ($scope, $rootScope) {
        ...
        var buttons = {
          ...
          getText: function (type) {
            ...
            if (button && $scope.get(button.fn)) return button.text;
          },
        };
        $scope.get = function (type) {
          if ($scope.config[type]) return $scope.config[type];
          if (!$scope[type]) {
            var text = buttons.getText(type);
            if (text) return ($scope.config[type] = text);
          }
          return ($scope.config[type] = $scope[type]);
        };
        if (!$scope.get('modalName')) $scope.config.modalName = 'confirm-modal';
        function call(type) {
        ...
        function close(type) {
          return function () {
            actionClosed = true;
            $rootScope.$broadcast(
              'dialog.' + $scope.config.modalName + '.close'
            );
            call(type);
          };
        }
        var actionClosed;
        $scope.$on(
          'dialog.' + $scope.get('modalName') + '.opened',
          function () {
            actionClosed = false;
            $scope.isOpen = true;
          }
        ...
        $scope.$on(
          'dialog.' + $scope.get('modalName') + '.closed',
          function () {
            $scope.isOpen = false;
            if (actionClosed) return;
            if (call('cancel')) return;
            if (call('alert')) return;
            call('ok');
          }
        );
      },
