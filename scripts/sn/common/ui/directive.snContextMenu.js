...
angular
  .module('sn.common.ui')
  .directive('contextMenu', function ($document, $window, snCustomEvent) {
    var $contextMenu, $ul;
    ...
    function setContextMenuPosition(event, $ul) {
      ...
      if (contextMenuItemHeight === 0) contextMenuItemHeight = 24;
      var cmWidth = 150;
      ...
      var startX =
        pageX + cmWidth >= $window.innerWidth ? pageX - cmWidth : pageX;
      var startY =
        pageY + cmHeight >= $window.innerHeight ? pageY - cmHeight : pageY;
      $ul.css({
        display: 'block',
        position: 'absolute',
        left: startX,
    ...
    var renderContextMenu = function ($scope, event, options) {
      ...
      snCustomEvent.observe(
        'partial.page.reload',
        $contextMenu.data('resizeHandler')
      );
    };
    function _clearContextMenus(event) {
      ...
      angular.forEach(els, function (el) {
        snCustomEvent.un(
          'partial.page.reload',
          angular.element(el).data('resizeHandler')
        );
    ...
    return function (scope, element, attrs) {
      element.on('contextmenu', function (event) {
        ...
        if (angular.element(element).attr('context-type')) return;
        showMenu(event);
      });
      ...
      element.on('touchend', function (event) {
        var distX = Math.abs(
          event.originalEvent.changedTouches[0].screenX -
            doubleTapStartPosition.x
        );
        var distY = Math.abs(
          event.originalEvent.changedTouches[0].screenY -
            doubleTapStartPosition.y
        );
      ...
      function handleSpace(evt) {
        ...
        if (!$target.hasClass('list-edit-cursor')) return;
        showMenu(evt);
      }
      function handleClick(event) {
        ...
        if (
          !$el.attr('context-type') &&
          !$target.hasClass('context-menu-click')
        )
          return;
        showMenu(event);
      }
            ...
            scope.$watch(
              ...
              function (newValue, oldValue) {
                if (newValue !== oldValue)
                  renderContextMenuItems(scope, event, menu);
              },
              true
            );
