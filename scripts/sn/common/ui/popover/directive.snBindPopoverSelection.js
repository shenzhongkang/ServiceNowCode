...
angular
  ...
  .directive('snBindPopoverSelection', function (snCustomEvent) {
    'use strict';
    return {
      restrict: 'A',
      controller: function ($scope, $element, $attrs, snCustomEvent) {
          ...
          if (data.ref === ref) {
            if (window.g_form) {
              if ($attrs.addOption) {
                ...
                var fieldValue =
                  typeof $attrs.ref === 'undefined' ? data.ref : $attrs.ref;
                window.g_form._setValue(
                  fieldValue,
                  data.value,
                  data.displayValue
                );
        ...
        function clearDerivedFields(value) {
          if (window.DerivedFields) {
            var df = new DerivedFields(
              $scope.field ? $scope.field.ref : $attrs.ref
            );
            df.clearRelated();
