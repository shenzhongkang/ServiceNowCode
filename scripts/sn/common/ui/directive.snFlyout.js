...
angular
  .module('sn.common.ui')
  .directive('snFlyout', function (getTemplateUrl) {
    'use strict';
    return {
      restrict: 'E',
      transclude: true,
      ...
      link: function ($scope, element, attrs) {
        ...
        var setup = function () {
          ...
          if ($scope.open) element.css(position, 0);
          else element.css(position, -distance);
        };
        var calculatePosition = function () {
          if ($scope.open) {
            ...
            else animation[position] = -body.outerHeight();
          }
        };
        ...
        $scope.$on(
          $scope.register + '.bounceTabByIndex',
        ...
        $scope.$on(
          $scope.register + '.selectTabByIndex',
          function (event, index) {
      ...
      controller: function ($scope, $element) {
        ...
        $scope.selectTab = function (tab) {
          if ($scope.selectedTab) $scope.selectedTab.selected = false;
          tab.selected = true;
        ...
        $scope.bounceTab = function (index) {
          if (index >= $scope.tabs.length || index < 0) return;
          ...
          if (!tabScope.selected) {
            ...
            if (!highLightColor) highLightColor = invertColor(baseColor);
            if (tabScope.highlighted) contractTab(tabElem);
            for (var i = 0; i < 2; i++) {
              expandTab(tabElem);
              contractTab(tabElem);
            }
        ...
        this.addTab = function (tab) {
          $scope.tabs.push(tab);
          if ($scope.tabs.length === 1) $scope.selectTab(tab);
        };
        function normalizeTab(index) {
          if (
            index < 0 ||
            index >= $scope.tabs.length ||
            !$scope.tabs[index].highlighted
          )
            return;
          var tabElem = $element.find('.flyout-tab').eq(index);
          tabElem.velocity(
        ...
        function invertColor(rgb) {
          if (typeof rgb === 'string') var color = rgb.match(/[0-9]+/g);
          else var color = rgb.slice(0);
  ...
  .directive('snFlyoutTab', function () {
    ...
    return {
      ...
      transclude: true,
      template:
        "<div ng-show='selected' ng-transclude='' style='height: 100%'></div>",
