  ...
  $httpProvider.interceptors.push(function (
    ...
    function error(response) {
      ...
      if (status == 401) {
        ...
        if (newPromise) return newPromise;
      }
    ...
    function canResendRequest(response) {
      ...
      if (
        headers['x-sessionloggedin'] != 'true' &&
        !headers['x-rotate-guest-usertoken']
      )
        return false;
      if (headers['x-usertoken-allowresubmit'] == 'false') return false;
    ...
    function setToken(token) {
      $window.g_ck = token;
      if (!token) {
        $httpProvider.defaults.headers.common['X-UserToken'] =
          'token_intentionally_left_blank';
      } else {
      ...
      if ($window.Zepto) {
        if (!Zepto.ajaxSettings.headers) Zepto.ajaxSettings.headers = {};
