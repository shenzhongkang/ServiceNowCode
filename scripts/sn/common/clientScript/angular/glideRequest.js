...
angular
  .module('sn.common.clientScript')
  .factory(
    'glideRequest',
    function ($q, $log, $http, $window, urlTools, xmlUtil) {
      'use strict';
      function _sendRequestByType(method, url, options) {
        ...
        return $http(options).then(
          function (response) {
            ...
            response.responseXML = getXml
              ? xmlUtil.xmlToElement(response.data)
              : null;
            return response;
          },
          function (error) {
            ...
            error.responseXML = getXml
              ? xmlUtil.xmlToElement(error.data)
              : null;
