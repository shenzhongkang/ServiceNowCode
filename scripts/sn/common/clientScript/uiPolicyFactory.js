...
(function (exports, undefined) {
  ...
  function createUiPolicy(g_form, uiPolicyMap, scripts) {
    ...
    var _fields =
      uiPolicyMap.condition_fields || getConditionFields(_conditions);
    ...
    function evaluateCondition() {
      ...
      for (var i = 0; i < terms.length; i++) {
        ...
        if (t.or) {
          if (result != 't') result = _evaluateTermTF(t);
        } else {
    ...
    function evaluateTerm(term) {
      ...
      if (uiPolicyTypes.dateTypes[type]) return evaluateTermDate(term);
      ...
      switch (oper) {
        ...
        case 'STARTSWITH':
          if (type == 'reference') userValue = g_form.getDisplayValue(field);
          return userValue ? userValue.indexOf(value) === 0 : false;
        case 'ENDSWITH':
          if (type == 'reference') userValue = g_form.getDisplayValue(field);
          return userValue
            ? userValue.lastIndexOf(value) == userValue.length - value.length
            : false;
        case 'LIKE':
          if (type == 'reference') userValue = g_form.getDisplayValue(field);
        ...
        case 'BETWEEN':
          ...
          return (
            userValue && userValue >= values3[0] && userValue <= values3[1]
          );
        case 'SAMEAS':
          userValue =
            type == 'reference'
              ? g_form.getDisplayValue(term.field)
              : g_form.getValue(term.field);
          value =
            type == 'reference'
              ? g_form.getDisplayValue(term.value)
              : g_form.getValue(term.value);
          return userValue == value;
        case 'NSAMEAS':
          userValue =
            type == 'reference'
    ...
    function evaluateTermNumber(term) {
      ...
      var oper = term.oper;
      var value = term.value;
      if (value !== '' && value.indexOf(',') === -1) value = parseFloat(value);
      ...
      switch (oper) {
        ...
        case 'IN':
          ...
          return (
            userValue.toString() &&
            _inArray(userValue.toString(), values) !== -1
          );
        case 'NOT IN':
          var values2 = (value + '').split(',');
    ...
    function evaluateTermDate(term) {
      ...
      if (userValue) {
        if (
          typeof g_user_date_time_format != 'undefined' &&
          typeof g_user_date_format != 'undefined'
        ) {
          userFormat = isDateTime
            ? g_user_date_time_format
            : g_user_date_format;
          standardFormat = g_user_date_time_format;
        } else if (!isDateTime) userValue += ' 00:00:00';
      }
      if (term.oper == 'ISEMPTY') return userValue === '';
      ...
      switch (term.oper) {
        ...
        case 'NOTON':
          ...
          return (
            valueDate1 !== 0 &&
            valueDate2 !== 0 &&
            (userDate < valueDate1 || userDate > valueDate2)
          );
        case 'BETWEEN':
        ...
        default:
          _debug(
            "evaluateTermDate - unsupported operator '" +
              term.oper +
              "'. Returning FALSE."
          );
          return false;
    ...
    function _dateComparisonHelper(left, right, comparison) {
      ...
      if (_isNullField(value) || _isNullField(userValue)) {
        _debug(
          'ui policy could not find a valid field to compare against. Returning FALSE.'
        );
        return false;
      }
      var userDate = _getDate(userValue, userFormat);
      ...
      if (parsed.beforeAfter == 'before' && diff > 0) return false;
      if (parsed.beforeAfter == 'after' && diff < 0) return false;
      var xDiff;
      if (parsed.interval == 'quarter') {
        var Qdiff = Math.abs(
          _getAbsQuarter(theDate) - _getAbsQuarter(userDate)
        );
        xDiff = Qdiff - parsed.intervalValue;
      } else {
        var timeSpan = _getIntervalInMilliSeconds(
          parsed.interval,
          parsed.intervalValue
        );
        xDiff = Math.abs(diff) - Math.abs(timeSpan);
      }
      if (comparison === 'GT') return xDiff > 0;
      if (comparison === 'LT') return xDiff < 0;
    }
    ...
    function _evaluateTermDateTrend(userDateMilliseconds, trendValueString) {
      ...
      if (trendParams.length != 2 || !trendParams[1]) return;
      var trendParamParts = trendParams[1].split(',');
      if (trendParamParts.length != 3) return;
      ...
      switch (trendType) {
        ...
        default:
          _debug(
            "_evaluateTermDateTrend - unsupported trend type '" +
              trendType +
              "'. Returning FALSE."
          );
          return false;
    ...
    function _evaluateTermDateRelative(
      userDateMilliseconds,
      relativeValueString,
      isDateTime
    ) {
      ...
      if (
        !relativeValues ||
        relativeValues.length != 4 ||
        isNaN(relativeValues[3])
      )
        return false;
      ...
      var modifier = 1;
      if (termWhen == 'ahead') modifier = -1;
      var relativeValueMilliseconds;
      switch (termType) {
        case 'hour':
          relativeValueMilliseconds = hoursAgoInMilliseconds(
            modifier * termValue
          );
          break;
        case 'minute':
          relativeValueMilliseconds = minutesAgoInMilliseconds(
            modifier * termValue
          );
          break;
        case 'dayofweek':
          relativeValueMilliseconds = daysAgoInMilliseconds(
            modifier * termValue
          );
          break;
        case 'month':
          relativeValueMilliseconds = monthsAgoInMilliseconds(
            modifier * termValue
          );
          break;
        case 'quarter':
          relativeValueMilliseconds = quartersAgoInMilliseconds(
            modifier * termValue
          );
          break;
        case 'year':
          relativeValueMilliseconds = yearsAgoInMilliseconds(
            modifier * termValue
          );
          break;
        default:
          _debug(
            "_evaluateTermDateRelative - unsupported type '" +
              termType +
              "'. Returning FALSE."
          );
          return false;
    ...
    function _evaluateDateValues(checkVals, oper) {
      ...
      switch (oper) {
        ...
        default:
          _debug(
            "_evaluateDateValues - unsupported operator '" +
              oper +
              "'. Returning FALSE."
          );
          return false;
    ...
    function _trendWeek(userDate, trendValue) {
      ...
      var userWeek =
        Math.ceil(
          ((userDate.getTime() - checkDate) / 86400000 +
            checkDate.getDay() +
            1) /
            7
        ) - 1;
    ...
    function getUserValue(term) {
      ...
      if (userValue) return g_form.getDecimalValue(term.field + '.storage');
    ...
    function runActions(result) {
      ...
      if (result) {
        for (var i = 0; i < uiPolicyMap.actions.length; i++) {
          ...
          if (action.relatedList) _runListAction(action, result);
          else _runAction(action, result);
        }
        ...
        for (var j = 0; j < uiPolicyMap.actions.length; j++) {
          ...
          if (action2.relatedList) _runListAction(action2, result);
          else _runAction(action2, result);
        }
    ...
    function _runListAction(action, result) {
      ...
      if (result) {
        if (visible == 'true') g_form.showRelatedList(list);
        else if (visible == 'false') g_form.hideRelatedList(list);
      } else {
        if (visible == 'true') g_form.hideRelatedList(list);
        else if (visible == 'false') g_form.showRelatedList(list);
      }
    }
    ...
    function _debugTerm(term, result) {
      ...
      if (term.or) or = '  or ';
      var userValue;
      if (term.field) userValue = g_form.getValue(term.field) + '';
      else userValue = '(null)';
      if (!userValue) userValue = '<blank>';
      var dspValue = '';
      if (dspValue) dspValue = ' [' + dspValue + '] ';
