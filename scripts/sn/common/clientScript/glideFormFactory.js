...
(function (
  ...
  function createGlideForm(tableName, sysId, fields, uiActions, options) {
    ...
    function GlideForm() {
      ...
      this.getDecimalValue = function (fieldName) {
        ...
        if (!value) value = this.getValue(fieldName);
        if (!value || value.length === 0) return 0;
        if (!_options.cleanDecimals) return parseFloat(value);
        var sVal = '';
        var nVal = value.length;
        var sChar = '';
        var nChar = '';
        var decimalSeparator =
          typeof g_user_decimal_separator != 'undefined'
      ...
      this.getReference = function (fieldName, callback) {
        if (!callback) {
          _logWarn(
            'GETREF:NOCB',
            'Mobile scripts must specify a callback function'
          );
          return;
        }
        var field = _getField(fieldName);
      ...
      this.hideFieldMsg = function (fieldName, clearAll) {
        ...
        if (!hasError) _setInvalid(this, field, false);
        this.$private.events.propertyChange(
          PROPERTY_CHANGE_FIELD,
          field.name,
          'messages',
      ...
      this.getActionName = function () {
        if (uiActions && uiActions.getActiveActionName) {
          ...
          if (activeUiAction !== DEFAULT_ACTION_NAME) return activeUiAction;
        }
        return _submitAction.name || DEFAULT_ACTION_NAME;
      };
      this.reload = function () {
        if (_options.reloadForm) _options.reloadForm();
        else
          _logWarn(
            'UNSUPPORTED',
            'g_form.reload is not supported on mobile or service portal.'
          );
      };
      this.save = function () {
      ...
      this.submit = function (submitActionName) {
        ...
        if (!_submitAction.sysId || _submitAction.name !== submitActionName) {
          this.$private.userState.setRunningAction(
            submitActionName || SUBMIT_ACTION_NAME
          );
        }
        if (
          submitActionName !== 'sysverb_ws_delete' &&
          !this.$private.validateForm(_submitAction.name)
        ) {
      ...
      this.serialize = function (onlyDirtyFields) {
        var serializeField = function (field, fields) {
          ...
          if (
            ...
            typeof fieldCopy.value === 'undefined'
          ) {
            fieldCopy.value = '';
          }
      ...
      this.onUserChangeValue = function (handler) {
        ...
        return function () {
          ...
          if (index > -1) _onUserChangedHandlers.splice(index, 1);
        };
      };
      ...
      this.$private = {
        ...
        events: {
          ...
          userChangeValue: function (field, value) {
            if (
              _onUserChangedHandlers.length == 0 ||
              _equals(field.originalValue, value)
            ) {
              return;
            }
          ...
          stateChange: function (state) {
            ...
            _onStateChangeHandlers.forEach(function (fn) {
              fn.call(fn, previousState, _formState);
            });
          },
        ...
        applyLiveUpdate: function (updatedRecord, updatedBy) {
          ...
          Object.keys(updatedRecord).forEach(function (fieldName) {
            ...
            if (
              isUserModified &&
              _equals(value, _userModifiedFields[fieldName].newValue) &&
              field.liveUpdate
            ) {
              ...
              if (displayString) liveUpdate.displayString = displayString;
        ...
        setAdditionalFieldState: function (fieldName, propName, value) {
          ...
          switch (field.type) {
            case 'masked': {
              allowToSet =
                field.useConfirmation && propName === 'confirmationValue';
              break;
            }
          }
          if (allowToSet) {
      ...
      function _getField(fieldName) {
        ...
        if (variableField) return variableField;
    ...
    function _setValueGlideList(
      ...
      if (Array.isArray(displayValue)) {
        ...
        displayValue &&
        field.value.split(',').length == displayValue.split(',').length
      ) {
      ...
      var requestUri =
        '/api/now/ui/glide/list/element/display/' + table + '/' + field.value;
    ...
    function _setValue(g_form, field, value, displayValue, options) {
      ...
      if (
        !field ||
        (_equals(field.value, value) &&
          (noDisplayValue || _equals(field.displayValue, displayValue)))
      ) {
        return;
      }
      if (value === null || value === undefined) value = '';
      if (_isLiveUpdating && (!options || !options.fromLiveUpdate)) return;
        ...
        if (!glideFormFieldFactory.hasValue(field)) {
          ...
          if (displayValue && !Array.isArray(displayValue))
            _logWarn(
              ...
              'When using setValue with a glide_list element: displayValue should be an array of display values.'
            );
      ...
      field.displayValue =
        typeof displayValue !== 'undefined' && displayValue != null
          ? displayValue
          : value;
      ...
      if (
        !_equals(oldValue, value) ||
        !_equals(oldDisplayValue, displayValue)
      ) {
    ...
    function _fireValueChange(g_form, field, value) {
      ...
      if (_valueCalls === 0) {
        g_form.$private.events.stateChange(
          g_form.isUserModified() ? FORM_STATE_MODIFIED : FORM_STATE_UNMODIFIED
        );
    ...
    function _updateDerivedFields(g_form, originatingField) {
      ...
      derivedFields.forEach(function (field) {
        var relativeField = _relativeDerivedFieldName(
          field.name,
          originatingField.name
        );
      ...
      var referenceKey = originatingField.reference_key
        ? originatingField.reference_key
        : 'sys_id';
          ...
          if (result.length > 0) {
            ...
            keys.forEach(function (fieldName) {
              ...
              _setValue(
                ...
                newFieldValues.display_value,
                { skipDerivedFieldUpdate: true, skipDisplayValueUpdate: true }
              );
            });
          }
        });
    ...
    function _getDirtyQueryFields(fields) {
      ...
      if (fields) {
        fields.forEach(function (field) {
          if (
            typeof field.dirtyQueryField !== 'undefined' &&
    ...
    function _fireUiMessage(g_form, type, message) {
      ...
      g_form.$private.events.propertyChange(PROPERTY_CHANGE_FORM, null, type);
    }
    function _fireFormMessage(g_form, action, messageType, message) {
      var handledMessage = false;
      if (_options.formMessageHandler) {
        var response = _options.formMessageHandler(
          g_form,
          action,
          messageType,
          message
    ...
    methods.forEach(function (method) {
      if (!instance[method]) {
        instance[method] = function () {
          _logWarn(
            'UNSUPPORTED',
            'Method ' + method + ' is not supported on mobile'
          );
        };
      }
    });
    ...
    methods.forEach(function (method) {
      if (!instance[method]) {
        instance[method] = function () {
          _logWarn(
            'DEPRECATED',
            'Method ' + method + ' is deprecated and unsupported on mobile'
          );
        };
      }
    });
  ...
  function _copy(source, serializedExclusionList) {
    ...
    for (var prop in source) {
      if (
        serializedExclusionList &&
        serializedExclusionList.indexOf &&
        serializedExclusionList.indexOf(prop) >= 0
      )
        continue;
  ...
  window,
  window.document || {},
  window.glideFormFieldFactory,
  window.catalogVariableHandler
);
