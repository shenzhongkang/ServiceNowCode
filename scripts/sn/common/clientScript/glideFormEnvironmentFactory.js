...
(function (exports, $log, undefined) {
  ...
  function createGlideFormEnvironment(
    g_form,
    g_scratchpad,
    g_user,
    g_modal,
    ...
    function getOnSubmitGlideForm(g_form) {
      ...
      facade.save = function () {
        _logInfo('g_form', 'save', 'is not available');
        return false;
      };
      facade.submit = function () {
        _logInfo('g_form', 'submit', 'is not available');
        return false;
      };
      return facade;
    }
      ...
      if (scriptMap.onLoad) {
        for (var i = 0; i < scriptMap.onLoad.length; i++) {
          try {
            ...
            _logError(
              'CS:ONLOAD',
              'Could not load onLoad Client Script "' + cs.name + '": ' + e
            );
          }
        }
      }
      if (scriptMap.onChange) {
        for (var j = 0; j < scriptMap.onChange.length; j++) {
          try {
            ...
            _logError(
              'CS:ONCHANGE',
              'Could not load onChange Client Script "' + cs.name + '": ' + e
            );
          }
        }
      }
      if (validationScripts) {
        ...
        for (var k = 0; k < validationScripts.length; k++) {
          try {
            ...
            var wrappedScript = _wrapExecuteClientScript(
              script,
              g_form,
              cs.name
            );
            fieldNames.forEach(function (fieldName) {
              ...
              _validationScripts[fieldName].push(wrappedScript);
            });
          } catch (e) {
            _logError(
              'CS:VALIDATE',
              'Could not load Validation Client Script "' + cs.name + '": ' + e
            );
          }
        }
      }
      if (scriptMap.onSubmit) {
        for (var k = 0; k < scriptMap.onSubmit.length; k++) {
          try {
            ...
            _logError(
              'CS:ONSUBMIT',
              'Could not load onSubmit Client Script "' + cs.name + '": ' + e
            );
          }
        }
      }
      ...
      ['script_true', 'script_false'].forEach(function (type) {
        ...
        try {
          ...
          resultScriptMap[policyScript.name] = {
            execute: _wrapExecuteClientScript(
              wrappedScript,
              g_form,
              uiPolicyMap.short_description
            ),
          };
        } catch (e) {
    ...
    function _runValidationScripts() {
      ...
      registerExtensionPoint(
        'g_validation_script_field_count',
        totalFieldValidationsRequired
      );
          ...
          for (var j = 0, jM = scripts.length; j < jM; j++) {
            ...
            if (result === false || isString) {
              if (isString) {
                ...
                if (!invalidFieldExists) invalidFieldExists = true;
              }
      ...
      if (invalidFieldExists) g_form.$private.showFieldInvalidMsg(g_form);
    ...
    function _wrapScript(script, parameters, mainFuncName) {
      ...
      try {
        fn = new Function(
          allParams,
          'return (' + script + ')(' + scriptParams.join(',') + ')'
        );
      } catch (e) {
        if (mainFuncName) {
          script = new Function(
            [],
            script + ' return ' + mainFuncName + '.apply(this, arguments);'
          );
          fn = new Function(
            allParams,
            'return (' + script + ')(' + scriptParams.join(',') + ')'
    ...
    function _executeClientScript(script, g_form, name, apiParams, event) {
      ...
      if (typeof injectedParams === 'string') {
        _logError(
          'SCRIPT:EXEC',
          'Invalid params passed into Client Script "' + name + '"'
        );
        return;
      }
      var baseParams = {
      ...
      try {
        ...
        _logError(
          'SCRIPT:EXEC',
          'Error while running Client Script "' + name + '": ' + e
        );
      }
    }
