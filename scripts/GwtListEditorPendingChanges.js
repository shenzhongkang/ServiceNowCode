...
var GwtListEditorPendingChanges = Class.create({
  ...
  get: function (sysId) {
    if (this.isEmptyRecord(sysId)) return this.emptyRecord;
  ...
  addRecord: function (sysId, operation) {
    ...
    if (operation) record.setOperation(operation);
  ...
  setReferenceValid: function (sysId, fieldName, valid) {
    var field = this.getField(sysId, fieldName);
    if (field) field.setReferenceValid(valid);
  },
  isReferenceValid: function (sysId, fieldName) {
    var field = this.getField(sysId, fieldName);
    if (field) return field.isReferenceValid();
    return true;
  },
  ...
  getRecordOperation: function (sysId) {
    ...
    if (r) return r.operation;
    return '';
  },
  setRecordOperation: function (sysId, operation) {
    var r = this.get(sysId);
    if (r) r.operation = operation;
  },
  ...
  clearModified: function (sysId) {
    if (!this.get(sysId)) return;
    this.get(sysId).clearModified();
  },
  clearAllModified: function () {
    for (var id in this.changes) this.clearModified(id);
  },
  remove: function (sysId) {
    if (this.modifiedRecords[sysId]) delete this.modifiedRecords[sysId];
  },
  removeAll: function () {
    for (var i in this.modifiedRecords) if (i != -1) this.remove(i);
  },
  getValueToRender: function (sysId, fieldName) {
    var field = this.getField(sysId, fieldName);
    if (!field || (this.emptyRecord && sysId == this.emptyRecord.sysId))
      return '';
    if (field.isRenderValueSet()) return field.getRenderValue();
    if (field.isDisplayValueSet()) return field.getDisplayValue();
    if (field.isValueSet()) return field.getValue();
    if (field.getRenderValue()) return field.getRenderValue();
    if (field.getDisplayValue()) return field.getDisplayValue();
    return field.getValue();
  },
  ...
  addToAggregates: function (record) {
    ...
    for (var n in fields) {
      ...
      if (field.isDisplayValueSet()) value = field.getDisplayValue();
...
GwtListEditorPendingChanges.ChangeExporter = Class.create(
  GwtListEditorPendingChanges.RecordReceiver,
  {
    ...
    changedRecord: function (sysID, record) {
      if (record.isDefaultValues()) return;
...
GwtListEditorPendingChanges.ComposableChangeReceiver = Class.create(
  GwtListEditorPendingChanges.ChangeReceiver,
  {
    ...
    endRecord: function (sysID) {
      for (i = 0; i < this.limit; i++) this.receivers[i].endRecord(sysID);
    },
    changedField: function (sysID, field) {
