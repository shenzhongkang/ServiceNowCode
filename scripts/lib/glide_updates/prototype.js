...
Prototype.ScriptFragmentDetailed =
  '<script(?:[^>]*type="([^>]*?)")?(?:[^>]*src="([^>]*?)")?[^>]*>([\\S\\s]*?)</script>';
  ...
  (function () {
    function extractScriptsDetailed() {
      ...
      return (this.match(matchAll) || []).map(function (scriptTag) {
        ...
        if (m[1] == 'application/xml') return;
        if (m[1] == 'text/html') return;
        if (m[1] == 'text/ng-template') return;
        return {
          script: m[3],
          src: m[2],
        };
    ...
    function evalScripts(evalGlobal) {
      (function _executer(scripts) {
        ...
        if (!script) return _executer(scripts);
        if (script.src) {
          if (!g_evalScriptCache[script.src]) {
            ...
            ajaxRequest(script.src, null, true, function (r) {
              ...
              if (r && r.responseText) evalScript(r.responseText, evalGlobal);
              return _executer(toEvalScripts);
            });
          } else if (g_evalScriptCache[script.src].state == 'loading') {
            g_evalScriptCache[script.src].scripts =
              g_evalScriptCache[script.src].scripts.concat(scripts);
            return;
          } else if (g_evalScriptCache[script.src].state == 'loaded')
            return _executer(scripts);
        } else {
          if (script.script) evalScript(script.script, evalGlobal);
          return _executer(scripts);
        }
...
function evalScript(s, evalGlobal) {
  ...
  if (!evalGlobal) return eval(s);
  if (window.execScript) return window.execScript(s);
  return window.eval ? window.eval(s) : eval(s);
}
if (
  document.getElementsByClassName &&
  typeof Element.Methods.getElementsByClassName === 'function'
)
    ...
    switch (typeof o1[p]) {
      ...
      case 'object':
        if (
          o1[p] !== null &&
          o2[p] !== null &&
          (o1[p].constructor.toString() !== o2[p].constructor.toString() ||
            !Object.equals(o1[p], o2[p]))
        )
          return false;
        break;
      case 'function':
