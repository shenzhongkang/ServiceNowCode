...
(function () {
  ...
  Prototype.effects = {
    speed: function (speed, easing, fn) {
      ...
      opt.duration = Prototype.effects.fx.off
        ? 0
        : typeof opt.duration === 'number'
        ? opt.duration
        : opt.duration in Prototype.effects.fx.speeds
        ? Prototype.effects.fx.speeds[opt.duration]
        : Prototype.effects.fx.speeds._default;
  ...
  Prototype.effects.fx.prototype = {
    update: function () {
      ...
      (
        Prototype.effects.fx.step[this.prop] ||
        Prototype.effects.fx.step._default
      )(this);
    },
    cur: function () {
      if (
      ...
      if (gotoEnd || t >= this.options.duration + this.startTime) {
        ...
        if (done) {
          ...
          if (this.options.hide) {
            $(this.elem).hide();
            if (Prototype.Browser.IE) $(this.elem).setStyle({ filter: '' });
          }
        ...
        var defaultEasing =
          this.options.easing ||
          (Prototype.effects.easing.swing ? 'swing' : 'linear');
        this.pos = Prototype.effects.easing[specialEasing || defaultEasing](
          this.state,
          n,
          0,
          1,
  ...
  Object.extend(Prototype.effects.fx, {
    tick: function () {
      ...
      if (!timers.length) Prototype.effects.fx.stop();
    },
    interval: 13,
    stop: function () {
  ...
  Object.extend(Prototype, {
    ...
    queue: function (elem, type, data) {
      ...
      if (!data) return q || [];
      if (!q || Object.isArray(data)) q = elem.store(type, makeArray(data));
      else q.push(data);
      return q;
    },
  ...
  function hide(elem, speed, easing, callback) {
    ...
    if (display !== 'none') elem.store('olddisplay', elem.getStyle('display'));
  ...
  function toggle(elem, fn, fn2, callback) {
    ...
    } else animate(elem, genFx('toggle', 3), fn, fn2, callback);
    return elem;
  }
  ...
  function animate(elem, prop, speed, easing, callback) {
    ...
    if (isEmptyObject(prop)) return optall.complete;
    return elem[optall.queue === false ? '_execute' : 'queue'](function () {
      ...
      for (p in prop) {
        ...
        if (isElement && (p === 'height' || p === 'width')) {
          ...
          if (
            elem.getStyle('display') === 'inline' &&
            elem.getStyle('float') === 'none'
          ) {
            if (!inlineBlockNeedsLayout) elem.style.display = 'inline-block';
            else {
              var display = defaultDisplay(this.nodeName);
              if (display === 'inline') elem.style.display = 'inline-block';
              else {
      ...
      if (opt.overflow != null) elem.style.overflow = 'hidden';
  ...
  function queue(elem, type, data) {
    ...
    if (data === undefined) return Prototype.queue(elem, type);
  ...
  function delay(elem, time, type) {
    time = Prototype.effects.fx
      ? Prototype.effects.fx.speeds[time] || time
      : time;
    type = type || 'fx';
