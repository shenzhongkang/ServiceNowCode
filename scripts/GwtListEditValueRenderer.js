...
var GwtListEditValueRenderer = Class.create({
  ...
  initialize: function (changes, tableController) {
    ...
    this.decor = new GwtListEditRecordDecorations(
      this.changes,
  ...
  setValue: function (sysId, fieldName, value) {
    if (this.changes.isEmptyRecord(sysId)) this._saveEmptyRecord(sysId);
    return this.changes.setValue(
      sysId,
      fieldName,
      value,
      this._callOnChange.bind(this)
    );
  },
  setDisplayValue: function (sysId, fieldName, value) {
    if (this.changes.isEmptyRecord(sysId)) this._saveEmptyRecord(sysId);
    return this.changes.setDisplayValue(
      sysId,
      fieldName,
      value,
      this._callOnChange.bind(this)
  ...
  addRow: function () {
    if (
      GlideLists2[this.tableController.listID] &&
      GlideLists2[this.tableController.listID].disableRowAdd
    )
      return;
  ...
  addRowNoInsert: function () {
    if (!this.tableController.canCreate) return;
    if (
      GlideLists2[this.tableController.listID] &&
      GlideLists2[this.tableController.listID].disableRowAdd
    )
  ...
  renderValue: function (sysId, fieldName, valueToDisplay, newRow) {
    ...
    if (!cell) return;
    var cellIndex =
      document.documentElement.getAttribute('data-doctype') == 'true' ? 2 : 1;
  ...
  _renderContents: function (cell, sysId, fieldName, dspValue, newRow) {
    if (dspValue) dspValue = dspValue.escapeHTML();
    ...
    if (fieldElement.isReference()) {
      ...
      if (referenceValue) {
        if ('NULL' === referenceValue) {
          if (!hasClassName(cell, 'list_edit_new_row')) dspValue = '';
    ...
    var defaultTemplate = doctypeMode
      ? GwtListEditValueRenderer.Templates.RENDER_DEFAULT_DOCTYPE
      : GwtListEditValueRenderer.Templates.RENDER_DEFAULT;
  ...
  _getValueToRender: function (sysId, fieldName, valueToDisplay) {
    if (valueToDisplay) return valueToDisplay;
  ...
  insertRowForRecord: function (record) {
    ...
    for (var n in fields) this.renderValue(record.sysId, n, '', isNew);
    var info = {
  ...
  deleteRowWithRecord: function (record) {
    ...
    if (tr) {
      ...
      img.setAttribute(
        'data-dynamic-title',
        this.msgs['Undo mark for deletion']
      );
      if (this.isDoctype()) img.setStyle({ color: '#cfcfcf' });
    }
    this.changes.removeFromAggregates(record);
  },
  deleteRowToggle: function (sysId) {
    var record = this.changes.get(sysId);
    if (record && record.operation == 'delete_pending') return;
    if (!record) record = this.changes.addRecord(sysId, 'delete_pending');
    ...
    if (record.operation == 'delete') {
      ...
      if (tr) {
        ...
        if (this.isDoctype()) img.setStyle({ color: '#FF402C' });
      }
      ...
      this.changes.loadValues(
        [sysId],
        this.changes.getAggregateFields(),
        this._deleteRowHandler.bind(this, sysId)
      );
    }
  },
  ...
  _callOnChange: function (sysId, fieldName, oldValue, value) {
    var old_g_list = null;
    if (typeof g_list != 'undefined') old_g_list = g_list;
    g_list = this.gList;
    var scriptName =
      'onChange_' + this.tableController.listID + '_' + fieldName;
  ...
  _markChangedField: function (cell, sysId, fieldName) {
    if (!this.tableController.hasDeferredChanges()) return;
    if (!this.changes.isFieldDirty(sysId, fieldName)) return;
  ...
  _resetBackgroundColor: function (tr) {
    ...
    if (previousRows.length > 0) {
      if (previousRows[0].hasClassName('list_odd')) tr.className = 'list_even';
      else tr.className = 'list_odd';
