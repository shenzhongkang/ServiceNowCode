  ...
  createEditControls: function () {
    ...
    function onTagAdded(evt, ui) {
      ...
      for (var i = 0; i < rowIds.length; i++) {
        ...
        var labelNames = row
          .find('.tagit-label:contains(' + ui.tagLabel + ')')
          .filter(function () {
    ...
    function onTagRemoved(evt, ui) {
      ...
      var validRowIds = [];
      for (var i = 0; i < rowIds.length; i++) {
        var label = $("tr[sys_id='" + rowIds[i] + "']").find(
          "li[id='" + ui.tag + "']"
        );
        if (label.length != 0) {
          label.remove();
    ...
    function labelResponse() {
      ...
      share
        ...
        .click(function () {
          window.location.href =
            json.table + '_list.do?sysparm_query=' + json.query;
        });
      if (json.owner != true) {
      ...
      var rowIds = that.editor.getCanWriteIds();
      for (var i = 0; i < rowIds.length; i++) {
        var rowShare = $("tr[sys_id='" + rowIds[i] + "']").find(
          "[id='" + json.label + "'].tagit-share"
        );
        rowShare
          .parent()
          .attr('id', json.sysId)
