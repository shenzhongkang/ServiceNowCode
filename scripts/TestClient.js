...
function popTestClient(test_definition, test_subject) {
  var test_execution;
  if (!test_subject) test_execution = test_definition;
...
TestClient.prototype = {
  ...
  STATUS_IMAGES: [
    'images/workflow_skipped.gif',
    'images/loading_anim2.gifx',
    'images/workflow_complete.gifx',
  ...
  getStatusRow: function (obj, order) {
    ...
    if (hasDetails || hasPercent) {
      ...
      if (
        typeof this.detailStates[obj.sys_id] == 'boolean' &&
        this.detailStates[obj.sys_id] == false &&
        dtl != null
      )
        dtl.onclick();
      tr.appendChild(dtd);
  ...
  _getDetailsText: function (message, obj) {
    if (message.length > 150) {
      ...
      new_message.innerHTML =
        '<b>' +
        this.translator.getMessage('Details') +
        ': </b>' +
        message.slice(0, 150) +
        '... ';
      var anch = new Element('a');
      anch.href = 'test_execution.do?sys_id=' + obj.sys_id;
      anch.innerHTML = '<b>(' + this.translator.getMessage('more') + ')</b>';
      new_message.appendChild(anch);
      return new_message;
    } else {
      var new_message = new Element('span');
      new_message.innerHTML =
        '<b>' + this.translator.getMessage('Details') + ': </b>' + message;
      return new_message;
    }
  },
  ...
  renderChildren: function (so, pr_cont) {
    if (!so.results) return;
    for (var i = 0; i < so.results.length; i++) {
      pr_cont.appendChild(
        this.getStatusRow(so.results[i], i + 1)
