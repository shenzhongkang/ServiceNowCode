...
var GlideWindow = Class.create(GwtObservable, {
  ...
  initialize: function (id, readOnly) {
    if (typeof this.DEFAULT_BODY == 'undefined')
      this.DEFAULT_BODY =
        '<center> ' +
        getMessage('Loading...') +
        " <br/><img src='images/ajax-loader.gifx' alt='" +
        getMessage('Loading...') +
        "' /></center>";
  ...
  destroy: function () {
    ...
    if (this.container) {
      ...
      if (parent) parent.removeChild(gWindow);
    }
    this.setShim(false);
    if (isMSIE) this.container.outerHTML = '';
    this.release();
    this.valid = false;
    this.closeDecoration = null;
  },
  ...
  getDecorations: function (left) {
    if (left) return this.leftDecorations;
  ...
  getDescribingXML: function () {
    ...
    for (var name in preferences) {
      ...
      if (v && typeof v.escapeHTML === 'function') v = v.escapeHTML();
  ...
  locate: function (domElement) {
    while (domElement.parentNode) {
      ...
      if (domElement.gWindow) return domElement.gWindow;
  ...
  get: function (id) {
    if (!id || !gel('window.' + id)) return this;
  ...
  initDecorations: function () {
    if (!this.isReadOnly()) {
      this.closeDecoration = this.addFunctionDecoration(
        'images/x.gifx',
        'Close',
  ...
  showCloseButton: function () {
    if (this.closeDecoration) this.closeDecoration.style.visibility = 'visible';
  },
  hideCloseButton: function () {
    if (this.closeDecoration) this.closeDecoration.style.visibility = 'hidden';
  },
  insert: function (element, beforeElement, invisible) {
    ...
    if (!div) {
      ...
      if (invisible) div.style.visibility = 'hidden';
      if (beforeElement) element.insertBefore(div, beforeElement);
  ...
  render: function () {
    ...
    if (this.nologValue) ajax.addParam('ni.nolog.sysparm_value', true);
  ...
  setBody: function (html, noEvaluate, setAlt) {
    ...
    if (typeof html == 'string') {
      var showBody = true;
      if (html.length == 0) showBody = false;
      this.showBody(showBody);
      html = this._substituteGet(html);
      html = this._fixBrowserTags(html);
      this.body.innerHTML = html;
      if (setAlt) this.setBodyAlt(html);
      if (!noEvaluate) this._evalScripts(html);
    } else {
    ...
    if (prefs.length > 0) {
      for (var i = 0; i < prefs.length; i++) {
        ...
        if (name == 'title') this.setTitle(valu);
  ...
  showBody: function (show) {
    if (this.divMode) var tr = this.body;
    else var tr = this.body.parentNode.parentNode;
  ...
  setClassName: function (className) {
    ...
    if (this.getWindowDOM()) this.getWindowDOM().className = className;
  },
  setColor: function (color) {
    this.windowBackground = color;
    if (this.getBody()) this.getBody().parentNode.style.backgroundColor = color;
  },
  ...
  getWidth: function () {
    if (this.window) return this.window.clientWidth;
    else return;
  },
  ...
  getHeight: function () {
    if (this.window) return this.window.clientHeight;
  },
  setSize: function (w, h) {
    var obj = this.getContainer();
    var bo = this.getBody();
    var setWidth = w;
    if (setWidth && !isNaN(setWidth)) setWidth = setWidth + 'px';
    bo.style.width = setWidth;
    obj.style.width = setWidth;
    if (obj != this.window) this.window.style.width = setWidth;
    var setHeight = h;
  ...
  toggleActive: function () {
    if (this.active) this.setWindowInactive();
    else this.setWindowActive();
  },
  setHeaderWrap: function (wrap) {
    this.headerWrap = wrap;
    if (wrap) this.title.style.whiteSpace = 'normal';
    else this.title.style.whiteSpace = 'nowrap';
  },
  ...
  setTitle: function (html) {
    ...
    var titleNode =
      this.titleContents && !this.divMode ? 'titleContents' : 'title';
    if (typeof html == 'string') this[titleNode].innerHTML = html;
    else {
  ...
  setHeaderBorderVisibility: function (visible) {
    if (visible !== 'false' && visible) this.header.style.border = 'solid';
    else this.header.style.border = 'none';
  },
  ...
  setTitleVisibility: function (visible) {
    if (visible !== 'false' && visible) this.setTitle(this.titleValue);
  ...
  on: function (name, func) {
    if (this.FORWARD_EVENTS[name]) this.forward(name, this.window, func);
    else GwtObservable.prototype.on.call(this, name, func);
    if (name == 'dblclick') {
      if (isMSIE) this.window.style.cursor = 'hand';
      else this.window.style.cursor = 'pointer';
    }
  },
  ...
  _centerOnScreen: function (width, height) {
    ...
    if (
      topWindowBody &&
      topWindowBody.select &&
      topWindowBody.select('table.navpage').length == 0
    ) {
      ...
      if (currentIframe && currentIframe.getBoundingClientRect) {
        ...
        if (rectTop < 0) {
          ...
          else topY += parseInt((d - windowHeight) / 2) - rectTop;
        } else {
          if (d > browserHeight)
    ...
    if (leftX < 0) leftX = 0;
    if (this.container) {
      var headerSpacer = $(this.container.parentNode).select(
        '.section_header_spacer'
      );
      if (headerSpacer.length > 0) {
        headerSpacer = headerSpacer[0];
        if (topY < headerSpacer.offsetHeight) topY = headerSpacer.offsetHeight;
      }
    }
    ...
    try {
      if (win && win.parent) {
        ...
        for (var i = 0; i < e.length; i++) {
          if (e[i].contentWindow == win) return e[i];
        }
      }
    } catch (e) {}
    return null;
  ...
  _drawIndicator: function (classNames, id, altText) {
    ...
    if (altText) this.filterIndicator.title = getMessage(altText);
    dragTableTD.id = id;
  ...
  _toggleIndicator: function (indicator) {
    var $parentEl;
    var preference;
    if (indicator === 'real-time') preference = 'real_time';
    if (indicator === 'filter') preference = 'filter_indicator';
    $parentEl = $(indicator + '-indicator-' + this.id);
    if ($parentEl) {
      if (this.preferences[preference] === 'true')
        $parentEl.addClassName('active');
      else $parentEl.removeClassName('active');
    }
  },
  _enableDragging: function (element) {
    var id = element.getAttribute('dragpart');
    if (!id || this.isReadOnly() || this.getPreference('pinned')) return;
  ...
  _evalScripts: function (html) {
    ...
    if (x) {
      ...
      for (var i = 0; i < scripts.length; i++) {
        ...
        var s = '';
        if (script.getAttribute('type') == 'application/xml') continue;
        if (script.getAttribute('type') == 'text/ng-template') continue;
        if (script.getAttribute('src')) {
          ...
          if (!s) s = script.innerHTML;
        }
        if (s) evalScript(s, true);
      }
    }
  ...
  _onCloseClicked: function () {
    if (!this.fireEvent('closeconfirm', this)) return false;
    if (this.window) {
      var gWindow = $(this.window.gWindow);
      if (gWindow) gWindow.destroy();
    }
  },
  _setShimBounds: function (x, y, w, h) {
    if (x !== '') this.cacheX = parseInt(x);
    if (y !== '') this.cacheY = parseInt(y);
    if (w != '' && !isNaN(w)) this.cacheWidth = parseInt(w);
    if (h != '' && !isNaN(h)) this.cacheHeight = parseInt(h);
    if (!this.shim) return;
    if (x !== '') this.shim.style.left = x + 'px';
    if (y !== '') this.shim.style.top = y + 'px';
    if (w != '') {
      if (!isNaN(w)) this.shim.style.width = w + 'px';
      else this.shim.style.width = w;
    }
    if (h != '') {
      if (!isNaN(h)) this.shim.style.height = h + 'px';
      else this.shim.style.height = h;
    }
  },
  ...
  _substituteGet: function (html) {
    ...
    for (var i = 0; i < substitutions.length; i++) {
      ...
      html = html.replace(
        reg,
        this.type() + ".prototype.get('" + this.getID() + "'"
      );
    }
    return html;
  },
