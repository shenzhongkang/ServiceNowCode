...
var GlideListElement = Class.create({
  ...
  setValue: function (newValue, newDisplayValue) {
    ...
    if (newValue || newDisplayValue) {
      ...
      if (
        newDisplayValue &&
        newValue &&
        newDisplayValue.length != newValue.length
      ) {
        newDisplayValue = '';
        isArbitraryEmail = true;
      }
      ...
      if (typeof newDisplayValue == 'undefined' || newDisplayValue == '') {
        ...
        if (!allEmail) {
          ...
          } else aj.addParam('sysparm_type', 'getDisplayValues');
      ...
      if (
        typeof newDisplayValue != 'undefined' &&
        newValue.length > 0 &&
        newDisplayValue.length != newValue.length
      ) {
        jslog(
          'Error: Length of first and second parameter arrays to setValue for ' +
            this.name +
            ' are not the same'
        );
        return;
      ...
      if (typeof newDisplayValue != 'undefined') {
        for (i = 0; i < newDisplayValue.length; i++) {
          if (allEmail) this._setValue('', newValue[i]);
          else this._setValue(newValue[i], newDisplayValue[i]);
        }
  ...
  clearValue: function (noOnChange) {
    ...
    if (visibleElement) visibleElement.options.length = 0;
  ...
  _setValue: function (newValue, displayValue, allEmails) {
    if (!newValue)
      ...
      if (allEmails !== true) toggleAddMe(this.name);
    }
  },
  _glideListGetDisplayValuesDone: function (response, args) {
    ...
    for (var i = 0; i < references.length; i++) {
      var displayValue =
        references[i].attributes.getNamedItem('display').nodeValue;
      var referenceValue =
        references[i].attributes.getNamedItem('sys_id').nodeValue;
      this._setValue(referenceValue, displayValue);
    }
    for (var i = 0; i < choices.length; i++) {
      var displayValue =
        choices[i].attributes.getNamedItem('display').nodeValue;
  ...
  _showSpacer: function (display) {
    ...
    if (spacer) spacer.style.display = 'inline';
  },
  _updateDisplay: function (performOnChange) {
    ...
    var buttonContainerVisible = buttonContainer
  ...
  updateLockedList: function (current_value_id, update_id) {
    ...
    if (current_value.options) {
      for (var i = 0; i < current_value.options.length; i++) {
        if (i > 0) the_value += g_glide_list_separator;
        the_value += current_value.options[i].text;
      }
    } else the_value = current_value.value;
    var update_element = gel(update_id);
    if (update_element.href) update_element.href = the_value;
  ...
  setReadOnly: function (disabled) {
    ...
    if (addMe) addMe.setAttribute('list-read-only', 'false');
    if (disabled) {
  ...
  isDisabled: function () {
    ...
    if (unlockElement.style.visibility == 'visible') return false;
    return true;
  },
...
function editList(tableName, urlBase, idField, reference, maintainOrder) {
  ...
  if (view != null) {
    view = view.value;
    if (view != '') url = url + '&sysparm_view=' + view;
  }
  var changeset = gel('sysparm_changeset');
  if (changeset != null) {
    changeset = changeset.value;
    if (changeset != '') url = url + '&sysparm_changeset=' + changeset;
  }
  ...
  if (sysparm_record_list)
    url =
      url +
      '&sysparm_record_list=' +
      encodeURIComponent(sysparm_record_list.value);
...
function addGlideListChoice(selectID, value, displayValue, toggleIcons) {
  ...
  if (select.contains(value)) return;
...
function setRemovalNotification(id) {
  ...
  var text = !selectedOptions.length
    ...
    : selectedOptions
        ...
        .join(', ');
  if (notification.innerText === text) text += '!';
...
function toggleGlideListIcons(id, performOnChange) {
  ...
  if (!view2) view2 = gel('view2link.' + id);
  var select = gel('select_0' + id);
  var options = select.options;
  var selCnt = 0;
  var isMe = false;
  for (var i = 0; i != options.length; i++) {
    if (options[i].selected) selCnt++;
  ...
  if (view2) {
    ...
    if (selCnt == 1) {
      ...
      if (selectValue.indexOf('@') > -1) {
        if (selectValue == selectText) isEmail = true;
      }
    }
    ...
    if (selCnt == 1 && !isEmail) {
      ...
      var ariaLabel = clickThrough
        ? 'Open selected item: {0}'
        : 'Preview selected item: {0}';
      view2Link.setAttribute(
        'aria-label',
        new GwtMessage().getMessage(ariaLabel, selectText)
      );
    } else {
  ...
  if (remove) {
    if (selCnt > 0)
      if (remove.tagName == 'BUTTON') remove.disabled = false;
      else remove.src = 'images/delete_edit.gifx';
    else if (remove.tagName == 'BUTTON') remove.disabled = true;
    else remove.src = 'images/delete_edit_off.gifx';
  }
  if (add_me) {
    if (!isMe)
      if (add_me.tagName == 'BUTTON') add_me.disabled = false;
      else add_me.src = 'images/icons/user_obj.gifx';
    else if (add_me.tagName == 'BUTTON') add_me.disabled = true;
    else add_me.src = 'images/icons/user_obj_off.gifx';
  }
  add_me = null;
  remove = null;
  view2 = null;
  ...
  if (typeof performOnChange != 'undefined')
    if (performOnChange == false) return;
  glideListSaveList(id);
  if (isCatalogGlideList(id)) variableOnChange(id);
}
...
function glideListPopupSelection(event, id, table) {
  ...
  if (sysid) popRecordDiv(event, table, sysid);
}
function glideListInit(id, ref, reference) {
  ...
  if ($look.length) {
    $look.on('click keydown', function (e) {
      if (
        e.type === 'keydown' &&
        !($j.ui.keyCode.ENTER === e.which || $j.ui.keyCode.SPACE === e.which)
      )
        return;
