...
var GlideFilterHandler = Class.create({
  ...
  destroy: function () {
    if (this.tr) {
      if (this.tr.operSel) this.tr.operSel.onchange = null;
      this.tr = null;
    }
  ...
  _initInputs: function () {
    this.inputCnt = 0;
    this.inputs = [];
    for (var i = 0; i < this.maxValues; i++) this.inputs[i] = null;
  },
  _clearValues: function () {
    this.values = [];
    for (var i = 0; i < this.maxValues; i++) this.values[i] = '';
  },
  ...
  _getInputValue: function (el) {
    ...
    if (el.tagName.toUpperCase() == 'TEXTAREA') return el.value;
    ...
    if (el.multiple) {
      ...
      for (var i = 0; i < options.length; i++) {
        if (options[i].selected) vals.push(options[i].value);
      }
      return vals.join(',');
    }
  ...
  _addTextInput: function (type, td, label) {
    if (!td) td = this.tr.tdValue;
    if (!this._isEmptyOper()) {
      if (this.tr.tdMapping) this.tr.tdMapping.show();
    }
    var associatedField = this.item && this.item.label;
    var input = addTextInput(
      td,
      this.values[this.inputCnt],
      type,
      associatedField,
      label
    );
  ...
  _isTemplate: function () {
    ...
    while (t) {
      ...
      if (t) {
        var id = t.id + '';
        if (id.indexOf('filters_table') != -1) break;
      }
    }
    if (!t) return false;
  ...
  _renderDynamicOperandInput: function (op) {
    if (typeof g_dynamic_filter_options == 'undefined') return false;
    if (op != 'DYNAMIC') return false;
    ...
    for (var i = 0; i < arr.length; i++) {
      ...
      var isExclusiveDynamic =
        typeof gtf_exclusive_dynamics != 'undefined' &&
        gtf_exclusive_dynamics == 'true';
      if (
        (!isExclusiveDynamic &&
          this.item &&
          this.item.getReference() == aItemArr[2]) ||
        this.originalTableName == aItemArr[3]
      ) {
        if (!addedReferenceInput) var input = this._addSelect(180, false, 1);
        var arrInput = [];
        ...
        addOption(
          input,
          aItemArr[0],
          translated[aItemArr[1]],
          this.rightOperand == aItemArr[0]
        );
  ...
  setOriginalTable: function (tableNameFull) {
    ...
    else this.originalTableName = tableNameFull.split('.')[1];
  },
  getValues: function () {
    this._clearValues();
    if (this.inputCnt == 0) return '';
    ...
    if (this.inputCnt == 1) return this.values[0];
  ...
  _populateRightOperandChoices: function () {
    ...
    var table = '';
    if (typeof mainFilterTable == 'undefined') table = this._getFilterTable();
    else table = mainFilterTable;
    ...
    if (null != table && '' != table && null != field && '' != field) {
      ...
      if (selection) completeSelection = table + '.' + selection;
      else completeSelection = table;
      var showRelatedFields = 'no';
      if (
        this.isRightOperandShowingRelatedFields ||
        (selection && selection.indexOf('.') >= 0)
      )
      ...
      this.addChoices(
        ...
        completeSelection,
        '',
        this._filterRightOperand,
        selection,
        null,
        showRelatedFields,
        this.item
      );
      if (null != selection) this.selectRightOperand(s, selection);
    }
  },
  ...
  addChoices: function (
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (
        (!elementDef || elementDef.getType() != 'glide_var') &&
        !item.isReference()
      ) {
        savedItems[label] = t;
    ...
    if (
      ...
      (((fValue == TEXTQUERY || textIndex) && filterMethod == null) ||
        forFilter)
    ) {
  ...
  _addPlaceholderOption: function (s, tableDef, messages) {
    var o = addOption(s, PLACEHOLDER, messages['-- choose field --'], true);
    o.style.color = 'blue';
    o.tableName = tableDef.getName();
    o.fullLabel = messages['-- choose field --'];
  },
  _filterRightOperand: function (
    leftOperandElement,
    rightOperandElement,
    isRightOperandShowingRelatedFields
  ) {
    var filterOut = true;
    if (null != leftOperandElement && null != rightOperandElement) {
      ...
      if ('reference' != leftType) {
        if ('reference' == rightType && isRightOperandShowingRelatedFields)
          filterOut = false;
        if (leftType == rightType) filterOut = false;
      } else {
        if ('reference' == rightType) filterOut = false;
      }
    }
    return filterOut;
  },
  ...
  _addSameAsLabels: function (t, op) {
    if (op == 'SAMEAS' || op == 'NSAMEAS') {
      ...
      if (op == 'SAMEAS') span.innerHTML = ASMSG;
      else span.innerHTML = FROMMSG;
    }
  },
  ...
  destroyMappingSupport: function () {
    if (
      this._parentMappingMgr &&
      typeof this._parentMappingMgr.destroyElement === 'function'
    )
  ...
  activateMapping: function () {
    ...
    if (
      this._parentMappingMgr &&
      typeof this._parentMappingMgr.notifyStateChange === 'function'
    )
...
var GlideFilterString = Class.create(GlideFilterHandler, {
  ...
  _build: function () {
    ...
    if (this._isEmptyOper()) {
      ...
      if (this.tr.tdMapping) this.tr.tdMapping.hide();
      return;
    }
    var op = this._getOperator();
    if (this._renderDynamicOperandInput(op)) return;
  ...
  _initValues: function (values) {
    ...
    if (op == 'BETWEEN') {
      ...
      else this.values = this._parseValues(values, '@');
      for (var i = this.values.length; i < this.maxValues; i++)
        this.values[i] = '';
    } else {
      this.values = [];
...
var GlideFilterDuration = Class.create(GlideFilterHandler, {
  _setup: function (values) {
    ...
    if (this._getOperator() == 'BETWEEN') {
      ...
      else this.durationTo = new GlideDuration(valueArray[0], this.item);
    }
    this.listenForOperChange = true;
  },
  destroy: function () {
    if (this.tr) this.tr.tdValue.innerHTML = '';
    this.inputCnt = 0;
  ...
  _build: function () {
    ...
    if (this._isEmptyOper()) {
      ...
      if (this.tr.tdMapping) this.tr.tdMapping.hide();
      return;
    } else if (this.tr.tdMapping) this.tr.tdMapping.show();
    if (this._renderRightOperandAsFieldList(this._getOperator())) {
      ...
      if (this._getOperator() == 'BETWEEN' && this.maxValues == 2) {
        var txt = document.createTextNode(' ' + getMessage('and') + ' ');
        this.tr.tdValue.appendChild(txt);
        if (!this.durationTo) this.durationTo = new GlideDuration();
...
var GlideFilterStringMulti = Class.create(GlideFilterString, {
  ...
  _build: function () {
    ...
    if (this._renderDynamicOperandInput(this._getOperator())) return;
    if (this._renderRightOperandAsFieldList(this._getOperator())) {
      this._addSameAsLabels(this, this._getOperator());
      this._populateRightOperandChoices();
    } else this._addTextArea();
  },
...
var GlideFilterChoice = Class.create(GlideFilterHandler, {
  ...
  _fillSelect: function (inputIndex) {
    ...
    if (!this._isEmptyOper() && this.tr.tdMapping) this.tr.tdMapping.show();
    var vars = {};
    ...
    if (oper == 'IN' || oper == 'NOT IN') removeNone = true;
    if (isMSIE && this.inputs[0].multiple) {
      var isIE6 = /MSIE 6/.test(navigator.userAgent);
      if (isIE6) this.inputs[0].focus();
    }
    for (var i = 0; i < this.choices.length; i++) {
      var option = this.choices[i];
      if (option[0] == '' && removeNone) continue;
      var selected =
        vars[option[0]] != null || this.values[inputIndex] == option[0];
...
var GlideFilterChoiceDynamic = Class.create(GlideFilterChoice, {
  ...
  _build: function () {
    ...
    if (this._renderRightOperandAsFieldList(oper)) {
      ...
      if (
        this.prevOper &&
        this.prevOper != 'LIKE' &&
        this.prevOper != 'STARTSWITH' &&
        this.prevOper != 'ENDSWITH'
      )
        this._clearValues();
      this._addTextInput();
  ...
  _getChoices: function () {
    if (
      ...
      g_filter_description.getChoiceList(this.tr.tableField) != null
    ) {
      var response = loadXML(
        g_filter_description.getChoiceList(this.tr.tableField)
      );
      ...
      if (domain) ajax.addParam('sysparm_domain', domain.value);
  ...
  _addChoices: function (xml) {
    ...
    for (var i = 0; i < items.length; i++) {
      var item = items[i];
      if (item.getAttribute('value') == '') addNone = false;
    }
    if (addNone) this.choices[0] = ['', msg.getMessage('-- None --')];
    for (var i = 0; i < items.length; i++) {
      var item = items[i];
    ...
    if (dep != null) {
      this.choices = this.choices.sort(function (a, b) {
...
var GlideFilterCurrency = Class.create(GlideFilterString, {
  ...
  getValues: function () {
    ...
    var valList = v.split('@');
    var fromVal =
      "javascript:global.getCurrencyFilter('" +
      tn +
      "','" +
      fn +
      "', '" +
      this.currency_widget.value +
      ';' +
      valList[0] +
      "')";
    if (
      (valList.length > 1 && this._getOperator() == 'BETWEEN') ||
      valList.length > 2
    )
      return (
        fromVal +
