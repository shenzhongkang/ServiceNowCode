...
var GlideTimeElement = Class.create({
  ...
  setValue: function (value, displayValue) {
    ...
    if (value == '00:00:00') {
      ...
      if (!timeObject) {
        this.log(
          "setValue aborted; unable to interpret time '" +
            value +
            "' for format '" +
            dateFormat +
            "'"
    ...
    if (ampmw) if (ampm) ampmw.value = ampm;
  ...
  _LZ: function (val, padded) {
    if (!padded) {
      if (val.length > 1) if (val.charAt(0) == '0') val = val.substring(1);
      return val;
    }
    val += '';
  ...
  _getInteger: function (val) {
    ...
    for (var i = 0; i < val.length; i++) {
      if (digits.indexOf(val.charAt(i)) == -1) return val.substring(0, i);
    }
    return val;
  },
  log: function (msg) {
...
GlideTimeElement.updateRealTime = function (ref) {
  ...
  if (dateFormat.indexOf('a') > -1) {
    ...
    if (ampmw) ampm = ampmw.value;
  }
  if (dateFormat.indexOf('s') > -1) {
    var secondsInput = gel(ref + 'dur_sec');
    if (secondsInput) sec = secondsInput.value;
  }
  sdata.value = GlideTimeElement.formatTime(
    parseInt(hour.value || 0, 10),
...
GlideTimeElement.formatTime = function (hour, minute, second, ampm, format) {
  ...
  value['H'] = hour;
  value['HH'] = LZ(hour);
  if (hour == 0) value['h'] = 12;
  else if (hour > 12) value['h'] = hour - 12;
  else value['h'] = hour;
  value['hh'] = LZ(value['h']);
  value['a'] = ampm;
  value['aa'] = ampm;
