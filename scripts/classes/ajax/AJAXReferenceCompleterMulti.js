...
var AJAXReferenceCompleterMulti = Class.create(AJAXReferenceCompleter, {
  ...
  _rebuildFromInput: function () {
    ...
    for (var i = 0; i < arr.length; i++) {
      ...
      if (this._hash.keys().indexOf(a) != -1)
        ...
        if (this.allowInvalid) _hashNew.set(a, a.escapeHTML());
      }
    }
    this._hash = _hashNew;
    this._setFormValues();
    ...
    if (this.element.getAttribute('reference') == 'sys_user') {
      ...
      for (i = 0; i < keyContacts.length; i++) {
        if (
          !matchedRef &&
          keyContacts[i] != '' &&
          !isEmailValid(keyContacts[i]) &&
          !this._isGEMValue(keyContacts[i])
        ) {
          ...
          if (!hexSysId) {
            ...
            g_form.showFieldMsg(
              ...
              'Please enter a valid email address or User',
              'error'
            );
  ...
  _resolveReference: function (sys_id, displayValue) {
    ...
    ajax.getXML(
      this._resolveReferenceResponse.bind(this, sys_id, displayValue)
    );
  },
  ...
  setDropDownSize: function () {
    var e = this.element;
    var mLeft =
      grabOffsetLeft(e) + this.getDisplayElement().value.length * 5 + 'px';
