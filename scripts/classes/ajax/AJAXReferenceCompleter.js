...
var AJAXReferenceCompleter = Class.create(AJAXCompleter, {
  ...
  initialize: function (
    ...
    this.dynamicCreate =
      this.element.readAttribute('data-ref-dynamic') == 'true';
    this.isList = this.element.readAttribute('islist') == 'true';
    if (!this.simpleQualifier) this.refQual = '';
    this.isFilterUsingContains =
      this.element.readAttribute('is_filter_using_contains') == 'true';
  ...
  keyDown: function (evt) {
    ...
    if (typedChar == KEY_ARROWUP) {
      if (!this.selectPrevious()) this.hideDropDown();
      ...
      if (this.hasDropDown() && this.select()) this.clearTimeout();
      else this.onBlur();
    } else if (typedChar == KEY_TAB && window.g_accessibility) {
  ...
  keyUp: function (evt) {
    var typedChar = getKeyCode(evt);
    if (!this.isDeleteKey(typedChar)) return;
    this.clearTimeout();
  ...
  keyPress: function (eventArg) {
    ...
    var typedChar = getKeyCode(evt);
    if (typedChar != KEY_ENTER && typedChar != KEY_RETURN) this.clearTimeout();
    if (this.isNavigation(typedChar)) return true;
    if (this.isDeleteKey(typedChar)) return true;
  ...
  isNavigation: function (typedChar) {
    if (typedChar == this.KEY_TAB) return true;
    if (typedChar == this.KEY_LEFT) return true;
    if (typedChar == this.KEY_RIGHT) return true;
  },
  ...
  _getSearchChars: function () {
    ...
    else return this.getDisplayValue();
  },
  ...
  _translateDoubleByteIntoSingleByte: function (s) {
    ...
    for (var i = 0, l = s.length, char; i < l; i++) {
      char = s.charCodeAt(i);
      if (char == 12288) str += String.fromCharCode(32);
  ...
  ajaxResponse: function (response) {
    ...
    this.processXML(xml);
    this.isResolvingFlag = false;
    if (this.onResolveCallback) this.onResolveCallback();
  },
  ...
  _processBlurValue: function (values, recents) {
    ...
    if (!this.oneMatchSelects || this.getDisplayValue() === '') return;
    ...
    if (targetLabel) this.referenceSelect(targetValue, targetLabel);
  },
  ...
  addSysParms: function () {
    ...
    if (this.elementName.indexOf('IO:') > -1)
      name = this.elementName.substring(
        this.elementName.indexOf('IO:'),
        this.elementName.length
      );
    var sp =
      'sysparm_name=' +
      name +
      ...
      this.max +
      '&sysparm_chars=' +
      encodeText(this.searchChars);
    if (this.guid) sp += '&sysparm_completer_id=' + this.guid;
    if (this.ignoreRefQual) sp += '&sysparm_ignore_ref_qual=true';
    else if (this.refQual != '' && typeof this.refQual != 'undefined')
      sp += '&sysparm_ref_qual=' + this.refQual;
    var domain = gel('sysparm_domain');
    if (domain) sp += '&sysparm_domain=' + domain.value;
    return sp;
  },
  ...
  createDropDown: function (foundStrings, foundRecents) {
    ...
    if (foundStrings && foundStrings.length > 0) {
      ...
      for (var c = 0; c < foundStrings.length; c++) {
        if (this.max > 0 && c >= this.max) break;
    ...
    if (this.currentMenuCount) {
      ...
      if (isTextDirectionRTL()) {
        ...
        if (
          ...
          isMSIE7 ||
          isMSIE6 ||
          (isMSIE9 && getPreference('glide.ui11.use') == 'false')
        ) {
          if (typeof g_form != 'undefined')
            w = this.element.offsetParent
              ? this.element.offsetParent.clientWidth
              : 0;
        }
        this.dropDown.style.left =
          parseInt(this.dropDown.style.left) - diff + w + 'px';
        this.iFrame.style.left =
          parseInt(this.iFrame.style.left) - diff + w + 'px';
      ...
      this.firefoxBump();
      var msg =
        '{0} suggestions. Please use the up and down arrow keys to select a value';
      ...
      messageAPI.fetch(
        [msg],
        function (msgs) {
          var msgWithValues = messageAPI.format(
            msgs[msg],
            this.currentMenuCount
          );
  ...
  _showMax: function (foundStrings, foundRecents) {
    if (foundRecents && foundRecents.length > 0) this._addHeaderRow('Search');
  },
  ...
  select: function () {
    if (this.selectedItemNum < 0) return false;
  ...
  referenceSelectTimeout: function (sys_id, displayValue) {
    ...
    if (typeof reflistModalPick == 'function') this._referenceSelectTimeout();
    else setTimeout(this._referenceSelectTimeout.bind(this), 0);
  },
  ...
  referenceSelect: function (sys_id, displayValue, referenceInvalid) {
    ...
    var glideListHasItem =
      e && e.value && sys_id && 'glide-list' === e.getAttribute('data-type')
        ? !!e.value.match(new RegExp('(^|,)(' + sys_id + ')(,|$)', 'gm'))
        : false;
    ...
    this.showViewImage();
    if (!referenceInvalid) this.clearInvalid();
    this._clearDerivedFields();
    if (this.selectionCallBack && sys_id) {
      if (!glideListHasItem) eval(this.selectionCallBack);
      else if (this.getDisplayElement()) this.getDisplayElement().value = '';
    }
    if (e['filterCallBack']) {
      e.filterCallBack();
    }
  ...
  setDropDownSize: function () {
    ...
    if (window.$j) {
      ...
      var parentScrolltop =
        parentIsBody || parent.css('overflow') == 'visible'
          ? 0
          : parent.scrollTop();
      if (!parentIsBody) parentOffset = parent.offset();
    ...
    if (dd.offsetWidth > parseInt(mWidth)) mWidth = dd.offsetWidth;
  ...
  onFocus: function () {
    if (this.ignoreFocusEvent || this.element.getAttribute('readonly')) return;
    this.hasFocus = true;
    this.currentDisplayValue = this.getDisplayValue();
    this.currentRefValue = this.getKeyValue();
    this._setUpDocMouseDown();
  ...
  onBlurEvent: function (evt) {
    if (this.element.getAttribute('readonly')) return;
  ...
  onBlur: function () {
    ...
    if (this.getDisplayValue().length == 0) {
      ...
      if (!this.isFilterUsingContains) {
        ...
        } else if (this.currentRefValue != this.keyElement.value) {
          if (
            this.keyElement.id.indexOf('LIST_EDIT') == -1 ||
            this.currentRefValue != ''
          )
            refInvalid = false;
        }
        if (refInvalid) this.setInvalid();
    ...
    if (this.isPopulated()) {
      if (this.getMenuCount() == 1) {
        ...
        if (
          o0['label']
            .toUpperCase()
            .startsWith(this.getDisplayValue().toUpperCase())
        )
          return true;
        return false;
      }
      ...
      if (
        o0['label'] == this.getDisplayValue() &&
        o1['label'] != this.getDisplayValue()
      )
        return true;
    }
  },
  ...
  setDependent: function (dependentReference) {
    ...
    var h = new GlideEventHandler(
      'onChange_' + n,
      this.onDependentChange.bind(this),
      dependentReference
    );
  ...
  addDependentValue: function () {
    ...
    if (!el) {
      if (
        ...
        this.element.hasAttribute('data-dependent-value')
      ) {
        return (
          '&sysparm_value=' + this.element.readAttribute('data-dependent-value')
        );
      } else {
        return '';
      }
    ...
    var depValue = '';
    if (el.tagName == 'INPUT') depValue = el.value;
    else depValue = el.options[el.selectedIndex].value;
    return '&sysparm_value=' + depValue;
  },
  setRefQualElements: function (elements) {
    this.simpleQualifier = false;
    if (!elements) this.refQualElements = null;
    else {
      ...
      if (a == '*') {
        ...
        for (var i = 0; i < elements.length; i++) {
          if (
            elements[i].id != this.keyElement.id &&
            elements[i].id.startsWith(tableDot)
          )
  ...
  firefoxBump: function () {
    ...
    for (var i = 0; i < children.length; i++) {
      if (children[i] && children[i].firstChild) {
        ...
        if (dchildSpanWidth > dparentDivWidth) this.setWidth(dchildSpanWidth);
      }
    }
  },
  ...
  cachePut: function (name, value) {
    if (this.refQualElements) return;
  ...
  cacheEmpty: function () {
    ...
    while (s.length > 2) {
      ...
      if (rowCount == 0 && e.childElementCount == 0) return true;
      break;
    }
    return false;
  },
