...
var AJAXTableCompleter = Class.create(AJAXReferenceCompleter, {
  ...
  createInnerDropDown: function () {
    if (this.dropDown.childNodes.length > 0) return;
    this._createTable();
  },
  ...
  _addColumns: function (tr, item) {
    ...
    for (var i = 0; i < fields.length; i++) {
      ...
      if (this.prevText[i] == value) td.addClassName('ac_additional_repeat');
      else this.prevText[i] = value;
  ...
  _showMax: function (foundStrings, foundRecents) {
    ...
    if (this.rowCount >= 250) {
      a.innerHTML = new GwtMessage().getMessage(
        'Showing 1 through {0} of more than {1}',
        showing,
        total
      );
    } else {
      a.innerHTML = new GwtMessage().getMessage(
        'Showing 1 through {0} of {1}',
        showing,
        total
      );
    }
