...
var AJAXCompleter = Class.create({
  ...
  setHeight: function (height) {
    ...
    if (g_isInternetExplorer) height += this.ieIFrameAdjust;
  ...
  hideDropDown: function () {
    if (this.dropDown.style.visibility == 'hidden') return;
  ...
  showDropDown: function () {
    if (this.dropDown.style.visibility == 'visible') return;
  ...
  appendItem: function (item) {
    ...
    if (this.currentMenuItems == null) this.currentMenuItems = [];
  ...
  selectNext: function () {
    ...
    if (this.selectedItemNum < this.getMenuCount() - 1) itemNumber++;
  ...
  unsetSelection: function () {
    if (this.selectedItemNum == -1) return;
