...
var GlideAjax = Class.create(GlideURL, {
  ...
  initialize: function initialize($super, processor, url) {
    ...
    if (typeof GlideTransactionScope != 'undefined')
      GlideTransactionScope.appendTransactionScope(
  ...
  getAnswer: function () {
    if (!(this.requestObject && this.requestObject.responseXML)) return null;
    return this.requestObject.responseXML.documentElement.getAttribute(
      'answer'
    );
  },
  ...
  getXMLWait: function (additionalParams) {
    ...
    this._makeRequest();
    var m =
      '*** WARNING *** GlideAjax.getXMLWait - synchronous function - processor: ' +
      this.processor;
    sw.jslog(m);
    if (window.console && window.console.log)
      console.log('%c ' + m, 'background: darkred; color: white;');
    if (this.requestObject.status != 200) this._handleError();
  ...
  _makeRequest: function () {
    ...
    if (this.requestObject == null) return null;
    if (!GlideAjax.want_session_messages) this.setWantSessionMessages(false);
    ...
    if (this.queryString) {
      if (this.postString) this.postString += '&';
      this.postString += this.queryString;
    }
    if (this._isEligibleForBatching()) this._enqueueBatchRequest();
  ...
  _enqueueBatchRequest: function () {
    if (window.NOW && window.NOW.GlideAjaxBatchRequestQueue)
      ...
      console.warn(
        'GlideAjaxBatchRequestQueue not available, falling back to immediate dispatch'
      );
      console.debug(
        'Add scripts/ga_batch/js_includes_batchga.js to your page to enable batching'
      );
      this._sendRequest();
    }
  },
  _sendRequest: function () {
    ...
    this.requestObject.setRequestHeader(
      'Content-Type',
    ...
    if (!this.async || this.callbackFunction == null) this._hideLoading();
  },
  _processReqChange: function () {
    if (this.requestObject.readyState != 4) return;
  ...
  _handle401Error: function () {
    if (getTopWindow().loggingOut) return false;
    var sessionLoggedIn =
      this.requestObject.getResponseHeader('X-SessionLoggedIn');
    if ('true' != sessionLoggedIn) {
      if (
        window.confirm(
          new GwtMessage().getMessage('ajax_session_timeout_goto_login_confirm')
        )
      ) {
      ...
      var allowResubmit = this.requestObject.getResponseHeader(
        'X-UserToken-AllowResubmit'
      );
      if ('true' == allowResubmit) {
        var autoResubmit =
          this.requestObject.getResponseHeader('X-AutoResubmit');
        ...
        if (
          window.confirm(
            new GwtMessage().getMessage(
              'ajax_session_timeout_resubmit_request_confirm'
            )
          )
        ) {
          this._sendRequest();
        ...
        if ('true' == sessionLoggedIn) return false;
        var msg = new GwtMessage().getMessage(
          'ajax_session_timeout_refresh_screen'
        );
        if (!msg) msg = 'Your session has expired. Click OK to continue.';
  ...
  _handleError: function () {
    ...
    if (responseCode == 401) {
      var requestedToken = this.requestObject.getResponseHeader(
        'X-UserToken-Request'
      );
      var respondedToken = this.requestObject.getResponseHeader(
        'X-UserToken-Response'
      );
      if (requestedToken && respondedToken && requestedToken != respondedToken)
        CustomEvent.fireAll('ck_updated', respondedToken);
      var handleTimeOut =
        this.requestObject.getResponseHeader('X-HandleTimeOut');
      if ('true' == handleTimeOut) if (this._handle401Error()) return;
    } else if (responseCode == 404 && this._outOfScope()) {
      var err_options = {
        text:
          'Access to Script Include ' +
          this.processor +
          ' blocked from scope: ' +
      ...
      this._wasCanceled() &&
      this.callbackFunction &&
      !this._getSuppressCancelNotification()
    ) {
      var err_options = {
  ...
  _outOfScope: function () {
    var callerScope = this.getParam('sysparm_scope')
      ? this.getParam('sysparm_scope')
      : 'global';
    var isAppScope = callerScope != 'global';
    return (
      isAppScope &&
      this.requestObject.responseXML.documentElement
        .getAttribute('error')
        .indexOf('HTTP Processor class not found') == 0
    );
  },
  _responseReceived: function () {
    ...
    if (this.callbackFunction) {
      ...
      else this.callbackFunction(this.requestObject, this.callbackArgs);
    }
    if (this.wantRequestObject) return this.requestObject;
  ...
  _fireUINotifications: function () {
    ...
    var notifications =
      this.requestObject.responseXML.getElementsByTagName('ui_notifications');
  ...
  _showSessionNotifications: function () {
    if (!this.requestObject || !this.requestObject.responseXML) return;
    var notifications = this.requestObject.responseXML.getElementsByTagName(
