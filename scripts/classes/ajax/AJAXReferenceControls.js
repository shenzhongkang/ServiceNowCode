...
var AJAXReferenceControls = Class.create({
  ...
  clearDropDown: function () {
    if (this.ac) this.ac.clearDropDown();
  },
  ...
  createInput: function (parentElement) {
    ...
    input = this.input;
    if (doctype) input.className = 'form-control list-edit-input';
    ...
    input.setAttribute(
      'data-ref-dynamic',
      this.tableElement.isDynamicCreation()
    );
  },
  resolveReference: function () {
    if (this.ac) this.ac.onBlur();
  },
  ...
  createLookup: function (parent) {
    ...
    if (doctype) image = $(createIcon('icon-search', title));
    image.width = 18;
    image.height = 16;
  ...
  createDependent: function (parent) {
    if (!this.tableElement.isDependent()) return;
    var input = cel('input');
    input.type = 'hidden';
  ...
  createInputGroup: function (parent) {
    if (document.documentElement.getAttribute('data-doctype') != 'true') return;
  ...
  _setDependent: function () {
    if (this.dependent == null) return;
    var value = this.record.getValue(this.tableElement.getDependent());
    if ('NULL' === value) this.dependentInput.value = '';
    else this.dependentInput.value = value;
  },
  _onFocus: function (evt) {
    if (this.ac) return;
    this._setDependent();
    var dep = '';
    if (this.dependentInput) dep = 'sys_dependent';
    var referenceValid = true;
    if (this.record && this.record.isReferenceValid)
      referenceValid = this.record.isReferenceValid();
    this.ac = new AJAXTableCompleter(
      this.input,
      this.id,
      dep,
      null,
