...
var AJAXEmailClientCompleter = Class.create(AJAXTableCompleter, {
  ...
  keyPress: function (evtArg) {
    ...
    if (
      !evt.shiftKey &&
      (typedChar == KEY_ARROWDOWN || typedChar == KEY_ARROWUP)
    )
      return false;
    if (this.isDeleteKey(typedChar)) return true;
    if (
      typedChar == KEY_ENTER ||
      typedChar == KEY_RETURN ||
      typedChar == KEY_TAB
    ) {
      if (this.hasDropDown() && this.select()) this.clearTimeout();
      else {
        this.onBlur();
        if (typedChar != KEY_TAB) this.hasFocus = true;
      }
      return false;
    }
  ...
  isNavigation: function (typedChar) {
    ...
    if (typedChar == KEY_TAB && this.currentDisplayValue == '') return true;
  },
  ...
  processXML: function (xml) {
    ...
    if (!this.hasFocus) {
      ...
      if (
        ...
        this.log(
          'setting value without focus to ' +
            values[0]['label'] +
            '->' +
            values[0]['name']
        );
        ...
        selectionObj.label =
          values[0]['label'] ||
          this.getEmailAddressFromACResultXML(values[0].XML);
        var addressFilterIds = this.getAddressFilterIds();
        if (addressFilterIds)
          this.proccessXmlResultAndValidateAgainstFilters(
            selectionObj,
            xml,
            addressFilterIds
          );
        else this.renderSelection(selectionObj.sysId, selectionObj.label);
      } else {
        if (e.getAttribute('allow_invalid') != 'true') this.setInvalid();
      }
      return;
    }
  ...
  select: function () {
    ...
    selectionObj.label =
      acItem['label'] || this.getEmailAddressFromACResultXML(acItem.XML);
    var addressFilterIds = this.getAddressFilterIds();
    if (addressFilterIds)
      this.proccessXmlResultAndValidateAgainstFilters(
        selectionObj,
  ...
  renderSelection: function (sysId, label) {
    ...
    if (this.renderItemTemplate) {
      ...
      s.innerHTML =
        s.innerHTML +
        '<span class="address" tabindex="-1" onclick="addressOnClick(event, this)" style="white-space:nowrap;" value="' +
        sysId +
        '" aria-label="' +
        label +
        '">' +
  ...
  validateAgainstFilters: function (selectionObj, email, addressFilters) {
    ...
    ga.getXML(
      this.validateAgainstFiltersCallback.bind(this),
      null,
      selectionObj
    );
  },
  validateAgainstFiltersCallback: function (answer, requestParams) {
    var resultXml =
      answer.responseXML.documentElement.getElementsByTagName('result');
    if (resultXml && resultXml.length != 0) {
      ...
      } else this.renderInvalidFromFilterResult(result);
    }
  },
  proccessXmlResultAndValidateAgainstFilters: function (
    selectionObj,
    resultXml,
    addressFilters
  ) {
  ...
  renderInvalidFromFilterResult: function (result) {
    ...
    if (resizeInput) resizeInput(this.element);
  ...
  onBlur: function () {
    ...
    if (this.getDisplayValue().length == 0) {
      if (this.currentDisplayValue != '') this.referenceSelect('', '');
      ...
      if (this.isExactMatch()) {
        ...
        selectionObj.label =
          o['label'] || this.getEmailAddressFromACResultXML(o.XML);
        var addressFilterIds = this.getAddressFilterIds();
        if (addressFilterIds)
          this.proccessXmlResultAndValidateAgainstFilters(
            selectionObj,
      ...
      if (refInvalid || !this.isPopulated()) {
        this.log(
          'onBlur with no menu items requesting the reference for ' +
  ...
  isExactMatch: function () {
    if (this.isPopulated()) {
      if (this.getMenuCount() == 1) {
        ...
        if (displayValue) displayValue = displayValue.toLocaleLowerCase();
        if (label) label = label.toLocaleLowerCase();
        if (label == displayValue) return true;
        return false;
      }
  ...
  setTitle: function (message) {
    ...
    else this.getDisplayElement().setAttribute('data-original-title', message);
    this.getDisplayElement().title = message;
    var srInvalidMsg = document.getElementById('sr_invalid_msg');
    if (srInvalidMsg) srInvalidMsg.innerText = message;
  },
