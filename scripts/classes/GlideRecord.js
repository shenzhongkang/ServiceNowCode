...
var GlideRecord = Class.create({
  ...
  deleteRecord: function (responseFunction) {
    ...
    if (typeof responseFunction == 'undefined') {
      ...
      sw.jslog(
        '*** WARNING *** GlideRecord synchronous query for table: ' +
          this.getTableName()
      );
    }
    ...
    for (var i = 0; i < r.length; i++) {
      ...
      if (this.isDotWalkField(name)) {
        ...
        for (var p = 0; p < parts.length - 1; p++) {
          var part = parts[p];
          if (typeof start[part] != 'object') start[part] = new Object();
          start = start[part];
        }
  ...
  isDotWalkField: function (name) {
    for (var i = 0; i < this.displayFields.length; i++) {
      ...
      if (name == encodedFieldName) return true;
    }
    return false;
  },
  ...
  update: function (responseFunction) {
    ...
    if (typeof responseFunction == 'undefined') {
      var sw = new StopWatch();
      sw.jslog(
        '*** WARNING *** GlideRecord synchronous update or insert for table: ' +
          this.getTableName()
      );
      var rxml = ajax.getXMLWait();
      return this._updateResponse(rxml);
    }
    if (typeof responseFunction != 'function') responseFunction = doNothing;
  ...
  _updateResponse: function (rxml) {
    var items = rxml.getElementsByTagName('item');
    if (items && items.length > 0) return getTextValue(items[0]);
  },
  ...
  _getXMLSerialized: function () {
    ...
    if (this.sys_id) root.setAttribute('id', this.sys_id);
