/*! RESOURCE: /scripts/classes/GenericHierarchy.js */
var GenericHierarchy = Class.create(GwtDiagram, {
  ...
  _pathRouterFactory: function () {
    ...
    else return new GwtManhattanRouteCurve();
  },
  load: function (attributes) {
    if (attributes)
      for (var i = 0; i < attributes.length; i++) {
        var attr = attributes[i].split('=');
        if (attr.length == 1) attr[1] = '';
        this.setParam(attr[0], attr[1]);
        if (attr[0] == 'id') this.id = attr[1];
        else if (attr[0] == 'baseid') this.baseid = attr[1];
        else if (attr[0] == 'nocontext') this.contextMenus = false;
      }
    ...
    if (fontSize) layout.setDefaultNodeFontSize(fontSize);
    ...
    } else layout.setDefaultNodeSize(200, 50);
...
function reloadSelf(newBaseId, idName) {
  ...
  if (p2 > -1) sysparmContent = sysparmContent.substring(0, p2);
  var attributes = '';
