...
var GlideFilterVariables = Class.create(GlideFilterHandler, {
  ...
  _initValues: function (values) {
    ...
    if (values) {
      ...
      var query = new GlideEncodedQuery(
        this.tableName,
        'IO:' + this.values[0] + this.refQuery,
        null
      );
      query.parse();
  ...
  _hideOperators: function () {
    if (this.tr.tdOper) this.tr.tdOper.style.display = 'none';
  },
  _refListOpen: function () {
    if (this.values[2])
      reflistOpen(
        this.id,
        this.item.getName(),
        this.item.getReference(),
        '',
        false,
        '',
        'cat_item=' + this.variableRefQual
      );
    else reflistOpen(this.id, this.item.getName(), this.item.getReference());
    return false;
  },
  ...
  getFilterText: function (oper) {
    ...
    return (
      'variables.' + this.getValues() + oper + this.variableFilter.getValues()
    );
  },
  _useDisplayValue: function (oper) {
    return (
      this.item.getType() != 'glide_list' &&
      oper != '=' &&
      oper != '!=' &&
      oper != 'CHANGESFROM' &&
      oper != 'CHANGESTO'
  ...
  _onFocus: function (evt) {
    if (!this.inputs[1].ac) {
      ...
      if (this.values[2])
        this.inputs[1].ac.setAdditionalValue(
          'sysparm_ref_qual',
          'cat_item=' + this.variableRefQual
        );
      else this.inputs[1].ac.setAdditionalValue('sysparm_ref_qual', '');
  ...
  _refValueChange: function () {
    ...
    this._clearQuery();
    this.values[0] = id;
    if (id) this._getVariableType(id);
  },
  _catValueSet: function (catValue) {
    if (this.values[2] == catValue) return;
  ...
  _filterVariableReference: function () {
    this._onFocus();
    if (this.values[2]) this._setQualifier(this.inputs[1].ac, this.values[2]);
    else this.inputs[1].ac.setAdditionalValue('sysparm_ref_qual', '');
    this._clearQuery();
  },
  _setQualifier: function (ac, catItem) {
    if (ac) ac.setAdditionalValue('sysparm_ref_qual', 'cat_item=' + catItem);
  ...
  _getSetResponse: function (ac, catItem, sets) {
    if (!sets.hasNext()) return;
    var setList = new Array();
    while (sets.next()) setList.push(sets.variable_set + '');
    this.variableRefQual =
      catItem + '^OR' + 'variable_setIN' + setList.join(',');
    if (ac)
      ac.setAdditionalValue(
        'sysparm_ref_qual',
        'cat_item=' + this.variableRefQual
      );
  },
  ...
  _getVariableResponse: function () {
    ...
    } else this._buildType(this.grVariable);
  ...
  _setFilterBasedOnType: function () {
    ...
    if (this.tr.tdValue) this.tr.removeChild(this.tr.tdValue);
    td = conditionRow.addTD(this.tr, this.queryID);
  ...
  _buildType: function (gr) {
    ...
    if (gr.type == '1') this.vItem.setType('boolean');
    else if (gr.type == '8') {
  ...
  _getHandler: function () {
    ...
    if (this.vItem.type == 'multiple_choice') {
      ...
      var handler = new GlideFilterVariablesChoiceDynamic(
        'var.IO:' + this.values[0],
        this.item
      );
      return handler;
    }
    ...
    if (this.vItem.type == 'reference') {
      ...
      var handler = new GlideFilterReference(
        'var.IO:' + this.values[0],
        this.vItem
      );
      return handler;
  ...
  _onFocusCat: function (evt) {
    if (!this.inputs[3].ac) {
      this.inputs[3].ac = new AJAXTableCompleter(
        this.inputs[3],
        this.catID,
        ''
      );
      ...
      this.inputs[3].ac.setAdditionalValue(
        'ac_columns',
