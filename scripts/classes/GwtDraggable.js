...
var GwtDraggable = Class.create(GwtObservable, {
  initialize: function (header, itemDragged) {
    this.header = $(header);
    if (!itemDragged) itemDragged = this.header;
  ...
  start: function (e) {
    ...
    this.differenceX =
      ex - grabOffsetLeft(this.draggable) + grabScrollLeft(this.draggable);
    this.differenceY =
      ey - grabOffsetTop(this.draggable) + grabScrollTop(this.draggable);
  ...
  _restoreSelection: function () {
    if (this.parentElement) restoreSelection(this.parentElement);
  },
  z: function () {},
});
function createPageShim(parentElement) {
  if (!parentElement) return;
  var w = parentElement.scrollWidth
    ? parentElement.scrollWidth
    : parentElement.clientWidth;
  var h = parentElement.scrollHeight
...
function removePageShim(parentElement) {
  ...
  if (pageShim) parentElement.removeChild(pageShim);
}
