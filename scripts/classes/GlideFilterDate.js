...
var GlideFilterDate = Class.create(GlideFilterHandler, {
  ...
  _setup: function (values) {
    ...
    this.userTimeFormat = g_user_date_time_format.substring(
      g_user_date_format.length + 1
    );
  ...
  _build: function () {
    ...
    if (this.tableName) path = path + '?table_name=' + this.tableName;
    ...
    var responseFunction = function (request) {
      ...
      calendar.TRENDVALUES_WITH_FIELDS_PLURAL =
        result.trendValuesWithFieldsPlural;
  ...
  buildDate: function (dateFilterOptions) {
    ...
    if (window.sysvalues && window.sysvalues.calendar) {
      var getUniqueExternalDates = this.getUniqueExternalDates(
        this.calendar,
        window.sysvalues.calendar
      );
    ...
    if (this.prevOper && this.prevOper != oper) this._clearValues();
    switch (oper) {
      case 'SAMEAS':
      case 'NSAMEAS':
        if (this.getFilterClass() != 'GlideTemplateFilter') {
          this._addDateSelect(
            this.id,
            this.calendar['TRENDVALUES_WITH_FIELDS'],
            this.values[1],
            [1]
          );
          ...
          if (oper == 'SAMEAS') span.innerHTML = this.ASMSG;
          else span.innerHTML = this.FROMMSG;
        }
      ...
      case 'NOTON':
        ...
        if (val) val += '@' + this.values[1] + '@' + this.values[2];
        this._addDateChoices(this.id, val, [0, 1, 2], false);
        break;
      case '<':
      case '>=':
        var pos = 1;
        this._addDateChoices(
          this.id,
          this.values[0],
          [pos],
          this.allowTime,
      ...
      case 'DATEPART':
        ...
        if (this.values[1]) {
          ...
          if (parts.length == 3) trendOper = parts[2].substring(1, 3);
        }
        this._addDateSelect(this.id, this.calendar['RELATIVE'], trendOper, [1]);
        this._addDateSelect(
          this.id + '_2',
          this.calendar['DATEPART'],
          this.values[0],
          [0]
        );
        break;
      case 'RELATIVE':
        ...
        var newOptionsList = this.calendar['RELATIVE'].filter(function (el) {
          return !(
            el[1].match(/[GEL]E/i) &&
            !(thisValues && thisValues[0].match(el[1]))
          );
        });
      ...
      case 'LESSTHAN':
        var input = this._addTextInput();
        if (null == this.values[3] || '' == this.values[3]) this.values[3] = 0;
        if (isMSIE) input.style.marginTop = '-1px';
        input.value = this.values[3];
        input.style.width = '30px';
        ...
        this._addDateSelect(
          this.id + '_2',
          this.calendar['TRENDVALUES_WITH_FIELDS_PLURAL'],
          this.values[1],
          [1]
        );
        this._addDateSelect(
          this.id + '_3',
          this.calendar['WHEN_WITH_FIELDS'],
          this.values[2],
          [1]
        );
        this._populateRightOperandChoices();
      ...
      case 'SINCE':
        ...
        while (base.next()) {
          ...
          if (value == this.values[0]) selected = true;
    ...
    if (
      notOneOf(
        'BETWEEN ANYTHING RELATIVE SINCE MORETHAN LESSTHAN SAMEAS NSAMEAS DATEPART'
      )
    ) {
  ...
  getValues: function () {
    ...
    if (this.supportsMapping) {
      if (this._getOperator() == 'BETWEEN') {
        ...
        if ($in1.val() || $in2.val()) {
          return (
            ($in1.val() ||
              this._getDateGenerate(
                this._convertDate(this.values[0]),
                'start'
              )) +
            '@' +
            ($in2.val() ||
              this._getDateGenerate(this._convertDate(this.values[1]), 'end'))
          );
        }
      } else if (this._getOperator() == 'ON') {
        var gemVal = $j(this.tr.tdValue).find('#em_input_1').val();
        if (gemVal) return gemVal + '@' + gemVal + '@' + gemVal;
      } else {
        var gemVal = $j(this.tr.tdValue).find('#em_input_1').val();
        if (gemVal) {
          return gemVal;
    ...
    switch (this._getOperator()) {
      case 'ON':
      case 'NOTON':
        if (this.values[0].indexOf('@') != -1) return this.values[0];
        var value = this._convertDate(this.values[0]);
        return (
          value +
          '@' +
          this._getDateGenerate(value, 'start') +
          '@' +
          this._getDateGenerate(value, 'end')
        );
      case '<':
      case '<=':
        var value = this._convertDate(this.values[0]);
      ...
      case 'SAMEAS':
      case 'NSAMEAS':
        if (this.values[1] == '') return this.values[0];
        return this.values[1] + '@' + this.values[0];
        break;
      case 'MORETHAN':
      case 'LESSTHAN':
        if (isNaN(this.values[0])) this.values[0] = 0;
        return (
          this.values[3] +
          '@' +
          this.values[1] +
  ...
  _addDateChoices: function (
    ...
    var select = this._addDateSelect(
      id,
      this.calendar,
      matchValue,
      positions,
  ...
  _getDynamicCalendarOption: function () {
    if (
      typeof g_dynamic_filter_options_calendar == 'undefined' ||
      g_dynamic_filter_options_calendar == ''
    )
      return [];
    var dynamicOptions = [];
  ...
  _getDateGenerate: function (value, tag) {
    if (value.indexOf('javascript:') != -1) return value;
    if (isDate(value, this.SYS_DATE_TIME_FORMAT)) {
      var dt = getDateFromFormat(value, this.SYS_DATE_TIME_FORMAT);
  ...
  _calendarPopup: function (id, btn) {
    ...
    if (allowTime) {
      ...
      if (!isDate(currentDate, format)) {
        ...
        if (defaultTime == 'end') dt.setHours(23, 59, 59);
    ...
    if (!input) input = gel(this.id + '_input_1');
  ...
  _dateTimeComplete: function (id) {
    ...
    if (
      isDate(value, this.userDateFormat) ||
      isDate(value, this.userDateTimeFormat)
    ) {
      option.value = input.value;
      option.text = input.value;
    } else addOption(select, input.value, input.value, true);
  },
  initBetweenMapping: function (value1, value2) {
    ...
    this._initMappingComponent($field2, $btn2, id, {
      ...
      parent: {
        show: function () {
          $td
            .find('#cal_2, select[id*="' + id + '_select_' + id + '_2"]')
            .show();
    ...
    $j(this.tr.tdValue)[this.supportsMapping ? 'removeClass' : 'addClass'](
      'gem-unsupported'
    );
