...
var GlideTimerElement = Class.create({
  ...
  setReadOnly: function (readOnly) {
    ...
    g_form.setSensitiveDisplayValue(
      this.name + '.ui_policy_sensitive',
      readOnly ? 'none' : ''
    );
    return true;
  },
  ...
  _setTimer: function () {
    if (!this._timerStarted) this._timerStarted = new Date().getTime();
    if (!this._timerTicks) this._timerTicks = 0;
    var diff =
      new Date().getTime() -
      this._timerStarted -
      this._timerTicks * this._incrementInterval;
    this.timerID = setTimeout(
      this._incrementTimer.bind(this),
      this._incrementInterval - diff
    );
  },
  _incrementTimer: function () {
    if (!this._isPaused()) {
      ...
      if (this._isScrolling)
        this._deferredTime = !!this._deferredTime
          ? this._deferredTime + addedTime
          : currentTime;
      else this.setIncrementingValue(currentTime);
    }
    this._timerTicks++;
    this._setTimer();
  },
  ...
  fields2time: function () {
    ...
    if (eHour.value)
      currentTime =
        60 * 60 * parseInt(isNaN(eHour.value) ? 0 : eHour.value, 10);
    if (eMin.value)
  ...
  setIncrementingValue: function (currentTime) {
    ...
    if (updateTotal) this.updateTotal();
  },
  ...
  duration2timeValue: function (currentTime) {
    var days = 0;
    if (currentTime == 0) currentTime = '00:00:00';
  ...
  _cleanDate: function (dateToClean) {
    if (dateToClean.indexOf(' ') > -1) {
      ...
      if (days.indexOf('-') != -1) days = Math.floor(new Date(days) / 8.64e7);
      dateToClean =
        days + ' ' + dateToClean.substring(sp + 1, dateToClean.length);
    }
    return dateToClean;
  },
  toString: function () {
