...
var Table = Class.create({
  ...
  initialize: function (
    ...
    if (cols && this.cacheable) this.readResponse(cols);
  ...
  readColumns: function () {
    ...
    ajax.addParam(
      'sysparm_apply_template_acls',
      this.applyTemplateAcls ? 'true' : 'false'
    );
    ajax.addParam('sysparm_value', this.tableName);
    if (this.sys_id) ajax.addParam('sysparm_sys_id', this.sys_id);
    if (this.set_id) ajax.addParam('sysparm_set_id', this.set_id);
    if (this.vars_id) ajax.addParam('sysparm_vars_id', this.vars_id);
    if (this.parentTable)
    ...
    if (this.callback) ajax.getXML(this.readColumnsResponse.bind(this));
    else {
  ...
  readResponse: function (xml) {
    ...
    if (root == null || root.length == 0) root = this.columns;
    ...
    for (var i = 0; i < childNodes.length; i++) {
      ...
      if (childNodes[i].tagName == 'tables') this.setTables(childNodes[i]);
      if (childNodes[i].tagName == 'sys_choice_extensions')
        this.setChoiceExtensions(childNodes[i]);
      if (childNodes[i].tagName != 'item') continue;
  ...
  setDependencies: function () {
    for (var i = 0; i < this.elementsArray.length; i++) {
      ...
      if (element.isDependent()) {
        ...
        if (parent) parent.addDependentChild(element.getName());
      }
    }
  },
  ...
  getTableElements: function (tableName) {
    ...
    for (var i = 0; i < items.length; i++) {
      var item = items[i];
      if (item.getTableName() != tableName) continue;
      elements.push(item);
    }
    return elements;
  },
  getElement: function (elementName) {
    if (this.elements[elementName]) return this.elements[elementName];
  ...
  _nameIsExtension: function (name) {
    if (name.indexOf(this.REF_ELEMENT_PREFIX) != 0) return false;
    name = name.substring(this.REF_ELEMENT_PREFIX.length);
...
Table.get = function (
  ...
  return (topWindow.Table || Table).getInTopWindow(
    tableName,
    parentTable,
    isTemplate,
    loadExtensions,
...
Table.setColumns = function (tableName, parentTable, xmlString) {
  ...
  if (parentCache && answer.cacheable) parentCache.put(cachedName, answer);
};
...
Table.getInTopWindow = function (
  ...
  Table.setCachable(t);
  if (t.cacheable) {
    var cachedName = Table.getCacheKey(
      tableName,
      parentTable,
      isTemplate,
      loadExtensions
    ...
    if (parentCache) {
      ...
      if (table) {
        try {
          ...
          console.warn(
            'An error occurred with the Table object or the callback applied to the Table object. ' +
              'Due to limitations in IE11 this is expected when the form is inside the frameset and is ' +
              'refreshed (saved, updated,  etc) and should not affect functionality.',
            error
          );
        }
      }
...
Table.getCacheKey = function (tableName, parentTable, isTemplate) {
  return (
    (parentTable ? parentTable + '.' : '') + tableName + '.' + !!isTemplate
  );
};
Table.getCache = function () {
  var cache = getTopWindow().g_cache_td;
  if (cache) return cache;
  if (!window.g_cache_td) window.g_cache_td = new GlideClientCache(400);
