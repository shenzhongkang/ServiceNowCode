...
var SlushBucket = Class.create({
  ...
  onKeyMoveLeftToRight: function (evt) {
    ...
    if (evt.keyCode != desiredKeyCode) return;
  ...
  moveOptions: function (sourceSelect, targetSelect, copyFlag) {
    ...
    for (var i = 0; i < selectedIds.length; i++) {
      ...
      if (
        ...
        this._isDuplicate(targetOptions, soption.value)
      )
        continue;
    ...
    if (targetSelect['onchange']) targetSelect.onchange();
    if (sourceSelect['onchange']) sourceSelect.onchange();
  ...
  moveUp: function () {
    ...
    for (var i = 0; i < selectedIds.length; i++) {
      ...
      if (selId == 0) break;
      if (window['privateMoveUp']) privateMoveUp(options, selId);
      else this.swap(options[selId], options[selId - 1]);
      options[selId].selected = false;
      options[selId - 1].selected = true;
    }
    this.evenOddColorize();
    sourceSelect.focus();
    if (sourceSelect['onLocalMoveUp']) sourceSelect.onLocalMoveUp();
  ...
  moveDown: function () {
    ...
    for (var i = 0; i < selectedIds.length; i++) {
      var selId = selectedIds[i];
      if (selId + 1 == options.length) break;
      if (window['privateMoveDown']) privateMoveDown(options, selId);
      else this.swap(options[selId], options[selId + 1]);
      options[selId].selected = false;
      options[selId + 1].selected = true;
    }
    this.evenOddColorize();
    sourceSelect.focus();
    if (sourceSelect['onLocalMoveDown']) sourceSelect.onLocalMoveDown();
    function resetFields() {
      sourceSelect.removeAttribute('multiple');
      setTimeout(function () {
        sourceSelect.setAttribute('multiple', 'multiple');
  ...
  evenOddColorize: function () {
    if (!this.evenOddColoring) return;
    rightSelect = this.getRightSelect();
    if (rightSelect.length < 1) return;
    var options = rightSelect.options;
    for (var i = 0; i < rightSelect.length; i++) {
      if (i % 2 == 0) rightSelect.options[i].style.background = 'white';
      else rightSelect.options[i].style.background = '#dddddd';
    }
  },
