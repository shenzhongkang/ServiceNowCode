...
(function (global, $) {
  ...
  GlideModalForm.prototype = $.extend({}, GlideModal.prototype, {
    ...
    render: function () {
      ...
      $body.append(
        '<center>' +
          getMessage('Loading') +
          '... <br/><img src="images/ajax-loader.gifx"/></center></span>'
      );
      ...
      addHidden(
        f,
        'sysparm_goto_url',
        'modal_dialog_form_response.do?' + parms.join('&')
      );
      f.submit();
    },
    ...
    _formLoaded: function () {
      ...
      var pathname = frame
        .contents()
        .get(0)
        .location.pathname.indexOf('modal_dialog_form_response.do');
      if (pathname >= 0) {
      ...
      if (this.onFormLoad) this.onFormLoad(this);
    },
  ...
  function getFormTemplate() {
    return (
      '<form class="modal_dialog_form_poster" target="dialog_frame" method="POST" style="display: inline;"/>' +
