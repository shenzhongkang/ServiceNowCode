...
var GlideTabs2 = Class.create({
  initialize: function (
    ...
    if (offset == 1) this.tabs.shift();
    if (this.isFormTabs) {
      CustomEvent.observe('form.loaded', this.markMandatoryTabs.bind(this));
      CustomEvent.observe(
        'ui_policy.loaded',
        this.startCatchingMandatory.bind(this)
      );
      var initialIndex = parseInt(
        $j('#tabs2_section').attr('data-initial-tab-index'),
        10
      );
    ...
    if (this.tabDiv) {
      $j(this.tabDiv)
        .attr('role', 'tablist')
        .on('keydown', this, this.handleKeydown);
      setTimeout(
        function (tabDiv) {
          $j(tabDiv.getElementsByClassName('tabs2_tab'))
            .not('.tabs2_active')
  ...
  handleKeydown: function (e) {
    ...
    if (e.which == 37) {
      var newIndex =
        (self.activeTab - 1 + self.tabsTabs.length) % self.tabsTabs.length;
      var loopCount = 0;
      while (
        self.tabsTabs[newIndex].element.style['display'] == 'none' &&
        loopCount < self.tabsTabs.length
      ) {
    ...
    activeTab.focus();
    $j(this.getElementsByClassName('tabs2_tab'))
      .not('.tabs2_active')
      .attr({ tabindex: -1, 'aria-selected': false });
  },
  setActive: function (index) {
    if (index < 0 || index > this.tabs.length - 1) index = 0;
  ...
  deactivate: function () {
    ...
    for (var i = 0; i < count; i++) {
      ...
      if (tabsTab.isVisible()) show(this.tabs[i]);
    }
    for (var i = 0; i < this.tabs.length; i++) {
  ...
  activate: function () {
    if (this.className === 'tabs2_vars') return;
    removeClassName(this.parentElement, 'tabs_enabled');
    removeClassName(this.parentElement, 'tabs_disabled');
  ...
  showAll: function () {
    var tabs = this.tabs;
    for (var i = 0; i < tabs.length; i++) show(tabs[i]);
  },
  ...
  findTabIndexByID: function (tabID) {
    ...
    for (var i = 0; i < count; i++) {
      if (this.tabIDs[i] == tabID) return i;
    }
  ...
  findTabIndexByName: function (tabName) {
    ...
    if (index === -1) this.log('findTabIndexByName could not find ' + tabName);
    return index;
  },
  createTabs: function (tabClassPrefix, isPreloaded) {
    ...
    for (var i = 0, n = tabs.length, tab, title, name; i < n; i++) {
      ...
      this.tabsTabs[i] = new GlideTabs2Tab(
        this,
        i,
        title,
        tabClassPrefix,
  ...
  markTabMandatoryByField: function (field) {
    ...
    if (!this.isVisible(i)) this.showTab(i);
  ...
  findTabIndex: function (fieldName) {
    ...
    while (tabSpan) {
      if (hasClassName(tabSpan, 'tabs2_section')) break;
  ...
  _bumpSpacer: function (newHeight) {
    ...
    if (!spacerDiv || !newHeight) return;
    var spacerHeight = spacerDiv.offsetHeight;
    if (newHeight < spacerHeight) return;
  ...
  _getRowCount: function (tab) {
    ...
    if (tab) {
      ...
      if (omitCount) {
        ...
        if (
          this.activeTab != -1 &&
          this.tabs[this.activeTab].id != idList &&
          !this.endPage[id]
        )
          return Math.max(totalRows - 1, this.omitCountTotalCount[id]) + '+';
        if (
          this.doesNextPageExist[id] &&
          !this.endPage[id] &&
          firstRow > this.omitCountTotalCount[id]
        ) {
        ...
        if (this.endPage[id]) return this.endPageCount[id];
  ...
  getChildNodesWithClass: function (p, className) {
    ...
    for (var i = 0, nodes = 0, n = children.length; i < n; i++) {
      var node = children[i];
      if (hasClassName(node, className)) answer.push(node);
    }
    return answer;
  },
  log: function (msg) {
