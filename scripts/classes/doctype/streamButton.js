...
$j(function ($) {
  ...
  $('.list_stream_button').click(function () {
    ...
    if (!isOpen) {
      ...
      var url =
        '$stream.do?sysparm_table=' +
        listid +
        '&sysparm_nostack=yes&sysparm_query=' +
        query;
      var target = 'parent';
      if (shouldUseFormPane()) target = 'form_pane';
  ...
  function createStreamReader(url) {
    if ($('.list_stream_reader').length) return;
    var frame =
      '	<iframe src="' + url + '" id="list_stream_reader_frame"></iframe>';
    var $div = $(
      '<div class="list_stream_reader" aria-labelledby="stream_header">' +
  ...
  function resizeStreamReader($div) {
    ...
    var top = 50;
    if (typeof g_form == 'undefined') top = $('.list_nav_spacer').offset().top;
    else top = $('.section_header_content_no_scroll').offset().top;
    $div.css('top', top);
