...
(function (global, $) {
  ...
  GlideModal.prototype = {
    ...
    setSize: function (width) {
      this.size = 'modal-md';
      if (!width) this.size = 'modal-md';
      else if (typeof width == 'string' && width.indexOf('modal-') == 0)
        this.size = width;
      else if (width < 350) this.size = 'modal-alert';
      else if (width < 450) this.size = 'modal-sm';
      else if (width < 650) this.size = 'modal-md';
      else this.size = 'modal-lg';
    },
    ...
    setPreferenceAndReload: function (params) {
      for (var key in params) this.preferences[key] = params[key];
      this.render();
    },
    ...
    renderWithContent: function (content) {
      ...
      else $('.modal-body', this.$window).html(content);
      var self = this;
      this.$window
        .on('show.bs.modal', function () {
          self.isOpen = true;
          self.$modalContent = self.$window.find('.modal-content');
          if (self.isAutoFullHeight) self.maximizeHeight();
        })
    ...
    renderIframe: function (url, onloadCallback) {
      ...
      if (loading.textContent) loading.textContent = loadingMessage;
      else loading.innerText = loadingMessage;
      var iframe = document.createElement('iframe');
      iframe.setAttribute(
        'style',
        'width: 100%; height: 100%; border: 0; background-color: white; visibility: hidden;'
      );
      ...
      this.on('bodyrendered', function () {
        context.$modalContent.find('iframe').load(function (evt) {
          ...
          iframe.setAttribute(
            'style',
            'width: 100%; height: 100%; border: 0; background-color: white;'
          );
    ...
    _createModal: function () {
      ...
      autoFocus =
        typeof autoFocus === 'boolean' ? autoFocus : autoFocus === 'true';
    ...
    setPreferencesFromBody: function (xml) {
      ...
      if (prefs.length > 0) {
        for (var i = 0; i < prefs.length; i++) {
          ...
          if (name == 'render_title' && valu == 'false') this.setTitle('');
        }
      }
    },
    ...
    getDescribingXML: function () {
      ...
      for (var name in preferences) {
        if (!preferences.hasOwnProperty(name)) continue;
    ...
    destroy: function () {
      if (!this.fireEvent('closeconfirm', this)) return;
      if (this.$window) this.$window.modal('hide');
    },
    ...
    _watchHelp: function () {
      this.$window.on(
        ...
        '.help',
        function () {
          if (this._helpCallback) this._helpCallback.call();
        }.bind(this)
      );
    },
    ...
    fireEvent: function () {
      ...
      if (!this._events || !this._events[evtName]) return true;
      for (var i = 0; i < this._events[evtName].length; i++) {
        var ev = this._events[evtName][i];
        if (!ev) continue;
        if (ev.apply(this, args) === false) return false;
      }
      return true;
    },
    ...
    addFunctionDecoration: function (imgSrc, imgAlt, func, side) {
      if (imgSrc == 'images/help.gif') this.addHelpDecoration(func);
    },
