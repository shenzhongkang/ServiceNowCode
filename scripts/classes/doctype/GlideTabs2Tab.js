...
var GlideTabs2Tab = Class.create({
  initialize: function (parent, index, caption, classPrefix, isPreloaded) {
    ...
    if (isPreloaded && tabs.length !== 0 && tabs[index]) {
      ...
      el = this.element;
      if (!classPrefix) this.classPrefix = 'tabs2';
      else this.classPrefix = classPrefix;
    ...
    if (
      ...
      (isMSIE6 ||
        isMSIE7 ||
        isMSIE8 ||
        (isMSIE9 && !getTopWindow().document.getElementById('edge_west')))
    )
  ...
  setActive: function (yesNo) {
    if (yesNo) {
      ...
      CustomEvent.fire(
        'tab.activated',
        this.parent.className + (this.index + 1)
      );
    } else {
  ...
  showTab: function (yesNo) {
    ...
    if (!elementParent || elementParent.tagName != 'H3') return;
  ...
  updateCaption: function (caption) {
    ...
    this.getElement().getElementsByClassName('tab_caption_text')[0].innerHTML =
      this.caption;
  },
  isVisible: function () {
  ...
  markAsComplete: function (yesNo) {
    ...
    if (yesNo) this.element.removeAttribute('aria-describedby');
    else
      this.element.setAttribute(
        'aria-describedby',
        'tab2_section_mandatory_text_' + this.index
      );
    if (isDoctype())
