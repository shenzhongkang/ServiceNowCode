...
var NotificationMessage = Class.create({
  ...
  initialize: function (options) {
    ...
    if (this.options.type == 'warn') this.options.type = 'warning';
    if (this.options.type === '') this.options.type = 'info';
    if (this.options.type == 'system') this.options.type = 'info';
    this.options.fadeIn = this._validNumber(
      this.options.fadeIn,
      this.FADE_IN_DEFAULT_MS
    );
    this.options.fadeOut = this._validNumber(
      this.options.fadeOut,
      this.FADE_OUT_DEFAULT_MS
    );
    this.options.closeDelay = this._validNumber(
      this.options.closeDelay,
      this.CLOSE_DEFAULT_MS
    );
    this._show();
  },
  _show: function () {
    ...
    if (this.options.singleMessage) container.update('');
    this.notification = this._create();
    if (
      !this.options.bundleMessages ||
      container.childElements().length === 0
    ) {
      container.insert(this.notification);
      if (!this.options.sticky) {
        this.timeoutId = setTimeout(
          this._close.bind(this, false),
      ...
      this._updateMoreText(
        notification,
        NotificationMessage.prototype.messages.length + ' more...'
      );
    }
    this.notification.on(
      'click',
      '.notification_close_action',
      this._close.bind(this, true)
    );
  ...
  _close: function (boolCloseImmediately, closeEvent) {
    if (!this.notification || this._isClosing === true) return;
    ...
    function _onClose(notification) {
      ...
      if (
        notification.up('.' + this.options.classContainer) &&
        notification
          .up('.' + this.options.classContainer)
          .select('.' + this.options.classPrefix).length <= 1
      )
    ...
    if (closeEvent && closeEvent.element)
      notification = closeEvent
        .element()
        .up('.' + this.options.classPrefix + '-closable');
    if (boolCloseImmediately) _onClose.call(this, notification);
    else
      notification.animate(
        { height: 0, opacity: 0.2 },
        this.options.fadeOut,
        _onClose.bind(this, notification)
      );
  },
  _makeSticky: function () {
    ...
    this.notification.addClassName(
      this.options.classPrefix + '_message_sticky'
    );
  },
  ...
  _getContainer: function (n) {
    var c = $(n);
    if (c) return c.down('.' + this.options.classContainer);
    c = new Element('div', {
      id: n,
  ...
  _createHeading: function (container) {
    ...
    heading.insert(
      '<h3 class="panel-title">Current Notification Messages</h3>'
    );
  ...
  _showOuterPanel: function (notification) {
    ...
    notification
      .up('.notification_container')
      .down('.panel-body').style.padding = '';
    notification
      .up('.notification_container')
  ...
  _updateMoreText: function (notification, text) {
    notification
      .up('.panel')
      .select('.notification-more')[0]
      .update(sanitizeHtml(text));
  },
