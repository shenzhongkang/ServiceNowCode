...
var GlideDialogWindow = Class.create(GlideWindow, {
  ...
  destroy: function () {
    ...
    window.setTimeout(function () {
      ...
      if (isMSIE10 || isMSIE11) document.body.focus();
    }, 0);
  },
  ...
  setBody: function (html, noEvaluate, setAlt) {
    ...
    if (this.getPreference('focusTrap')) this.enableFocusTrap();
    this.onResize();
  },
  ...
  _getOverlayHeight: function () {
    ...
    if (elements && elements.length > 0) return elements[0].scrollHeight;
  ...
  _determineZIndex: function () {
    if (!window.$j) return this.ZINDEX;
    var zIndex = this.ZINDEX;
    $j('.modal:visible').each(function (index, el) {
      var elZindex = $j(el).zIndex();
      if (elZindex >= zIndex) zIndex = elZindex + 2;
    });
    return zIndex;
  },
  type: function () {
