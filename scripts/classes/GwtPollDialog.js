...
var GwtPollDialog = Class.create(GlideDialogWindow, {
  initialize: function (
    ...
    var keys = [
      ...
      'Export failed',
      'Internal error',
      'Finished Export. Exported {0} of {1} rows',
    ];
  ...
  startPolling: function () {
    ...
    if (poll_form) {
      ...
      if (poll_form.sysparm_query && poll_form.sysparm_query.value) {
        poll_form.sysparm_query.value = poll_form.sysparm_query.value.replace(
          /@99@/g,
          '&'
        );
        poll_form.sysparm_query.value = poll_form.sysparm_query.value.replace(
  ...
  ack: function (response) {
    ...
    if (answer.indexOf('complete') == 0) {
      ...
      var splits = answer.split(',');
      if (splits.length > 1) $('err_text').innerHTML = splits[1];
      $('poll_img').hide();
      $('export_failed').show();
  ...
  _showExportStatusMsg: function (gRecord) {
    if (gRecord.next()) {
      ...
      if (this.rows > maxRows) {
        $('poll_text_export_status').innerHTML = formatMessage(
          this.msgs['Finished Export. Exported {0} of {1} rows'],
          maxRows,
          this.rows
        );
        $('poll_text_wiki').show();
  ...
  _makeRequest: function (postString, callback) {
    var ga = new GlideAjax(
      'poll_processor',
      'poll_processor.do?sysparm_auto_request=true'
    );
  ...
  _isDestroyed: function () {
    if ($('poll_text') == null) g_poll_dialog = null;
