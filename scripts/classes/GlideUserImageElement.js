...
var GlideUserImageElement = Class.create({
  ...
  isReadOnly: function () {
    ...
    return (
      $element.hasClassName('readonly') ||
      $element.getAttribute('writeaccess') == 'false'
    );
  },
  setReadOnly: function (disabled) {
    ...
    if (disabled) {
      ...
      if (!image) showObjectInline(addButton);
    }
  },
  isDisabled: function () {
    var editButtons = $('edit.' + this.name);
    if (editButtons && editButtons.visible()) return false;
    var addButton = $('add.' + this.name);
    if (addButton && addButton.visible()) return false;
    return true;
  },
  _ieResize: function () {
    ...
    if (isMSIE && iInput) {
      ...
      if (height > maxHeight || width > maxWidth) {
        ...
        if (widthRatio > heightRatio) image.width = maxWidth;
