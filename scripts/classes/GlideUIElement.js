...
var GlideUIElement = Class.create({
  ...
  getLabelElement: function () {
    ...
    if (!parentElementNode) parentElementNode = document;
    var labels = parentElementNode.getElementsByTagName('label');
    for (var i = 0; (label = labels[i]); i++) {
      if (label.htmlFor == this.getID()) return label;
    }
  ...
  getChildElementById: function (id) {
    if (this.fetchedNodes[id]) return this.fetchedNodes[id];
    var element = this.getChildElementById0(id);
    if (element) this.fetchedNodes[id] = element;
    return element;
  },
  ...
  _findSubChild: function (startNode, id) {
    if (!startNode || (startNode.id && startNode.id == id)) return startNode;
...
function newGlideUIElement(
  ...
  if (mandatory && fieldName && fieldName.indexOf('.') > -1)
    return new GlideMandatoryDerivedUIElement(
      tableName,
      fieldName,
      type,
      mandatory,
