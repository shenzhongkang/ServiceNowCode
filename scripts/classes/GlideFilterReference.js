...
var GlideFilterReference = Class.create(GlideFilterHandler, {
  ...
  _initValues: function (values) {
    this.values = [];
    if (values) this.values[0] = values;
  },
  _build: function () {
    ...
    if (inDoctypeMode) {
      input.setAttribute(
        'class',
        'pull-left form-control filter-reference-input'
      );
    ...
    if (view && view.value == 'sys_ref_list') return;
    if (inDoctypeMode) {
      var btn = cel('button');
  ...
  _refListOpen: function () {
    ...
    if (this.useRefQual && this.item.getRefQual())
      ...
      if (!reference) {
        ...
        qualifier =
          'table=' + this.getTableName() + '^element=' + this.item.getName();
      }
      reflistOpen(
        target,
        this.item.getName(),
  ...
  getValues: function () {
    ...
    if (this.inputCnt == 2) {
      ...
      if (userInputVal != null && userInputVal.indexOf('javascript:') > -1)
        input = userInput;
      else if (this._useDisplayValue(oper)) input = userInput;
