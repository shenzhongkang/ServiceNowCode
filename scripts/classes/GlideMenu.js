...
GlideMenu.prototype = {
  ...
  isEmpty: function () {
    ...
    for (var i = 0; i < this.menuItems.length; i++) {
      if (this.menuItems[i].parentId == '') return false;
    }
    return true;
  },
  load: function () {},
  ...
  showContextMenu: function (evt, id, variables) {
    ...
    id += this.suffix;
    if (!getMenuByName(id)) this._createMenu(id);
    var cm = getMenuByName(id);
    if (cm.context.isEmpty()) return;
  ...
  _buildMenu: function (parentId, cm) {
    ...
    for (var i = 0; i < this.menuItems.length; i++) {
      ...
      if (lastType == 'line' && item.type == 'line') continue;
      if (lastType == 'line' && itemsAfterLine > 0) {
        this._addLine(cm);
        itemsAfterLine = 0;
      }
      lastType = item.type;
      if (lastType == 'line') continue;
      if (this._addMenuItem(cm, item)) itemsAfterLine++;
    }
  },
  ...
  _addMenuItem: function (cm, item) {
    ...
    var mi;
    if (item.type == 'action') {
      if (!this._getAction(item)) mi = cm.addLabel(item.label);
      else
        mi = cm.addFunc(
          item.label,
          this._runMenuAction.bind(this, item),
          item.id
        );
      ...
      if (item.trackSelected) sm.setTrackSelected(true);
      this._buildMenu(item.id, sm);
      if (sm.isEmpty()) added = false;
      else mi = cm.addMenu(item.label, sm, item.id);
    }
    if (mi && item.image) cm.setImage(mi, item.image);
  ...
  _getAction: function (item) {
    ...
    if (item.sysId) action += '\n' + GlideMenu.scripts[item.sysId];
    return action;
  },
  _getOnShowScript: function (item) {
    if (item.sysId) return GlideMenu.onScripts[item.sysId];
...
GlideMenu.addScripts = function (o) {
  if (o == null) return;
  for (var s in o.scripts) GlideMenu.scripts[s] = o.scripts[s];
  for (var s in o.onScripts) GlideMenu.onScripts[s] = o.onScripts[s];
};
