...
var UserRolesElement = Class.create({
  ...
  setReadOnly: function (disabled) {
    ...
    if (disabled) {
      if (lockElement) hideObject(lockElement);
      ...
      if (lockElement && lockElement.style.display != 'none') return true;
      var unlockElement = gel(this.name + '_unlock');
  ...
  isDisabled: function () {
    ...
    if (lockElement && lockElement.visible()) return false;
  ...
  keyDownLeft: function (evt) {
    var key = evt.keyCode;
    if (this._isSpaceOrEnterKey(key)) this.selectRole();
  },
  keyDownRight: function (evt) {
    var key = evt.keyCode;
    if (this._isSpaceOrEnterKey(key)) this.deselectRole();
  },
  ...
  _updateNonEditValue: function (newValue) {
    ...
    if (newValue.length < 2) {
      elem.setStyle('display', 'inline-block').update(newValue);
    } else {
      elem.setStyle('display', 'inline-block').update(newValue.join(', '));
    }
  },
  _initializeEvents: function () {
    ...
    $(this.name + '_edit').on(
      'click',
      '[data-action="add"]',
      this.selectRole.bind(this)
    );
    $(this.name + '_edit').on(
      'click',
      '[data-action="remove"]',
      this.deselectRole.bind(this)
    );
  },
});
