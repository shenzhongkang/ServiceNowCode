...
var GwtDraggableSnap = Class.create(GwtDraggable, {
  ...
  setCreateFloat: function (f) {
    ...
    if (!f) this.onCreateFloat = this._createFloat.bind(this);
  },
  ...
  snapDrag: function (dragObj, x, y, e) {
    ...
    x = pos[0];
    y = pos[1];
    if (!this.dragFloat) this.dragFloat = this.onCreateFloat(dragObj, x, y);
    if (this.dragFloat) {
      this.dragFloat.style.left = x;
      this.dragFloat.style.top = y;
    }
    this._findDropZoneAndMove(
      dragObj,
      x + this.differenceX,
      y + this.differenceY
    );
  ...
  _boundDragging: function (x, y) {
    if (this.boundDirection == 'l-r') y = this.origY;
    else if (this.boundDirection == 'u-d') x = this.origX;
    if (this.boundElement) {
      if (y < this.boundTop) y = this.boundTop;
      if (y > this.boundBottom) y = this.boundBottom;
      if (x < this.boundLeft) x = this.boundLeft;
      if (x > this.boundRight) x = this.boundRight;
    }
    return [x, y];
  },
  _findDropZoneAndMove: function (dragObj, x, y) {
    if (this.snapMode == 'absolute') {
      ...
      if (dz && dz != this.currentDropZone) {
        ...
        if (
          !this.fireEvent('beforedrop', dragObj, dz.element, dz.element, x, y)
        )
          return false;
  ...
  _initDragBounds: function (x, y) {
    ...
    if (this.boundElement) {
      this.boundLeft =
        grabOffsetLeft(this.boundElement) - grabScrollLeft(this.boundElement);
      this.boundTop =
        grabOffsetTop(this.boundElement) - grabScrollTop(this.boundElement);
      this.boundRight =
        this.boundLeft +
        this.boundElement.offsetWidth -
        this.draggable.offsetWidth;
      this.boundBottom =
        this.boundTop +
        this.boundElement.offsetHeight -
        this.draggable.offsetHeight;
      this.boundLeft -= 4;
      this.boundTop -= 4;
      this.boundRight += 4;
      this.boundBottom += 4;
  ...
  _initDropZonesFromTable: function (t) {
    ...
    for (var row = 0; row < rowCnt; row++) {
      for (var col = 0; col < colCnt; col++) {
        ...
        if (
          getAttributeValue(cell, 'dropzone') == 'true' ||
          cell.dropzone == 'true'
        )
          zones.push(cell);
      }
  ...
  _initDropZonesRelative: function (dragObj, zone) {
    ...
    for (var i = 0; i < zone.childNodes.length; i++) {
      ...
      if (
        getAttributeValue(node, 'dragpart') ||
        node.dragpart == 'true' ||
  ...
  _floatBackAndDelete: function (gd, tTime, tMoves) {
    ...
    var backX =
      (currentX - grabOffsetLeft(baseObj) - grabScrollLeft(baseObj)) / tMoves;
    var backY =
      (currentY - grabOffsetTop(baseObj) - grabScrollTop(baseObj)) / tMoves;
