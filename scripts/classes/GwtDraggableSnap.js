...
var GwtDraggableSnap = Class.create(GwtDraggable, {
  ...
  _initDropZonesRelative: function(dragObj, zone) {
      ...
      for (var i = 0; i < zone.childNodes.length; i++) {
          ...
          if (getAttributeValue(node, "dragpart") || node.dragpart == "true" || getAttributeValue(node, "dropzone") || node.dropzone == "true") {
  ...
  _isInScrollRegion: function(element, region) {
      ...
      if (top < 0)
          top = 0;
      return (left >= region.scrollLeft) && (top >= region.scrollTop) && (left <= (region.scrollLeft + region.offsetWidth)) && (top <= (region.scrollTop + region.offsetHeight));
  },
