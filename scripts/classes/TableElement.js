...
var TableElement = Class.create({
  ...
  hasDependentChildren: function () {
    for (var key in this.dependentChildren) return true;
    return false;
  },
  ...
  setType: function (type) {
    this.type = type;
    if (type == 'glide_list') this.isRef = false;
    if (type == 'glide_var') this.isRef = true;
  },
  setReference: function (reference) {
    if (reference && reference != '') this.reference = reference;
    this.isRef = false;
    switch (this.type) {
      case 'glide_list':
        if (this.reference) this.isRef = true;
        break;
      case 'reference':
      case 'domain_id':
      case 'glide_var':
  ...
  setRefRotated: function (rotated) {
    if ('yes' == rotated) this.refRotated = true;
    else this.refRotated = false;
  },
  setCanWrite: function (ra) {
    if ('no' == ra) this.canwrite = false;
    else this.canwrite = true;
  },
  setSaveAsTemplate: function (ra) {
    if ('no' == ra) this.saveastemplate = false;
    else this.saveastemplate = true;
  },
  setCanRead: function (ra) {
    if ('no' == ra) this.canread = false;
    else this.canread = true;
  },
  setActive: function (active) {
    if ('no' == active) this.active = false;
  ...
  setDependent: function (dep) {
    if (dep && dep != '') this.dependent = dep;
  },
  addDependentChild: function (name) {
    if (name) this.dependentChildren[name] = true;
  },
  ...
  setNamedAttributes: function (attrs) {
    ...
    for (var i = 0; i < pairs.length; i++) {
      ...
      if (parts.length == 2) this.namedAttributes[parts[0]] = parts[1];
    }
  },
  ...
  isDateOnly: function () {
    if (dateOnlyTypes[this.type]) return true;
    else return false;
  },
  isDateTime: function () {
    if (dateTimeTypes[this.type]) return true;
    else return false;
  },
  getName: function () {
    return this.name;
  ...
  getBaseTableName: function () {
    ...
    if (!table || !table.tablesArray) return '';
    if (table.tablesArray.length === 0) return this.getTableName();
  ...
  canRead: function () {
    if (this.canread == 'unknown') return this.getBooleanAttribute('canread');
    return this.canread;
  },
  ...
  canWrite: function () {
    if (this.canwrite == 'unknown') return this.getBooleanAttribute('canwrite');
  ...
  isActive: function () {
    if (this.active == 'unknown') return this.getBooleanAttribute('active');
    return this.active;
  },
  isNumber: function () {
    return (
  ...
  isArray: function () {
    if (this.array && this.array == 'yes') return true;
    return false;
  },
  canSort: function () {
    if (!this.getBooleanAttribute('cansort')) return false;
    if (this.name.indexOf('password') > -1) return false;
    if (this.name == 'sys_id') return false;
    if (this.type == 'journal' || this.type == 'journal_input') return false;
  ...
  canGroup: function () {
    if (this.getNamedAttribute('can_group') == 'true') return true;
    if (this.isEdgeEncrypted() && this.canMatch()) return true;
    if (!this.canSort()) return false;
    if (this.isMulti()) return false;
    if (
      this.name.indexOf('.') > -1 &&
      this.name.indexOf(this.REF_ELEMENT_PREFIX) > -1
    )
      return false;
    if (this.type == 'glide_duration') return true;
    if (
      this.type == 'glide_date_time' ||
  ...
  getNamedAttribute: function (name) {
    if (this.namedAttributes[name]) return this.namedAttributes[name];
    else return null;
  },
  type: function () {
    return 'TableElement';
  ...
  isEncrypted: function () {
    return (
      this.getBooleanAttribute('glide_encrypted', false) ||
      this.getType() == 'glide_encrypted'
    );
  },
});
TableElement.get = function (name) {
  ...
  for (var i = 1; i < names.length; i++) {
    ...
    if (e.type === 'glide_var' || !e.isReference()) break;
