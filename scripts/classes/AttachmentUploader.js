...
var AttachmentUploader = Class.create({
  initialize: function (
    ...
    if (!sys_id) sys_id = gel('sysparm_item_guid');
    this.parent_sys_id = sys_id.value;
    var table = gel('sys_target');
    if (!table) table = gel('ni.attachment_target');
  ...
  send: function (uploadedFunction) {
    ...
    Event.observe(
      ...
      function () {
        ...
        if (self.control) self.control.style.backgroundColor = 'tomato';
        self.destroy();
      },
      false
    );
    Event.observe(
      ...
      function () {
        ...
        if (self.control) self.control.style.backgroundColor = 'LightCyan';
      },
      false
    );
    Event.observe(
      ...
      if (self.xhr.readyState === 4) {
        ...
        if (xml) {
          ...
          if (sys_id == 'attachment.refused') {
            alert(
              'Attachment refused because file type not allowed or type does not match file contents.'
            );
            ...
            if (!imgSrc) imgSrc = 'images/attachment.gifx';
    ...
    this._checkCompatibleFile(function () {
      ...
      if (window.g_ck) formData.append('sysparm_ck', window.g_ck);
  ...
  _updateProgress: function (txt) {
    if (this.progress) this.progress.innerHTML = txt;
  },
  ...
  _outputIncompatible: function () {
    ...
    errMsg +=
      'Please select the file and upload through the paperclip icon above.';
    g_form.addErrorMessage(new GwtMessage().getMessage(errMsg, this.file.name));
    if (this.control) this.control.style.display = 'none';
    this.destroy();
  },
  type: function () {
...
AttachmentUploader.uploadBlob = function (
  blob,
  filename,
  url,
  data,
    ...
    if (xhr.readyState === 4) {
      try {
        ...
        if (!sys_id) {
          return onError(
            new Error(
              'Attachment failed because file type does not match with file contents.'
            )
          );
        }
        if (sys_id == 'create.permission') {
          return onError(
            new Error('You do not have ability to attach file to this record.')
          );
        }
  ...
  function listen(event, listener) {
    if (!xhr.addEventListener) xhr.attachEvent(event, listener);
    else xhr.addEventListener(event, listener, false);
  }
...
AttachmentUploader.uploadDataURLAttachment = function (
  dataUrl,
  name,
  table,
  sys_id
...
AttachmentUploader._getFormBuilder = function () {
  ...
  if (window.g_ck) form.append('sysparm_ck', window.g_ck);
  return form;
};
...
AttachmentUploader.addDropZone = function (
  dropZone,
  maxMegabytes,
  canAttach,
  showView,
  ...
  if (maxMegabytes !== '') maxMegabytes = parseInt(maxMegabytes, 10);
  else maxMegabytes = 0;
  if (isNaN(maxMegabytes)) maxMegabytes = 0;
  ...
  Event.observe(
    ...
    function (event) {
      ...
      if (
        ...
        if (
          dropData != undefined &&
          dropData.toString().startsWith(window.location.origin + '/')
        )
          return;
      }
      Event.stop(event);
      ...
      var line = gel('header_attachment_line');
      if (line) line.reverseOnCancel = 'false';
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        if (maxMegabytes > 0) {
          if (megs > maxMegabytes) {
            filesTooLarge.push(
              file.name +
                ' (' +
                AttachmentUploader.getDisplaySize(file.size) +
                ')'
            );
            files[i] = null;
            continue;
        ...
        if (gotExtensions) {
          ...
          var extension = '(none)';
          if (periodIndex == -1) badExtension = true;
          else {
            extension = file.name.substring(periodIndex + 1).toLowerCase();
            if (extensions.indexOf(extension) == -1) badExtension = true;
          }
          if (badExtension) {
        ...
        p.innerHTML =
          files[i].name +
          ' (' +
          AttachmentUploader.getDisplaySize(files[i].size) +
          ') ';
        var span = cel('span');
...
AttachmentUploader._resetBackground = function (doNow) {
  ...
  if (doNow != true && typeof line.resetDisplay != 'undefined') {
    ...
    if (resetDisplay < 10) return;
  }
  var headerAttachment = gel('header_attachment');
  if (headerAttachment) headerAttachment.style.backgroundColor = '';
...
AttachmentUploader.getDisplaySize = function (sizeInBytes) {
  ...
  if (kilobytes < 1) kilobytes = 1;
  var reportSize = kilobytes + 'K';
  if (kilobytes > 1024) reportSize = Math.round(kilobytes / 1024) + 'MB';
  return reportSize;
};
AttachmentUploader.noDropZone = function (message) {
  ...
  Event.observe(
    ...
    function (event) {
      Event.stop(event);
      if (isChrome) event.dataTransfer.dropEffect = 'copy';
      return true;
    },
    true
  );
...
AttachmentUploader.setWarning = function (
  filesTooLarge,
  filesBadExtension,
  maxFileSize
) {
  ...
  if (filesTooLarge.length > 0) {
    warningString +=
      'Files larger than the maximum file size (' + maxFileSize + 'MB):\n';
