...
var GlideDuration = Class.create({
  ...
  initialize: function (value, item) {
    ...
    if (item) {
      this.item = item;
      this.maxLevel = this._parseMaxUnit(
        this.item.getNamedAttribute('max_unit')
      );
    }
    if (value) this.values = GlideDuration.parseDurationToParts(value);
    for (var i = this.values.length; i < 4; i++) this.values[i] = '0';
  },
  buildHTML: function (parent) {
    ...
    } else this.hour = this.getDays() * 24;
    if (this.maxLevel >= this.MAX_UNIT_HOURS) {
      this._addSpan(this.hourMessage);
      this.hour = this._addInput(
        'dur_hour',
        this.hour + parseInt(this.getHours(), 10)
      );
      if (this.maxLevel == this.MAX_UNIT_HOURS) this.hour.maxLength = '7';
      this._addSpan(':');
    } else this.minute = (this.hour + parseInt(this.getHours(), 10)) * 60;
    if (this.maxLevel == this.MAX_UNIT_MINUTES)
      this._addSpan(this.minutesMessage);
    if (this.maxLevel >= this.MAX_UNIT_MINUTES) {
      this.minute = this._addInput(
        'dur_min',
        this.minute + parseInt(this.getMinutes(), 10)
      );
      if (this.maxLevel == this.MAX_UNIT_MINUTES) this.minute.maxLength = '9';
      this._addSpan(':');
    } else this.second = (this.minute + parseInt(this.getMinutes(), 10)) * 60;
    if (this.maxLevel == this.MAX_UNIT_SECONDS)
      this._addSpan(this.secondsMessage);
    this.second = this._addInput(
      'dur_sec',
      this.second + parseInt(this.getSeconds(), 10)
    );
    if (this.maxLevel == this.MAX_UNIT_SECONDS) this.second.maxLength = '10';
  },
...
GlideDuration.parseDurationToParts = function (value) {
  ...
  if (parts.length == 2) {
    ...
    for (var i = 0; i < times.length; i++) newPart[1 + i] = times[i];
    parts = newPart;
  }
  return parts;
};
