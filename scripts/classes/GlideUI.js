...
var GlideUI = Class.create({
  initialize: function () {
    ...
    CustomEvent.observe(
      GlideUI.UI_NOTIFICATION_SYSTEM,
      this._systemNotification.bind(this)
    );
    CustomEvent.observe(
      GlideUI.UI_NOTIFICATION_INFO,
      this._systemNotification.bind(this)
    );
    CustomEvent.observe(
      GlideUI.UI_NOTIFICATION_ERROR,
      this._systemNotification.bind(this)
    );
    CustomEvent.observe(
      GlideUI.UI_NOTIFICATION_SYSTEM_EVENT,
      this._eventNotification.bind(this)
    );
  ...
  fire: function (notification) {
    this.topWindow.CustomEvent.fireTop(
      GlideUI.UI_NOTIFICATION + '.' + notification.getType(),
      notification
    );
  },
  ...
  _eventNotification: function (notification) {
    ...
    if (type == 'refresh_nav') CustomEvent.fireTop('navigator.refresh');
  },
  addOutputMessage: function (options) {
    ...
    document.addEventListener('DOMContentLoaded', function (event) {
      if (window.nowapi) {
        window.nowapi.g_i18n.getMessages(
          [
            srMessage.info,
            srMessage.warning,
            srMessage.error,
    ...
    else newMsg = GlideUI.OUTPUT_MESSAGE_TEMPLATE_WITH_ID.evaluate(options);
    var messageKey = options.type + '_' + options.msg;
    if (options.preventDuplicates && this.messages[messageKey]) return false;
  ...
  clearOutputMessages: function (closeImg) {
    ...
    } else divs = this._getOutputMessageDivs();
    if (!divs) return false;
  ...
  clearNavMessage: function () {
    var navMessage = $('nav_message');
    if (!navMessage) return false;
    navMessage.remove();
    var firstNavElement = $$('nav.navbar')[0];
    if (firstNavElement) firstNavElement.setStyle({ marginBottom: '5px' });
    this.adjustTopScrollHeight();
    return true;
  },
  ...
  _scrapePageLoadMessages: function () {
    ...
    setTimeout(function () {
      ...
      for (var i = 0; i < messages.length; i++) {
        ...
        var type = 'info';
        if (msg.classList.contains('outputmsg_error')) type = 'error';
        if (msg.classList.contains('outputmsg_warning')) type = 'warning';
        if (self._shouldAnnounce(msg)) self._announce(type, text);
      }
    }, 2000);
  },
  ...
  _pruneOldMessages: function () {
    ...
    for (var i = 0; i < nodes.length; i++) {
      var node = nodes[i];
      if (node.readyToDelete) node.parentNode.removeChild(node);
    }
  },
...
GlideUI.UI_NOTIFICATION_SYSTEM_EVENT =
  GlideUI.UI_NOTIFICATION + '.system_event';
