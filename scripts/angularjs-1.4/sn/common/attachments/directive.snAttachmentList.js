      ...
      return {
        ...
        controller: function ($scope) {
          ...
          function refreshResources() {
            var handler = snAttachmentHandler.create(
              $scope.tableName,
              $scope.sysID
            );
          ...
          $scope.attachFiles = function (files) {
            if ($scope.tableName && $scope.sysID) {
              ...
              if ($scope.uploadFileFn) $scope.uploadFileFn({ files: files });
            }
          };
          ...
          $scope.deleteAttachment = function () {
            snAttachmentHandler
              .deleteAttachment($scope.attachmentToDelete.sys_id)
              .then(function () {
                var index = $scope.attachmentList.indexOf(
      ...
      return {
        ...
        link: function (scope, element, attrs) {
          ...
          scope.editModeToggle = function ($event) {
            ...
            if (scope.editMode) editFileName();
            else viewFileName();
          };
          scope.updateName = function () {
            scope.editMode = false;
            viewFileName();
            snAttachmentHandler.renameAttachment(
              scope.attachment.sys_id,
              scope.attachment.file_name
            );
          };
        },
        controller: function ($scope, snCustomEvent, i18n, $http) {
          ...
          $scope.removeAttachment = function (attachment, index) {
            if (
              $scope.deleteFileFn !== undefined &&
              $scope.deleteFileFn instanceof Function
            ) {
            ...
            snAttachmentHandler
              .deleteAttachment($scope.attachment.sys_id)
              .then(function () {
          ...
          $scope.handleAttachmentClick = function (event) {
            ...
            if ($scope.editMode || !$scope.attachment) return;
          ...
          function getAttachmentAvailability(isPreview, event) {
            $http
              .get(
                '/api/now/as_attachment/getAvailability/' +
                  $scope.attachment.sys_id
              )
              .then(
                function (response) {
                  ...
                  if (_res && _res.availability) {
                    ...
                    else attachmentUnavailableMsg(_res.message);
                  } else {
                    previewOrDownloadAttachment(isPreview, event);
                  }
                },
          ...
          function previewOrDownloadAttachment(isPreview, event) {
            if (isPreview && $scope.attachment.image) {
              if (!$scope.attachment.size)
                $scope.attachment.size = $scope.getSize(
              ...
              if (!$scope.attachment.sys_id) return;
              $window.location.href =
                'sys_attachment.do?sys_id=' + $scope.attachment.sys_id;
            }
          }
