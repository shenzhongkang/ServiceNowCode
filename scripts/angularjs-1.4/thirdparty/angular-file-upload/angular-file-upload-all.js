...
(function () {
    ...
    function ($http, $q, $timeout) {
      ...
      this.upload = function (config) {
        ...
        config.transformRequest = config.transformRequest
          ? Object.prototype.toString.call(config.transformRequest) ===
            '[object Array]'
            ? config.transformRequest
            : [config.transformRequest]
          : [];
        config.transformRequest.push(function (data, headerGetter) {
          ...
          if (config.file != null) {
            ...
            if (
              Object.prototype.toString.call(config.file) === '[object Array]'
            ) {
              var isFileFormNameString =
                Object.prototype.toString.call(fileFormName) ===
                '[object String]';
  ...
  function handleFileSelect(
    ...
    function watchForRecompile(attrVal) {
      $timeout(function () {
        if (elem.parent().length) {
          watchers.push(
            scope.$watch(attrVal, function (val, oldVal) {
              if (val != oldVal) {
                recompileElem();
              }
            })
    ...
    function recompileElem() {
      ...
      if (elem.attr('__afu_gen__')) {
        angular
          .element(document.getElementById(elem.attr('id').substring(1)))
          .remove();
      }
      if (elem.parent().length) {
    ...
    if (!isInputTypeFile()) {
      ...
      if (elem.attr('multiple'))
        fileElem.attr('multiple', elem.attr('multiple'));
    ...
    function updateModel(fileList, attr, ngModel, scope, evt) {
      ...
      var regexp =
        angular.isString(accept) && accept
          ? new RegExp(globStringToRegex(accept), 'gi')
          : null;
  ...
  function handleDrop(scope, elem, attr, ngModel, $parse, $timeout, $location) {
    ...
    if (attr['dropAvailable']) {
      $timeout(function () {
        scope.dropAvailable
          ? (scope.dropAvailable.value = available)
    ...
    elem[0].addEventListener(
      ...
      function (evt) {
        ...
        extractFiles(
          ...
          $parse(attr.allowDir)(scope) != false,
          attr.multiple || $parse(attr.ngMultiple)(scope)
        );
      },
      false
    );
        ...
        if (items != null) {
          for (var i = 0; i < items.length && valid; i++) {
            valid =
              ...
              ((acceptFn &&
                $parse(acceptFn)(scope, { $file: items[i], $event: evt })) ||
                (regexp &&
                  items[i].type != null &&
                  items[i].type.match(regexp)) ||
...
(function () {
  ...
  if (
    ...
    (window.FileAPI && FileAPI.forceLoad)
  ) {
    ...
    var addFlash = function (elem) {
      ...
      if (!el.attr('disabled')) {
        ...
        if (
          !el.hasClass('js-fileapi-wrapper') &&
          (hasFileSelect || el.attr('__afu_gen__') != null)
        ) {
          el.addClass('js-fileapi-wrapper');
          if (el.attr('__afu_gen__') != null) {
            var ref =
              (el[0].__refElem__ && angular.element(el[0].__refElem__)) || el;
            ...
            ref.bind('mouseover', function () {
              ...
              el.css('position', 'absolute')
                .css('top', ref[0].offsetTop + 'px')
                .css('left', ref[0].offsetLeft + 'px')
                .css('width', ref[0].offsetWidth + 'px')
                .css('height', ref[0].offsetHeight + 'px')
                .css('padding', ref.css('padding'))
                .css('margin', ref.css('margin'))
