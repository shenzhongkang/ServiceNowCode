...
angular
  .module('snm.auth.data')
  .provider('glideUserSession', function glideUserSessionProvider() {
    'use strict';
    ...
    this.$get = function glideUserSession(
      $rootScope,
      $q,
      $http,
      $log,
      ...
      function login(username, password, rememberMe) {
        ...
        return _sendLoginRequest(params).then(
          function (response) {
            if (!response.data) {
      ...
      function requestMfaCode() {
        return _sendLoginRequest({ send_mfa_code: true }).then(
          function (response) {
            var status;
            if (!response.data) {
        ...
        $http({
          ...
          function (response) {
            ...
            if (data) {
              ...
              $d.reject(
                'No external identity provider found for the username: ' +
                  username
              );
            }
          },
