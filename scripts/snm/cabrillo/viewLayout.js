...
(function (window, cabrillo, undefined) {
  ...
  function getTitle() {
    return packageUtils.call('getTitle').then(
      function (data) {
        return data.results;
      },
      function (err) {
  ...
  function setNavigationBarButtons(buttons, execute) {
    var externalHandler = _registerCallbackHandler(
      'registerNavigationBarButtons',
      buttons,
      execute
    );
    return packageUtils
      .call('setNavigationBarButtons', {
  ...
  function _registerCallbackHandler(handlerClass, payloads, executeCallback) {
    ...
    payloads.forEach(function (payload, payloadIndex) {
      var callbackName =
        cabrillo.EXPORT_NAME + '.' + PACKAGE + '.executeCallback';
      var handlerPayload = {
      ...
      handlerPayload.callbackScript =
        callbackName + '(' + JSON.stringify(handlerPayload) + ')';
