...
(function (window, cabrillo, undefined) {
  ...
  var ADD_ATTACHMENTS_URL =
    '/angular.do?sysparm_type=ngk_attachments&action=add&load_attachment_record=true';
  function addFile(tableName, sysID, params, options) {
    ...
    var apiPath =
      ADD_ATTACHMENTS_URL + '&sys_id=' + sysID + '&table=' + tableName;
    options = options || {};
  ...
  function callMethod(methodName, data) {
    return cabrillo.callMethod(
      cabrillo.PACKAGE + '.' + PACKAGE + '.' + methodName,
      data
    );
  }
