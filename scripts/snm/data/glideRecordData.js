...
angular
  .module('snm.data')
  .factory('glideRecordData', function ($log, $q, $http) {
    'use strict';
    function loadRecord(tableName, sysId, viewName, queryValues) {
      var record = new GlideRecordData(
        tableName,
        'sys_id',
        sysId,
        viewName,
          ...
          angular.forEach(layoutSection.columns, function (layoutColumn) {
            ...
            angular.forEach(layoutColumn.fields, function (layoutField) {
              ...
              if (realField) {
                ...
                $log.warn(
                  '(FormLayout)',
                  'Could not find field for layout item: ',
                  layoutField.name
                );
              }
            });
