...
angular
  ...
  .factory('catalogDataLookup', function ($q, $log, $http, urlTools) {
    'use strict';
    var DEBUG_LOG = false;
    return {
      initItem: initItem,
    ...
    function _loadDataLookup(url, itemSysId, g_form) {
      return $http.get(url).then(function (response) {
        ...
        return _createDataLookupEnv(
          itemSysId,
          g_form,
          data.catalog_data_lookup
        );
      });
    }
    ...
    function _wrapChangeHandler(_itemSysId, _g_form, _fieldDataLookups) {
      ...
      return function _onChangeForm(
        ...
        if (DEBUG_LOG) {
          $log.log(
            '(catalogDataLookup) Found field lookup:',
            control,
            dataLookup
          );
        }
