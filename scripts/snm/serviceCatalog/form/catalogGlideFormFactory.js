...
angular
  .module('snm.serviceCatalog.form')
  .factory(
    'catalogGlideFormFactory',
    function ($q, $log, $http, urlTools, glideFormFactory, catalogDataLookup) {
      'use strict';
      return {
      ...
      function getCatalogFields(fields, arr) {
        ...
        for (var i = 0; i < fields.length; i++) {
          var field = fields[i];
          field.catalogFieldName = getCatalogFieldName(
            field.name,
            field.sys_id
          );
          arr.push(field);
      ...
      function getCatalogField(fields, fieldName) {
        for (var i = 0, iM = fields.length; i < iM; i++) {
          if (
            ...
            fields[i].catalogFieldName == fieldName ||
            fields[i].variable_name == fieldName
          ) {
            return fields[i];
          }
        }
        ...
        for (var i = 0; i < fieldList.length; i++) {
          ...
          switch (field.catalogType) {
            default:
              ...
              params['sys_original.' + prefix + ':' + sysId] =
                field.initial_value;
              switch (field.type) {
                case 'masked':
                  if (prefix === 'IO') {
