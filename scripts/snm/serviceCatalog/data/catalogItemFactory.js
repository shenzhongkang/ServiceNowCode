...
angular
  .module('snm.serviceCatalog.data')
  .factory(
    'catalogItemFactory',
    function ($q, $log, $http, glideRecordData, urlTools) {
      'use strict';
      var DEBUG_LOG = false;
      var TYPE_ITEM = 'item';
      ...
      function loadProducer(categorySysId, itemSysId) {
        return loadItem(categorySysId, itemSysId, TYPE_PRODUCER).then(function (
          item
        ) {
          angular.extend(item, {
            recordParams: {
      ...
      function createCatalogItem(data, itemType, viewLayout) {
        ...
        angular.extend(data, {
          ...
          getPrice: function () {
            return data.price_raw
              ? data.price_raw
              : parseFloat(data.price.replace(/\$|,/g, ''));
          },
          getRecurringPrice: function () {
            return data.recurring_price_raw
              ? data.recurring_price_raw
              : parseFloat(data.recurring_price.replace(/\$|,/g, ''));
          },
      ...
      function loadItemScripts(itemSysId) {
        return $q
          .all([loadItemUiPolicy(itemSysId), loadItemClientScripts(itemSysId)])
      ...
      function executeProducerSubmit(
        itemSysId,
        itemGuid,
        fieldParams,
        fieldSequence
