...
window.NOW.FormTags = (function ($) {
  ...
  function _initializeResponseMenu() {
    ...
    if (json.set.length > 0) $tagsNumber.text(json.set.length);
    ...
    if (!window.isTablet) $documentTags.find('input').hide();
  }
  ...
  function _onTagRemoved(evt, ui, rowId) {
    ...
    if (!isNaN(number)) {
      ...
      if (number > 0) $tagsNumber.text(number);
      else $tagsNumber.hide();
    }
  ...
  function _labelResponse() {
    ...
    share
      .parent()
      .attr('id', json.sysId)
      .css({ 'background-color': json.bgcolor, color: json.tcolor });
    share
      .parent()
      .find('.tagit-label')
      .unbind('click')
    ...
    var shareLevel =
      json.type == 'SHARED' ? 'tagit-share-group' : 'tagit-share-user';
    share
      .parent()
      .removeClass('tagit-share-user tagit-share-group')
      .addClass(shareLevel);
    if (json.owner != true) {
...
var showTagForm = function (id) {
  ...
  $j('#tag_form').on({
    'select2-open': function () {
      form.disableFocusTrap();
    },
    'select2-close': function () {
      form.enableFocusTrap();
    },
  });
};
