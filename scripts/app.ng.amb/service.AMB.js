...
angular
  ...
  .service(
    'amb',
    function (AMBOverlay, $window, $q, $log, $rootScope, $timeout) {
      'use strict';
      var ambClient = null;
      ...
      if (sameScope) {
        ...
        serverConnection.loginShow = function () {
          if (!serverConnection.isLoginWindowEnabled()) return;
          if (loginWindow && loginWindow.isVisible()) return;
          if (serverConnection.isLoginWindowOverride()) return;
          var modalConfig = {
      ...
      function addOverrideStyles(el) {
        ...
        if (backdrop !== undefined)
          backdrop.style.cssText = [
            backdrop.style.cssText,
            serverConnection.getLoginWindowOverlayStyle(),
          ].join('; ');
      }
      ...
      var connectOpenedEventId = ambClient.subscribeToEvent(
        'connection.opened',
        function () {
      ...
      var connectClosedEventId = ambClient.subscribeToEvent(
        'connection.closed',
        function () {
          setInterrupted('amb.connection.closed');
        }
      );
      var connectBrokenEventId = ambClient.subscribeToEvent(
        'connection.broken',
        function () {
          setInterrupted('amb.connection.broken');
        }
      );
      var connectSessionLoggedin = ambClient.subscribeToEvent(
        'session.logged.in',
        function () {
          $rootScope.$broadcast('amb.session.logged.in');
        }
      );
      var connectSessionReestablished = ambClient.subscribeToEvent(
      ...
      var documentReadyState = $window.document
