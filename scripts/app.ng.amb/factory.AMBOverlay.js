...
angular
  .module('ng.amb')
  .factory('AMBOverlay', function ($templateCache, $compile, $rootScope) {
    'use strict';
    var showCallbacks = [],
      beforeShowCallbacks = [],
      hideCallbacks = [],
    ...
    function AMBOverlay(config) {
      ...
      if (angular.isFunction(config.onShow)) showCallbacks.push(config.onShow);
      if (angular.isFunction(config.onHide)) hideCallbacks.push(config.onHide);
      function lazyRender() {
      ...
      function renderModal() {
        ...
        modal = $compile($templateCache.get('amb_disconnect_modal.xml'))(
          modalScope
        );
      ...
      function showModal() {
        if (isRendered) modal.modal('show');
      }
      function hideModal() {
        if (isRendered) modal.modal('hide');
      }
      function destroyModal() {
        ...
        if (pos >= 0) showCallbacks.splice(pos, 1);
        pos = hideCallbacks.indexOf(config.onShow);
        if (pos >= 0) hideCallbacks.splice(pos, 1);
      }
      return {
        render: lazyRender,
