...
var GwtListEditRecordDecorations = Class.create({
  ...
  updateTable: function () {
    ...
    this.tableController.observe(
      'glide:list_v2.edit.changes_deferred',
      this._handleChangesDeferred.bind(this)
    );
  },
  showView: function (sysId) {
    if (!this.tableController.hasActions) return;
    var currDecor = this._getCurrDecor(sysId);
    var showDecor = this._getViewDecor(sysId);
    this._displayDecor(sysId, showDecor, currDecor);
    if (this._isDoctype()) this._adjustView();
  },
  ...
  _displayDecor: function (sysId, showDecor, currDecor) {
    if (!showDecor) return;
    if (currDecor && currDecor !== showDecor) currDecor.hide();
  ...
  resetSaveIcons: function () {
    ...
    this.tableController.tableElementDOM
      ...
      .each(function (elem) {
        ...
        if (
          sys_id &&
          (elem.hasClassName('list_add') || elem.down('.list_edit_dirty')) &&
          !elem.down('td.list_edit_new_row')
        )
  ...
  _getPendingDecor: function (sysId) {
    ...
    var cell = this.tableController.getDecorationCell(sysId);
    cell = $(cell);
    if (cell.down("[icon_type='save']")) return;
  ...
  _getServerViewDecorations: function (sysId, cell) {
    ...
    if (!Object.isElement(decor)) return null;
    if ('SPAN' !== decor.tagName) return null;
    if (decor.id) return null;
    return $(decor);
  },
  _buildDecorationSpan: function (sysId, cell, label) {
    var decor = $(
      new Element('span', { id: sysId + '_' + label, class: 'list_decoration' })
    );
    decor.hide();
  ...
  _addPendingDecorations: function (sysId, span, existingImg) {
    if (this.tableController.isFormUI()) {
      if (this._isDoctype() && this.changes.isUpdatedRow(sysId)) return;
      this._addDeleteIcon(span, sysId);
      ...
      else this._addNewEmbeddedListIcon(span, sysId);
      return;
    }
  ...
  sortZebra: function () {
    ...
    this.tableController.tableElementDOM
      ...
      .each(function (row) {
        if (flip) {
          if (row.hasClassName('list_odd')) row.removeClassName('list_odd');
          if (!row.hasClassName('list_even')) row.addClassName('list_even');
        } else {
          if (row.hasClassName('list_even')) row.removeClassName('list_even');
          if (!row.hasClassName('list_odd')) row.addClassName('list_odd');
        }
        flip = !flip;
      });
  },
  _getTemplate: function (name) {
    var doctypeTempl =
      GwtListEditRecordDecorations.Templates[name + '_DOCTYPE'];
    if (this._isDoctype() && doctypeTempl) return doctypeTempl;
...
GwtListEditRecordDecorations.Templates = {
  ...
  EMBEDDED_LIST_NEW_ICON: new Template(
    '<img src="images/dirty.gifx" tabindex="0" width="16" height="16" class="clsshort button list_decoration" title="#{title}" ></img>'
  ),
