...
var refreshLabelRate =
  refreshRateProperty != null && refreshRateProperty > 0
    ? refreshRateProperty
    : 60;
...
function labelsGetRequest(labelIds) {
  if (g_label_status.loading) return true;
...
function labelsGetResponse(request) {
  ...
  if (request.status == 200) g_label_status.error_count = 0;
  else g_label_status.error_count += 1;
...
function createLabelMod(parent, style, id, url, title, image, appid) {
  ...
  var scrollIcon = isTextDirectionRTL()
    ? 'images/scroll_lft.gifx'
    : 'images/scroll_rt.gifx';
  if (image == 'images/s.gifx') image = scrollIcon;
  var img;
  if (image == null || image == '')
    img =
      '<img style="width:16px; cursor:hand" src="images/icons/remove.gifx" alt="Click me to remove the tag entry" onmouseover="this.src = \'images/closex_hover.gifx\'" onmouseout="this.src = \'images/icons/remove.gifx\'" src="images/icons/remove.gifx"/>';
  else img = "<img style='width:16px' src='" + image + "' alt='' />";
  var tdimg = cel('td', tr);
  tdimg.style.width = '16px';
  var tdhtml;
  if (image == scrollIcon) tdhtml = img;
  else
    tdhtml =
      ...
      "','" +
      id +
      '\');" onmouseover="this.src = \'images/closex_hover.gifx\'" onmouseout="this.src = \'images/icons/remove.gifx\'" title="Click me to remove the tag entry">' +
      img +
      '</a>';
...
function updateLabelReadCount(appTD, notRead) {
  ...
  if (paren > -1) {
    ...
    inner = inner.trim();
    if (notRead > 0) inner = inner + ' (' + notRead + ')';
    inner = inner + term;
    clearNodes(appTD);
...
function doAssignLabel(tableName, label, sysId) {
  ...
  if (sysId == null || !sysId) {
    if (!populateParmQuery(form, '', 'NULL')) return false;
  } else {
  ...
  if (
    ...
    option.getAttribute('gsft_base_label')
  )
...
function newLabel(tableName, sysID, callback, focusTrap) {
  ...
  if (isDoctype) {
    ...
    tagForm.render();
    if (focusTrap) {
      $j('#tag_form').on('ft-ready', function () {
        tagForm.enableFocusTrap();
      });
      $j('#tag_form').on('select2-open', function () {
        tagForm.disableFocusTrap();
      });
      $j('#tag_form').on('select2-close', function () {
        tagForm.enableFocusTrap();
      });
    }
  } else {
    ...
    if (!callback)
      gsftPrompt(
        msgs['New tag'],
        msgs['Please enter the name for the new tag'],
        function (labelName) {
...
function newLabelRequest(tableName, labelName, sysID) {
  ...
  if (view != null) viewName = view.value;
...
function assignLabel(labelName, tableName, sysId, viewName) {
  ...
  if (viewName) url.addParam('sysparm_view', viewName);
  url.getXML(function () {
    reloadWindow(window);
  });
}
...
function removeLabelResponse(response, args) {
  var labelId =
    response.responseXML.documentElement.getAttribute('sysparm_name');
  if (!labelId) refreshNavIfNotDoctypeUI();
  else {
...
function assignLabelToCheckedSysIds(labelName, tableName, listId) {
  if (!labelName || labelName.strip() == '') return;
  var list = GlideList2.get(listId);
  if (!list) return;
  var sysIds = list.getChecked();
  if (!sysIds) return;
...
function getFormByTableName(tableName) {
  ...
  if (!form) form = document.forms[tableName + '.do'];
  return form;
}
function refreshNavIfNotDoctypeUI() {
  var isDoctype =
    document.documentElement.getAttribute('data-doctype') == 'true';
  if (!isDoctype) refreshNav();
}
