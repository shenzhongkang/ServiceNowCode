...
var GwtListEditReference = Class.create(GwtListEditWindow, {
  ...
  _createInputControls: function (parent) {
    ...
    this.inputControls = new AJAXReferenceControls(
      this.editor.tableElement,
      this.id,
      parent,
      refSysId,
      rowSysId,
  ...
  setReferenceValue: function (value, valid) {
    this.setValue(value);
    if (this.editor) this.editor.setReferenceValid(valid);
  },
  dismiss: function ($super) {
    if (this.destroyed || !this.inputControls) return false;
  ...
  _getDisplayValue: function () {
    if (this.editor.getValue() == 'NULL') return '';
