...
var GwtListEditTableController = Class.create({
  ...
  updateTable: function (tableElementDOM) {
    ...
    if (tableName !== this.tableName)
      jslog(
        'New table name ' +
          tableName +
          ' does not match existing table name ' +
          this.tableName
      );
    this.tableElementDOM = $(tableElementDOM);
    this._updateTableData();
  ...
  _initMeta: function () {
    ...
    this.hasActions =
      this.getAttribute('glide_list_has_actions') == 'false' ? false : true;
    this.groupFields = this._getGroupFields();
    if (
      this.rowCount == 0 &&
      'save_with_form' == this.listEditType &&
      !this.hasHeader
    )
    ...
    var cells = [];
    if (
      this.tableElementDOM &&
      this.tableElementDOM.rows &&
      this.tableElementDOM.rows.length > 0 &&
      this.tableElementDOM.rows[0].cells
    ) {
    ...
    for (var i = 0; i < cells.length; i++) {
      var cell = cells[i];
      if (
        hasClassName(cell, 'column_head') ||
        hasClassName(cell, 'list_header_cell')
      ) {
        ...
        if (fieldName) {
          ...
          if (!this.firstField) this.firstField = fieldName;
        }
      }
    }
  },
  ...
  _updateTableData: function () {
    ...
    this.observe(
      'glide:list_v2.edit.changes_saved',
      this._handleChangesSaved.bind(this)
    );
  ...
  _updateSysIdToRowMapping: function () {
    ...
    for (var i = 0, n = rows.length; i < n; i++) {
      var row = rows[i];
      if (
        !hasClassName(row, 'list_row') &&
        !hasClassName(row, 'list_row_detail')
      )
        continue;
    ...
    for (var i = 0; i < rowCount; i++) {
      ...
      if (sysId) {
        if (this.sysIdsToRows.hasOwnProperty(sysId)) continue;
  ...
  getFqField: function (sysId, fqFieldName) {
    ...
    if (!col) {
      var row = this.tableElementDOM.select(
        'tr[data-detail-row="' + fqFieldName + '"][sys_id=' + sysId + ']'
      );
      if (row.length > 0) return $(row[0].cells[0]);
      return null;
    }
  ...
  getRelatedSysID: function () {
    if (!this.relatedField || this.relatedField == 'null') return null;
    var indexOfSysId =
      this.query.indexOf(this.relatedField + '=') +
      this.relatedField.length +
      1;
  ...
  _getInsertRow: function (body) {
    ...
    for (var i = rows.length; i > 0; i--) {
      var row = $(rows[i - 1]);
      if (row.hasClassName('aggregate')) continue;
      var aggMsg = row.down('td.aggregate_message');
      if (aggMsg) continue;
      if (row.hasClassName('calculationLine')) return i - 1;
      return i;
    }
    return 0;
  },
  _getRowCss: function (tr) {
    if (this.listEditType != 'save_with_form') return 'list_unsaved';
    if (tr.rowIndex % 2 == 0) return 'list_even';
    return 'list_odd';
  },
  ...
  buildRow: function (sysId) {
    ...
    var cellCnt = 0;
    for (var i = 0; i < row.cells.length; i++) cellCnt += row.cells[i].colSpan;
    ...
    this._updateSysIdToRowMapping();
    if (this.hasShowableHierarchy()) this._buildHierarchyRow(sysId);
    return row;
  },
  ...
  needsInsertRow: function () {
    if (this.tableElementDOM.select('.list_unsaved') != 0) return false;
    if (this.tableElementDOM.select('.list_edit_new_row') != 0) return false;
    if (typeof isTablet != 'undefined' && isTablet) return false;
    return this.insertRow;
  },
  fire: function (evt, info) {
    if (!info.listId) jslog('Event ' + evt + ' fired with no listId');
  ...
  hasDeferredChanges: function () {
    if ('save_with_form' === this.listEditType) return true;
    if ('save_by_row' === this.listEditType) return true;
    return false;
  },
  isFormUI: function () {
    if ('save_with_form' === this.listEditType) return true;
  ...
  focus: function () {
    if (this.tableElementDOM.setActive) this.tableElementDOM.setActive();
    else this.tableElementDOM.focus();
  },
  focusBody: function () {
  ...
  _handleChangesSaved: function (evt) {
    if (evt.memo.listId !== this.listID) return;
    if (!this.insertRow) return;
    if ('save_with_form' === this.listEditType) return;
  ...
  getRecordIndex: function (recPos) {
    ...
    if (rows.length > 0 && extra > 0) {
      ...
      if (this.hasHierarchy) return lastNdx + extra + extra;
  ...
  getRowByPos: function (recPos) {
    if (recPos < 1) return null;
    var rows = this.getListRows();
    if (recPos > rows.length) return null;
    return rows[recPos - 1];
  },
  getCellByPos: function (recPos, cellNdx) {
    var row = this.getRowByPos(recPos);
    if (!row) return null;
    if (hasClassName(row, 'list_row_detail')) return $(row.cells[0]);
    return $(row.cells[cellNdx]);
  },
  ...
  _getGroupFields: function () {
    ...
    if (0 === groupBy.length) return null;
    return groupBy.join(',');
  },
  _isGrouped: function () {
  ...
  _ensureGroupSeparator: function (sysId) {
    ...
    if (null !== this.groupSeparator) return;
  ...
  hasShowableHierarchy: function () {
    if (this._isGrouped()) return false;
  ...
  getNextRowByPos: function (fromPos) {
    ...
    while (ndx <= limit) {
      ...
      if (row && row.style.display != 'none') return ndx;
      ndx++;
    }
    return null;
  },
...
GwtListEditTableController.addClassToRow = function (tr, className) {
  ...
  var cells = tr.cells;
  for (var i = 1; i < cells.length; i++) addClassName(cells[i], className);
};
GwtListEditTableController.removeClassFromRow = function (tr, className) {
  removeClassName(tr, className);
  var cells = tr.cells;
  for (var i = 0; i < cells.length; i++) removeClassName(cells[i], className);
};
GwtListEditTableController.Templates = {
  NEW_ROWS: new Template('<span class="list_group">#{newRows}</span>'),
};
