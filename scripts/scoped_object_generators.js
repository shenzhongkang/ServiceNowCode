...
function ScopedGlideAjaxGenerator(scope) {
  ...
  ScopedGlideAjax.prototype = classExtendForScope(
    {},
    window.GlideAjax.prototype,
    {
      scope: scope,
...
function ScopedGFormGenerator(scope) {
  ...
  function inScope(fieldName) {
    try {
      ...
      if (ge && scope == ge.getScope()) return true;
      if (ge && ge.isInherited && scope == g_form.getScope()) return true;
    } catch (e) {
      jslog(e);
    }
    return false;
  ...
  function _getUIElement(fieldName) {
    if (g_form.hasVariablesPrefix && g_form.removeVariablesPrefix)
      if (g_form.hasVariablesPrefix(fieldName))
        fieldName = g_form.resolveNameMap(
          g_form.removeVariablesPrefix(fieldName)
        );
    return (
      g_form.getGlideUIElement(fieldName) ||
      g_form.getGlideUIElement(g_form.resolveNameMap(fieldName))
    );
  }
  function _noCallbackError(displayName, fieldName) {
    var text =
      ...
      ' for ' +
      fieldName +
      ' not allowed: missing callback function as parameter';
    var err_options = {
      text: text,
      type: 'system',
      attributes: { type: 'error' },
  ...
  scoped_g_form.setReadOnly = function (fieldName, disabled) {
    if (!validField(fieldName)) return;
    if (inScope(fieldName)) return g_form.setReadOnly(fieldName, disabled);
  ...
  scoped_g_form.setMandatory = function (fieldName, mandatory) {
    if (!validField(fieldName)) return;
    if (inScope(fieldName)) return g_form.setMandatory(fieldName, mandatory);
    _showScopeError('Mandatory', fieldName, mandatory);
  };
  scoped_g_form.setDisplay = function (fieldName, display) {
    if (!validField(fieldName)) return;
    if (inScope(fieldName)) return g_form.setDisplay(fieldName, display);
    _showScopeError('Display', fieldName, display);
  };
  scoped_g_form.setDisabled = function (fieldName, disabled) {
    if (!validField(fieldName)) return;
    if (inScope(fieldName)) return g_form.setDisabled(fieldName, disabled);
...
function ScopedGlideDialogWindowGenerator(scope) {
  var extendFrom = window.GlideDialogWindow
    ? GlideDialogWindow.prototype
...
function classExtendForScope(extended, defaults, options) {
  if (window.jQuery) return jQuery.extend(extended, defaults, options);
  var prop;
...
function notifyFromWrappedScopedObject(msgObject) {
  ...
  if (
    typeof nowapi !== 'undefined' &&
    nowapi &&
    typeof nowapi.hasOwnProperty('g_notification')
  )
