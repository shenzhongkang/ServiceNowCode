...
var GwtListEditDependent = Class.create(GwtListEditWindow, {
  ...
  onKeyTab: function (evt) {
    if (window.g_accessibility && evt.target) {
      if (
        evt.shiftKey &&
        (evt.target.id == 'cell_edit_ok' || evt.target.id == 'cell_edit_cancel')
      )
        return;
  ...
  _renderControls: function () {
    ...
    for (; fieldArrayIndex < this.fields.length; fieldArrayIndex++) {
      ...
      if (!tableElement || !tableElement.isChoice()) continue;
  ...
  _renderControl: function (tableElement, fieldName, focus) {
    ...
    select.id = id;
    if (this.doctype) select.className = 'form-control list-edit-input';
    ...
    if (focus) this.focusElement = select;
  },
  _handleOnChange: function (e) {
    ...
    for (var k in children) {
      var focusElement = gel(
        'LIST_EDIT_' + this._getTargetName(parent.name, k)
      );
      if (!focusElement) return;
      focusElement.setAttribute('dbVal', '');
      this._addOptions(
        focusElement,
        parent.options[parent.options.selectedIndex].value
      );
    }
  },
  ...
  _createOptions: function (element, response) {
    ...
    if (this.controlsCount > 0) this.controlsCount--;
  ...
  _checkDuplicateOptions: function (element, value, label) {
    ...
    for (var i = 0; i < opts.length; i++)
      if (opts[i].value == value && opts[i].text === label) return true;
    return false;
  },
  save: function () {
    var tbody = gel('editControls');
    ...
    if (choiceLists) {
      for (var i = 0; i < choiceLists.length; i++) {
        ...
        var dspValue;
        if (!option.value) dspValue = '';
        else dspValue = option.text;
