...
var GwtListEditWindow = Class.create(GlideWindow, {
  ...
  _showUpdateMessage: function () {
    ...
    var msgStr;
    if (this.numCanEdit == 1) msgStr = this.msgRowUpdated;
    else msgStr = this.numCanEdit + this.msgRowsUpdated;
    if (this.numCannotEdit > 0) {
      msgStr = msgStr + '<br/>';
      if (this.numCannotEdit == 1) msgStr += this.msgRowNotUpdated;
      else msgStr += this.numCannotEdit + this.msgRowsNotUpdated;
    }
  ...
  _createControls: function () {
    ...
    if (!(ret === false)) this.createEditControlsComplete();
  },
  ...
  createTextInput: function () {
    ...
    if (this.doctype) answer.addClassName('form-control list-edit-input');
    if (!this.doctype) answer.setStyle({ width: this.preferredWidth });
  ...
  _getOffsetLeftOverflow: function (cell, offset) {
    ...
    var offsetLeft =
      formList.length != 0 ? offset - formList.scrollLeft() : offset;
    return offsetLeft;
  },
  _getOffsetLeft: function (cell) {
    ...
    offsetLeft = isDoctype()
      ? this._getOffsetLeftOverflow(cell, offsetLeft)
      : offsetLeft;
    ...
    if (drawWidth <= cell.clientWidth) return offsetLeft - scrollOffset;
    return offsetLeft + cell.clientWidth - drawWidth - scrollOffset;
  },
  _getOffsetTopOverflow: function (cell, offset) {
    var formList = $j(cell).closest('div.list_div');
    var offsetTop =
      formList.length != 0 ? offset - formList.scrollTop() : offset;
    return offsetTop;
  },
  _getOffsetTop: function (cell) {
    ...
    offsetTop = isDoctype()
      ? this._getOffsetTopOverflow(cell, offsetTop)
      : offsetTop;
    ...
    if (drawHeight <= cell.clientHeight) return offsetTop - scrollOffset;
  ...
  _getParentOffset: function (cell, isTop) {
    ...
    while (cellParent !== this.container.parentNode) {
      if (isTop) scrollOffset += cellParent.scrollTop;
      else scrollOffset += cellParent.scrollLeft;
      cellParent = cellParent.parentNode;
      if (cellParent.getAttribute('snListEditContainer') === 'true')
        inParentContainer = true;
    }
    if (!inParentContainer) scrollOffset = 0;
    return scrollOffset;
  },
  addMandatoryDecorator: function () {
    if (!this.editor.isMandatory()) return;
    var mandatory = cel('span');
    if (!this.doctype) {
      if (this.originalRenderValue != '')
        mandatory.className = 'mandatory_populated';
      else mandatory.className = 'mandatory';
    }
    if (this.doctype) mandatory.className = 'required-marker';
    mandatory.style.fontSize = '18px';
    mandatory.innerHTML = '&nbsp;';
    this.addDecoration(mandatory, true);
  },
  createOkCancel: function () {
    var cancel = createImage(
      'images/workflow_approval_rejected.gif',
      this.msgCancelButton,
      this,
      this.dismissEvent
    );
    if (this.doctype)
      cancel = createIcon(
        'btn btn-icon icon-cross-circle color-red',
        this.msgCancelButton,
        this,
        this.dismissEvent
      );
    ...
    var save = createImage(
      'images/workflow_approved.gifx',
      this.msgSaveButton,
      this,
      this.saveEvent
    );
    if (this.doctype)
      save = createIcon(
        'btn btn-icon icon-check-circle color-green',
        this.msgSaveButton,
        this,
        this.saveEvent
      );
  ...
  saveAndClose: function (keyCallback, evt) {
    ...
    if (!input) input = $('LIST_EDIT_' + this.editor.name);
    var value = '';
    if (input) {
      value = input.value;
    }
    ...
    if (
      value.trim() == '' ||
      (value == 'NULL' && this.editor.tableElement.isReference())
    ) {
      isEmpty = true;
    }
          ...
          if (dur[prop] && dur[prop].value.trim() === '') {
            ...
            alert(
              gMessage.getMessage(
                ...
                this.editor.tableElement.getLabel()
              )
            );
            return false;
          }
    ...
    var depFields = Object.keys(
      this.editor.tableElement.dependentChildren || {}
    );
    ...
    if (input && (depFields.length === 0 || editControls === null))
      ...
      result = this.runScriptsForMultipleFields(
        0,
        editControls.getElementsByTagName('select')
      );
    else this._commitSave();
  ...
  runScript: function (
    ...
    if (this.editor && this.editor.table && fieldName)
      scripts =
        g_event_handlers_onCellEdit[this.editor.table + '.' + fieldName];
  ...
  _commitSave: function () {
    ...
    this.save();
    this.dismiss();
    if (this.editor) this.editor.saveValues();
    return;
  },
  save: function () {
    var input = GwtListEditWindow.getCellEditValue();
  ...
  setValue: function (value, displayValue) {
    ...
    if (this.editor) this.editor.setValue(value, displayValue);
  },
  setRenderValue: function (value) {
    if (!value) value = '';
    if (this.editor) this.editor.setRenderValue(value);
  },
  ...
  onKeyReturn: function (evt) {
    if (
      window.g_accessibility &&
      evt &&
      evt.target &&
      evt.target.id == 'cell_edit_cancel'
    )
      return this.onKeyEsc(evt);
    evt.stop();
    if (this.saveAndClose()) this._placeCursorReturn(evt);
  },
  onKeyTab: function (evt) {
    if (window.g_accessibility && evt.target) {
      if (
        evt.shiftKey &&
        (evt.target.id == 'cell_edit_ok' ||
          evt.target.id == 'cell_edit_cancel' ||
          evt.target.getAttribute('role') == 'button')
      )
        return;
      else if (!evt.shiftKey && evt.target.id != 'cell_edit_cancel') return;
    }
    evt.stop();
    if (!this.saveAndClose(this._placeCursorTab.bind(this), evt)) return;
    this._placeCursorTab(evt);
  },
  _placeCursorReturn: function (evt) {
    if (!evt.ctrlKey) this.gridEdit.refreshCursor();
    else this.gridEdit.editNextRow();
  },
  _placeCursorTab: function (evt) {
    if (evt.shiftKey) this.prevColumn();
  ...
  setPreferredWidth: function () {
    ...
    if (this.preferredWidth > 400) this.preferredWidth = 400;
    if (this.preferredWidth < 150) this.preferredWidth = 150;
    this.setWidth(10);
  },
  ...
  saveEvent: function (evt) {
    Event.stop(evt);
    if (this.saveAndClose()) this.gridEdit.refreshCursor();
  },
  setTitle: function ($super, html) {
    ...
    if (typeof html == 'string') inner.innerHTML = html;
    else inner.appendChild(html);
