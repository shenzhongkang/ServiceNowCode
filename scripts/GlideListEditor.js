...
var GlideListEditor = Class.create({
  ...
  initialize: function (table) {
    ...
    this.renderer = new GwtListEditValueRenderer(
      this.changes,
  ...
  updateTable: function (tableDOM) {
    if (this.tableController.isForTable(tableDOM)) return;
  ...
  _observeTableEvents: function () {
    ...
    this.tableController.observe(
      'glide:list_v2.edit.cells_changed',
      policyCallback
    );
    this.tableController.observe(
      'glide:list_v2.edit.focus_moved',
      policyCallback
    );
    this.tableController.observe('glide:list_v2.edit.save_now', policyCallback);
    Event.observe(
      this.tableController.tableElementDOM,
      'click',
      policyCallback
    );
    this.tableController.observe(
    ...
    this.tableController.observe(
      'glide:list_v2.edit.rows_deleted',
      this._handleRowsDeleted.bind(this)
    );
    this.tableController.observe(
      'glide:list_v2.edit.focus_moved',
      this._handleFocusMoved.bind(this)
    );
    Event.observe(
      this.tableController.tableElementDOM,
      'glide:list_v2.edit.update_column',
      this._updateColumn.bind(this)
    );
    Event.observe(
      this.tableController.tableElementDOM,
      'glide:list_v2.edit.check_enter_once',
      this._checkEnterOnce.bind(this)
    );
    if (window.GlideListEditorMessaging) new GlideListEditorMessaging(this);
  },
  createCellEditor: function (element, ignoreTypes) {
    if (!this.tableController.canEdit()) return null;
    this.gridEdit.hideCursor();
    this.gridEdit.clearRanges();
    this.gridEdit.setAnchorCell(element);
    this.cellEditor = new GwtCellEditor(
      this.gridEdit,
      ignoreTypes,
      this.changes,
      this.tableController,
  ...
  _handleFocusMoved: function (evt) {
    ...
    if (!this.cellEditor.isActive()) return;
  ...
  applyUpdates: function () {
    ...
    var updater = new GlideListEditor.RowUpdater(
      this.changes,
      this.tableController,
      this.renderer
    );
    this.changes.exportRecords(updater);
    if (this.tableController.needsInsertRow()) this.renderer.addRowNoInsert();
  },
  ...
  saveUpdatesInForm: function () {
    addHidden(
      this._getForm(),
      this._getXmlIslandName(),
      this._serializeModified()
    );
  },
  _serializeModified: function () {
    ...
    var receiver = new GlideListEditor.XmlSerializingReceiver(
      xml,
      xml.documentElement,
      selector
    );
  ...
  _getForm: function () {
    if (typeof g_form == 'undefined' || !g_form) return null;
    return gel(g_form.getTableName() + '.do');
  },
  _getXmlIslandName: function () {
    return (
      'ni.java.com.glide.ui_list_edit.ListEditFormatterAction[' +
  ...
  saveChanges: function (onComplete) {
    var saver = new GwtListEditAjaxChangeSaver(
      this.changes,
      this.tableController,
      onComplete
    );
  ...
  _updateColumn: function (evt) {
    ...
    var ids =
      evt.memo.ids != null
        ? evt.memo.ids
        : this.changes.calcWritableSysIds(this.tableController.sysIds, [
            columnName,
          ]);
    ...
    for (var i = 0; i < numIds; i++) {
      ...
      if (
        evt.memo.ids != null ||
        this.changes.emptyRecord == null ||
        this.changes.emptyRecord.sysId != id
      ) {
    ...
    for (var j = 0; j < l; j++) {
      ...
      for (var i = 0; i < numIds; i++) {
        ...
        if (
          ...
          if (
            evt.memo.oldValues != null ||
            this.changes.isFieldDirty(id, columnName)
          ) {
            var field = this.changes.getField(id, columnName);
            var originalValue =
              evt.memo.oldValues != null
                ? evt.memo.oldValues[i]
                : field.getOriginalDisplay();
            var currentValue =
              evt.memo.newValue != null
                ? evt.memo.newValue
                : field.getRenderValue();
...
function getQueryForList(listID) {
  ...
  if (t) return getAttributeValue(t, 'glide_list_query') + '';
  return '';
}
...
GlideListEditor.XmlSerializingReceiver = Class.create(
  GwtListEditorPendingChanges.ChangeReceiver,
  {
    ...
    beginRecord: function (sysID, op) {
      if (!this.selector.isSelected(sysID, op)) return;
    ...
    endRecord: function (sysID) {
      if (this.record) this.dst.appendChild(this.record);
      this.record = null;
    },
...
GlideListEditor.ChangeCounter = Class.create(
  GwtListEditorPendingChanges.RecordReceiver,
  {
    ...
    changedRecord: function (sysID, record) {
      if (this.selector.isSelected(sysID, record.operation)) this.changeCount++;
    },
    isDone: function () {
