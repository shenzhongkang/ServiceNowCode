...
var GwtListEditAjaxValueLoader = Class.create({
  ...
  _buildIdList: function (ajax, sysIds, fields) {
    ...
    for (var i = 0; i < sysIds.length; i++) {
      ...
      for (var j = 0; j < fields.length; j++) {
        if (!this.changes.getField(id, fields[j])) fieldList.push(fields[j]);
      }
  ...
  _waitForAjax: function (ajax, callback, timeout) {
    ...
    if (!this.ajax) {
      ...
      setTimeout(
        this._waitForAjax.bind(this, ajax, callback),
        10,
        timeout - 10
      );
    }
  ...
  _getValuesResponse: function (callback, response) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      var fields;
      if (fieldList) fields = fieldList.split(',');
      else fields = [];
      for (var j = 0; j < fields.length; j++) {
        var n = fields[j];
        ...
        var canWrite = true;
        if (fieldItem.getAttribute('can_write') == 'false') canWrite = false;
        var mandatory = false;
        if (fieldItem.getAttribute('mandatory') == 'true') mandatory = true;
        ...
        if (!this.changes.getField(sysId, n)) {
          ...
          f.setInitialValues(
            this._getXMLValue(fieldItem, 'value'),
            this._getXMLValue(fieldItem, 'displayvalue')
          );
  ...
  _getXMLFieldItem: function (parent, n) {
    for (var i = 0; i < parent.childNodes.length; i++) {
      ...
      if (item.nodeName.toLowerCase() == n) return item;
    }
    return null;
  },
  _getXMLValue: function (item, type) {
    ...
    if (e && e.length > 0) value = getTextValue(e[0]);
    if (!value) value = '';
    return value;
  },
  _addField: function (sysId, field) {
    var record = this.changes.get(sysId);
    if (!record) record = this.changes.addRecord(sysId);
