...
(function () {
  ...
  addAfterPageLoadedEvent(function () {
    ...
    if (gaBatchQueue) gaBatchQueue.processQueue();
    var timeoutDuration = window.NOW.batch_glide_ajax_disable_time;
    if (!timeoutDuration || timeoutDuration < 0) timeoutDuration = 1000;
    setTimeout(function () {
  ...
  var ms = Math.round(
    NOW.xperf.loadedFunctionsEnd - NOW.xperf.loadedFunctionsBegin
  );
