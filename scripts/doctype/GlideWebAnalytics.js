...
var GlideWebAnalytics = (function () {
  function subscribe() {
    ...
    if (
      window.snWebaConfig.subscribed &&
      window.snWebaConfig.subscribed == true
    )
      return;
    var ambClient = getAMB();
    if (ambClient == undefined || ambClient == '') return;
    ...
    webaCh.subscribe(function (response) {
      ...
      var oldConfig = {
        ...
        trackerURL: window.snWebaConfig.trackerURL
  ...
  function getAMB() {
    ...
    if (ambClient) return ambClient;
    window.snWebaConfig.ambClient = window.g_ambClient
      ? window.g_ambClient
      : window.amb
      ? window.amb.getClient()
      : '';
    return window.snWebaConfig.ambClient;
  }
  function handleConfigUpdate(oldConfig, newConfig) {
    if (shouldRemoveTracker(oldConfig, newConfig)) removeTracker();
  ...
  function shouldRemoveTracker(oldConfig, newConfig) {
    if (newConfig.siteId == '0' || newConfig.trackerURL == '') return true;
    return false;
  }
  function shouldUpdateTracker(oldConfig, newConfig) {
    if (
      oldConfig.siteId &&
      oldConfig.siteId != '0' &&
      oldConfig.siteId != newConfig.siteId
    )
      return true;
  ...
  function shouldInsertTracker(oldConfig, newConfig) {
    if (oldConfig.siteId == undefined || oldConfig.siteId == '0') return true;
  ...
  function removeWebaScript() {
    ...
    if (asyncTrackEle == undefined) return;
  ...
  function insertTracker(newConfig, additionalData) {
    ...
    var head = document.head || document.getElementsByTagName('head')[0];
    if (trackerExists()) return;
    if (!isConfigValid(newConfig)) return;
  ...
  function insertEventTracker(category, key, value, additionalValue) {
    if (!isConfigValid(window.snWebaConfig)) return;
    if (!trackerExists()) insertTracker(window.snWebaConfig);
    if (
    ...
    if (additionalValue)
      additionalValue =
        typeof additionalValue == 'number' ? additionalValue : 0;
  ...
  function isConfigValid(newConfig) {
    ...
    var trackerURL =
      newConfig && newConfig.trackerURL ? newConfig.trackerURL : '';
    if (webaSiteId == null || webaSiteId == '') return false;
    if (webaSiteId == zero) return false;
    if (trackerURL == null || trackerURL == '') return false;
    return true;
  }
  ...
  function getUserId(additionalData) {
    if (window.NOW && window.NOW.user_id && window.NOW.user_id != '')
      ...
      if (userObj && userObj.userID) return userObj.userID;
    }
    return '';
  }
  ...
  var api = {
    ...
    trackEvent: function (category, key, value, additionalValue, delayInMs) {
      if (delayInMs == undefined) delayInMs = 3000;
