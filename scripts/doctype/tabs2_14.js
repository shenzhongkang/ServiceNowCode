...
(function () {
  ...
  function initFormTabs() {
    ...
    window.g_tabs2Sections = new GlideTabs2(
      'tabs2_section',
      f,
      1,
      undefined,
  ...
  function initRelatedListTabs() {
    ...
    for (var i = 0; i < hiddenOnLoad.length; i++) hideTab(hiddenOnLoad[i]);
    for (var i = 0; i < showOnLoad.length; i++) showTab(showOnLoad[i]);
    showOnLoad = [];
    hiddenOnLoad = [];
  }
  ...
  function initTabs(tabSet) {
    ...
    if (window.g_tabs_preference) tabSet.activate();
    else tabSet.deactivate();
    if (!hasTabs(window.g_tabs2Sections) && !hasTabs(window.g_tabs2List))
      tabs2ToggleDisable();
  }
  function hasTabs(tabSet) {
    if (tabSet === null) return true;
  ...
  function setTabbed() {
    if (window.g_tabs_preference) {
      ...
      if (window.g_tabs2List) window.g_tabs2List.activate();
      CustomEvent.fire('tabs.enable');
    } else {
      window.g_tabs2Sections.deactivate();
      if (window.g_tabs2List) window.g_tabs2List.deactivate();
  ...
  function showTab(listTableName) {
    ...
    if (!window.g_tabs2List.isActivated()) show(relatedListID);
  ...
  function hideTab(listTableName) {
    ...
    if (!g_tabs2List.isActivated()) hide(relatedListID);
    formatLastTabRoundedBorder();
  }
