...
function gsftSubmitBack() {
  ...
  checkDirtyForm(
    function save() {
      if (isDirtyFormAutosave()) gsftSubmit(backButton);
      else
        gsftSubmit(
          null,
          $j(backButton).closest('form')[0],
          'sysverb_check_save'
        );
    },
    function discard() {
...
function gsftSubmitOnExternalClick(e, tableName) {
  if (!e.target || !e.target.href) return false;
  if (e.metaKey || e.ctrlKey) return true;
  var url = e.target.href;
  if (g_form && g_form.modified) {
    e.preventDefault();
    if (window.g_form_dirty_support) {
      if (window.g_form_dirty_autosave) saveAndRedirect(tableName, url);
      else {
...
function isFormDataBig(form) {
  ...
  for (var i = 0, iMax = form.elements.length; i < iMax; i += 1) {
    ...
    if (id === '' || id === 'kb_url_prefix') continue;
    if (id.startsWith('ni.') && !id.startsWith('ni.dependent')) continue;
    if (id.startsWith('select_0') || id.startsWith('sys_readonly.')) continue;
...
function gsftSubmit(control, form, action_name) {
  ...
  if (typeof form == 'undefined') {
    ...
    if (typeof form == 'undefined') {
      ...
      if (sectionFormId) f = $(sectionFormId.value);
    }
  } else f = form;
  if (g_submitted) return false;
  if (typeof action_name == 'undefined' && control)
    action_name = control.hasAttribute('data-action-name')
  ...
  if (action_name == 'sysverb_back') g_submitted = false;
  else g_submitted = true;
...
function setCheckBox(box) {
  ...
  var frm = box.form;
  if (frm) frm[id].value = box.checked;
  else {
    var widget = $(id);
    if (widget) widget.value = box.checked;
  }
...
function populateParmQuery(form, prefix, defaultNULL, action) {
  ...
  if (!action) action = form.sys_action.value;
  if (action.indexOf('sysverb') != 0) {
    if (keyset == '') {
      if (!alert(msgs['No records selected'])) return false;
    } else {
      if (action == 'delete_checked') {
        var items = keyset.split(',');
        if (items.length == 1) {
          if (!confirm(msgs['Delete the selected item?'])) return false;
        } else if (items.length > 0) {
          if (
            !confirm(
              msgs['Delete these'] + ' ' + items.length + ' ' + msgs['items?']
            )
          )
            return false;
        }
  ...
  if (keyset == '' && defaultNULL) keyset = defaultNULL;
  if (prefix) keyset = prefix + keyset;
...
function getChecked(form) {
  ...
  for (i = 0; i < lookup.elements.length; i++) {
    if (lookup.elements[i].type != 'checkbox') continue;
...
function iterateList(e, row, value, update) {
  ...
  if (update) g_form.setMandatoryOnlyIfModified(true);
  ...
  addInput(
    form,
    'HIDDEN',
    'sysparm_exclude_tiny_parms',
    ['sys_id', 'sysparm_record_row'].join()
  );
...
function checkSaveID(tableName, urlBase, sysid, refKey, viewOverride) {
  ...
  if (refKey) url += '&sysparm_refkey=' + refKey;
  if (viewOverride) {
    ...
    if (view != null) {
      view = view.value;
      if (view != '') url += '&sysparm_view=' + view;
    }
  }
  var nameOfStack = $('sysparm_nameofstack');
  if (nameOfStack != null) {
    nameOfStack = nameOfStack.value;
    if (nameOfStack != '') url += '&sysparm_nameofstack=' + nameOfStack;
  }
...
function isDirtyFormAutosave() {
  return (
    !window.g_form_dirty_support ||
    window.g_form_dirty_autosave ||
    !window.g_form ||
    !g_form.modified
  );
}
...
function renderSaveChangesDialog(saveCallback, discardCallback) {
  if (isMSIE11) {
    if (
      confirm(
        getMessage(
          "Changes have been made, continuing will discard the changes. Click 'OK' to discard changes."
        )
      )
    ) {
  ...
  var content = $j(
    new XMLTemplate('dirty_form_modal').evaluate({
...
function saveAndRedirect(tableName, url) {
  ...
  if (!g_form.isNewRecord()) g_form.setMandatoryOnlyIfModified(true);
  ...
  g_submitted = true;
  if (typeof g_form != 'undefined' && g_form) g_form.enableUIPolicyFields();
  f.submit();
  return false;
}
function submitTextSearch(event, tableName) {
  if (event != true && event.keyCode != 13) return;
...
function getControlForm(name) {
  ...
  if (isSafari || isChrome) {
    if (form) {
      ...
      if (collectionType != '[object HTMLFormElement]') form = form[0];
    }
  }
  return form;
}
...
function hideReveal(sectionName, imagePrefix, snap) {
  ...
  if ($s.length == 0) return _hideRevealDirect(sectionName, imagePrefix, snap);
  var $b = $j('.section-content', $s);
  if ($s.hasClass('state-closed')) $b.show('medium');
  else $b.hide('medium');
  ...
  setPreference(
    'collapse.section.' + sectionName,
    $s.hasClass('state-closed') ? 'true' : 'false'
  );
}
function _hideRevealDirect(sectionName, imagePrefix, snap) {
  ...
  if (imagePrefix) imageName = imagePrefix;
...
function setPreference(name, value, func) {
  var u = getActiveUser();
  if (u) u.setPreference(name, value);
...
function getPreference(name) {
  ...
  if (u) {
    ...
    if (typeof opinion != 'undefined') return opinion;
  }
  console.warn(
    'Preference ' +
      name +
      ' not found in the active user cache. Making a blocking AJAX call to load it. Consider pre-loading this property in glide_user.'
  );
  var url = new GlideAjax('UserPreference');
...
function labelClicked(label, elementType) {
  ...
  if (hFor) {
    ...
    if (!elpaco || elpaco.type == 'hidden') elpaco = $(hFor);
    if (elpaco === null) {
    ...
    if (
      elpaco &&
      elpaco.type != 'hidden' &&
      elpaco.style.visibility != 'hidden'
    ) {
...
function insertFieldName(textBoxName, label) {
  ...
  if (index > -1) insertAtCursor(textBox, '\n' + label);
  else insertAtCursor(textBox, label);
...
function replaceRegEx(text, doc, tableName) {
  ...
  if (tableName.indexOf('.') > 0) tableName = tableName.split('.')[0];
  if (s.length > 0) {
    ...
    if (val.length == 0) {
      ...
      if (labelText.indexOf(':') > 0) labelText = labelText.split(':')[0];
      val = labelText;
    }
    re = new RegExp('%\\{' + field + '[\\}]', 'g');
    var result = text.replace(re, val);
    if (result.indexOf('%{') > 0) result = replaceRegEx(result, doc, tableName);
    return result;
  }
  return text;
}
...
function textareaSizer(id, rows) {
  var element = $(id);
  if (element) setAttributeValue(element, 'rows', rows);
}
...
function handleMaxMinHeights(element, height) {
  ...
  if (height >= maxHeight) {
    if ($('sizer_plus_' + id)) $('sizer_plus_' + id).addClassName('disabled');
    $element.setStyle({
      overflowY: 'auto',
    });
    return;
  ...
  if (height <= minHeight) {
    if ($('sizer_minus_' + id)) $('sizer_minus_' + id).addClassName('disabled');
    return;
  }
...
function resizeTextAreaIframe() {
      ...
      if (doctype) {
        ...
        if (tfDoc && isReadonly) {
          function setHeightToScrollHeight() {
            if (
              tfDoc.documentElement.scrollHeight !==
              parseInt(tf.style.height, 10)
            ) {
        ...
        if (tbl) {
          tbl.style.height =
            parseInt(tbl.style.height) + (newHeight - elHeight) + 'px';
        }
        if (readOnlyDiv) {
  ...
  if (!tf) return;
  if (!tf.parentNode) return;
  if (!tf.parentNode.nextSibling) return;
...
function toggleQuestionRows(thisclass, display, fl) {
  forcelabels = false;
  if (fl == true) forcelabels = true;
  var rows = $(document.body).select('.' + thisclass);
  for (i = 0; i < rows.length; i++) {
    var element = rows[i];
    var id = element.id;
    if ('CATEGORY_LABEL' != id || forcelabels) element.style.display = display;
  }
  var openStyle = 'none';
  var closedStyle = 'none';
  if ('none' == display) openStyle = '';
  else closedStyle = '';
...
function toggleWorkflow(id, expandPref) {
  ...
  for (var i = 0; i != spans.length; i++) {
    var span = spans[i];
    if (!span.getAttribute('stage')) continue;
...
function toggleItems(id, force) {
  ...
  for (var i = 0; i < tables.length; i++) {
    ...
    if (tableId.indexOf('workflow.') == -1) continue;
    var idParts = tables[i].id.split('.');
    if (id && tableId != 'workflow.' + id) continue;
    var pref = getPref(idParts[1]);
    if (force != pref) toggleWorkflow(idParts[1], pref);
    if (id) break;
  }
}
function getPref(id) {
  var filterImgSrc = $('filterimg.' + id).src;
  if (filterImgSrc.indexOf('filter_hide') != -1) return 'true';
  return 'false';
}
...
function validateHex(field) {
  ...
  if (!valid) {
    var sName = '';
    if (field.name != null) sName = ' of ' + field.name + ' ';
...
function enterSubmitsForm(e, enter_submits_form) {
  ...
  var $source = $j(source);
  if (
    source.getAttribute('data-type') &&
    e.keyCode == 13 &&
    source.getAttribute('data-type') == 'ac_reference_input'
  )
    return false;
  if (source && source.type == 'textarea') return true;
  if (
    source.getAttribute('contenteditable') === 'true' &&
    $source.is('.multiLinePill')
  )
    return true;
  if ($source.is('.delete_pill.remove-pill')) return true;
  if ($source.is('.dot_walk_btn')) return true;
  ...
  if (enter_submits_form == 'false') return false;
  var submitHeaderElements = $(document.body)
    .select('.header')
    .filter(function (el) {
      return el.type === 'submit' && !el.disabled;
    });
  if (!submitHeaderElements.length) return false;
  var submitButton =
    submitHeaderElements.find(function (el) {
      return (
        el.value === 'sysverb_insert' ||
...
function tsIndexCreatorPopup(tableName) {
  ...
  if (tableName != '') gDialog.setTitle('Generate Text Index');
  else gDialog.setTitle('Regenerate All Text Indexes');
  gDialog.render();
}
...
function sortSelect(obj) {
  ...
  if (!maxSort || maxSort == 0) maxSort = 500;
  ...
  for (var i = 0; i < obj.options.length; i++) {
    ...
    if (newOption.value.indexOf('split') > 0) o2[o2.length] = newOption;
    else if (
      newOption.value &&
      newOption.value.substr(0, 2) !== 'u_' &&
      (newOption.value.indexOf('formatter') > 0 ||
        newOption.value.indexOf('component') > 0 ||
        newOption.value.indexOf('annotation') > 0 ||
        newOption.value.indexOf('chart') > 0)
    )
      o3[o3.length] = newOption;
    else o[o.length] = newOption;
  }
  ...
  var counter = 0;
  for (
    var i = o.length + o2.length;
    i < o.length + o2.length + o3.length;
    i++
  ) {
    var newOption = new Option(
      o3[counter].text,
      o3[counter].value,
      o3[counter].defaultSelected,
      o3[counter].selected
    );
