...
function isFormDataBig(form) {
  ...
  for (var i = 0, iMax = form.elements.length; i < iMax; i += 1) {
      ...
      if (id === '' || id === 'kb_url_prefix')
          continue;
      if (id.startsWith("ni.") && !id.startsWith("ni.dependent"))
          continue;
...
function resizeTextAreaIframe() {
  var args = Array.prototype.slice.call(arguments, 0), tf = $("textarea_iframe." + id), doctype = args[0], id = args[1], height, oldRows, rows;
  if (doctype) {
      height = args[2];
  } else {
      rows = args[2];
...
function enterSubmitsForm(e, enter_submits_form) {
  ...
  var submitButton = submitHeaderElements.find(function(el) {
      return (el.value === 'sysverb_insert' || el.value === 'sysverb_insert_and_stay' || el.value === 'sysverb_update' || el.value === 'sysverb_update_and_stay' || el.value === 'sysverb_save' || el.value === 'sys_security_acl_insert_with_roles');
...
function sortSelect(obj) {
  ...
  for (var i = 0; i < obj.options.length; i++) {
      ...
      else if (newOption.value && newOption.value.substr(0, 2) !== "u_" && (newOption.value.indexOf('formatter') > 0 || newOption.value.indexOf('component') > 0 || newOption.value.indexOf('annotation') > 0 || newOption.value.indexOf('chart') > 0))
