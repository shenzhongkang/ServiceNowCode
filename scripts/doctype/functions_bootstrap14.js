...
(function () {
  ...
  function listen(id, then) {
    if (typeof id !== 'string') listenMany(id, then);
    if (defined[id]) {
...
function runAfterAllLoaded() {
  ...
  for (var i = 0; i != g_load_functions.length; i++) {
    ...
    if (t > 5000) jslog('Time: ' + t + ' for: [' + i + '] ' + f);
  }
...
function addRenderEvent(func) {
  if (isRenderEventRegistered(func)) return;
...
function isRenderEventRegistered(func) {
  var s = func.toString();
  for (var i = 0; i < g_render_functions.length; i++)
    if (g_render_functions[i].toString() == s) return true;
  return false;
}
...
function runAfterPageLoadedEvents() {
  ...
  sw.jslog(
    'after page loaded complete, functions called: ' +
      g_afterPageLoadedFunctions.length
  );
...
function cel(name, parent) {
  ...
  if (arguments.length > 1) parent.appendChild(e);
  return e;
}
function rel(id) {
  var e = gel(id);
...
function clearNodes(t) {
  if (!t) return;
  while (t.hasChildNodes()) t.removeChild(t.childNodes[0]);
}
...
function addOnSubmitEvent(form, func, funcname) {
  ...
  if (typeof form.onsubmit != 'function') form.onsubmit = func;
  else {
    form.onsubmit = function () {
      var formFuncCalled = false;
      try {
        if (oldonsubmit() == false) return false;
        CustomEvent.fire(
...
function show(element) {
  var e = typeof element === 'string' ? gel(element) : element;
  if (!e) return;
  if (e.tagName == 'TR') e.style.display = 'table-row';
...
function hideObject(o, visibilityOnly) {
  ...
  if (!visibilityOnly) o.style.display = 'none';
  _frameChanged();
}
function showObject(o, visibilityOnly) {
  if (!o) return;
  o.style.visibility = 'visible';
  if (!visibilityOnly) o.style.display = 'block';
  _frameChanged();
}
...
function findFirstEditableElement(form) {
  ...
  for (var i = 0, n = elements.length; i < n; i++) {
    ...
    if (!tags.include(tagName)) continue;
    element = $(element);
...
function setMandatoryExplained(enforce) {
  ...
  if (!showexp) return;
  if (enforce || foundAMandatoryField()) showexp.style.display = 'inline';
  else showexp.style.display = 'none';
}
function foundAMandatoryField() {
  var spanTags = document.getElementsByTagName('span');
  if (!spanTags) return false;
...
function _frameChanged() {
  if (_frameChangedTimer) clearTimeout(_frameChangedTimer);
...
function getFormContentParent() {
  ...
  var validFormElementCount = document.querySelectorAll(
    '.modal.in input:not([type="hidden"])'
  ).length;
  if (glideOverlay.length > 0 && exposeMask && exposeMask.visible())
    _returnedValue = glideOverlay[0];
  if (bsModal && validFormElementCount > 0) return bsModal;
  if (typeof g_section_contents == 'undefined' || !g_section_contents)
    g_section_contents = $(document.body).select(
      '.section_header_content_no_scroll'
    );
  if (g_section_contents.length > 0) _returnedValue = g_section_contents[0];
  else _returnedValue = document.body;
  return _returnedValue;
}
...
function hasClassName(element, name) {
  if (!element) return;
  if (!element.hasClassName) return;
...
function writeTitle(element, title) {
  element.title = title;
  if (element.alt) element.alt = title;
...
function contextMenuHide(e) {
  if (typeof contextHide === 'undefined') return;
  if (!isMSIE && e) {
    if (isTouchDevice && !isTouchRightClick(e)) {
      if (e.type == 'touchend' && $(e.target).up('.context_menu')) return;
      contextHide();
...
function loadXML(r) {
  ...
  if (typeof xml != 'undefined') return xml;
  var dom = null;
  try {
