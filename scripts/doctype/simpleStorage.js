...
(function (root, factory) {
  ...
  else root.simpleStorage = factory();
})(this, function () {
  'use strict';
  var VERSION = '0.1.2',
    _storage = false,
  ...
  function _setupUpdateObserver() {
    ...
    else document.attachEvent('onstorage', _reloadData);
  }
  ...
  function _setTTL(key, ttl) {
    ...
    if (ttl !== 0) {
      if (_storage.hasOwnProperty(key)) {
        if (!_storage.__simpleStorage_meta) _storage.__simpleStorage_meta = {};
        if (!_storage.__simpleStorage_meta.TTL)
          _storage.__simpleStorage_meta.TTL = { expire: {}, keys: [] };
        _storage.__simpleStorage_meta.TTL.expire[key] = curtime + ttl;
        if (_storage.__simpleStorage_meta.TTL.expire.hasOwnProperty(key)) {
          for (
            i = 0, len = _storage.__simpleStorage_meta.TTL.keys.length;
            i < len;
            i++
          ) {
        ...
        for (
          ...
          if (
            _storage.__simpleStorage_meta.TTL.expire[
              _storage.__simpleStorage_meta.TTL.keys[i]
            ] >
            curtime + ttl
          ) {
        ...
        if (!added) _storage.__simpleStorage_meta.TTL.keys.push(key);
      } else return false;
    } else {
      if (
        _storage &&
    ...
    if (
      ...
      _storage.__simpleStorage_meta &&
      _storage.__simpleStorage_meta.TTL &&
      _storage.__simpleStorage_meta.TTL.keys.length
    ) {
      _ttl_timeout = setTimeout(
        _handleTTL,
        Math.min(
          Math.max(
            _storage.__simpleStorage_meta.TTL.expire[
              _storage.__simpleStorage_meta.TTL.keys[0]
            ] - curtime,
            0
          ),
          0x7fffffff
  ...
  return {
    ...
    set: function (key, value, options) {
      if (key == '__simpleStorage_meta') return false;
    ...
    getTTL: function (key) {
      ...
      if (_storage.hasOwnProperty(key)) {
        if (
          ...
          var ttl = Math.max(
            _storage.__simpleStorage_meta.TTL.expire[key] - +new Date() || 0,
            0
          );
          return ttl || false;
        } else {
