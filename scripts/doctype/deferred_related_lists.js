...
$j(function ($) {
  ...
  setTimeout(function () {
    if (!window.g_form || window.g_related_list_timing != 'deferred') return;
    getRelatedLists();
  }, timeout);
  ...
  function getRelatedLists() {
    ...
    ga.getXML(function (response) {
      ...
      CustomEvent.fire('page_timing', {
        name: 'RLV2',
        child: 'Network time',
        ms: lists_loaded_time - deferred_loadtime,
      });
    });
  }
  CustomEvent.observe('list.loaded', function (table, list) {
    ...
    if (
      list.getReferringURL().indexOf('list2_deferred_related_lists.do') != -1 ||
