...
var GwtMessage = Class.create({
  ...
  initialize: function () {},
  set: function (n, v) {
    if (!v) GwtMessage._messages[n] = true;
    else GwtMessage._messages[n] = v;
  },
  format: function (msg) {
    if (!msg) return '';
    var str = msg;
  ...
  getMessages: function (resolveList) {
    ...
    for (var i = 0; i < resolveList.length; i++) {
      ...
      if (v === true) pageMsgs[resolveList[i]] = resolveList[i];
      else if (v) pageMsgs[resolveList[i]] = v;
      else dataRequiringAjaxCall.push(resolveList[i]);
    }
    ...
    for (var key in pageMsgs) results[key] = pageMsgs[key];
    return results;
  },
  _loadNewMessages: function (resolveList) {
    ...
    ajax.addParam(
      'sysparm_prefetch',
      this._shouldPrefetch() ? 'true' : 'false'
    );
  ...
  _removeCachedEntries: function (resolveList) {
    ...
    for (var i = 0; i < resolveList.length; i++) {
      ...
      if (this._getCache().get(key)) continue;
  ...
  _getCache: function () {
    ...
    try {
      if (getTopWindow().g_cache_message) return getTopWindow().g_cache_message;
    } catch (e) {}
    if (typeof g_cache_message != 'undefined') return g_cache_message;
...
function getMessage(msg, callback) {
  if (typeof callback == 'function') {
    if (typeof msg == 'object') new GwtMessage().fetch(msg, callback);
    else
      new GwtMessage().fetch([msg], function (o) {
        callback(o[msg]);
      });
  } else {
    if (typeof msg == 'object') return new GwtMessage().getMessages(msg);
