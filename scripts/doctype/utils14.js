...
function toggleDivDisplayAndReturn(divName) {
  ...
  var div = $(divName);
  if (!div) return;
  if (div.style.display == 'none') showObject(div);
  else hideObject(div);
  return div;
}
...
function findParentByTag(element, tag) {
  ...
  while (element && element.parentNode && element.parentNode.tagName) {
    ...
    if (element.tagName.toLowerCase() == tag.toLowerCase()) return element;
  }
  return ret;
}
...
function expandEffect(el, duration, steps, stepCallback, completionCallback) {
  if (!useAnimation()) {
    showObject(el);
    if (completionCallback) completionCallback(el);
    return;
  }
  var h;
  if (el.originalHeight) h = el.originalHeight;
  else {
    h = getHeight(el);
    if (h == 0) {
      showObject(el);
...
function expandAnimationEffect(
  el,
  height,
  duration,
  steps,
  ...
  new Rico.Effect.Size(el.id, null, height, duration, steps, {
    step: function () {
      if (stepCallback) stepCallback(el);
    },
    complete: function () {
...
function getScrollOffset(item, attr) {
  ...
  while (item && item.tagName && item != parentElement) {
    wb += item[attr];
    if (isMSIE) item = item.offsetParent;
...
function getValue(evt) {
  ...
  try {
    ...
    var msg =
      typeof e == 'string' ? e : e.message ? e.message : 'Unknown Error';
...
function getEventCoords(e) {
  ...
  if (fudge == document.body) return answer;
...
function getRelativeTop() {
  ...
  var topLeft = new Point(
    port.getScrollOffsets().left,
    port.getScrollOffsets().top
  );
  var fudge = getFormContentParent();
...
function getTextValue(node) {
  ...
  if (firstNode.data) return firstNode.data;
...
function addForm() {
  ...
  if (window.g_ck) addHidden(form, 'sysparm_ck', g_ck);
  return form;
}
...
function menuIsEmpty(selectName) {
  var selectMenu = document.getElementById(selectName);
  if (!selectMenu || selectMenu.selectedIndex <= 0) return true;
  return false;
}
...
function createImage(src, title, object, onClick) {
  ...
  img.title = title;
  img.alt = title;
  if (arguments.length == 4) img.onclick = onClick.bindAsEventListener(object);
  return img;
}
function createIcon(cls, title, object, onClick) {
  ...
  if (arguments.length == 4) icn.onclick = onClick.bindAsEventListener(object);
  return icn;
}
...
function getFormattedDate(date) {
  ...
  return (
    doubleDigitFormat(curr_mon) +
    '/' +
    doubleDigitFormat(curr_day) +
    '/' +
...
function getFormattedTime(date) {
  ...
  return (
    doubleDigitFormat(curr_hour) +
    ':' +
    doubleDigitFormat(curr_min) +
    ':' +
...
function opticsLog(tablename, fieldname, message, oldvalue, newvalue) {
  ...
  if (g_form._pushAction) g_form._pushAction(info);
...
function handleAriaInvalidState(fieldId, fieldIsEmpty) {
  ...
  if (
    displayNode &&
    displayNode.classList.contains('element_reference_input')
  ) {
