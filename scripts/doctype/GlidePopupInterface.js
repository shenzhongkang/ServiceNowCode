...
(function (exports) {
  function isApiLoaded(m) {
    return (
      ...
      exports.nowapi.g_popup_manager[m]
    );
  }
  function popListDiv(
    evt,
    ...
    if (isApiLoaded('popListDiv')) onReady();
    else
  ...
  function popReferenceDiv(evt, inputId, view, form, refKey) {
    ...
    if (isApiLoaded('popReferenceDiv')) onReady();
    else
      ScriptLoader.getScripts(
        'scripts/classes/nowapi/ui/GlidePopup.js',
        onReady
  ...
  function popRecordDiv(evt, table, sys_id, view, width) {
    ...
    if (isApiLoaded('popRecordDiv')) onReady();
    else
      ScriptLoader.getScripts(
        'scripts/classes/nowapi/ui/GlidePopup.js',
        onReady
  ...
  function popLightWeightReferenceDiv(evt, inputid, hideOpenButton) {
    ScriptLoader.getScripts(
      'scripts/classes/nowapi/ui/GlidePopup.js',
      function () {
        nowapi.g_popup_manager.popLightWeightReferenceDiv(
          evt,
          inputid,
          hideOpenButton
        );
