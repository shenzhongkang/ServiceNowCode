...
var GlideForm = Class.create({
  ...
  _setFieldReadOnly: function(ge, s, disabled, skipProcessLabelChange) {
      ...
      if (control) {
          ...
          if (!skipProcessLabelChange) {
              value = (ge.type === "reference" || ge.type === "glide_list") && ge.getElement() ? ge.getElement().value : control.value;
  ...
  isReadOnly: function(ge, control) {
      ...
      return control.disabled || control.readOnly || ((typeof control.hasClassName === "function") && control.hasClassName("readonly")) || ((typeof control.getAttribute === "function") && control.getAttribute("writeaccess") == "false");
  },
  ...
  resolvePrettyNameMap: function(realName) {
      var pname = realName;
      for (var i = 0; i < this.nameMap.length; i++)
          if ('ni.VE' + this.nameMap[i].realName == realName || 'ni.QS' + this.nameMap[i].realName.substring(3) == realName || this.nameMap[i].realName == realName) {
  ...
  getLabelOf: function(fieldName) {
      ...
      var labelContent = "";
      if (widgetLabel) {
          labelContent = $j(widgetLabel).find('.label-text').text() || widgetLabel.innerText || widgetLabel.textContent;
  ...
  _setDecorations: function(fieldName, decorations) {
      ...
      if (labelEl) {
          ...
          for (var i = 0; i < decorations.length; i++) {
              ...
              var $dec = $j('<span class="field_decoration ' + dec.icon + ' ' + dec.color + '" ' + 'title="' + dec.text + '" ' + 'data-placement="right" data-container=".touch_scroll"></span>')
              decorEl.append($dec);
              $dec.tooltip().hideFix();
          }
          return true;
  ...
  changedFieldsFilter: function(element) {
      if (element.changed && element.id.startsWith(this.getTableName() + ".") && (element.tagName.toUpperCase() != "TEXTAREA"))
          return true;
  ...
  _opticsInspectorLog: function(fieldName, oldValue) {
      var value = this.getValue(fieldName);
      opticsLog(this.tableName, fieldName, "Value changed from " + (oldValue ? oldValue : "(empty)") + " to " + (value ? value : "(empty)"), oldValue, value);
  },
