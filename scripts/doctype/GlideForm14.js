...
var GlideForm = Class.create({
  ...
  initialize: function (
    ...
    this.personalizePrefKey =
      'personalize_' + this.tableName + '_' + this.getViewName();
  ...
  fieldChanged: function (elementName, changeFlag) {
    ...
    if (field && this._isDerivedWaiting(elementName)) {
      ...
      if (newValue && field.getElementMandatory() != 'true')
        ...
        !newValue &&
        !field.isMandatory() &&
        field.getElementMandatory() != 'false'
      )
  ...
  _setDerivedFieldsMandatory: function (ed) {
    if (ed.getType() !== 'reference') return;
    for (var name in this.mandatoryDerivedFields) {
      ...
      if (field.referenceFieldName == ed.fieldName) {
        if (!field.referenceField) field.setReferenceField(ed);
  ...
  getPrefixHandler: function (id) {
    if (!id) return;
    if (id.indexOf('.') < 0) id = 'variables.' + id;
  ...
  getElement: function (id) {
    if (
      this.elementHandlers[id] &&
      typeof this.elementHandlers[id].getElement == 'function'
    )
      return this.elementHandlers[id].getElement();
    else return this.getControl(id);
  },
  getParameter: function (parm) {
    if (!(parm.substr(0, 8) == 'sysparm_')) parm = 'sysparm_' + parm;
    var pcel = gel(parm);
    if (pcel) return pcel.value;
    else return '';
  },
  hasAttribute: function (s) {
    if (this.attributes[s]) return true;
    return false;
  },
  ...
  _getPersonalizeHiddenFields: function () {
    if (this.personalizeHiddenFields == null) {
      var prefVal =
        NOW[this.personalizePrefKey] || getPreference(this.personalizePrefKey);
      if ('false' == prefVal) this.personalizeHiddenFields = [];
      else if (prefVal.length)
        this.personalizeHiddenFields = prefVal.split(',');
      else this.personalizeHiddenFields = [];
    }
  ...
  _savePersonalizeHiddenFields: function (callback) {
    setPreference(
      this.personalizePrefKey,
      this.personalizeHiddenFields.join(','),
      callback
    );
  },
  isUserPersonalizedField: function (fieldName) {
    fieldName = this.removeCurrentPrefix(fieldName);
    return this.personalizeHiddenFields === null
      ? false
      : this.personalizeHiddenFields.indexOf(fieldName) != -1;
  },
  ...
  _setDisplay: function (fieldName, display, isMandatory, fieldValue) {
    ...
    var opticsVisibleMessage =
      'Visibility set to ' + (display ? 'visible' : 'hidden');
    if (!control) {
    ...
    if (display != true && isMandatory && fieldValue == '') return;
    var theElement = control;
    if (
      this.elementHandlers[control.id] &&
    ...
    if (glideElement)
      if (
        glideElement.type == 'script' ||
        glideElement.type == 'script_plain' ||
        glideElement.type == 'xml'
      )
  ...
  getViewName: function () {
    ...
    var view = 'default';
    if (sysparmView && sysparmView.value) view = sysparmView.value;
    return view;
  },
  ...
  changeElementStyle: function (fieldName, name, value) {
    ...
    if (!ge) return;
    if (this.changeElementParent(ge, name, value)) return;
    var labelElement = ge.getLabelElement();
    if (labelElement) labelElement.parentNode.parentNode.style[name] = value;
    var parentTR = findParentByTag(ge.getElement(), 'tr');
    if (parentTR && parentTR != labelElement) parentTR.style[name] = value;
  },
  changeElementParent: function (ge, name, value) {
    ...
    if (element) {
      ...
      var isEmptyRef =
        ge.type == 'reference' && this.getValue(ge.fieldName) == '';
      if (decoration && decoration.length > 0) {
        for (var i = 0; i < decoration.length; i++) {
          if (decoration[i].getAttribute('list-read-only') === 'true') continue;
          if (
            isEmptyRef &&
            decoration[i].getAttribute('data-type') == 'reference_popup'
          )
            decoration[i].style[name] = 'none';
          else decoration[i].style[name] = value;
        }
      }
      return true;
    }
  ...
  setRequiredChecked: function (fieldName, required) {
    if (
      !fieldName ||
      !fieldName.startsWith('ni.VE') ||
      !fieldName.startsWith('ni.QS')
    ) {
  ...
  setMandatory: function (fieldName, mandatory) {
    ...
    if (this.isSecurityReadOnlyField(fieldName)) {
      opticsLog(
        this.tableName,
        fieldName,
        "Unable to set security read-only field's mandatory state to " +
          mandatory
      );
      return;
    }
  ...
  _setMandatory: function (fieldName, thisElement, mandatory) {
    ...
    if (
      ...
      control.id &&
      this.elementHandlers[control.id] &&
      typeof this.elementHandlers[control.id].setMandatory == 'function'
    )
  ...
  _setDerivedWaitingReadonly: function (fieldName, disabled) {
    ...
    var control = this.getControl(fieldName);
    if (!control) return;
    if (disabled) control.setAttribute('derived-waiting', true);
    else control.removeAttribute('derived-waiting');
  },
  _setReadonly: function (fieldName, disabled, isMandatory, fieldValue) {
    ...
    if (!ge) {
      opticsLog(
        this.tableName,
        fieldName,
        "Unable to set invalid field's ReadOnly to " + disabled
      );
      return;
    }
    this._setReadonly0(
  ...
  _setReadonly0: function (
    ...
    if (disabled && isMandatory && fieldValue == '') {
      opticsLog(
        this.tableName,
        fieldName,
        "Unable to set blank mandatory field's read-only state to " + disabled
      );
      return;
    }
    if (this.isSecurityReadOnlyField(fieldName)) {
      opticsLog(
        this.tableName,
        fieldName,
        "Unable to set security read-only field's read-only state to " +
          disabled
      );
      return;
    }
    if (control.getAttribute('gsftlocked') == 'true') {
      opticsLog(
        this.tableName,
        fieldName,
        "Unable to set locked field's ReadOnly to " + disabled
      );
      return;
    }
    if (
      ...
      if (this.elementHandlers[control.id].setReadOnly(disabled) == true)
        return;
    } else this.setDisabledControl(control, disabled);
    ...
    if ($inputGroup.length) {
      if (disabled) $inputGroup.addClass('input-group-disabled');
      else $inputGroup.removeClass('input-group-disabled');
    }
    this.setSensitiveDisplayValue(
      s + '.ui_policy_sensitive',
      disabled ? 'none' : ''
    );
    this._setFieldReadOnly(
      ge,
      'ni.' + this.tableName + '.' + fieldName,
      disabled
    );
    if (this.tableName == 'ni')
      this._setFieldReadOnly(ge, 'ni.' + ge.fieldName, disabled);
    var suggestionExists = gel(
      'ni.dependent_reverse.' + this.tableName + '.' + fieldName
    );
    if (suggestionExists) {
      this._setFieldReadOnly(
        ge,
        'lookup.' + this.tableName + '.' + fieldName,
        disabled,
        true
      );
    }
  ...
  _setFieldReadOnly: function (ge, s, disabled, skipProcessLabelChange) {
    ...
    if (control) {
      ...
      if (!skipProcessLabelChange) {
        value =
          (ge.type === 'reference' || ge.type === 'glide_list') &&
          ge.getElement()
            ? ge.getElement().value
            : control.value;
  ...
  onSubmit: function () {
    ...
    if (
      action == 'sysverb_back' ||
      action == 'sysverb_cancel' ||
      action == 'sysverb_delete' ||
    ...
    for (var i = 0; i < fields.length && !focusDetermined; i++) {
      ...
      if (!tryLabel) {
        try {
          ...
          if (formEle) {
            var firstInvalidEle = formEle.querySelector(
              "[aria-invalid='true']"
            );
      ...
      if (tryLabel) {
        ...
        if (displayWidget && displayWidget.getAttribute('type') == 'hidden') {
          var streamDisplayWidget = gel(
            'activity-stream-' + field + '-textarea'
          );
  ...
  mandatoryCheck: function () {
    if (
      !this.checkMandatory ||
      (!this.modified && this.mandatoryOnlyIfModified)
    )
      return true;
    var rc = true;
    ...
    if (!rc) {
      ...
      theText =
        getMessage('The following mandatory fields are not filled in') +
        ': ' +
        theText;
      try {
  ...
  setVariablesReadOnly: function (readOnly) {
    for (var x = 0; x < g_form.elements.length; x++) {
      for (var i = 0; i < this.nameMap.length; i++) {
        ...
        if (
          entry.realName == element.fieldName &&
          element.tableName == 'variable'
        ) {
          if (!this._hasSCForm()) this.setReadOnly(entry.prettyName, readOnly);
          else g_sc_form.setReadOnly(entry.prettyName, readOnly);
        }
      }
    }
  },
  getHelpTextControl: function (variableName) {
    ...
    if (handler) {
      ...
      ele = handlerObject.getHelpTextControl(
        handlerObject.removeCurrentPrefix(variableName)
      );
    }
    if (!ele) {
      jslog(
        'getHelpTextControl is supported for only Service Catalog Variables'
      );
      return;
    }
    return ele;
  ...
  getEditableFields: function () {
    ...
    for (var i = 0; i < fa.length; i++) {
      ...
      if (!widget) continue;
      if (this.isEditableField(ed, widget)) answer.push(ed.fieldName);
    }
    return answer;
  },
  isEditableField: function (ge, control) {
    var isEditableFieldFn = this._getHandlerFn(control, 'isEditableField');
    if (isEditableFieldFn) return isEditableFieldFn(ge, control);
    if (!this.isTemplateCompatible(ge, control)) return false;
    if (!this.isVisible(ge, control)) return false;
    if (this.isReadOnly(ge, control)) return false;
    if (this.isDisplayNone(ge, control)) return false;
    return true;
  },
  ...
  isVisible: function (ge, control) {
    ...
    if (
      control &&
      ge.getType() != 'glide_duration' &&
      ge.getType != 'glide_time'
    ) {
      var xx = control.style['visibility'];
      if (xx == 'hidden') {
        var readOnlyField = gel('sys_readonly.' + control.id);
        if (
          (readOnlyField && readOnlyField.style['visibility'] == 'hidden') ||
          !readOnlyField
        )
          return false;
      }
    ...
    if (xx == 'hidden' || this._elementHasClass(ge, 'hidden')) return false;
    var tab = this._getTabNameForElement(control);
    if (tab && !this.isSectionVisible(tab)) return false;
    if (!control && !ge.getElementParentNode()) return false;
    return true;
  },
  ...
  isReadOnly: function (ge, control) {
    ...
    return (
      control.disabled ||
      control.readOnly ||
      (typeof control.hasClassName === 'function' &&
        control.hasClassName('readonly')) ||
      (typeof control.getAttribute === 'function' &&
        control.getAttribute('writeaccess') == 'false')
    );
  },
  isDisplayNone: function (ge, control) {
    var parentNode = ge.getElementParentNode();
    if (parentNode && parentNode.style.display == 'none') return true;
    if (
      ge.getType() == 'html' ||
      ge.getType() == 'html_script' ||
      ge.getType() == 'translated_html' ||
      ge.getType() == 'composite_name' ||
      ge.getType() == 'url'
    )
      return false;
    if (!control) return;
      ...
      if (xx == 'none') {
        var readOnlyField = gel('sys_readonly.' + control.id);
        if (
          (readOnlyField && readOnlyField.style['display'] == 'none') ||
          !readOnlyField
        )
          return true;
      }
    ...
    if (xx == 'none') return true;
    return false;
  },
  _getElementStyle: function (ge, style) {
    var element = ge.getElementParentNode();
    if (element) return element.style[style];
    var labelElement = ge.getLabelElement();
    if (labelElement) return labelElement.parentNode.parentNode.style[style];
    var parentTR = findParentByTag(ge.getElement(), 'tr');
    if (parentTR && parentTR != labelElement) return parentTR.style[name];
    return '';
  },
  ...
  getMissingFields: function () {
    ...
    for (var i = 0; i < fa.length; i++) {
      var ed = fa[i];
      if (!ed.isMandatory()) continue;
      var widget = this.getControl(ed.fieldName);
      if (!widget) continue;
      if (this._isMandatoryFieldEmpty(ed)) answer.push(ed.fieldName);
    }
    if (this._hasSCForm()) g_sc_form.getMissingFields(answer);
    return answer;
  },
  _isMandatoryFieldEmpty: function (ed) {
    var widgetValue = this.getValue(ed.fieldName);
    if (widgetValue != null && widgetValue != '') return false;
    if (ed.supportsMapping) {
      var id = 'sys_mapping.' + ed.tableName + '.' + ed.fieldName;
      var mappingValue = this.getValue(id);
      if (mappingValue.trim()) return false;
      var nonMappedFieldValue = this.getValue(
        ed.tableName + '.' + ed.fieldName
      );
      if (nonMappedFieldValue.trim()) return false;
      return true;
    }
    ...
    if (displayBox != null) {
      ...
      if (
        displayValue != null &&
        displayValue != '' &&
        displayBox.getAttribute('data-ref-dynamic') == 'true'
      ) {
        return false;
      }
    }
    if (this.isNewRecord() || this.mandatory) return true;
  ...
  resolvePrettyNameMap: function (realName) {
    ...
    for (var i = 0; i < this.nameMap.length; i++)
      if (
        'ni.VE' + this.nameMap[i].realName == realName ||
  ...
  getControlByForm: function (fieldName) {
    ...
    if (!widget) widget = form[fieldName];
    if (!widget)
      widget = $j(form)
        .find("[data-action-name='" + fieldName + "']")
        .get(0);
    if (
      widget &&
      ...
      for (var i = 0; i < widget.length; i++) {
        if (widget[i].checked) return widget[i];
        else if (
          widget[i].id == fieldName &&
          widget[i].tagName == 'INPUT' &&
          widget[i].type != 'radio'
        )
          return widget[i];
      }
      var wt = widget[0].type;
      if (typeof wt != 'undefined' && wt == 'radio') return widget[0];
    }
    return widget;
  },
  ...
  getLabelOf: function (fieldName) {
    ...
    if (widgetLabel) {
      labelContent =
        $j(widgetLabel).find('.label-text').text() ||
        widgetLabel.innerText ||
        widgetLabel.textContent;
      if (
        labelContent.indexOf('*') == 0 &&
      ...
      if (labelContent.lastIndexOf(':') + 1 == labelContent.length)
        labelContent = labelContent
          .toString()
          .substring(0, labelContent.length - 1);
    }
    if (labelContent == null || labelContent == '')
      labelContent = this._tryLabelRow(fieldName);
    if (labelContent == null || labelContent == '') {
      var handler = this.getPrefixHandler(this.resolvePrettyNameMap(fieldName));
      if (handler) labelContent = handler.getObject().getLabelOf(fieldName);
      else labelContent = fieldName;
    }
  ...
  setLabelOf: function (fieldName, value) {
    ...
    if (
      this.elementHandlers[control.id] &&
      typeof this.elementHandlers[control.id].setLabelOf == 'function'
    )
  ...
  _getDecorationsEl: function (field) {
    ...
    if (!decorations.length) {
      $j(
        '<span class="field_decorations" data-label-decorations="[]" />'
      ).prependTo(label);
  ...
  _setDecorations: function (fieldName, decorations) {
    ...
    if (!isArr(decorations)) return false;
    var attrName = 'data-label-decorations';
    var labelEl = this._getLabelEl(fieldName);
    if (labelEl) {
  ...
  addDecoration: function (field, icon, text, color) {
    ...
    for (var i = 0; i < maxi; i++) {
      var dec = decorations[i];
      if (dec.icon == icon && dec.text == text && dec.color == color) {
        isDuplicate = true;
      }
    }
    if (!isDuplicate) decorations.push(deco);
  ...
  setSectionDisplay: function (name, display) {
    ...
    if (display) g_tabs2Sections.showTab(index);
    else g_tabs2Sections.hideTab(index);
    return true;
  },
  isSectionVisible: function (name) {
    var index = g_tabs2Sections.findTabIndexByName(name);
    if (index !== -1) return g_tabs2Sections.isVisible(index);
    return false;
  },
  activateTab: function (name) {
    var index = g_tabs2Sections.findTabIndexByName(name);
    if (index !== -1) return g_tabs2Sections.setActive(index);
    return false;
  },
  getTabNameForField: function (fieldName) {
    if (!g_form.hasField(fieldName)) return null;
    var control = g_form.getControl(fieldName);
  ...
  _instanceofAny: function (obj, classes) {
    ...
    for (var i = 0, iMax = classes.length; i < iMax; i += 1) {
      ...
      if (typeof aClass === 'undefined') continue;
      var constructor = obj.constructor;
      while (constructor) {
        if (constructor === aClass) return true;
        constructor = constructor.parent;
      }
      if (obj instanceof aClass) return true;
    }
    return false;
  },
  _getVisibleControls: function (field) {
    ...
    if (
      widgets.length &&
      this._instanceofAny(widgets, ['RadioNodeList', 'HTMLCollection'])
    )
  ...
  validate: function () {
    ...
    for (var i = 0; i < fa.length; i++) {
      ...
      var widget = this.getControl(ed.fieldName);
      if (!widget) continue;
      if (!this.isEditableField(ed, widget)) continue;
      if (widget.getAttribute('validate') == 'false') continue;
      var widgetValue = widget.value;
      var widgetType = ed.type;
      var specialType = widget.getAttribute('specialtype');
      if (specialType) widgetType = specialType;
    ...
    for (var i = 0; i < labels.length; i++) this.flash(labels[i], '#FFFACD', 0);
    for (var i = 0; i < invalidWidgets.length; i++) {
      for (var j = 0; j < invalidWidgets[i].length; j++) {
        invalidWidgets[i][j].setAttribute('aria-invalid', 'true');
      }
  ...
  isNumeric: function (internaltype) {
    if (internaltype == 'decimal') return true;
    if (internaltype == 'float') return true;
    if (internaltype == 'integer') return true;
    if (internaltype == 'numeric') return true;
    return false;
  },
  ...
  setTemplateValue: function (fieldName, value, displayValue) {
    fieldName = this.removeCurrentPrefix(fieldName);
    var control = this.getControl(fieldName);
    if (control) control.templateValue = 'true';
  ...
  setValue: function (fieldName, value, displayValue, fromSetTemplateValue) {
    ...
    if (!control) {
      ...
      if (handler)
        handler
          .getObject()
          .setValue(handler.getFieldName(), value, displayValue);
      return;
    } else {
      ...
      if (control.options && control.options.length) {
        for (var i = 0; i < control.options.length; i++) {
          ...
          if (control.options[i].value === value) isChoicePresent = true;
        }
        if (
          !isChoicePresent &&
  ...
  getDisplayBox: function (fieldName) {
    ...
    if (handler) {
      var handlerObject = handler.getObject();
      return handlerObject.getDisplayBox(
        handlerObject.removeCurrentPrefix(fieldName)
      );
    }
    return;
  },
  clearValue: function (fieldName) {
    ...
    if (
      control.id &&
      this.elementHandlers[control.id] &&
      typeof this.elementHandlers[control.id].clearValue === 'function'
    ) {
  ...
  _setReadOnlyValue: function (fieldName, value, displayValue, control) {
    ...
    if (readOnlyField) {
      if (g_form.getGlideUIElement(fieldName).type === 'currency') {
        ...
        else readOnlyField.innerHTML = htmlEscape(value);
      ...
      if (readOnlyField) {
        ...
        else readOnlyField.value = displayValue;
      }
    }
    if (readOnlyField) $j(readOnlyField).trigger('autosize.resize');
    return updatedValue;
  },
  _setValue: function (
    fieldName,
    ...
    var defaultControlValue = this._setReadOnlyValue(
      fieldName,
      value,
      displayValue,
      control
    ...
    if (
      ...
      if (isSelect2) $j(control).select2('destroy');
      var i = this._getSelectedIndex(control, value, displayValue);
      control.selectedIndex = i;
      if (isSelect2) $j(control).select2().select2('val', value);
      if (!control.options[i].value)
        this._setEmptyMandatoryFieldsVisible(fieldName);
      onChange(this.tableName + '.' + fieldName, fromSetTemplateValue);
      ...
      if (nibox && nibox.type == 'checkbox') {
        ...
        if (value && value == 'false') nibox.checked = null;
        else if (value || value == 'true') nibox.checked = 'true';
        else nibox.checked = null;
        setCheckBox(nibox);
        return;
      }
      ...
      if (displaybox) {
        ...
        if (typeof displayValue === 'undefined' && value)
          displayValue = this._ensureDisplayValue(
            fieldName,
            value,
            displayValue,
            displaybox,
        ...
        $(control).hasClassName('glide_destroy_filter') ||
        $(control).hasClassName('glideform-set-value')
      ) {
        $j(control).val(value);
        onChange(this.tableName + '.' + fieldName);
      } else {
  ...
  _getDependentFieldErrorObject: function (fieldName) {
    return {
      success: false,
      message: new GwtMessage().getMessage(
        'Value of the field <b>{0}</b> is incorrect.',
        fieldName
      ),
    };
  },
  ...
  _ensureDisplayValue: function (
    ...
    if (
      ...
      var dependentFieldValue = dependentField
        ? g_form.getValue(dependentField)
        : '';
  ...
  getValue: function (fieldName) {
    ...
    if (!control) {
      var handler = this.getPrefixHandler(fieldName);
      if (handler) return handler.getObject().getValue(handler.getFieldName());
      return '';
    }
  ...
  getBooleanValue: function (fieldName) {
    var val = this.getValue(fieldName);
    val = val ? val + '' : val;
    if (!val || val.length == 0 || val == 'false') return false;
    return true;
  },
  addOption: function (fieldName, choiceValue, choiceLabel, choiceIndex) {
    ...
    if (isSelect2) {
      if (!choiceLabel) choiceLabel = choiceValue;
    ...
    var evalLength;
    if (!isSelect2) evalLength = len;
    else if (isSelect2) evalLength = len - 1;
    for (i = 0; i < evalLength; i++) {
      if (control.options[i].text == choiceLabel) {
        return;
      }
    }
    if (choiceIndex == undefined) choiceIndex = len;
    if (choiceIndex < 0 || choiceIndex > len) choiceIndex = len;
    var newOption = new Option(choiceLabel, choiceValue);
    ...
    if (isSelect2) control.options[len].remove();
  ...
  enableOption: function (control, choiceValue, choiceLabel) {
    ...
    for (var i = len - 1; i >= 0; i--) {
      if (
        control.options[i].text == choiceLabel &&
        control.options[i].value == choiceValue
      ) {
  ...
  clearOptions: function (fieldName) {
    ...
    if (!control) {
      var handler = this.getPrefixHandler(fieldName);
      if (handler) handler.getObject().clearOptions(handler.getFieldName());
      return;
    }
    if (!control.options) return;
    control.innerHTML = '';
  ...
  getOption: function (fieldName, choiceValue) {
    ...
    for (var i = 0; i < options.length; i++) {
      var option = options[i];
      if (option.value == choiceValue) return option;
    }
    return null;
  },
  removeContextItem: function (itemID) {
    for (av in contextMenus) {
      if (contextMenus[av]) {
        ...
        var c = menu.context;
        if (c) this.removeItem(menu, itemID);
      }
    }
  },
  ...
  getGlideUIElement: function (fieldName) {
    ...
    for (var i = 0; i < this.elements.length; i++) {
      ...
      if (thisElement.fieldName == fieldName) return thisElement;
    }
  },
  getDerivedFields: function (fieldName, immediateOnly) {
    ...
    if (immediateOnly !== true) return list;
  ...
  _hasDerivedInBetween: function (derived, allDerived) {
    ...
    for (var i = 0; i < allDerived.length; i++) {
      var d = allDerived[i];
      if (d === derived || d.split('.').length >= levels) break;
      if (derived.startsWith(d + '.')) return true;
    }
    return false;
  },
  _addDerivedWaiting: function (fieldName, refFieldName, isReadOnly) {
    if (!this.derivedWaiting[fieldName])
      this.derivedWaiting[fieldName] = {
        fieldName: fieldName,
        refFieldName: refFieldName,
        isReadOnly: isReadOnly || false,
      };
    if (!isReadOnly) this._setDerivedWaitingReadonly(fieldName, true);
  },
  ...
  getReference: function (fieldName, callback) {
    ...
    if (
      window['g_optics_inspect_handler'] &&
      g_optics_inspect_handler.opticsContextStack.length > 0
    )
      opticsContext =
        g_optics_inspect_handler.opticsContextStack[
          g_optics_inspect_handler.opticsContextStack.length - 1
        ];
    fieldName = this.removeCurrentPrefix(fieldName);
    var ed = this.getGlideUIElement(fieldName);
    if (!ed) {
      ...
      if (handler)
        return handler
          .getObject()
          .getReference(handler.getFieldName(), callback);
      return;
    }
    if (ed.type != 'reference' && ed.type != 'domain_id') return;
    var value = this.getValue(fieldName);
    var gr = new GlideRecord(ed.reference);
    if (value == '') {
      if (callback) callback(gr);
      return gr;
    }
    if (window.g_event_handlers_localCache && !callback) {
      ...
      if (typeof cachedObj !== 'undefined' && cachedObj.sys_id === value) {
        jslog(
          'GlideForm: getReference cache hit on ' +
            this.getTableName() +
            '.' +
            fieldName +
            ':' +
            value +
            ', skipped synchronous ajax call'
        );
        return cachedObj;
      }
    }
    ...
    if (callback) {
      var fn = function (gr) {
        ...
        if (opticsContext)
          CustomEvent.fire(
            'glide_optics_inspect_put_context',
            opticsContext['category'],
            opticsContext['name'],
            opticsContext['sys_id']
          );
        callback(gr);
        if (opticsContext) CustomEvent.fire('glide_optics_inspect_pop_context');
      };
      gr.query(fn);
      return;
    } else {
      ...
      sw.jslog(
        '*** WARNING *** GlideForm: synchronous getReference call on ' +
          this.getTableName() +
          ' for ' +
          fieldName
      );
      if (g_event_handlers_queryTracking) {
        gr.addAdditionalParam(
          'sysparm_reference_query_table',
          this.getTableName()
        );
  ...
  submit: function (actionName) {
    ...
    if (!action) action = this._getLinkActionByAttribute(actionName);
    if (action) return gsftSubmit(action);
  },
  ...
  getActionName: function () {
    ...
    if (form) {
      ...
      if (theButton) return theButton.value;
    }
    return this.action;
  },
    ...
    for (var i = 0; i < elements.length; i++) {
      if (filterFunc(elements[i])) {
        ...
        if (elements[i].disabled)
          queryComponent =
            elements[i].id + '=' + encodeURIComponent(elements[i].value);
        else queryComponent = Form.Element.serialize(elements[i]);
  ...
  _serializeDisabled: function () {
    ...
    for (var i = 0; i < n; i++) {
      ...
      if (!e.value || !e.id) continue;
      dfa.push(e.id + '=' + encodeURIComponent(e.value));
    }
    if (dfa.length) return '&' + dfa.join('&');
    return '';
  },
  ...
  serializeTargetFields: function () {
    var s =
      this._serializeElement('sys_target') ||
      this._serializeElementIfEmpty('sys_target', this.getTableName());
  ...
  _serializeElement: function (id) {
    ...
    if (e) {
      ...
      if (queryComponent) return '&' + queryComponent;
    }
    return '';
  },
  changedFieldsFilter: function (element) {
    ...
    if (element.id.startsWith('ni.VE') || element.id.startsWith('ni.QS')) {
      if (element.id.endsWith('read_only')) return false;
      return true;
    }
    if (element.name.startsWith('ni.WATERMARK')) return true;
    return false;
  },
  ...
  flash: function (widgetName, color, count) {
    ...
    if (realLabel) row = $(realLabel.parentNode.parentNode);
    else {
      jslog(
        "flash() called for '" + widgetName + "' but there is no label for it"
      );
      return;
    }
    var temp = row.select('.label_left');
    for (var i = 0; i < temp.length; i++) labels.push(temp[i]);
  ...
  getRelatedListNames: function () {
    if (window.NOW.g_relatedLists) return window.NOW.g_relatedLists.lists;
  ...
  _getRelatedListID: function (listTableName) {
    ...
    var relatedListId;
    if (relatedList) relatedListId = relatedList.getAttribute('id');
    if (!relatedListId)
  ...
  findV2RelatedListName: function (listTableName) {
    ...
    for (var i = 0; i < rlw.childNodes.length; i++) {
      ...
      if (idName.length == 0) continue;
      if (idName.indexOf(compareId) > -1) return node;
    }
    if (typeof GlideLists2 == 'undefined') return;
    for (var id in GlideLists2) {
      var list = GlideLists2[id];
      if (
        list.getTableName() == listTableName &&
        list.getParentTable() == tableName
      )
  ...
  _addFormMessage: function (msg, type, id) {
    ...
    if (scrollToMsg && scrollToMsg.value === 'true') msg = htmlEscape(msg);
  ...
  showFieldMsg: function (input, message, type, scrollForm, key) {
    ...
    if (!inputElement || !message) {
      jslog(
        "ERROR: invalid field or missing message passed to g_form.showFieldMsg('" +
          input +
          "','" +
          message +
          "')"
    ...
    if (
      inputElement.id &&
      this.elementHandlers[inputElement.id] &&
      typeof this.elementHandlers[inputElement.id].showFieldMsg == 'function'
    )
    ...
    if (!fieldRow) {
      jslog(
        "Error adding field message. Couldn't find the row to add the field message to."
      );
      return;
    }
    doc = fieldRow.ownerDocument;
    var inputContainer = inputElement.up('.form-field');
    if (!inputContainer) inputContainer = inputElement.up();
    ...
    if (!fieldMsgContainer) {
      ...
      if (isVariable) fieldMsgContainer.className = 'variable-field-msg';
      ...
      if (
        inputElement.getAttribute('data-type') === 'glide_element_date_time'
      ) {
    ...
    if (key)
      fieldMsgRow.setAttribute(
        'data-fieldmsg-key',
        inputElement.id + '_fieldmsg_' + key
      );
    ...
    if (
      !fieldRow.visible() &&
      fieldRow.getAttribute('data-type') == 'journal_input'
    ) {
      ...
      else fieldRow = $('activity-stream-textarea');
    }
    if (!positionedCursor)
  ...
  _scrollToElementTR: function (labelTR, msgTR, scrollForm) {
    ...
    if (typeof scroll == 'undefined') {
      ...
      if (scrollToMsg == null) scroll = true;
      else {
        if (scrollToMsg.value != 'false') scroll = true;
        else scroll = false;
      }
    }
    if (!scroll) return;
    ...
    var headerHeight = 0;
    if (titleDiv && titleDiv.clientHeight) headerHeight = titleDiv.clientHeight;
    ...
    if (scrollDiv.id)
      setTimeout(
        "$('" + scrollDiv.id + "').scrollTop = " + scrollTopSetting,
        100
      );
    else scrollDiv.scrollTop = scrollTopSetting;
  },
  ...
  hideFieldMsg: function (input, clearAll, key) {
    ...
    if (!inputElement) {
      jslog(
        "ERROR: invalid field ('" + input + "') passed to g_form.hideFieldMsg"
      );
      return;
    }
    if (
      inputElement.id &&
      this.elementHandlers[inputElement.id] &&
      typeof this.elementHandlers[inputElement.id].hideFieldMsg == 'function'
    )
    ...
    if (!tr) {
      ...
      if (!parent) parent = inputElement.up('td');
      if (parent) {
    ...
    if (clearAll) msgContainer.update('');
    else {
      var selector;
      if (key)
        selector =
          '[data-fieldmsg-key="' + inputElement.id + '_fieldmsg_' + key + '"]';
      else selector = '.fieldmsg';
  ...
  _positionCursorAtError: function (elem, message) {
    if (typeof elem == 'undefined' || elem.disabled) return false;
    ...
    if (index > -1) {
      ...
      if (parenIndex > -1) {
        ...
        } else return this._setCaretPositionLineColumn(elem, lineNo, 1);
      }
    }
    return false;
  },
  _setCaretPositionLineColumn: function (elem, lineNo, columnNo) {
    ...
    position += columnNo - 1;
    if (elem.createTextRange) position -= lineNo - 1;
    try {
  ...
  _setCaretPosition: function (elem, caretPos) {
    if (elem.createTextRange) {
      ...
      if (elem.setSelectionRange) {
        ...
        if (isSafari || isChrome) elem.focus();
        else {
  ...
  setLiveUpdateOriginalValue: function (fieldName, value, displayValue) {
    ...
    if (!element) return;
    if (
      this.elementHandlers[element.id] &&
      typeof this.elementHandlers[element.id].setLiveUpdateOriginalValue ==
        'function'
    )
      return this.elementHandlers[element.id].setLiveUpdateOriginalValue(
        value,
        displayValue
      );
  ...
  insertContentAtCursor: function (fieldName, content) {
    ...
    if (
      this.elementHandlers[element.id] &&
      typeof this.elementHandlers[element.id].insertContentAtCursor ==
        'function'
    )
    ...
    if (typeof originalContent == 'string') {
      var newContent =
        originalContent.substr(0, cursorLocation) +
        content +
        originalContent.substr(cursorLocation);
      this.setValue(fieldName, newContent);
    }
  },
  onUserChangeValue: function (handler) {
    if (
      this._onUserChangedHandlers != null &&
      this._onUserChangedHandlers.indexOf(handler) === -1
    )
      this._onUserChangedHandlers.push(handler);
    var gf = this;
    return function () {
      var index = gf._onUserChangedHandlers.indexOf(handler);
      if (index > -1) gf._onUserChangedHandlers.splice(index, 1);
    };
  },
  type: 'GlideForm',
});
