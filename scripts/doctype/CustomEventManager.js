...
var CustomEventManager = (function (existingCustomEvent) {
  ...
  function observe(eventName, fn) {
    if (trace) jslog('$CustomEventManager observing: ' + eventName);
    on(eventName, fn);
  }
  ...
  function un(name, func) {
    ...
    if (idx >= 0) events[name].splice(idx, 1);
  }
  function unAll(name) {
  ...
  function fireUp(eventName, args) {
    ...
    while (win) {
      try {
        if (win.CustomEvent.fireEvent.apply(null, arguments) === false) return;
  ...
  function shouldProcessMessage(sourceOrigin) {
    if (
      !window.g_concourse_onmessage_enforce_same_origin ||
      sourceOrigin === window.location.origin
    ) {
      return true;
    }
    return isOriginInWhiteList(
      sourceOrigin,
      window.g_concourse_onmessage_enforce_same_origin_whitelist
    );
  }
  ...
  function fireTop(eventName, args) {
    ...
    var t = getTopWindow();
    if (t !== null && window !== t) t.CustomEvent.fire(eventName, args);
  }
    ...
    function notifyAllFrom(rootFrame) {
      ...
      for (var i = 0; i < rootFrame.length; i++) {
        try {
          ...
          if (
            childFrame.CustomEvent &&
            typeof childFrame.CustomEvent.fireEvent === 'function'
          ) {
  ...
  function fireToWindow(
    ...
    if (trace)
      jslog(
        '$CustomEventManager firing: ' + eventName + ' args: ' + args.length
      );
    if (usePostMessage) {
  ...
  function getTopWindow() {
    ...
    try {
      while (
        topWindow.CustomEvent.fireEvent &&
