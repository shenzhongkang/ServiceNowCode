...
(function () {
  if (window.NOW.htmlClassSetterInitialized) return;
  ...
  function setDateClass() {
    ...
    if (df.dateBoth) {
      ...
      if (shortDateFormat) $h.addClass('date-calendar-short');
      else $h.addClass('date-calendar');
    } else if (df.timeAgo) $h.addClass('date-timeago');
    else {
      if (shortDateFormat) $h.addClass('date-calendar-short');
      else $h.addClass('date-calendar');
    }
  ...
  function setDateTimeVisibilityToSR() {
    if (!window.g_accessibility) return;
  ...
  var toggleTemplate = function (trueFalse) {
    var bool =
      typeof trueFalse !== 'undefined' ? trueFalse : !window.NOW.templateToggle;
  ...
  function setTemplateToggle() {
    ...
    if (window.NOW.templateToggle) {
      ...
      ariaLiveMsg = getMessage(
        'Added Template bar landmark to bottom of form.'
      );
    } else {
  ...
  function setCompact() {
    ...
    if (modalDiv) modalDiv = modalDiv.childNodes[0];
    var $pH;
    if (parent.$j) $pH = parent.$j('HTML');
    if (window.NOW.compact) {
      $h.addClass('compact');
      if ($pH) $pH.addClass('compact');
      if (modalDiv && modalDiv.className.indexOf(' compact') == -1)
        modalDiv.className += ' compact';
    } else {
      $h.removeClass('compact');
      if ($pH) $pH.removeClass('compact');
      if (modalDiv && modalDiv.className.indexOf(' compact') > -1)
  ...
  function setTabbed() {
    if (window.NOW.tabbed) $h.addClass('tabbed');
    else $h.removeClass('tabbed');
  }
  setTabbed();
  function setListTableWrap() {
    if (window.NOW.listTableWrap)
      $j('HTML').removeClass('list-nowrap-whitespace');
    else $j('HTML').addClass('list-nowrap-whitespace');
  }
  setListTableWrap();
...
function printList(maxRows) {
  ...
  if (
    mainWin &&
    mainWin.CustomEvent &&
    mainWin.CustomEvent.fire &&
    mainWin.CustomEvent.fire('print', maxRows) === false
  )
    return false;
  var veryLargeNumber = '999999999';
  var print = true;
  var features =
    'resizable=yes,scrollbars=yes,status=yes,toolbar=no,menubar=yes,location=no';
  if (isChrome && isMacintosh) features = '';
  var href = '';
  ...
  if (!href) {
    ...
    if (form && form.sysparm_referring_url) {
      ...
      if (
        href.indexOf('?sys_id=-1') != -1 &&
        !href.startsWith('sys_report_template')
      ) {
  ...
  href +=
    'sysparm_stack=no&sysparm_force_row_count=' +
    veryLargeNumber +
    '&sysparm_media=print';
  if (print) {
    if (href != null && href != '') {
