...
addAfterPageLoadedEvent(function () {
  ...
  document.on(
    'click',
    '[data-type="reference_popup"], [data-type="reference_hover"], [data-type="new_window_reference_popup"]',
    function (evt, element) {
      evt.preventDefault();
      var ref = element.getAttribute('data-ref');
  ...
  document.body.on(
    'click',
    'img[data-type="section_toggle"], span[data-type="section_toggle"]',
    function (evt, element) {
  ...
  document.body.on(
    'click',
    '[data-type="user_roles_lock"]',
    function (evt, element) {
      var ref = element.getAttribute('data-ref');
      $j(gel(ref + '_edit_modal'))
        .data('gWindow')
        .destroy();
      evt.stop();
    }
  ...
  document.body.on(
    ...
    function (evt, element) {
      ...
      var spaceElement = $('make_spacing_ok_' + ref);
      if (spaceElement) spaceElement.style.display = 'inline';
      toggleAddMe(ref);
      evt.stop();
    }
  );
  ...
  document.body.on(
    'click',
    '[data-type="glide_list_add_me_locked"]',
    function (evt, element) {
      var ref = element.getAttribute('data-ref');
      var u = element.getAttribute('data-user-id');
      var name = element.getAttribute('data-user').replace(/\\'/, "'");
  ...
  document.on('contextmenu', 'div[data-type="label"]', function (evt, element) {
    if (!elementAction(element, evt)) evt.stop();
  });
  document.on(
    'contextmenu',
    'nav[data-type="section_head"]',
    function (evt, element) {
      var id = element.getAttribute('data-id');
      if (!contextShow(evt, '1', -1, 0, 0)) evt.stop();
    }
  );
});
