...
MESSAGES_FILTER_BUTTONS = [
  ...
  'Delete',
  'Remove Condition',
  'Add AND Condition To Condition',
  'Add OR Condition To Condition',
  'Operator For Condition',
  'Insert HTML color name or hex value',
  'Separate individual references with a comma',
  'Use the following format: TableName.FieldName',
  "1 for Monday, 2 for Tuesday, etc. Multiple values can be entered, like '135' for Monday, Wednesday, and Friday",
  'Enter text in HTML format',
];
MESSAGES_FILTER_MISC = [
  'Run Filter',
  'Run',
  'Order results by the following fields',
  'New sort order condition added, {0} of {0}',
];
...
GlideFilter.prototype = {
  ...
  initialize: function (
    ...
    if (typeof synchronous != 'undefined') this.synchronous = synchronous;
    ...
    if (fDiv != null) this.divName = fDiv + 'gcond_filters';
    ...
    var mappingId = (this.mappingId = this.fDiv.id.replace(
      /gcond_filters$/,
      ''
    ));
    this.elementSupportsMapping = !!gel('sys_mapping.' + this.mappingId);
    this.sortElement = $('gcond_sort_order');
    if (this.sortElement) this.sortElement.filterObject = this;
    else this.sortElement = this.fDiv;
    this.sections = [];
  ...
  init2: function () {
    if (typeof this.query != 'undefined') {
      if (this.synchronous) this.setQuery(this.query);
      else this.setQueryAsync(this.query);
    } else {
      this.reset();
    }
  ...
  setSectionClasses: function () {
    ...
    if (!this.conditionsWanted) return;
    var showOr = false;
    $j('button.new-or-button').hide();
    var sections = this.sections;
    if (sections.length == 1) {
      var i = sections[0].conditions.length;
      if (i == 0) tbody.addClassName('sn-filter-empty');
      else if (i == 1) showOr = true;
      else if (i > 1) {
    ...
    if (sections.length > 0)
      $j(sections[sections.length - 1].table).addClass(
        'sn-animate-filter-clause'
      );
    if (showOr) $j('button.new-or-button').show();
  },
  setOptionsFromParmsElement: function (name) {
    ...
    if (ut) this.setUsageContext(ut);
    var uv = elem.getAttribute('data-use-variables');
    if (uv) this.setUseVariables(uv);
    ...
    if (eo) this.setIncludeExtended(eo);
  },
  ...
  _clearSections: function () {
    for (var i = 0; i < this.sections.length; i++) this.sections[i].destroy();
    if (this.sortSection) this.sortSection.destroy();
    this.sections = [];
  ...
  initMessageCache: function () {
    ...
    for (var key in sysvalues) {
      ...
      for (var i = 0; i < keys.length; i++) all[keys[i]] = 't';
    }
    for (key in sysopers) {
    ...
    for (var i = 0; i < m.length; i++) all[m[i]] = 't';
    var send = [];
    for (key in all) send.push(key);
  ...
  setRestrictedFields: function (fields) {
    ...
    if (!fields) return;
    var delimiter = ',';
    if (fields.indexOf(';') > -1) delimiter = ';';
    var fa = fields.split(delimiter);
    if (fa.length == 0) return;
    this.restrictedFields = {};
    for (var i = 0; i < fa.length; i++) this.restrictedFields[fa[i]] = fa[i];
  },
  ...
  addIgnoreFields: function (fields) {
    var fa = fields.split(',');
    if (fa.length == 0) return;
    for (var i = 0; i < fa.length; i++) this.ignoreFields[fa[i]] = fa[i];
  },
  ...
  filterFields: function (item) {
    ...
    if (!this.restrictedFields) {
      if (this.ignoreFields[fieldName]) return false;
      if (this.ignoreFieldTypes[fieldType]) return false;
      return true;
    }
    if (fieldName && fieldName.indexOf('.') > -1) return false;
    if (this.restrictedFields[fieldName]) return true;
    return false;
  },
  ...
  setQueryAsync: function (query, defaultVal) {
    this.addLoadingIcon();
    this.glideQuery = new GlideEncodedQuery(
      this.tableName,
      query,
      this.setQueryCallback.bind(this)
    );
    if (defaultVal) this.defaultVal = defaultVal.split(',');
    this.glideQuery.parse();
  ...
  setHasKeyword: function (b) {
    ...
    if (!controlsNQ[0]) {
      ...
      if (b) this.createDisabledOrWrap(orButton);
      else this.unwrapAndEnableOrButton(orButton.parent()[0]);
    }
  },
  ...
  createDisabledOrWrap: function (orButton) {
    ...
    orButton.wrap(
      $j("<span title='" + this.orMessage + "' tabIndex='0'></span>")[0]
    );
  },
  ...
  reset: function () {
    ...
    if (e.tagName == 'TBODY') {
      ...
      for (var i = 0; i < e.childNodes.length; i++) {
        ...
        if ($(tr).hasClassName('no_remove')) continue;
        toRemove.unshift(i);
      }
  ...
  build: function () {
    ...
    this.buildQuery();
    this.buildOrderBy();
    if (this.sections.length > 0) this.orCount += this.sections.length - 1;
    else this.orCount = 0;
  },
  buildOrderBy: function () {
    ...
    for (var i = 0; i < orderArray.length; i++) {
      ...
      if (order.isAscending()) this.addSortRow(order.getName(), 'ascending');
      else this.addSortRow(order.getName(), 'descending');
    }
  },
  buildQuery: function () {
    'use strict';
  ...
  _loadTablesForField: function (fieldName) {
    ...
    for (var i = 0; i < parts.length; i++) {
      var tableDef = loadFilterTableReference(
        tableName,
        this.isTemplate,
        this.applyTemplateAcls
      );
      if (!tableDef) return;
  ...
  findSection: function (queryID) {
    if (queryID) {
      ...
      if (i != null) return this.sections[i];
    }
    return null;
  },
  ...
  addRunButton: function () {
    ...
    if (section == null) section = this.sections[this.sections.length - 1];
  ...
  _toggleReadOnlyAttribute: function (disabled) {
    ...
    if (fieldName) element = gel(fieldName);
  ...
  _hideClass: function (parentElement, className, hideIt) {
    var elements = $(parentElement).select('.' + className);
    for (var i = 0; i < elements.length; i++) {
      if (hideIt) hideObject(elements[i]);
      else showObjectInline(elements[i]);
    }
    if (this.type == 'GlideTemplateFilter') {
      if (hideIt && parentElement && parentElement.parentElement)
        parentElement.parentElement.removeChild(parentElement);
      else showObjectInline(parentElement);
    }
  },
  getValue: function () {
...
GlideFilterSection.prototype = {
  ...
  destroy: function () {
    this.runRow = null;
    if (this.runCondition) this.runCondition.destroy();
  ...
  _setup: function (link, first) {
    ...
    var e =
      this.filter.getDiv() ||
      getThing(this.filter.tableName, this.filter.divName);
    if (this.sort) e = this.filter.sortElement;
    ...
    if (this.sort) this.addSortHeader();
    else this.addConditionHeader(first);
    return this.queryID;
  },
  addSortHeader: function () {
    ...
    tr +=
      '<hr></hr><h3>' +
      this.answer['Order results by the following fields'] +
      '</h3>';
    tr += '</td></tr>';
  ...
  _addHRTR: function (first) {
    var tr = $j(
      '<tr><td class="sn-filter-top sn_multiple_conditions"><hr></hr><td></tr>'
    );
    $j(this.tbody).append(tr);
    if (first) tr.hide();
  },
  _addMessageTR: function (first) {
    if (first) var message = getMessage('All of these conditions must be met');
    else message = getMessage('OR all of these conditions must be met');
    this.description = message;
    var tr =
      '<tr class="sn-filter-section-message"><td class="sn-filter-top"><h3 class="small">';
    tr += message;
    tr += '</h3></td></tr>';
  ...
  setFirst: function (length) {
    this.sectionSepMessage
      .find('td')
      .html(getMessage('All of these conditions must be met'));
  },
  addSortCondition: function () {
    ...
    if (this.runRow == null) condition.build(this.tbody);
    else {
  ...
  removePlaceHolder: function () {
    if (this.placeHolderCondition) this.placeHolderCondition.remove();
  },
  clearPlaceHolder: function () {
    if (!this.filter.getMaintainPlaceHolder()) this.placeHolderCondition = null;
    else this.newPlaceHolder();
  },
  addPlaceHolder: function (wantOR) {
    this.placeHolderCondition = new GlideSectionCondition(
      this,
      this.queryID,
      this.description
    );
  ...
  addCondition: function (wantOR, field, oper, value) {
    ...
    var condition = new GlideSectionCondition(
      this,
      this.queryID,
      this.description
    );
    ...
    if (this.runRow == null) condition.buildRow(this.tbody, field, oper, value);
    else {
  ...
  unescapeQueryValue: function (oper, val) {
    if (!val) return val;
    if (oper === 'MATCH_RGX') val = val.replace(/\^\^/gm, '^');
  ...
  addConditionWithPlaceHolder: function (wantOR, field, oper, value) {
    ...
    this.tbody.insertBefore(
      condition.getRow(),
      this.placeHolderCondition.getRow()
    );
    return condition;
  },
  addRunButton: function () {
    if (this.runRow != null) return;
  ...
  removeCondition: function (id) {
    ...
    if (
      (this.placeHolderCondition != null && this.conditions.length == 1) ||
    ...
    } else if (i == 0) this.conditions[0].makeFirst();
    updateFilterAria();
  },
  refreshSelectList: function () {
    for (var i = 0; i < this.conditions.length; i++)
...
GlideSectionCondition.prototype = {
  ...
  build: function (parent) {
    ...
    if (!this.filter.getConditionsWanted()) table.className = 'wide';
    ...
    this.conditionRow = new GlideConditionRow(
      this,
      this.queryID,
      this.wantOR,
      first,
  ...
  buildRow: function (parent, field, oper, value) {
    this.field = field;
    this.oper = oper;
    if (value) this.value = value.replace(/&amp;/g, '&');
  ...
  remove: function () {
    if (this.conditionRow) this.conditionRow.destroy();
  ...
  removePlaceHolder: function () {
    if (this.section) this.section.removePlaceHolder();
  },
  newPlaceHolder: function () {
    if (this.section) this.section.newPlaceHolder();
  },
  clearPlaceHolder: function () {
    if (this.section) this.section.clearPlaceHolder();
  },
  refreshSelectList: function () {
    if (this.conditionRow) this.conditionRow.refreshSelectList();
  },
  z: null,
};
...
GlideSubCondition.prototype = {
  ...
  buildRow: function (parent, field, oper, value) {
    ...
    this.row = new GlideConditionRow(
      this.condition,
      queryID,
      false,
      false,
...
GlideConditionRow.prototype = {
  ...
  initialize: function (condition, queryID, wantOr, first, sectionDescription) {
    ...
    tr.tdOper = td;
    if (!this.filter.getOpsWanted()) td.style.display = 'none';
    td.style.width = 'auto';
    td = this.addTD(tr, queryID);
    this.tdValue = td;
    if (this.filter.listCollectorId)
    ...
    if (this.filter.getTextAreasWanted()) td.style.width = '90%';
    else td.setAttribute('textAreaWanted', 'false');
  ...
  destroy: function () {
    ...
    if (this.row.handler) this.row.handler.destroy();
  ...
  setAsRunRow: function (field) {
    ...
    if (this.filter.runCode) runCode = this.filter.runCode;
  ...
  addAndOrTextCell: function () {
    ...
    if (!this.filter.getConditionsWanted()) td.style.display = 'none';
  },
  ...
  addRemoveButtonCell: function () {
    ...
    if (this.wantOr) td.hasOrButton = 'true';
  },
  ...
  build: function (field, oper, value) {
    ...
    var sname =
      isRelated && filterTables.length > 1 ? filterTables[1] : filterTables[0];
    if (this.field != null) sname = sname + '.' + field;
    this.filter.setFieldUsed(field);
    addFirstLevelFields(
      this.fieldSelect,
      sname,
      field,
      this.filter.filterFields.bind(this.filter),
      null,
      this.filter,
      this.filter.tableOptions
    );
    if (!this.tdName) {
      return [];
    }
    ...
    updateFields(
      ...
      oper,
      value,
      this.filter.getIncludeExtended(),
      this.filter,
      this.sectionDescription
    );
    this.fieldSelect.onchange = this.fieldOnChange.bind(this);
    var select2Offscreen = $j(this.tdName).find('select.select2-offscreen');
    var keywordOption = $j(select2Offscreen).find(
      'option[value="' + this.KEYWORDS_CONVERTED + '"]'
    );
    this.setKeywordOptionIndex(keywordOption.index());
    if (this.getKeywordOptionIndex() !== -1) {
      this.setKeywordOption(keywordOption.first());
      $j(this.fieldSelect).on(
        'select2-opening',
        this.handleKeywordCondition.bind(this)
      );
    }
    if (this.filter.isProtectedField(field)) this._setReadOnly();
    if (
      this.filter.fieldName == 'sys_template.template' ||
      this.filter.isTemplate
    )
      this.addHelpText();
  ...
  handleKeywordCondition: function () {
    ...
    var hasSubCondition =
      this.condition &&
      this.condition.subConditions &&
      this.condition.subConditions.length > 0;
    var select2Offscreen = $j(this.tdName).find('select.select2-offscreen');
    if (
      (hasKeyword && this.field !== this.KEYWORDS_CONVERTED) ||
      orCount > 0 ||
      hasSubCondition
    ) {
      select2Offscreen
        .find('option[value="' + this.KEYWORDS_CONVERTED + '"]')
        .remove();
    } else {
      var optionAtIndex =
        select2Offscreen.find('option')[this.getKeywordOptionIndex()];
  ...
  fieldOnChange: function () {
    ...
    if (b) {
      ...
      if (this.condition.isFirst()) this.makeFirst();
    }
    ...
    if (form) {
      var nameWithoutTablePrefix = this.getName().substring(
        this.getName().indexOf('.') + 1
      );
  ...
  addHelpText: function () {
    ...
    if (this.fieldElements) {
      switch (this.fieldElements) {
        ...
        case 'user_roles':
          this.helpText(
            this.answer['Separate individual references with a comma']
          );
          break;
        case 'composite_name':
          this.helpText(
            this.answer['Use the following format: TableName.FieldName']
          );
          break;
        case 'days_of_week':
          this.helpText(
  ...
  showFields: function () {
    ...
    if (!this.first) this.tdAndOrText.style.visibility = 'visible';
  ...
  addLeftButtons: function () {
    if (this.wantOr) {
      ...
      var andOnClick =
        "addConditionSpec('" +
        htmlEscape(this.tableName) +
        "','" +
        htmlEscape(this.queryID) +
        "','','','','" +
        fDiv +
        "'); return false;";
      var orOnClick = "newSubRow(this,'" + fDiv + "'); return false;";
      tdAddOr.innerHTML =
        this.getAndButtonHTML(andOnClick) + this.getOrButtonHTML(orOnClick);
      if (this.condition.isPlaceHolder()) tdAddOr.style.visibility = 'hidden';
    }
    ...
    if (!this.wantOr) {
      if (td.parentNode.sortSpec != true) {
        ...
        var orSpan =
          "<span class='sn-or-message'>" + this.answer['or'] + '</span>';
    ...
    if (!this.condition.isPlaceHolder()) return;
    if (!this.filter.defaultPlaceHolder) return;
  ...
  getAndButtonHTML: function (onClick) {
    return (
      ...
      onClick +
      '" title=\'' +
      this.answer['Add AND Condition'] +
      "' name='and-button' alt='" +
      this.answer['Add AND Condition'] +
      "' class='btn btn-default filerTableAction'>" +
      this.answer['and'].toUpperCase() +
      '</button>'
    );
  },
  getOrButtonHTML: function (onClick) {
    return (
      ...
      onClick +
      '" title=\'' +
      this.answer['Add OR Condition'] +
      "' name='or-button' alt='" +
      this.answer['Add OR Condition'] +
      "' class='btn btn-default filerTableAction'>" +
      this.answer['or'].toUpperCase() +
      '</button>'
    );
  },
  getDeleteButtonHTML: function (id, onClick) {
    return (
      ...
      '" title=\'' +
      this.answer['Delete'] +
      "' type='button' class='filerTableAction btn btn-default deleteButton'><span class='icon-cross'></span><span class=\"sr-only\">'" +
      this.answer['Delete'] +
      "'</span></button>"
    );
  },
  ...
  refreshSelectList: function () {
    ...
    if (this.field != null) sname = sname + '.' + this.field;
    addFirstLevelFields(
      this.fieldSelect,
      sname,
      this.field,
  ...
  buildRunButton: function (f) {
    var m = new GwtMessage();
    return (
      '<button class="btn btn-default" tabindex="0" onclick="' +
      f +
      '" title="' +
      m.getMessage('Run Filter') +
      '">' +
      m.getMessage('Run') +
      '</button>'
    );
...
GlideSortSection.prototype = {
  ...
  addField: function (field, oper) {
    ...
    row.sortSpec = true;
    var sectionLabel =
      getMessages(MESSAGES_FILTER_MISC)[
        'Order results by the following fields'
      ];
    var fSelect = addFields(
      this.getName(),
      field,
      true,
      this.filter.getIncludeExtended(),
...
GlideEncodedQuery.prototype = {
  ...
  destroy: function () {
    for (var i = 0; i < this.orderBy.length; i++) this.orderBy[i].destroy();
    for (var i = 0; i < this.groupBy.length; i++) this.groupBy[i].destroy();
  },
  ...
  getEncodedParts: function () {
    if (
      typeof g_filter_description != 'undefined' &&
      this.tableName == g_filter_description.getName() &&
    ...
    if (this.callback) ajax.getXML(this.getEncodedPartsResponse.bind(this));
    else {
  ...
  getEncodedPartsResponse: function (response) {
    if (!response || !response.responseXML) this.callback();
  ...
  parseXML: function () {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      } else this.terms[this.terms.length] = qp;
    }
...
GlideQueryPart.prototype = {
  ...
  extract: function () {
    ...
    for (i = 0; i < operators.length; i++) {
      if (this.operator == operators[i]) break;
    }
    ...
    if (this.field.startsWith('sys_tags.')) this.field = 'sys_tags';
...
function updateRemoveButtonAria(filterRow, index) {
  ...
  if (deleteButton) {
    deleteButton.setAttribute(
      'aria-label',
      answer['Remove Condition'] +
        ' ' +
        index +
        ': ' +
        getConditionTextFromFilterRow(filterRow)
    );
  }
}
function updateAndOrButtonAria(filterRow, index) {
  ...
  if (orButton && andButton) {
    var addLabel =
      answer['Add AND Condition To Condition'] +
      ' ' +
      index +
      ': ' +
      getConditionTextFromFilterRow(filterRow);
    var orLabel =
      answer['Add OR Condition To Condition'] +
      ' ' +
      index +
      ': ' +
      getConditionTextFromFilterRow(filterRow);
...
function updateOperatorAria(filterRow, index) {
  ...
  if (operatorField) {
    var operatorLabel =
      messages['Operator For Condition'] +
      ' ' +
      index +
      ': ' +
      getConditionTextFromFilterRow(filterRow);
...
function updateFilterAria() {
  var filterRows = $j('.filter_row_condition')
    .toArray()
...
function newSubRow(elBut, name) {
  ...
  if (fDiv && !checkFilterSize(fDiv.filterObject)) return;
  ...
  if (
    ...
    window.NOW.accessibility.ariaLivePolite(
      m.getMessage('New OR subcondition added, {0} of {0}', subConditionsCount),
      0
    );
  }
  _frameChanged();
...
function findInArray(a, searchID) {
  for (var i = 0; i < a.length; i++) {
    var condition = a[i];
    if (condition.getID() == searchID) return i;
  }
  return null;
}
...
function buildURL(tableName, query) {
  ...
  if (refQuery) url += '&sysparm_ref_list_query=' + refQuery.value;
  ...
  if (target) {
    ...
    if (e) url += '&sysparm_reflist_pinned=' + e.value;
  }
  return url;
}
function createCondFilter(tname, query, fieldName, elem) {
  ...
  useTextareas = false;
  new GlideConditionsHandler(
    tname,
    function (filter) {
      if (filter) {
        var filterDiv =
          filter.getDiv() || getThing(filter.tableName, filter.divName);
        if (filterDiv) {
...
function checkFilterSize(filterObject) {
  ...
  var filterString = '';
  if (
    isNaN(g_glide_list_filter_max_length) ||
    g_glide_list_filter_max_length <= 0
  )
  ...
  if (
    ...
    filterObject['type'] === 'GlideFilter'
  ) {
    var filterIDParts = filterObject.getDiv().id.split('gcond_filters', 1);
    filterString =
      filterIDParts && filterIDParts.length > 0
        ? getFilter(filterIDParts[0])
        : '';
  }
  if (filterString && filterString.length > g_glide_list_filter_max_length) {
    ...
    dialog.setBody(
      "<div style='padding: 10px'>" +
        getMessage(
          'The filter you are using is too big. Remove some conditions to make it smaller.'
        ) +
        '</div>',
      false,
      false
