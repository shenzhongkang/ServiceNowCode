...
var GwtListEditCalendar = Class.create(GwtListEditWindow, {
  ...
  _createCalendarHTML: function () {
    ...
    var img = createIcon(
      'icon-calendar',
      getMessage('Choose date...'),
      this,
      this._showCalendar.bind(this)
    );
    img.id = this.type();
  ...
  onFocus: function () {
    ...
    if (input) input.onfocus = null;
    this._showCalendar();
    if (g_full_calendar_edit) this._hideInput();
  },
  _hideInput: function () {
  ...
  _showCalendar: function () {
    if (gel('GwtDateTimePicker')) return;
    var format;
    var useTime;
    if (this.editor.tableElement.isDateTime()) {
      format = g_user_date_time_format;
      useTime = true;
    } else format = g_user_date_format;
    this.cal = new GwtDateTimePicker(
    ...
    this.cal.on(
      'save',
      function (evt) {
        if (evt === undefined) this.save();
        var el = document.getElementById('cell_edit_window');
        if (!el || !el.contains(evt.target)) this.save();
      }.bind(this)
    );
  ...
  addMultipleEditMsg: function () {
    if (!window.g_full_calendar_edit) return;
    var msgStr;
    if (this.numCanEdit == 1) return;
