...
(function () {
  ...
  var PersonalizeForm = Class.create({
    initialize: function () {
      ...
      this.$extraTitle = [];
      this.elements = g_form.elements.reduce(
      ...
      this.$toggleButton.click(
        function () {
          if (this.isOpen) this._hideSidebar();
          else if (!this.isInitialized) this._initializeSidebar();
          else this._showSidebar();
        }.bind(this)
      );
      this._addOrRemoveMandatoryObserver();
      this._deepClone(g_form._getPersonalizeHiddenFields()).forEach(function (
        fieldName
      ) {
        if (this._isMandatory(fieldName))
          g_form.setUserDisplay(fieldName, true);
        else g_form.setDisplay(fieldName, false);
      },
      this);
    },
    toString: function () {
    ...
    _initializeSidebar: function () {
      $j('html').click(
        function (event) {
          if (
            this.isOpen &&
            !this.$toggleButton.is(event.target) &&
            this.$container.parents('.popover').has($j(event.target)).length ===
              0
          )
            this._hideSidebar();
        }.bind(this)
      ...
      $j(document).keyup(
        function (event) {
          if (event.keyCode === 27)
            if (
              this.isOpen &&
              this.$container.parents('.popover').has($j(event.target))
                .length === 0
            )
              this._hideSidebar();
        }.bind(this)
    ...
    _showOrHideResetButton: function () {
      ...
      } else this._setResetButtonVisibility(false);
    },
    _showSidebar: function () {
      ...
      if (!this.$extraTitle.length)
        this.$extraTitle = this.$container
          .parent()
          .parent()
          .find('h3.popover-title');
      ...
      this.isOpen = true;
      if (
        !(this.lastFocusedElement && this.lastFocusedElement.is(':canBeSeen'))
      )
    ...
    _setPopoverHeight: function () {
      if (!this.$containerParent.length) {
        ...
        this.popoverContentPadding =
          this.$containerParent.innerHeight() - this.$containerParent.height();
        this.$container.height(
          this.$container.height() + this.popoverContentPadding
        );
      }
      var templateBarHeight =
        this.$templateBar.css('opacity') === '1'
          ? this.$templateBar.outerHeight(true)
          : 0;
      ...
      if (popoverContentHeight > popoverMaxHeight)
        ...
        this.$containerParent.height(
          popoverContentHeight - this.popoverContentPadding
        );
    },
    ...
    _attachHandlers: function () {
      ...
      this.elements.forEach(function (element) {
        ...
        element.$formElement
          .on(
            'mouseenter.personalizeForm',
            this._addHoverClass.bind(this, element)
          )
          .on(
            'mouseleave.personalizeForm',
            this._removeHoverClass.bind(this, element)
          );
        ...
        element.$popoverInput.on(
          'change.personalizeForm',
          function (event) {
            element.$popoverInput.attr(
              'aria-selected',
              element.$popoverInput.prop('checked') ? 'true' : 'false'
            );
        ...
        function (e) {
          switch (e.keyCode) {
            ...
            case $j.ui.keyCode.TAB:
              ...
              this.$toggleButton.click();
              this.$toggleButton.focus();
              if (e.shiftKey) this.tabOutPrevious();
              else this.tabOutNext();
              this._hideSidebar();
              break;
    ...
    _render: function () {
      ...
      this.elements.forEach(function (element) {
        ...
        if (!element.isDisabled) {
          ...
          var $hideButton = $j(
            '<a class="icon icon-delete personalize_form_hide_field" href="#"></a>'
          );
          if (!g_accessibility) {
            $hideButton.attr('title', 'Hide ' + element.label);
            $hideButton.data('input', $input);
            $fieldLabelTd.find('label').prepend($hideButton);
            this.$fieldLabelHideButtons =
              this.$fieldLabelHideButtons.add($hideButton);
    ...
    tabNextOrPrevOutOfMenu: function (goPrevious) {
      var selectables = $j(this.tabOutsideMenuSelector).not(
        this.$container.find(this.menuInputSelector)
      );
    ...
    _isFieldInHiddenSection: function ($control) {
      if (typeof g_tabs2Sections === 'undefined') return false;
    ...
    _setIsCheckedIsDisabled: function (element) {
      ...
      element.$popoverInput.attr(
        'aria-selected',
        element.isChecked ? 'true' : 'false'
      );
      element.isDisabled =
        isMandatory || (this._isDisplayNone(name) && !isPersonalized);
      element.$popoverInput
        .attr('disabled', element.isDisabled)
        .prop('checked', element.isChecked);
      if (
        element.isDisabled &&
        element.$hideButton &&
        element.$hideButton.is(':visible')
      )
    ...
    _mandatoryObserver: function (prefixedField) {
      ...
      if (isMandatory && isPersonalized) g_form.setDisplay(fieldName, true);
    },
    _addOrRemoveMandatoryObserver: function () {
      ...
      else CustomEvent.un('mandatory.changed', this._mandatoryObserver);
    },
