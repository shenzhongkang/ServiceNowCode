...
window.ANGULAR_BOOTSTRAPPER = (function angularBootstrapper() {
  ...
  function scheduleCompilation(selectors) {
    ...
    if (!elems.length > 0) {
      jslog(
        'Element matching selector not found, skipping Angular compilation of: ' +
          selectors
      );
      return;
    }
  ...
  function getCompiledParent(elem) {
    var parent = elem;
    while (parent != null) {
      if (parent.getAttribute('sn-ng-compiled')) return parent;
      if (parent.getAttribute('ng-non-bindable')) return null;
