...
NOW.c14 = (function () {
  ...
  return {
    ...
    shouldShowDynamicReferenceOperator: function (
      type,
      elementDef,
      tableNameFull
    ) {
      if (
        ...
        return false;
      if (!window.g_dynamic_filter_options) return false;
      var table = elementDef.getReference();
      var gotOne = false;
      var arr = g_dynamic_filter_options.split('##');
      for (var i = 0; i < arr.length; i++) {
        var aItem = arr[i];
        if (aItem.length == 0) continue;
        var aItemArr = aItem.split('::');
        if (aItemArr.length < 3) continue;
        var contextTableName = tableNameFull.split('.')[1];
        var isExclusive =
          typeof gtf_exclusive_dynamics != 'undefined' &&
          gtf_exclusive_dynamics == 'true';
        if (
          (!isExclusive && table == aItemArr[2]) ||
          contextTableName == aItemArr[3]
        ) {
          gotOne = true;
          break;
        }
      ...
      if (event.keyCode == 13 || event.keyCode == 3) {
        ...
        if (slushbucketDOMcheck()) {
          ...
          if (elem && elem.getAttribute) {
            ...
            if (name) timeout = name + 'acRequest();';
          }
          if (!timeout) timeout = 'acRequest();';
        } else {
          var name = getCurrentTable();
          if (name) timeout = "runFilter('" + name + "');";
        }
        if (timeout) setTimeout(timeout, 0);
        Event.stop(event);
        return false;
      }
      return true;
    ...
    conditionColumnResponse: function (
      columns,
      tableName,
      mfunc,
      tableOptions
      ...
      if (typeof mfunc === 'function') mfunc(tableName);
    },
  };
})();
function slushbucketDOMcheck() {
  ...
  if ((epObject = gel('ep'))) {
    ...
    for (var tdArrIndex = 0; tdArrIndex < tdArr.length; tdArrIndex++) {
      if (tdArr[tdArrIndex].className == 'slushbody') return true;
    }
  }
  return false;
}
...
function _createFilterSelect(width, multi, associatedField) {
  ...
  if (associatedField)
    s.setAttribute(
      'aria-label',
      msg.getMessage('Choose option for field: {0}', associatedField)
    );
  else s.setAttribute('aria-label', defaultLabel);
...
function getTableReference(
  ...
  if (firstTable == '') firstTable = tableName;
  return Table.get(
    tableName,
    parentTable,
    isTemplate,
...
function allowConditionsForJournal(type, filter) {
  if (type != 'journal' && type != 'journal_input') return false;
  if (filter && filter.type == 'GlideTemplateFilter' && filter.allowJournal)
    return true;
  if (!filter || filter.getUsageContext() != 'element_conditions') return false;
...
function updateFields(
  ...
  if (idx != -1) NOW.c14.setShowRelated(fieldName, idx, name, select, filter);
...
function addOperators(
  ...
  if (associatedField)
    s.setAttribute(
      'aria-label',
      msg.getMessage('Choose operator for field: {0}', associatedField)
    );
  else s.setAttribute('aria-label', defaultLabel);
  if (td.parentNode.conditionObject)
    if (td.parentNode.conditionObject.isPlaceHolder()) s.disabled = true;
  var opers;
  if (isChoice) opers = sysopers[type + '_choice'];
  var translated =
  ...
  if (
    elementDef &&
    (elementDef.isEdgeEncrypted() || elementDef.isEncrypted())
  ) {
    if (canSort) opers = edge_order_Types[type];
    else opers = edge_equality_Types[type];
  }
  if (!opers && sysopers[type]) opers = sysopers[type];
  if (type && type.indexOf(':') > 0) {
    var complexTypeArray = type.split(':');
    if (null != complexTypeArray[0]) opers = sysopers[complexTypeArray[0]];
  }
  if (!opers) opers = sysopers['default'];
  if (noOps) dValue = '=';
  if (filterClass == 'GlideTemplateFilter') {
    if (typeof gtfOperators != 'undefined' && gtfOperators.length != 0)
      opers = gtfOperators;
    else opers = sysopers_template['default'];
  }
  ...
  for (var ii = 0; ii < opers.length; ii++) {
    ...
    if (extopers[opInfo[0]] && !includeExtended[opInfo[0]]) continue;
    if (opInfo[0] == 'DYNAMIC' && !showDynamicReferenceOperator) continue;
  ...
  td.fieldType = 'select';
  if (so) td.currentOper = getSelectedOption(s).value;
  td.subType = type;
  td.appendChild(s);
  return s;
}
...
function addTextInput(td, dValue, type, associatedField, label) {
  ...
  if (td.parentNode.conditionObject)
    if (td.parentNode.conditionObject.isPlaceHolder()) input.disabled = true;
  if (isMSIE) {
    input.onkeypress = function () {
      return NOW.c14.enterKey(event);
    };
  } else input.onkeypress = NOW.c14.enterKey;
  td.fieldType = 'input';
  if (dValue) input.value = dValue;
  input.className = 'filerTableInput form-control';
  var msg = NOW.msg;
  var defaultLabel = label
    ? msg.getMessage(label)
    : msg.getMessage('Input value');
  input.title = defaultLabel;
  if (associatedField)
    input.setAttribute(
      'aria-label',
      msg.getMessage('Input value for field: {0}', associatedField)
    );
  else input.setAttribute('aria-label', defaultLabel);
...
function loadFilterTableReference(
  mft,
  isTemplate,
  applyTemplateAcls,
  tableOptions
...
function decodeFilter(tableName, tableOptions) {
  ...
  if (filter) {
    if (
      typeof sc_condition_builder == 'undefined' &&
...
function refreshFilter(name) {
  ...
  for (var i = 0; i < fQueries.length; i++) {
    ...
    if (queryTR.queryPart != 'true') continue;
...
function getFilter(name, doEscape, queryStringValidator) {
  ...
  if (fDiv) divName = fDiv + divName;
  var fDiv = getThing(name, divName);
  if (!fDiv) return '';
  if ('gcond_filters' == divName) addOrderBy();
  var fQueries = fDiv.getElementsByTagName('tr');
  for (var i = 0; i < fQueries.length; i++) {
    var queryTR = fQueries[i];
    if (queryTR.queryPart != 'true') continue;
    ...
    if (fullFilter.length > 0 && queryString.length > 0) fullFilter += '^NQ';
    fullFilter = fullFilter + queryString;
  }
  if (fullFilter.length > 0) fullFilter += '^EQ';
  fullFilter += orderBy;
  filter = fullFilter;
  if (doEscape) filter = encodeURIComponent(filter);
  return filter;
  function addOrderBy() {
    'use strict';
...
function getQueryForTR(trItem, queryStringValidator) {
  ...
  for (var i = 0; i < trs.length; i++) {
    ...
    if (field == PLACEHOLDER || !tr.operSel) continue;
    ...
    if (!tr.sortSpec && !tr.aggSpec) {
      ...
      if (oper == 'ascending') orderBy += '^' + 'ORDERBY' + field;
      else if (oper == 'descending') orderBy += '^' + 'ORDERBYDESC' + field;
    }
  }
  function getTDFieldValue(td) {
    ...
    if (fType != 'select') return field.value;
    var options = field.options;
    if (!field.multiple) return options[field.selectedIndex].value;
    var retVal = [];
    for (var i = 0; i < options.length; i++) {
      if (options[i].selected) retVal[retVal.length] = options[i].value;
    }
    return retVal.join(',');
  }
...
function deleteFilterByID(tablename, id, mappingId) {
  ...
  function deleteTD(tableName, butTD) {
    ...
    if (butTR.conditionObject) butTR.conditionObject.remove();
    else {
  ...
  function updateConditionFocus(
    ...
    var trs = table.find('tr.filter_row_condition');
    if (!isRelatedList && trs.length > 1) trs.splice(-1, 1);
    var position =
      trs.length - 1 >= originalPosition
        ? originalPosition
        : originalPosition - 1;
    var tr = $j(trs[position]);
    var container = tr.find(
      'div.select2-container.select2.form-control.filter_type'
    );
    container.addClass('select2-container-active');
    container.find('input.select2-focusser.select2-offscreen').focus();
  }
}
function buildFieldsPerType(
  tableName,
  tr,
  descriptorName,
  fOper,
  ...
  if (elementDef == null && tableDef.glide_var)
    elementDef = tableDef.getElement(
      parts[parts.length - 2] + '.' + parts[parts.length - 1]
    );
  if (elementDef == null) {
    if (descriptorName != TEXTQUERY && descriptorName != PLACEHOLDER) return;
  ...
  if (tr.handler) tr.handler.destroy();
  tr.handler = null;
  if (tr.sortSpec == true) {
    tr.varType = 'sortspec';
    type = 'sortspec';
  ...
  else if (descriptorName.startsWith('IO:')) tr.tableField = descriptorName;
  else tr.tableField = tableName + '.' + descriptorName;
  tdOperator.innerHTML = '';
  tdValue.innerHTML = '';
  tdValue.style.minWidth = '';
  if (type == 'float') type = 'integer';
  else if (type == 'domain_number') type = 'integer';
  else if (
    type == 'wide_text' ||
    type == 'ref_ext' ||
    type == 'sc_email' ||
    ...
    type = 'calendar';
    if (fValue && fValue.indexOf('datePart') > -1) fOper = 'DATEPART';
    else if (fValue && fValue.indexOf('getBaseFilter') > 0) fOper = 'SINCE';
  }
  var showDynamicReferenceOperator = NOW.c14.shouldShowDynamicReferenceOperator(
    type,
    elementDef,
    tableNameFull
  );
  var associatedField = elementDef && elementDef.label;
  var operSel = addOperators(
    tdOperator,
    type,
    fOper,
    isChoice,
  ...
  if (operSel) {
    operSel.observe('change', function () {
      ...
      if (form) {
        var nameWithoutTablePrefix = tableNameFull.substring(
          tableNameFull.indexOf('.') + 1
        );
  ...
  tr.operSel = operSel;
  if (fOper == null && operSel) fOper = tdOperator.currentOper;
  ...
  if (tableNameFull.endsWith('.var_map')) {
    ...
    tr.handler =
      elementDef.isEdgeEncrypted() || elementDef.isEncrypted()
        ? new GlideFilterDateEncrypted(tableName, elementDef, filter.mappingId)
        : new GlideFilterDate(tableName, elementDef, filter.mappingId);
    ...
    else tr.handler = new GlideFilterVariables(tableName, elementDef);
    tr.handler.setOriginalTable(tableNameFull);
  ...
  if (tr.handler)
    tr.handler.initMappingSupport(
      supportsMapping,
      type,
      filter && filter.mappingMgr
    );
}
function addFirstLevelFields(
  s,
  target,
  fValue,
  filterMethod,
  ...
  if (!gotShowRelated) {
    ...
    else showRelated = getPreference('filter.show_related');
  }
  ...
  savedLabels.forEach(function (sname) {
    ...
    if (labelPrefix != '') o.fullLabel = labelPrefix + '.' + sname;
    else o.fullLabel = sname;
    if (indentLabel) o.innerHTML = '&nbsp;&nbsp;&nbsp;' + o.innerHTML;
    if (o.value.indexOf('...') != -1)
      if (o.fullLabel.indexOf('(+)') == -1) o.style.color = 'blue';
      else o.style.color = 'darkred';
  });
  if (
    typeof sc_condition_builder == 'undefined' &&
    filterExpanded &&
    ...
    if (showRelated != 'yes')
      ...
      o = addOption(
        ...
        messages['Remove Related Fields'],
        false
      );
    o.style.color = 'blue';
  }
  if (
    !placeholder &&
    s.selectedIndex == 0 &&
    ((textIndex && fValue != TEXTQUERY) || headersAdded)
  )
  ...
  function fixParts() {
    ...
    if (filterExpanded && parts.length > 2) {
      ...
      if (tableLabel == null) tableLabel = 'Parent';
      o = addOption(
        s,
        tableDef.getName() + '...',
        tableLabel + ' ' + messages['lowercase_fields'],
        false
      );
      o.tableName = tableDef.getName();
      o.style.color = 'blue';
    ...
    for (var i = 1; i < parts.length - 1; i++) {
      ...
      var parentTable =
        extension != '' ? extension : elementDef.getTable().getName();
      tableDef = getTableReference(childTable, parentTable);
      if (cname.length) cname = cname + '.';
...
function addSortSpec(name, fField, fOper) {
  if (!NOW.c14.setup(name)) return null;
  ...
  if (!checkFilterSize(fDiv.filterObject)) return;
...
function addFields(tableName, fValue, isSort, extendedFields, label) {
  ...
  if (fValue) sname = sname + '.' + fValue;
  if (isSort) addFirstLevelFields(s, sname, fValue, sortByFilter);
  else addFirstLevelFields(s, sname, fValue);
  return s;
}
...
function addCondition(name) {
  ...
  if (
    ...
    window.NOW.accessibility.ariaLivePolite(
      msg.getMessage('New AND condition added, {0} of {0}', conditionsCount),
      0
    );
  }
  _frameChanged();
...
function addConditionSpec(name, queryID, field, oper, value, fDiv) {
  if (firstTable == null) firstTable = currentTable;
  if (!NOW.c14.setup(name)) return null;
  var divName = 'gcond_filters';
  if (fDiv != null) divName = fDiv + 'gcond_filters';
  ...
  if (!checkFilterSize(filter)) return;
  ...
  if (
    ...
    if (isAndCondition) {
      ...
      window.NOW.accessibility.ariaLivePolite(
        m.getMessage('New AND condition added, {0} of {0}', conditionsCount),
        0
      );
    } else {
      window.NOW.accessibility.ariaLivePolite(
        m.getMessage(
          'New OR filter section added, {0} of {0}',
