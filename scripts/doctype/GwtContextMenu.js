...
var GwtContextMenu = Class.create({
  SUBMENU_INDICATOR:
    "<i class='icon-chevron-right context_submenu_indicator'/>",
  ...
  initialize: function (id, useBodyAsParent) {
    ...
    if (typeof useBodyAsParent == 'undefined') useBodyAsParent = false;
  ...
  _getDocRoot: function () {
    ...
    if (window.location.pathname.indexOf('/') > -1) {
      ...
      if (fp.substring(0, 1).indexOf('/') != -1) docRoot = docRoot + fp;
      else docRoot = docRoot + '/' + fp;
    }
    docRoot += '/';
    return docRoot;
  },
  add: function (label, id, keys) {
    ...
    d.innerHTML = l;
    if (keys) d.setAttribute('data-label', label);
    m.appendChild(d);
    return d;
  },
  ...
  addURL: function (label, url, target, id) {
    ...
    if (target) d.setAttribute('target', target);
    return d;
  },
  ...
  addAction: function (label, action, id) {
    return this.addHref(
      label,
      "contextAction('" + this.getTableName() + "', '" + action + "')",
      null,
      null,
      id
    );
  ...
  addConfirmedAction: function (label, action, id) {
    return this.addHref(
      label,
      "contextConfirm('" + this.getTableName() + "', '" + action + "')",
      null,
      null,
      id
    );
  ...
  addKeyShortcut: function (keys, href) {
    ...
    var keyboardEnabled =
      topWindow.com &&
      topWindow.com.glide &&
      topWindow.com.glide.ui &&
      topWindow.com.glide.ui.keyboard;
    if (!keyboardEnabled) return null;
    if (!keys) return keys;
    if (shortcuts[keys]) return keys;
    shortcuts[keys] = {};
    var callback = function (e) {
      var start = e.data.href.indexOf('javascript:') == 0 ? 11 : 0;
      var startParen = e.data.href.indexOf('(');
      var functionName =
        startParen > start ? e.data.href.substring(start, startParen) : {};
      var func = eval(functionName);
      if (typeof func == 'function') eval(e.data.href);
      else document.location.href = e.data.href;
    };
    addLoadEvent(function () {
      ...
      if (isMainFrame)
        shortcuts[keys] = keyboard
          .bind(keys, callback, { href: href })
          .global(keyboard.formFrame);
      else if (isFormFrame)
        shortcuts[keys] = keyboard
          .bind(keys, callback, { href: href })
          .formFrame();
      else shortcuts[keys] = null;
    });
    return keys;
  },
  getItem: function (itemId) {
    ...
    for (var i = 0; i < items.length; i++) {
      var item = items[i];
      if (item.getAttribute('item_id') == itemId) return item;
    }
    return null;
  },
  ...
  setChecked: function (item) {
    if (item) this.setImage(item, this.CHECKED_IMAGE_PATH);
  },
  ...
  destroy: function () {
    ...
    if (isMSIE) this.menu.outerHTML = null;
  ...
  display: function (e) {
    if (!this.getParent()) CustomEvent.fireAll('body_clicked', null);
    menuSort = true;
    this._dullMenu();
    ...
    if (this._isEmpty(menu)) return;
    menu.style.left = '0';
    menu.style.top = '0';
    this.parentElement.appendChild(menu);
    this.opener && this.opener.setAttribute('aria-expanded', 'true');
    if (
      this.getProperty('top') > 0 &&
      (this.getProperty('left') > 0 || this.getProperty('right') > 0)
    ) {
  ...
  hide: function (openModal) {
    ...
    if (firingObj) {
      if (firingObj.tagName == 'I') this._toggleFocusLock(true);
      if (!openModal) firingObj.focus();
    }
  ...
  execute: function (e) {
    ...
    if (x.subMenu) x.subMenu.hideAll();
    else this.hideAll();
    return false;
  },
  ...
  menuHighLight: function (e) {
    ...
    this._enableItem(x);
    if (x.subMenu) this._expandSubMenu(x, e);
  },
    ...
    switch (e.keyCode) {
      ...
      case 37:
        ...
        if (parentMenu && parentMenu.menu) {
          var parent = parentMenu.menu.select(
  ...
  _isFocusableMenuItem: function (element) {
    return (
      !element.classList.contains('context_item_hr') &&
  ...
  _focusNext: function (item) {
    ...
    if (nextSibling) nextSibling.focus();
      ...
      if (firstItem && this._isFocusableMenuItem(firstItem)) firstItem.focus();
    }
  },
  _focusPrevious: function (item) {
    ...
    if (previousSibling) previousSibling.focus();
      ...
      var lastItem = item.parentElement.querySelector(
        'div:last-child.context_item'
      );
      if (lastItem && this._isFocusableMenuItem(lastItem)) lastItem.focus();
    }
  },
  moveMenuToXY: function (e, left, top, right) {
    var menu = this.getMenu();
    if (right) left = right - menu.offsetWidth;
  ...
  moveMenuToParent: function (e, firingObject) {
    ...
    var borderLeftWidth = parseInt(
      $j(this.getParent().getMenu()).css('border-right-width'),
      10
    );
  ...
  getProperty: function (c) {
    if (this.properties) return this.properties[c];
    else return '';
  },
  ...
  _disableItem: function (item) {
    ...
    if (item && item.subMenu) item.setAttribute('aria-expanded', 'false');
  },
  _dullMenu: function () {
  ...
  _getElement: function (e) {
    var x = e.target;
    try {
      if (!x.isMenuItem && x.parentNode.isMenuItem) x = x.parentNode;
    } catch (err) {}
    return x;
  },
  ...
  _handleTimeout: function (lght, firingObject, movingTo) {
    if (this.getProperty('timeout') > 0) {
      if (lght) {
        ...
        if (gActiveContext && gActiveContext.menu.contains(movingTo)) return;
  ...
  _setMenuAttrs: function () {
    ...
    this.menu.style.zIndex = this.getParent()
      ? GwtContextMenu.zIndex + 1
      : GwtContextMenu.zIndex;
    ...
    else this.menu.onclick = this.execute.bind(this);
  },
  ...
  _isEmpty: function (menu) {
    if (!menu) return true;
    if (!menu.firstChild) return true;
    return false;
  },
  _updateURL: function (d, label, url) {
    ...
    if (typeof g_form != 'undefined') {
      this.dmap = this.dmap || {};
      this.dmap[label] = d;
      $(g_form.getFormElement()).observe(
        'glidescope:initialized',
        function (e) {
          e.memo.gts.appendTransactionScope(
            function (name, value) {
  ...
  _toggleFocusLock: function (focusLockOn) {
    ...
    if (!focusLockedElem[0]) return;
    if (focusLockOn) focusLockedElem.attr('tabindex', -1);
    else focusLockedElem.removeAttr('tabindex');
  },
});
GwtContextMenu.zIndex = 1100;
function displayContextMenu(e, name, filterable) {
  if (!getMenuByName(name)) return;
...
function contextShow(e, tableName, timeoutValue, ttop, lleft, rright) {
  ...
  try {
    ...
    if (
      frameWindow &&
      frameWindow.id == 'dialog_frame' &&
      frameWindow.noContext == true
    )
      return true;
  } catch (err) {}
  if (shouldSkipContextMenu(e)) return true;
  e = getRealEvent(e);
  menuTable = tableName;
  var name = tableName;
  if (name && name.substring(0, 8) != 'context_') name = 'context_' + name;
  if (
    document.readyState &&
    document.readyState != 'complete' &&
    document.readyState != 'interactive' &&
    typeof window.g_hasCompleted == 'undefined'
  ) {
    jslog(
      'Ignored context menu show for ' +
        name +
        ' because document was not ready'
    );
    return false;
  }
  ...
  if (getMenuByName(name)) {
    ...
    else contextMenu.display(e);
    if (contextMenu.onshow) contextMenu.onshow();
  }
  return false;
}
...
function addQuestionActionItems(id, type) {
  ...
  jr.addParam(
    'sysparm_contextual_security',
    g_form.hasAttribute('contextual_security')
  );
...
function shouldSkipContextMenu(e) {
  if (e.ctrlKey && trustCtrlKeyResponse()) return true;
  return false;
}
...
function contextTimeout(e, tableName, waitCount) {
  ...
  if (!getMenuByName(name)) return;
  var contextMenu = getMenuByName(name).context;
  if (typeof waitCount == 'undefined') waitCount = 500;
  contextMenu.setProperty('timeout', waitCount);
  var hideParam;
  if (contextMenu.hideOnlySelf == true) hideParam = '"' + name + '"';
  contextMenu.setTimeout(
    setTimeout('contextHide(' + hideParam + ')', waitCount)
  );
}
...
function getRowID(e) {
  ...
  if (cell == null) cell = e.target;
  var row = cell.parentNode;
  var id = row.id;
  if (id == null || id.length == 0) id = row.parentNode.id;
  return id;
}
function contextHide(name) {
  if (!gActiveContext) return;
  if (typeof name != 'undefined' && gActiveContext.getID() != name) return;
  if (gActiveContext.beforehide) {
    if (gActiveContext.beforehide() == false) return;
  }
...
function elementAction(e, event, gcm) {
  ...
  if (typeof g_user != 'undefined') {
    var count = 1;
    if (!gcm) gcm = addActionItems(fName, tableName, type, choice);
    if (gcm) return contextShow(event, gcm.getID(), -1, 0, 0);
  }
  return true;
}
...
function clearMenus() {
  ...
  for (var keys in shortcuts) {
    if (shortcuts[keys]) shortcuts[keys].clear();
  }
  shortcuts = null;
}
