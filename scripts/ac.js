...
function fieldProcess(
  ...
  if (type == 'Reference') {
    ...
    if (useInvisible) invisibleField = gel(elementName);
  } else {
    displayField = gel(elementName);
    if (useInvisible) invisibleField = gel('sys_display.' + elementName);
  }
  var updateField;
  if (uFieldName != null) updateField = gel(uFieldName);
  var ac = displayField.ac;
  if (ac == null) {
    ac = getAC(displayField.name);
    if (ac == null) {
      ac = newAC(displayField, invisibleField, updateField, elementName, type);
      if (ac.isOTM()) ac.refField = refField;
    }
  }
  if (typedChar != KEY_TAB) {
    ...
    ac.matched = false;
    ac.ignoreAJAX = false;
    if (ac.type == 'Reference') ac.dirty = true;
  }
  var waitTime = 50;
  if (
    typedChar == KEY_ARROWDOWN ||
    typedChar == KEY_ARROWUP ||
    typedChar == NO_INVISIBLE
  )
    waitTime = 0;
...
function initAutoCompleteField(ac) {
  ...
  setSavedText(
    ac,
    new Array(
      ac.getInvisibleField() ? ac.getInvisibleField().value : null,
      ac.getField().value
    )
  );
...
function setDropDownSize(ac) {
  ...
  var menu = ac.getMenu();
  if (menu.offsetWidth > parseInt(mWidth)) mWidth = menu.offsetWidth + 'px';
  acSetTopLeftWidth(menu.style, mTop, mLeft, mWidth);
  var iframe = ac.getIFrame();
  if (iframe) acSetTopLeftWidth(iframe.style, mTop, mLeft, mWidth);
}
...
function estimateWidth(ac) {
  var field = ac.getField();
  if (g_isInternetExplorer) return field.offsetWidth - ac.menuBorderSize * 2;
  else return field.offsetWidth;
}
...
function stripNewlines(data) {
  ...
  for (var c = 0; c < data.length; c++) {
    if (Ib.indexOf(data.charAt(c)) == -1) retData += data.charAt(c);
    else retData += ' ';
  }
  return retData;
}
function grabMenuInfo(j) {
  ...
  if (!spanTag) return spanInfo;
  for (var i = 0; i < spanTag.length; i++) {
    var e = spanTag[i];
    if (e.className != 'selected_item') continue;
...
function dropDownHilight(ac, direction) {
  setTextField(
    ac,
    new Array(
      ac.getInvisibleField() ? ac.savedInvisibleTextValue : null,
      ac.savedTextValue
    )
  );
  ac.matched = true;
  if (!ac.currentMenuItems || ac.currentMenuCount <= 0) return;
  ac.showDropDown();
  var toSelect = ac.selectedItemNum + direction;
  if (toSelect >= ac.currentMenuCount) toSelect = ac.currentMenuCount - 1;
...
function setPreviousText(ac, textArray) {
  if (textArray[1] != null) ac.previousTextValue = textArray[1];
}
...
function fireOnFormat(ac) {
  var f = ac.getField();
  if (f.getAttribute('onformat')) eval(f.getAttribute('onformat'));
}
...
function createDropDown(ac, foundStrings) {
  ...
  setSavedText(
    ac,
    new Array(
      ac.getInvisibleField() ? ac.invisibleTextValue : null,
      ac.textValue
    )
  );
...
function createChild(ac, sa) {
  ...
  if (ac.type == 'PickList') itemInRow.glabel = sa[2];
  else itemInRow.glabel = sa[1];
...
function encodeText(txt) {
  if (encodeURIComponent) return encodeURIComponent(txt);
  if (escape) return escape(txt);
}
...
function removeAC(name) {
  for (var i = 0; i < g_ac_objects.length; i++) {
    if (g_ac_objects[i] == null) continue;
    if (g_ac_objects[i].elementName == name) g_ac_objects[i] = null;
  }
}
function getAC(name) {
  for (var i = 0; i < g_ac_objects.length; i++) {
    if (g_ac_objects[i] == null) continue;
    if (g_ac_objects[i].name == name) return g_ac_objects[i];
  }
  return getacPerInput(name);
}
function getacPerInput(elementName) {
  var f = gel('sys_display.' + elementName);
  if (f != null && f.ac != null) return f.ac;
  f = gel(elementName);
  if (f != null && f.ac != null) return f.ac;
  return null;
}
...
function fieldChangeSlush2(
  ...
  var filter = getFilter(elementName, '');
  filter += '^' + queryAddOn;
  displayField = gel(elementName);
  displayField.value = filter;
  fieldProcess(
...
function updateSlushField(ac, values) {
  ...
  if (updateField['onchange']) updateField.onchange();
}
...
function fieldProcessNow(
  ...
  if (itemName != null) ac.fCall = itemName;
  setTextValue(
    ac,
    new Array(invisibleField ? invisibleField.value : null, displayField.value)
  );
  if (typedChar == KEY_TAB) return;
  ac.fieldChanged = true;
  if (typedChar != 0 || updateField || ac.isOTM()) {
    if (
      ac.isOTM() ||
      ((typedChar == KEY_ARROWDOWN || typedChar == KEY_ARROWUP) && !updateField)
    ) {
      if (ac.isOTM() || !ac.isVisible()) {
        if (ac.isOTM())
          searchForData(ac, elementName, 'M2MList', noMax, additional);
        else searchForData(ac, elementName, type, noMax, additional);
      } else {
        ac.showDropDown();
      ...
      if (!updateField)
        setSavedText(
          ac,
          new Array(invisibleField ? ac.invisibleTextValue : null, ac.textValue)
        );
...
function updateRelatedGivenNameAndValue(elementName, elementValue) {
  ...
  if (viewField != null) {
    if (isDoctype()) viewField.style.display = '';
    else viewField.style.display = 'inline';
  }
  var viewRField = gel('viewr.' + elementName);
  var viewHideField = gel('view.' + elementName + '.no');
  if (viewRField != null) {
    if (isDoctype()) viewRField.style.display = '';
    else viewRField.style.display = 'inline';
  }
  if (viewHideField != null) viewHideField.style.display = 'none';
  if (typeof g_form == 'undefined') return;
  if (
    elementName &&
    elementName.indexOf('.') != elementName.lastIndexOf('.') &&
    g_form._isDerivedWaiting(elementName)
  )
    return;
...
function searchForData(ac, elementName, type, noMax, additional) {
  ...
  if (ac.textValue != '^' && emptySubstr(ac)) cachedData = emptySubstr(ac);
  if (cachedData) {
    ...
    var url =
      ...
      '&sysparm_type=' +
      type;
    if (ac.isOTM()) url += '&sysparm_field=' + ac.refField;
    if (additional) url += '&' + additional;
    ...
    if (target.type != 'hidden' && ac.ignoreAJAX != true) ac.getField().focus();
  }
...
function fieldChangeResponse(request, nozero) {
  ...
  if (request.responseXML.documentElement) {
    ...
    if (cancelMessage) {
      ...
      $j('#select_0')[0].firstChild.text = new GwtMessage().getMessage(
        "List's data could not be retrieved"
      );
      return;
    }
    ...
    window.status =
      ...
      items.length;
    if (items.length == 0) {
      if (ac.getInvisibleField()) ac.getInvisibleField().value = '';
      if (nozero != true) storeZeroString(ac, searchText, request);
    }
    if (searchText && ac.textValue && searchText != ac.textValue) return;
    ...
    if (type == 'Reference') {
      if (
        items.length == 1 &&
        searchText != null &&
        fieldMatches(searchText, items[0].getAttribute('label'))
      ) {
        ...
        var viewHideField = document.getElementById(
          'view.' + elementName + '.no'
        );
        if (viewField != null) viewField.style.display = 'none';
        if (viewHideField != null) viewHideField.style.display = 'inline';
    ...
    window.status = '';
    if (ac.getUpdateField()) updateSlushField(ac, null);
    else {
      clearRelated(ac);
      ac.clearDropDown();
    }
  }
  if (!ac.getUpdateField() && ac.currentMenuCount < 1) ac.hideDropDown();
}
