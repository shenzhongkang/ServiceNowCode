...
var GwtListEditGlideRecord = Class.create({
  ...
  initialize: function (changes, tableController, renderer) {
    ...
    this.sysIds = [];
    this.displayValues = {};
    if (!this.initialized) this._setIgnoreNames();
    else this._clearValues();
  ...
  insert: function () {
    ...
    for (var xname in this) {
      if (this.ignoreNames[xname]) continue;
  ...
  setDisplayValue: function (fieldName, dsp) {
    ...
    if ('undefined' === typeof this[fieldName]) this[fieldName] = dsp;
  },
  update: function () {
    var sysId = this._getRowSysId();
    if (!sysId) return;
    ...
    for (var xname in this) {
      ...
      if (changed) this.renderer.renderValue(sysId, xname);
    }
  },
  ...
  _getRowSysId: function () {
    if (this.currentRow < 0 || this.currentRow >= this.sysIds.length) return;
    return this.sysIds[this.currentRow];
  },
  _clearValues: function () {
    this.displayValues = {};
    for (var xname in this) {
      if (this.ignoreNames[xname] && this.ignoreNames[xname] == true) continue;
      delete this[xname];
    }
  },
