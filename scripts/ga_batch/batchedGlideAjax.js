...
window.NOW.batchedGlideAjax = function batchedGlideAjax(toProcess) {
  ...
  function batchResponseHandler(onCompletionFn) {
    return function (response) {
      ...
      var unprocessedGas = processIndividualResponses(
        Array.prototype.slice.apply(doc.childNodes)
      );
      if (onCompletionFn) onCompletionFn(unprocessedGas);
    };
  }
  function processIndividualResponses(nodes) {
    ...
    nodes.forEach(function (node) {
      ...
      try {
        if (response.succeeded) handleChildResponseSuccess(response);
        else handleChildResponseError(response);
      } catch (e) {
        console.warn(
          'BatchedGlideAjax: Error processing child response',
          response,
          ':',
          e
        );
  ...
  function responseNode(node) {
    ...
    if (processorIdx < 0 || processorIdx >= toProcess.length) {
      console.error(
        'BatchedGlideAjax: Processor index ' +
          processorIdx +
          ' out of bounds for batch queue',
        toProcess
      );
      return null;
    }
    ...
    return {
      ...
      get responseDocument() {
        if (responseDocument == null) {
          responseDocument = document.implementation.createDocument(
            '',
            '',
            null
          );
