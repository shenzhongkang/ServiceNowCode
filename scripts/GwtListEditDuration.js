...
var GwtListEditDuration = Class.create(GwtListEditWindow, {
  ...
  _previousChoice: function (e) {
    ...
    if (previous) previous = previous.previousSibling;
  ...
  renderControl: function () {
    var table = cel('table');
    var cls = '';
    if (this.doctype) cls = 'form-control list-edit-input';
    table.cellPadding = 0;
  ...
  save: function () {
    ...
    if (!day || day == null || !isNumber(day)) day = 0;
    if (!hour || hour == null || !isNumber(hour)) hour = 0;
    if (!min || min == null || !isNumber(min)) min = 0;
    if (!sec || sec == null || !isNumber(sec)) sec = 0;
    day = parseInt(day, 10);
    ...
    var dateStr =
      dt.getUTCFullYear() +
      '-' +
      padLeft(dt.getUTCMonth() + 1, 2, '0') +
      '-' +
      padLeft(dt.getUTCDate(), 2, '0') +
      ' ' +
      padLeft(hour, 2, '0') +
      ':' +
      padLeft(min, 2, '0') +
  ...
  _setRenderValue: function (day, hour, min, sec) {
    var dspVal = '';
    if (day > 0) dspVal += day + ' ' + this._getLabel('day', day) + ' ';
    if (hour > 0) dspVal += hour + ' ' + this._getLabel('hour', hour) + ' ';
    if (min > 0) dspVal += min + ' ' + this._getLabel('minute', min) + ' ';
    if (dspVal == '') dspVal += sec + ' ' + this._getLabel('second', sec) + ' ';
  ...
  _getUnitLevel: function () {
    ...
    if (max_unit == null) return this.MAX_UNIT_DAYS;
    switch (max_unit) {
      case 'hours':
