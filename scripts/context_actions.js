...
function switchView(type, tableName, viewName) {
  ScriptLoader.getScripts('scripts/classes/GlideViewManager.js', function () {
    if (type == 'list') new GlideViewManager(tableName, viewName).refreshList();
    else new GlideViewManager(tableName, viewName).refreshDetail();
  });
}
function copyRowToClipboard(base, ref, sysId, view) {
  var url = base + 'nav_to.do?uri=' + ref + '.do?sys_id=' + sysId;
  if (view) url += '%26sysparm_view=' + view;
  copyToClipboard(url);
}
function doUpdate(scope) {
  ...
  if (scope == 'selected') populateParmQuery(form, 'sys_idIN', 'NULL');
  else {
    if (
      !confirm(
        answer['Update the entire list?'] +
          ' (' +
          form.sysparm_total_rows.value +
          ' ' +
          answer['records'] +
          ')'
      )
...
function contextConfirm(tableName, actionName) {
  ...
  if (sysparm_query) sysparm_query = sysparm_query.value;
  else sysparm_query = '';
...
function executeRecentSearch(searchTerm, url) {
  parent.document.getElementById('sysparm_search').value =
...
function getView(tableName) {
  ...
  if (isReport()) {
    ...
    if (form) {
      ...
      if (temp) sysparm_view = temp.value;
    }
  }
  if (sysparm_view != '') return sysparm_view;
  var sp = gel('sysparm_view');
  if (sp) sysparm_view = sp.value;
  return sysparm_view;
  function isReport() {
...
var copyToClipboard =
  typeof window.NOW.g_clipboard !== 'undefined'
    ? window.NOW.g_clipboard.copyToClipboard
    : null;
function showQuickForm(id, action, width, height) {
  ...
  if (window.lastEvent) {
    ...
    } else tableName = srcElement.getAttribute('table');
    if ((action == undefined || action == '') && srcElement.value)
      action = srcElement.value;
    if (!form) keyset = g_list.getChecked();
    else keyset = getChecked(form);
    window.lastEvent = null;
  }
  if (tableName == undefined) {
    if (typeof gcm == 'undefined') gcm = crumbMenu;
    tableName = gcm.getTableName();
    form = getFormForList(tableName);
    if (typeof rowSysId != 'undefined') keyset = rowSysId;
    else keyset = getChecked(form);
    gcm.setFiringObject();
  }
  if ((!form && !tableName) || (!tableName && g_list)) return;
  if (!keyset || keyset == '') {
    alert('No records selected');
    return;
  }
  var gForm = new GlideDialogForm('', tableName + '_update');
  if (width && height) gForm.setDialogSize(width, height);
  gForm.addParm('sysparm_view', id);
  gForm.setMultiple(form);
  gForm.addParm('sysparm_checked_items', 'sys_idIN' + keyset);
  if (action && action != '') gForm.addParm('sysparm_action_name', action);
  gForm.render();
}
function personalizeResponses(id) {
  ...
  if (myreferurl) url += '&sysparm_referring_url=' + myreferurl.value;
  self.location = url;
}
function personalizeChoices(id) {
  ...
  if (mydependent != null) {
    ...
    if (el != null) {
      var selectValue;
      if (el.options) selectValue = el.options[el.selectedIndex].value;
      else selectValue = el.value;
...
function personalizeControl(strIdent, id, query) {
  ...
  if (query && query != '') url += '&sysparm_query=' + query;
  window.location = url;
}
function personalizer(strIdent, strForm, strSysId) {
  ...
  if (parentForm && parentForm['sysparm_collection_relationship'])
    addInput(
      ...
      parentForm['sysparm_collection_relationship'].value
    );
  else addInput(form, 'HIDDEN', 'sysparm_collection_relationship', '');
  ...
  if (parentForm && parentForm['sysparm_collection'])
    addInput(
      ...
      'HIDDEN',
      'sysparm_collection',
      parentForm['sysparm_collection'].value
    );
...
function personalizeList(listId, tableName) {
  ...
  else addInput(form, 'HIDDEN', 'sysparm_collection', '');
  if (typeof GlideTransactionScope !== 'undefined') {
    GlideTransactionScope.appendTransactionScope(function (name, value) {
      addInput(form, 'HIDDEN', name, value);
    });
...
function listCollection(coll_table, coll_field, of_table, view_name) {
  ...
  addQueryFilter(
    form,
    'CALCULATED:CollectionQueryCalculator',
    of_table + ',' + coll_field + ',' + view_name
  );
...
function exportToPDF(table, sys_id, isLandscape, sysparm_view, sysparm_domain) {
  ...
  if (window.g_tabs2List && g_tabs2List.tabIDs) {
    ...
    if (relatedListCount > 0) {
      for (var i = 0; i < relatedListCount; i++) {
        var relatedListName = relatedLists[i].substring(
          0,
          relatedLists[i].lastIndexOf('_list')
        );
  ...
  var url =
    ...
    '&sysparm_domain=' +
    sysparm_domain;
  if (isLandscape) url += '&landscape=true';
  window.location = url;
}
function showList(tableName, fields, ids) {
  if (!ids) ids = gActiveContext.getTableName();
  self.location =
    tableName +
    '_list.do?sysparm_query=' +
    addQueryFilter('', fields, ids, tableName, null, true);
}
function showItem(tableName, fields, ids, view) {
  ...
  var url =
    tableName +
    '.do?sysparm_query=' +
    addQueryFilter('', fields, ids, tableName);
...
function addQueryFilter(form, names, values, table, formName, useBaseTable) {
  ...
  for (var i = 0; i < vNames.length; i++) {
    ...
    } else query.push(vNames[i] + '=' + vValues[i]);
  }
  if (tableName) return query.join('^');
...
function securityQueryCalculator(values) {
  ...
  var element = null;
  var tableName = sa[0];
  if (sa.length > 1) fieldName = sa[1];
  var allTables = new Array();
  var table = new Table(tableName);
  if (fieldName == null) allTables = table.getTables();
  else {
...
function getRules(allTables, fieldName) {
  ...
  if (fieldName == null) {
    ...
    for (var i = 0; i < allTables.length; i++)
      rules +=
        '^ORname=' + allTables[i] + '^ORnameSTARTSWITH' + allTables[i] + '.';
    return rules;
  }
  var rc = new Array();
  for (var x = 0; x < allTables.length; x++) {
    ...
    rc.push(tableName);
    rc.push(tableName + '.*');
    if (fieldName != null) rc.push(tableName + '.' + fieldName);
  }
...
function setStack(form) {
  ...
  if (stack) addInput(form, 'HIDDEN', 'sysparm_nameofstack', stack);
}
