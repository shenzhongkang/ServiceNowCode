...
(function ($, undefined) {
      ...
      if (observer !== undefined) {
        ...
        this.propertyObserver = new observer(function (mutations) {
          ...
          if (disabledMutation[0])
            $(disabledMutation[0].target).trigger(
              'select2.disabled.toggle',
              disabledMutation[0].target
            );
        });
  ...
  SingleSelect2 = clazz(AbstractSelect2, {
    ...
    initContainer: function () {
      ...
      if (this.opts.element.attr('aria-required')) {
        this.focusser.attr(
          'aria-required',
          this.opts.element.attr('aria-required')
        );
      }
      ...
      if (this.originalLabel.length) {
        ...
        var id =
          this.originalLabel.attr('id') || this.focusser.attr('id') + '-label';
        this.originalLabel.attr('id', id);
        this.focusser.attr(
          'aria-labelledby',
          this.focusser.attr('aria-labelledby') +
            ' ' +
            this.originalLabel.attr('id')
        );
      }
      var originalTitle = this.opts.element.attr('title');
      this.opts.element.attr(
        'title',
        originalTitle || this.originalLabel.text()
      );
  ...
  MultiSelect2 = clazz(AbstractSelect2, {
    ...
    onSelect: function (data, options) {
      ...
      this.clearSearch();
      this.updateResults();
      var describedString = this.getVal()
        .map(function (val) {
