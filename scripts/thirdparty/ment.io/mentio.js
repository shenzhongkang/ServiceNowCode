        ...
        return {
          ...
          controller: [
            ...
            function ($scope, $timeout, $attrs) {
              ...
              $scope.defaultSearch = function (locals) {
                ...
                angular.forEach($scope.items, function (item) {
                  if (
                    item.label
                      .toUpperCase()
                      .indexOf(locals.term.toUpperCase()) >= 0
                  ) {
                    results.push(item);
                  }
                });
              ...
              $scope.bridgeSearch = function (termString) {
                var searchFn = $attrs.mentioSearch
                  ? $scope.search
                  : $scope.defaultSearch;
                searchFn({
                  term: termString,
              ...
              $scope.bridgeSelect = function (itemVar) {
                var selectFn = $attrs.mentioSelect
                  ? $scope.select
                  : $scope.defaultSelect;
                return selectFn({
                  item: itemVar,
              ...
              $scope.setTriggerText = function (text) {
                if ($scope.syncTriggerText) {
                  $scope.typedTerm =
                    $scope.trimTerm === undefined || $scope.trimTerm
                      ? text.trim()
                      : text;
                }
              };
              ...
              $scope.replaceText = function (text, hasTrailingSpace) {
                ...
                mentioUtil.replaceTriggerText(
                  ...
                  hasTrailingSpace,
                  $scope.suppressTrailingSpace
                );
                if (!hasTrailingSpace) {
                  $scope.setTriggerText('');
                  angular
                    .element($scope.targetElement)
              ...
              $scope.addMenu = function (menuScope) {
                if (
                  menuScope.parentScope &&
                  $scope.triggerCharMap.hasOwnProperty(menuScope.triggerChar)
                ) {
                  return;
                }
          ...
          link: function (scope, element, attrs, $timeout) {
            ...
            if (attrs.mentioItems) {
              ...
              var itemsRef = attrs.mentioSearch
                ? ' mentio-items="items"'
                : ' mentio-items="localItems"';
              scope.defaultTriggerChar = attrs.mentioTriggerChar
                ? scope.$eval(attrs.mentioTriggerChar)
                : '@';
              var html =
                '<mentio-menu' +
            ...
            scope.$watch('ngModel', function (newValue) {
              ...
              if (
                ...
                if (mentionInfo.mentionSelectedElement) {
                  ...
                  scope.targetElementSelectedOffset =
                    mentionInfo.mentionSelectedOffset;
                }
                ...
                scope.setTriggerText('');
                scope.hideAll();
                var macroMatchInfo = mentioUtil.getMacroMatch(
                  scope.context(),
                  scope.macros
                );
                if (macroMatchInfo !== undefined) {
                  ...
                  scope.targetElementSelectedOffset =
                    macroMatchInfo.macroSelectedOffset;
                  scope.replaceMacro(
                    macroMatchInfo.macroText,
                    macroMatchInfo.macroHasTrailingSpace
                  );
                } else if (
                  scope.selectNotFound &&
                  currentTypedTerm &&
                  currentTypedTerm !== ''
                ) {
                  var lastScope =
                    scope.triggerCharMap[scope.currentMentionTriggerChar];
                  if (lastScope) {
      ...
      function (mentioUtil, $rootScope, $log, $window, $document, $timeout) {
        return {
          ...
          templateUrl: function (tElement, tAttrs) {
            return tAttrs.mentioTemplateUrl !== undefined
              ? tAttrs.mentioTemplateUrl
          ...
          controller: [
            ...
            function ($scope) {
              ...
              $scope.activatePreviousItem = function () {
                ...
                this.activate(
                  $scope.items[
                    index === 0 ? $scope.items.length - 1 : index - 1
                  ]
                );
              };
          ...
          link: function (scope, element) {
            ...
            if (scope.parentScope) {
              ...
              if (!scope.forElem) {
                $log.error(
                  'mentio-menu requires a target element in tbe mentio-for attribute'
                );
                return;
              }
  ...
  angular.module('mentio').factory('mentioUtil', [
    ...
    function ($window, $location, $anchorScroll, $timeout) {
      function popUnderMention(
        ...
        var coordinates;
        var mentionInfo = getTriggerInfo(
          ctx,
          triggerCharSet,
          requireLeadingSpace,
          false
      ...
      function selectedElementIsTextAreaOrInput(ctx) {
        ...
        if (element !== null) {
          ...
          return (
            (nodeName === 'INPUT' && type === 'text') || nodeName === 'TEXTAREA'
          );
        }
        return false;
      }
      ...
      function getMacroMatch(ctx, macros) {
        ...
        if (effectiveRange !== undefined && effectiveRange !== null) {
          ...
          if (
            ...
            effectiveRange = effectiveRange.substring(
              0,
      ...
      function getTriggerInfo(
        ...
        if (effectiveRange !== undefined && effectiveRange !== null) {
          ...
          if (
            ...
            if (
              !leadingSpace &&
              (menuAlreadyActive || !/[\xA0\s]/g.test(currentTriggerSnippet))
            ) {
              return {
        ...
        if (selectedElementIsTextAreaOrInput(ctx)) {
          ...
          if (selectedElem != null) {
            ...
            var selectStartOffset =
              getWindowSelection(ctx).getRangeAt(0).startOffset;
