...
var GwtCellEditor = Class.create({
  ...
  initialize: function (
    ...
    this.savedFields = [];
    if (this.changes.isDeletedRow(this.sysId)) return;
    this.timer = setTimeout(
      this.showLoading.bind(this),
      this.WAIT_INITIAL_DELAY
    );
    this.valid = true;
  ...
  dismiss: function () {
    this.valid = false;
    this.hideLoading();
    if (this.editWindow) this.editWindow.dismiss();
  },
  saveAndClose: function () {
    this.valid = false;
    this.hideLoading();
    if (this.editWindow) return this.editWindow.saveAndClose();
    return false;
  },
  saveValues: function () {
    this._saveValues();
  ...
  setFieldValue: function (name, value, displayValue) {
    ...
    for (var i = 0; i < ids.length; i++) {
      ...
      if (value == null) this.renderer.setDisplayValue(id, name, displayValue);
      else this.renderer.setValue(id, name, value);
  ...
  renderEditor: function () {
    ...
    if (valueErr) this.errorMsg = valueErr;
    else this.errorMsg = this._checkSecurity();
    if (this.errorMsg) this.editWindow = this._buildErrorEditor();
    else this.editWindow = this._createEditWindow();
  },
  ...
  _checkIgnoreTypes: function () {
    ...
    for (var i = 0; i < this.ignoreTypes.length; i++) {
      if (type == this.ignoreTypes[i]) return getMessage(this.IGNORE_MSG);
    }
    if (this.tableElement.isArray()) return getMessage(this.IGNORE_MSG);
    return null;
  },
  _checkSecurity: function () {
    var canWriteIds = this.getCanWriteIds();
    if (canWriteIds.length != 0) return null;
    var sysIds = this._getSelectedSysIds();
    if (sysIds.length == 1) {
  ...
  _checkMultipleDerivedOrExtended: function () {
    ...
    if (sysIds.length > 1 && parts.length > 2) {
      if (parts[1].indexOf(this.REF_ELEMENT_PREFIX) == 0) {
        return getMessage(
          'Only one extended field can be list edited at a time'
        );
      }
  ...
  _createEditWindow: function () {
    ...
    if (
      (tableElement.isDependent() || tableElement.hasDependentChildren()) &&
      tableElement.isChoice()
    )
    ...
    if (
      tableElement.getType() == 'glide_duration' ||
      tableElement.getType() == 'timer'
    )
    ...
    if (tableElement.getType() == 'related_tags') {
      if (!isDoctype)
        return new GwtListEditError(
          this,
          this.gridEdit,
          getMessage(
            "In-cell editing not available, instead use the 'Assign Tag...' Context Menu to add a tag"
          )
        );
      else return new GwtListEditRelatedTags(this, this.gridEdit);
    }
    ...
    while (name) {
      ...
      if (tableElement) {
        ...
        if (name) {
          ...
          if (visited[name]) return topParent;
  ...
  _initFields: function (fieldName) {
    ...
    if (parts.length > 2) prefix = parts.slice(1, -1).join('.') + '.';
    if (this._addFieldName(prefix + tableElement.name)) {
      var children = tableElement.getDependentChildren();
      for (var k in children) this._initFields(this.table + '.' + prefix + k);
    }
  },
  _addFieldName: function (fieldName) {
    for (var i = 0; i < this.fields.length; i++)
      if (this.fields[i] == fieldName) return null;
  ...
  isActive: function () {
    ...
    if (!this.editWindow) return false;
