...
function formatPhone(field) {
  ...
  if (0 < ov.length && '+' != ov.charAt(0)) {
    var n = 0;
    if ('1' == ov.charAt(0)) ov = ov.substring(1, ov.length);
...
function formatCurrency(num) {
  ...
  sVal =
    minus +
    formatDollar(num, getGroupingSeparator()) +
    getDecimalSeparator() +
    formatCents(num);
  return sVal;
}
function formatNumber(num) {
  ...
  try {
    ...
    for (var i = 0; i < Math.floor((samount.length - (1 + i)) / 3); i++)
      samount =
        samount.substring(0, samount.length - (4 * i + 3)) +
...
function getDecimalSeparator() {
  if (g_user_decimal_separator) return g_user_decimal_separator;
  return '.';
}
function getGroupingSeparator() {
  if (g_user_grouping_separator) return g_user_grouping_separator;
  return ',';
}
