...
var GlideListAggregates = Class.create({
  ...
  getAggregateElement: function (fieldName, type) {
    ...
    for (var spanNdx = 0; spanNdx < spans.length; spanNdx++) {
      ...
      if (!hasClassName(span, 'aggregate_value')) continue;
      var aggtype = getAttributeValue(span, 'aggregate_type');
      if (aggtype == type) return span;
    }
    return null;
  },
  ...
  rowCountChanged: function (increment) {
    for (var k in this.aggregates) this.aggregates[k].rowNdx += increment;
  },
  _initAggregates: function () {
    ...
    for (var rowNdx = 0; rowNdx < rowCount; rowNdx++) {
      ...
      if (!hasClassName(row, 'aggregate')) continue;
  ...
  _updateAggregate: function (
    ...
    if (agg.type == 'MIN')
      ...
      aggValue = this._updateAggregateAverage(
        agg,
        oldValue,
        newValue,
        countChange
  ...
  _setAggregate: function (agg, fieldName, type, aggValue, countChange) {
    if (aggValue != null) {
      ...
      else aggSpan.innerHTML = aggValue;
      if (countChange) setAttributeValue(aggSpan, 'aggregate_count', agg.count);
    }
  },
  _updateAggregateMin: function (agg, oldValue, newValue) {
    if (agg.value == '?') return null;
    if (newValue != null && newValue < agg.value) return newValue;
    if (oldValue != null && oldValue == agg.value) return '?';
    return null;
  },
  ...
  _updateAggregateTotal: function (agg, oldValue, newValue) {
    ...
    if (isNaN(agg.value)) return;
    oldValue = new Number(oldValue);
    newValue = new Number(newValue);
    var total = new Number(agg.value);
    total += newValue - oldValue;
    total = parseFloat(
      total.toFixed(this._precision(agg.value, oldValue, newValue))
    );
    return total;
  },
  _precision: function (total, oldValue, newValue) {
    var len = Math.max(
      this._precisionLength(total),
      this._precisionLength(oldValue),
      this._precisionLength(newValue)
    );
    return len;
  },
  _precisionLength: function (value) {
    ...
    if (value.toString().indexOf('.') >= 0) {
      precisionLen = value
        .toString()
        .substr(value.toString().indexOf('.') + 1, value.toString().length);
    }
  ...
  _updateAggregateAverage: function (agg, oldValue, newValue, countChange) {
    if (!oldValue || isNaN(oldValue)) oldValue = '0';
    if (!newValue || isNaN(newValue)) newValue = '0';
    if (isNaN(agg.value) || isNaN(agg.count)) return;
