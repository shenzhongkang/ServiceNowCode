/*! RESOURCE: /scripts/util.requestManager/directives/directive.requestManager.js */
angular.module('cmdb.requests').directive('requestManager', [
  '$http',
  'snCustomEvent',
  'getTemplateUrl',
  '$timeout',
  ...
  function (
    ...
    return {
      ...
      templateUrl: getTemplateUrl('cmdb_request_manager.xml'),
      controller: function ($scope, $http) {
        ...
        snCustomEvent.observe('request_start', function () {
          if (timeOut) return;
          startTime = new Date();
          if (finalMsgTimeOut) $timeout.cancel(finalMsgTimeOut);
          finalMsgTimeOut = 0;
        ...
        function checkTransaction() {
          if (checkingWithServer) return;
          checkingWithServer = true;
          serverCheckTime = new Date();
          $http
            .get('/cancel_my_transaction.do?status=true&sysparm_output=json')
        ...
        function checkTransactionResponse(response) {
          if (response && response.data && response.data.status) {
            ...
            if ('No session, nothing to cancel' == message) clearMessage();
            else if ('complete' == message) handleRequestComplete();
            else startIntervalTimer();
          } else {
            clearMessage();
          }
        }
        ...
        function handleRequestComplete() {
          if (loadCancelValidation == false) {
            $scope.timer = getTime();
            if ($scope.cancelling) setStatus('Transaction cancelled after');
            else setStatus('Transaction completed in');
        ...
        function handleInterval() {
          ...
          if (timer >= transactionCheckDelay) checkTransaction();
          $scope.timer = getTime();
          if ($scope.cancelling) setStatus('Transaction cancelling:');
          else setStatus('Running Transaction:');
          setTimerMessage();
          if ($scope.timer == 0) {
            handleRequestComplete();
          }
        ...
        function setTimerMessage() {
          if ($scope.timer < $scope.timerDelay) return;
        ...
        $scope.handleRequestCancel = function () {
          ...
          $scope.cancelFired = true;
          $scope.cancelling = true;
          $scope.hideAlertButton(true);
          setStatus('Transaction cancelling:');
          $http
            .get('cancel_my_transaction.do?sysparm_output=xml')
        ...
        $scope.showAlertBar = function (enabled) {
          if ($scope.showAlert === enabled) return;
          if (enabled) MessagesStack.push('request-manager');
