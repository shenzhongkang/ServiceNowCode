...
var GwtListEditAjaxChangeSaver = Class.create({
  ...
  _buildXml: function () {
    ...
    var serializer = new GlideListEditor.XmlSerializingReceiver(
      xml,
      xml.documentElement,
      selector
    );
  ...
  _saveResponse: function (response) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (savedSysIds.indexOf(sysId) < 0) savedSysIds.push(sysId);
    ...
    if (savedSysIds.length > 0) this._fireChangesSaved(savedSysIds);
    if (this.onCompletion) this.onCompletion();
  },
  ...
  _showSaving: function () {
    for (var i = 0; i < this.changedEntries.length; i++) {
      ...
      if (element) this._showImage(element);
    }
  },
