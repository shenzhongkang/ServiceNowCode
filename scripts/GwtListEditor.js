...
var GwtListEditor = Class.create({
  ...
  edit: function (element) {
    ...
    if (cellEditor) this.cellEditor = cellEditor;
  },
  ...
  savePreviousEditor: function () {
    ...
    if (this.cellEditor.saveAndClose()) this.cellEditor = null;
  },
  ignoreClick: function (element) {
    if (element.tagName != 'TD') return true;
    if (!hasClassName(element, 'vt')) return true;
    if (hasClassName(element, 'vt-spacer')) return true;
  },
  onClickedAndDisabled: function (e) {
    var element = Event.element(e);
    if (element.tagName == 'DIV' && element.parentNode.tagName == 'TD')
      element = element.parentNode;
    if (this.ignoreClick(element)) return true;
    var warn = $('list_edit_disabled_warning');
    if (warn) warn.remove();
    var msg = getMessage(
      'List editing disabled - to enable, click the gear icon at top of list'
    );
    var imgHTML =
      '<img id="list_edit_disabled_warning" style="padding-left:2px" src="images/icons/cancel_circle_grey.gif" title="' +
      msg +
      '"/>';
    if (element.style.textAlign == 'right')
      element.innerHTML = imgHTML + element.innerHTML;
    else element.innerHTML = element.innerHTML + imgHTML;
  },
  onSelected: function (e) {
    ...
    if (this.ignoreClick(element)) return true;
    var listEditor = this._getListEditorForElement(element);
    if (listEditor) listEditor.setCursor(element);
  },
  onTriggerEdit: function (e) {
    ...
    if (listEditor) listEditor.gridEdit.startTableEdit();
  },
  onClicked: function (e) {
    ...
    if (e.ctrlKey || e.metaKey) {
      if (GwtListEditWindow.getCellEditWindow()) this.savePreviousEditor();
      return false;
    }
    this.edit(element);
  },
  ...
  setIgnoreTypes: function (types) {
    if (!types) this.ignoreTypes = [];
    else this.ignoreTypes = types.split(',');
  },
  addRow: function (listID) {
    var listEditor = this._getListEditor(listID);
    if (listEditor) listEditor.addRow();
  },
  deleteRowToggle: function (id, listID) {
    var listEditor = this._getListEditor(listID);
    if (listEditor) listEditor.deleteRowToggle(id);
  },
  ...
  _onTableLoad: function () {
    if (!window['g_list_edit_enable_property']) return;
    ...
    for (var i = 0, n = tableElements.length; i < n; i++) {
      if (this._preparedTables.indexOf(tableElements[i]) != -1) continue;
  ...
  _getClickedEvent: function () {
    if (g_list_edit_double) return 'dblclick';
    return 'click';
  },
  _prepareTable: function (table, eventName) {
    if (!table.down('tbody')) return;
    if (!this._isEditableList(table)) return;
    if (
      !window['g_list_edit_enable'] &&
      window['g_list_edit_enable_property']
    ) {
      Event.observe(
        table,
        eventName,
        this.onClickedAndDisabled.bindAsEventListener(this)
      );
      return;
    }
    ...
    if (g_list_edit_double)
      table
        .down('tbody')
        .observe('click', this.onSelected.bindAsEventListener(this));
    document.on(
      'click',
      '.list2-edit-button',
      this.onTriggerEdit.bindAsEventListener(this)
    );
    Event.observe(
      table,
      'mousedown',
  ...
  _getListEditTable: function (listID) {
    ...
    if (!table) return null;
    if (!hasClassName(table, 'list_table')) return null;
    return table;
  },
  _isEditableList: function (e) {
    if (e.tagName != 'TABLE') return false;
    if (!hasClassName(e, 'list_table')) return false;
  ...
  _hasChanges: function () {
    for (var listID in this.listEditorCache) {
      var listEditor = this.listEditorCache[listID];
      if (listEditor.hasChanges()) return true;
    }
    return false;
  },
  _getFormSubmitted: function () {
    if ('undefined' === typeof g_form) return false;
  ...
  _testBeforeUnload: function () {
    ...
    if (this.chainedUnloadHandler) {
      ...
      if (chainedResponse) return chainedResponse;
    }
    if (beingSubmitted) return;
    var message;
  ...
  _hookOnBeforeUnload: function () {
    if (!this.dirtyFormMessage) return;
  ...
  enableDirtyFormMessages: function () {
    ...
    if (tableElements && tableElements.length > 0) {
      this.dirtyFormMessage = getMessage(
        'Changes have been made. Continuing will discard the changes.'
      );
      this.openEditorMessage = getMessage(
        'There is a cell editor open on this page. Continuing will discard the changes.'
      );
    }
  },
  ...
  _onTabActivated: function (evt) {
    try {
      if ('string' !== typeof evt) return;
      if (!evt.startsWith('tabs2_list_')) return;
      ...
      if (!div) return;
      var divId = div.id;
      if (!divId || divId.length <= 5) return;
      var listId = divId.substring(0, divId.length - 5);
      if (this.listEditorCache[listId]) return;
      var table = GwtListEditor.getTableElem(listId);
      if (!table) return;
...
GwtListEditor._getListEditor = function (listID) {
  if (GwtListEditor.forPage) return GwtListEditor.forPage.getListEditor(listID);
  else return null;
};
...
function editListRegisterOnSubmit(listID) {
  if (!window['g_list_edit_enable']) return;
...
GwtListEditor._editListOnSubmitWrapper = function (listID) {
  ...
  g_list = null;
  if (result !== false) editor.saveUpdatesInForm();
  return result;
};
...
function initListEdit() {
  ...
  CustomEvent.observe(
    'partial.page.savePreviousEditor',
    GwtListEditor.forPage.savePreviousEditor.bind(GwtListEditor.forPage)
  );
  CustomEvent.observe(
    'partial.page.reload',
    GwtListEditor.forPage._onPartialLoad.bind(GwtListEditor.forPage)
  );
  CustomEvent.observe(
    'init.gwt_editor',
    GwtListEditor.forPage._onPartialLoad.bind(GwtListEditor.forPage)
