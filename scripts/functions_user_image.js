...
function saveUserImage(tableName, gotourl) {
  ...
  var okToSubmit = true;
  if (typeof form.onsubmit == 'function') okToSubmit = form.onsubmit();
...
function validateWithExtensions(fileName, fileExtensions) {
  ...
  if ('sysverb_cancel' == action.value) return true;
...
function validateSize(input) {
  ...
  var maxSize =
    form.max_size && form.max_size.value ? form.max_size.value : 1024;
...
function deleteUserImage(image_id, replacement) {
  ...
  if (delspan) delspan.innerHTML = '';
  var addanchor = $(image_id + '_update');
  if (addanchor) addanchor.innerHTML = '';
  var imagespan = $(image_id + '_image');
  if (imagespan) imagespan.style.visibiity = 'hidden';
  var noimagespan = $(image_id + '_noimage');
  if (noimagespan) noimagespan.style.visibility = '';
  return false;
}
