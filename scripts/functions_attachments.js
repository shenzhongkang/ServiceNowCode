...
function addAttachmentNameToForm(
  ...
  if (line) line.setStyle({ visibility: 'visible', display: '' });
  var span = $(cel('span'));
  ...
  span.innerHTML =
    ...
    '" title="' +
    hoverText +
    '" style="margin-right:4px;" tabindex="-1"><img src="' +
    image +
    '" alt="" /></a>';
  var txt = $(cel('a'));
  if ('innerText' in txt) txt.innerText = name;
  else txt.textContent = name;
  txt.href = '#';
  ...
  txt.href =
    'sys_attachment.do?sys_id=' + sysid + '&sysparm_this_url=' + thisURL;
    ...
    onAfterEdit: function (newName) {
      ...
      ga.getXML(function (response) {
        ...
        $$('a[data-id="' + sysid + '"]').each(function (elem) {
          ...
          else elem.textContent = answer === '0' ? newName : oldName;
        });
  ...
  if (showRename == 'true') {
    ...
    getMessage('Rename {0}', function (msg) {
      renameAttachment.setAttribute(
        'aria-label',
        new GwtMessage().format(msg, name)
      );
    });
  ...
  if (showView == 'true') {
    ...
    if (downloadAttachment) getMessage('Download {0}', setAriaLabel);
    else getMessage('View {0}', setAriaLabel);
    var newText = document.createTextNode(
      downloadAttachment ? getMessage('[download]') : getMessage('[view]')
    );
  ...
  if (attachList) attachList.appendChild(storage);
...
function getCurrentPageURL() {
  return encodeURIComponent(
    window.location.pathname.substr(1) + window.location.search
  );
}
...
function addEllipsesToAttachments() {
  ...
  if (more) {
    ...
    else setElementStyle(more, 'display:none');
  }
}
...
function addAttachmentNameToDialog(
  id,
  fileName,
  canDelete,
  createdBy,
  ...
  var thisURL = getCurrentPageURL();
  anchor.href =
    'sys_attachment.do?sys_id=' + id + '&sysparm_this_url=' + thisURL;
  ...
  if ('innerText' in text) text.innerText = fileName;
  else text.textContent = fileName;
  ...
  text.inPlaceEdit({
    ...
    onAfterEdit: function (newName) {
      ...
      $$('a[data-id="' + id + '"]').each(function (elem) {
        if ('innerText' in elem) elem.innerText = newName;
        else elem.textContent = newName;
      });
      $$('span[data-id="' + id + '"]').each(function (el) {
        if ('innerText' in el) el.innerText = newName;
        else el.textContent = newName;
      });
      updateAriaLabels(id, newName);
      text.removeAttribute('role');
    },
  });
  if (contentType == 'text/html') anchor.target = '_blank';
  ...
  if ($('ni.show_rename_link').value) {
    ...
    getMessage('Rename {0}', function (msg) {
      renameAttachment.setAttribute(
        'aria-label',
        new GwtMessage().format(msg, fileName)
      );
    });
    renameAttachment.onclick = function () {
      text.beginEdit();
  ...
  if (alert508 == 'true') alert(fileName + ' ' + anchor.title);
}
...
function getCurrentAttachmentNumber(sysid) {
  if ($$('.attachmentNumber_' + sysid)[0] == undefined) {
    if ($('header_attachment_list_label') == undefined) return undefined;
    else return $('header_attachment_list_label').down().innerHTML;
  }
...
function getPlusMoreAttachmentList() {
  var attachList = $('header_attachment_list');
  return !attachList || !attachList.hasClassName('with-plus-more')
    ? false
    : attachList;
}
function showHidePlusMore(number) {
  var header_attachment = $('header_attachment');
  if (!header_attachment) return;
  ...
  if (number <= limit) plusMore.addClassName('hidden');
  else plusMore.removeClassName('hidden');
  adjustAttachmentsVisibility();
}
function adjustAttachmentsVisibility() {
  ...
  attachList.select('.attachment_list_items').each(function (elem) {
    ...
    if (attachment && attachment.innerHTML) attachmentsFound++;
    if (attachmentsFound <= limit) elem.removeClassName('hidden');
...
function saveAttachment(tableName, sys_id, allowAttachment) {
  ...
  if (allowAttachment) g_dialog.setPreference('sc_override', 'true');
  g_dialog.setPreference(
    'attachment_disabled',
    window['AttachmentUploader']
      ? AttachmentUploader.isAttachmentDisabled()
      : 'false'
  );
  g_dialog.setPreference('focusTrap', true);
...
function _saveAttachmentConfirm(dialog) {
  ...
  if (!value.empty())
    if (
      !confirm(
        getMessage('Close before uploading attachments?') +
          '\n' +
...
function _saveAttachmentClose() {
  ...
  if (modified) {
    ...
    if (attachmentsModified != 'true') return;
  }
  if (typeof g_form == 'undefined') return;
  if (g_form.newRecord) g_form.modified = true;
  if (typeof GlideLists2 == 'undefined') return;
  for (var id in GlideLists2) {
    var list = GlideLists2[id];
    if (list.getTableName() === 'sys_attachment') list.refresh();
  }
}
function allowInPlaceEditModification(elem, event) {
  var length =
    elem.textContent != null ? elem.textContent.length : elem.innerText.length;
...
function maximumCharacterLimit() {
  ...
  if (f) return f.getAttribute('data-max-filename-length');
  return 100;
}
...
function saveFileAttachment(tableName, gotourl, options) {
  ...
  if (viewwidget) gotourl += '&sysparm_view=' + viewwidget.value;
  form.sys_action.value = 'sysverb_check_save';
  addInput(form, 'HIDDEN', 'sysparm_goto_url', gotourl);
  var okToSubmit = true;
  if (typeof form.onsubmit == 'function')
    if (runOnSubmit) okToSubmit = form.onsubmit();
    else g_form.submitted = true;
  if (okToSubmit) form.submit();
  return false;
}
function deleteFileAttachment(attachment_id, field_input_id) {
  ...
  if (attachmentspan) attachmentspan.parentNode.removeChild(attachmentspan);
  if (field_input_id) {
    ...
    if (noattachmentspan) noattachmentspan.style.visibility = 'visible';
...
function pushNotAvailableMessage(attachmentName) {
  g_form.addErrorMessage(
    new GwtMessage().getMessage(
      'The file <b> {0} </b> did not pass security scan and cannot be downloaded',
      attachmentName.escapeHTML()
    )
  );
}
function validateSizeAndExt(field) {
  ...
  var fileTypes =
    form.file_types && form.file_types.value ? form.file_types.value : '';
  ...
  if (file.size > allowedSize && allowedSize !== 0)
    warningMessages.push(
      gMessage.getMessage(
        '{0} is {1}. The maximum file size is {2}.',
        file.name,
        getDisplaySize(file.size),
        getDisplaySize(allowedSize)
      )
    );
  var fileExtension =
  ...
  if (!validExtension)
    warningMessages.push(
      gMessage.getMessage('{0} has a prohibited file extension.', file.name)
    );
