...
angular
  .module('sn.$sp')
  .directive('spVariableLayout', function () {
    'use strict';
    return {
      restrict: 'E',
        ...
        controller: function ($scope, $rootScope, $timeout) {
          ...
          c.populateMandatory = function populateMandatory(flatFields) {
            ...
            for (var f in flatFields) {
              field = flatFields[f];
              if (
                typeof field.mandatory_filled == 'undefined' ||
                field.type == spSCConf.CONTAINER_START ||
                field.type == spSCConf.LABEL
              )
                continue;
              if (field.mandatory_filled()) continue;
              if (field.visible && field.isMandatory()) mandatory.push(field);
            }
            $scope.$emit('variable.mandatory.change');
            return mandatory;
          };
          function onChange(fieldName, oldValue, newValue) {
            if (!(fieldName in formModel._fields)) return;
            $scope.$evalAsync(function () {
              ...
              var isProcessPrice =
                isCatalogItem &&
                $scope.formModel.process_price &&
                hasPricingImplications(field);
              if (isProcessPrice) {
                if (field.type == 'boolean' || field.type == 'boolean_confirm')
                  c.setBoolean(field, newValue);
                if (field.choices) c.setPrices(field, newValue);
              }
              ...
              if (isProcessPrice) c.calcPrice(formModel._fields);
            });
          }
          function hasPricingImplications(field) {
            if (!field._pricing) return false;
            if (field.type == 'boolean_confirm') return true;
            if (field.type == 'sc_multi_row') return true;
            if (
              field.type == 'reference' ||
              field.type == 'glide_list' ||
              field.type == 'boolean' ||
              field.choices
            )
              if (
          ...
          function getNestedVariables(fm, fields, viewField) {
            if (typeof viewField.variables !== 'undefined') {
              ...
              if (fieldModel) fieldModel._children = [];
          ...
          c.hasCatalogVariable = function hasCatalogVariable(flatFields) {
            for (var f in flatFields) {
              if (flatFields[f].hasOwnProperty('_cat_variable')) return true;
            }
            return false;
          };
          c.calcPrice = function calcPrice(fields) {
            function forcePriceUpdate() {
              for (var f in fields) {
                if (fields[f].type == 'sc_multi_row') return true;
                if (
                  fields[f].type == 'reference' ||
                  fields[f].type == 'glide_list'
                ) {
                  if (
                    fields[f]._pricing &&
                    fields[f]._pricing.pricing_implications === true
                  )
                    return true;
                }
              }
            }
            if (formModel.sys_class_name == 'sc_cat_item_producer') return;
            var price = 0;
            ...
            angular.forEach(fields, function (field) {
              ...
              if (field.price) price += Number(field.price);
              if (field.recurring_price)
                recurring_price += Number(field.recurring_price);
            });
            if (isMultiRowFields) return;
            var o = {
              price: price,
              recurring_price: recurring_price,
              force_update: forcePriceUpdate(),
            };
            if (fields) $scope.$emit('variable.price.change', o);
          };
          c.setBoolean = function setBoolean(field, value) {
            ...
            } else field.price = field.recurring_price = 0;
          };
          ...
          c.hasChoiceFields = function (flatFields) {
            for (var f in flatFields) {
              if (
                flatFields[f].type == 'choice' ||
                flatFields[f].type == 'multiple_choice'
              )
                return true;
            }
            return false;
          ...
          c.hasChoiceFieldsWithPrice = function (flatFields) {
            for (var f in flatFields) {
              ...
              if (
                ...
                field._pricing &&
                field._pricing.pricing_implications === true &&
                field._pricing.enhance_price_labels === true
              )
                return true;
            }
            return false;
          ...
          function initGlideForm() {
            var uiActions = spUIActionFactory.create(
              formModel._ui_actions || [],
              {
            ...
            g_form = glideFormFactory.create(
              $scope,
              isCatalogItem ? null : formModel.table,
              formModel.sys_id,
              flatFields,
              uiActions,
              {
              ...
              uiPolicies.forEach(function (policy) {
                policy.conditions.forEach(function (condition) {
                  if (condition.field.startsWith('variables.')) {
                    var conditionField =
                      $scope.formModel._fields[condition.field.substring(10)];
                    if (conditionField)
                ...
                policy.actions.forEach(function (action) {
                  if (action.name.startsWith('variables.')) {
                    var actionField =
                      $scope.formModel._fields[action.name.substring(10)];
                    if (actionField) action.name = actionField.variable_name;
                  }
                });
              });
            }
            ...
            g_form.$private.events.on(
              ...
              function (type, fieldName, propertyName, propertyValue) {
                ...
                if (
                  ...
                  if (section) {
                    var mandatoryFields = $scope.mandatory.filter(function (
                      field
                    ) {
          ...
          $scope.isContainerVisible = function isContainerVisible(container) {
            if (!isCatalogItem && typeof container.visible !== 'undefined') {
              if (container._parent) {
                ...
                return typeof parentVisibility !== 'undefined'
                  ? parentVisibility
                  : container.visible;
              }
          ...
          c.massageView = function massageView(formModel) {
            if (typeof formModel._view == 'undefined') return;
          ...
          function init() {
            ...
            if (formModel && formModel.delayView) {
              ...
              $scope.$on('spModel.view.render', function (evt, id, callback) {
              ...
              $scope.$evalAsync(function () {
                ...
                if (isCatalogItem && $scope.formModel.process_price) {
                  ...
                  if (c.hasChoiceFieldsWithPrice(flatFields))
                    spLabelHelper.getPriceLabelsForChoiceFields(
                      flatFields,
          ...
          function createEnvironment(user) {
            ...
            formEnvironment =
              glideFormEnvironmentFactory.createWithConfiguration(
                g_form,
                user,
                formModel.g_scratchpad,
                formModel.client_script || [],
                formModel.policy || [],
                g_modal,
                formModel.validation_scripts || [],
                uiScriptFactory.create(formModel.ui_scripts || [], $q)
              );
            ui_g_form = formEnvironment.getUserGlideForm();
            if (isCatalogItem) {
              formEnvironment.g_env.registerExtensionPoint(
                'g_service_catalog',
                {
              ...
              catalogGlideFormFactory.addItemEditor(
                g_form,
                formModel.sys_id,
                null,
                formModel.sys_id,
              ...
              catalogGlideFormFactory.addVariableEditor(
                g_form,
                formModel.sys_id,
                null,
                formModel.sys_id,
            ...
            formEnvironment.g_env.registerExtensionPoint(
              'g_list',
              glideListFactory.init(g_form, flatFields)
            );
