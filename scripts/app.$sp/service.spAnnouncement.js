...
angular
  ...
  .factory(
    'spAnnouncement',
    function ($rootScope, $http, $window, $timeout, $q, spConf, spUtil) {
      'use strict';
      ...
      function _cleanupStorage() {
        for (var key in $window.localStorage) {
          ...
          if (
            key.indexOf('dismissed_announcement_') === 0 &&
            $window.localStorage.getItem(key) !== _sessionId
          ) {
      ...
      function _handleHttpError(res) {
        console.log(
          spUtil.format('*** [HTTP::{code}] Unable to retrieve announcement', {
            code: res.status,
          })
        );
      }
      function _buildReq(path, method) {
        return {
          ...
          url: path
            ? spConf.announcementApi + '/' + path
            : spConf.announcementApi,
          headers: {
        ...
        if (!query) {
          ...
          result = list.filter(function (a) {
            ...
            try {
              ...
              console.log(
                '*** spAnnouncement.service: unable to process filter',
                e
              );
            }
            return include;
      ...
      function _sessionDismissed(id) {
        return (
          $window.localStorage.getItem('dismissed_announcement_' + id) ===
          _sessionId
        );
      }
      function _dismiss(id) {
        ...
        if (
          announcement.dismissOption === 'SESSION_DISMISSIBLE' ||
          !$rootScope.user.logged_in
        ) {
          try {
            $window.localStorage.setItem(
              'dismissed_announcement_' + id,
              _sessionId
            );
          } catch (e) {}
        } else {
      ...
      function _processAnnouncements() {
        ...
        _all.forEach(function (a) {
          if (
            a.dismissed ||
            ((a.dismissOption === 'SESSION_DISMISSIBLE' ||
              !$rootScope.user.logged_in) &&
              _sessionDismissed(a.id))
          ) {
            a.dismissed = true;
          }
          _list.push(a);
      ...
      function _init(announcements) {
        if (
          _initialized ||
          _initializing ||
          $window.NOW.sp.disableAnnouncements
        ) {
          return $q(function (resolve) {
            resolve();
          });
