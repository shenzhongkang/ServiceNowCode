...
angular
  ...
  .factory(
    'spPage',
    function ($rootScope, spConf, $location, $window, $sanitize, i18n) {
      'use strict';
      function getStyle(page) {
        return (
          '<style type="text/css" data-page-id="' +
          page.sys_id +
          '" data-page-title="' +
          $sanitize(page.title) +
          '">' +
      ...
      function getClasses(scope) {
        var style = [];
        if (scope.isNative) style.push('isNative');
        if (scope.theme.navbar_fixed) style.push('fixed-header');
        if (scope.theme.footer_fixed) style.push('fixed-footer');
        return style.join(' ');
      }
      ...
      function isHashChange(newUrl, oldUrl) {
        if (newUrl == oldUrl) return false;
        var newUrlParts = newUrl.split('#');
        var oldUrlParts = oldUrl.split('#');
        return newUrlParts.length > 1 && newUrlParts[0] == oldUrlParts[0];
      }
      function userLoggedIn(user) {
        if (user.hasOwnProperty('logged_in')) return user.logged_in;
        if (user.user_name === 'guest') return false;
        if (
          typeof user.user_name !== 'undefined' &&
          user.user_name &&
      ...
      function getTitle(response) {
        if (response.portal.title && !response.portal.hide_portal_name)
          return response.page.title
            ? response.page.title + ' - ' + response.portal.title
            : response.portal.title;
        return response.page.title;
      }
      function saveOnCtrlS(e) {
        if (e.keyCode != spConf.s) return;
      ...
      function getUrl(portalId, omitTheme) {
        ...
        if (omitTheme) params.omitTheme = omitTheme;
