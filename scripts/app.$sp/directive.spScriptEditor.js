...
angular
  ...
  .directive(
    ...
    function (
      $rootScope,
      $http,
      spCodeEditorAutocomplete,
      defaultJSAutocomplete,
      $timeout
    ) {
      return {
        template:
        ...
        link: function (scope, element, attrs, ctrl) {
          $timeout(function () {
            ...
            var server;
            spCodeEditorAutocomplete
              .getConfig('sp_widget', field.name)
            ...
            cmi.on('keyup', function (cm, event) {
              ...
              var ternTooltip = document.getElementsByClassName(
                'CodeMirror-Tern-tooltip'
              )[0];
              if (keyCode == 190)
                if (event.shiftKey) return;
                else server.complete(cmi, server);
            ...
            cmi.on('codemirror_hint_pick', function (i) {
              ...
              if (token && token.indexOf('fn(') != -1) {
                if (
                  editor.getTokenAt({ ch: cur.ch + 1, line: cur.line })
                    .string != '('
                ) {
                  editor.replaceRange(
                    '()',
                  ...
                  if (
                    token &&
                    token.substr(0, 4) !== 'fn()' &&
                    angular.element('div.CodeMirror-Tern-tooltip')[0]
                  ) {
