...
angular.module('sn.$sp').factory('spFacetManager', function ($location) {
  ...
  function clearFacetValue(facetID, facetValue) {
    ...
    if (
      !currentFacetValue.charAt(0) === '[' ||
      !currentFacetValue.charAt(currentFacetValue.length - 1) === ']'
    ) {
      clearFacet(facetID);
      return;
    }
    var currentFacetArr = currentFacetValue
      .substring(1, currentFacetValue.length - 1)
      .split(',');
  ...
  function isFacetSelected(key, searchSource) {
    var facetURLValue = $location.search()[key],
      t = $location.search().t;
    return (
      facetURLValue != undefined && facetURLValue != null && t === searchSource
    );
  }
