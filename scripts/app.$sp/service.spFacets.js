...
angular
  .module('sn.$sp')
  .factory(
    'spFacetsClientService',
    function ($rootScope, $location, $http, $q, spFacetManager) {
      'use strict';
      function _search(params) {
        return $http.post('/api/now/sp/search', params).then(
          function successCallback(response) {
            ...
            if (response.data.result) {
              var notifications = {};
              response.data.result.$$uiNotification.forEach(function (
                notification
              ) {
