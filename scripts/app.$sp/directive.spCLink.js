...
angular.module('sn.$sp').directive('spCLink', function (i18n) {
  return {
    ...
    replace: true,
    template:
      '<span class="sp-convenience-link-wrapper" ng-if="display()"><a href="{{href}}" target="_blank" class="sp-convenience-link">{{getText()}}</a></span>',
    link: function (scope, element, attrs, controller) {
      ...
      if (scope.target) {
        ...
        if (target == 'form') href = scope.table + '.do?sys_id=' + scope.id;
          ...
          scope.$watch(
            ...
            function (newValue, oldValue) {
              if (newValue != oldValue)
                scope.href =
                  scope.table + '_list.do?sysparm_query=' + scope.query;
            }
          );
