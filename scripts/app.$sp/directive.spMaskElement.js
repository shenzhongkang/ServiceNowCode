  ...
  return {
    ...
    controller: function ($scope) {
      ...
      ($scope.modelValueChange = function () {
        ...
        $scope
          .getGlideForm()
          .setValue($scope.field.name, $scope.field.stagedValue);
      }),
      ...
      (c.validate = function () {
        if (
          $scope.field.stagedValue !== $scope.field.confirmPassword &&
          $scope.field.useConfirmation == true
        )
          $scope.field.isInvalid = true;
        else $scope.field.isInvalid = false;
        $scope.onChange();
      }),
    ...
    link: function (scope, elem, attrs) {
      ...
      scope.reEnter = i18n.format(
        i18n.getMessage('Re-enter `{0}`'),
        scope.field.label
      );
