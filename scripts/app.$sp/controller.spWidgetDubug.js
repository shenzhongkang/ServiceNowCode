  ...
  .controller(
    ...
    function (
      ...
      var menu = [
        ...
        [
          i18n.getMessage('Instance in Page Editor {0}').withValues(['➚']),
          instancePageEdit,
        ],
        [
          i18n.getMessage('Page in Designer {0}').withValues(['➚']),
          openDesigner,
        ],
        null,
        [
          i18n.getMessage('Show Widget Customizations'),
          spWidgetDiagnostics.enableDiagnostics,
        ],
        null,
        ...
        [
          i18n.getMessage('Widget in Editor {0}').withValues(['➚']),
          openWidgetEditor,
        ],
        null,
        [
          i18n.getMessage('Log to console: {0}').withValues(['$scope.data']),
          logScopeData,
        ],
        [
          i18n.getMessage('Log to console: {0}').withValues(['$scope']),
          logScope,
        ],
      ];
      var basic_menu = [
      ...
      $scope.contextMenu = function (event) {
        ...
        if (g_user_is_admin) {
          if (spWidgetDiagnostics.diagnostics.isEnabled)
            menu[5] = [
              i18n.getMessage('Hide Widget Customizations'),
              spWidgetDiagnostics.disableDiagnostics,
            ];
          else
            menu[5] = [
              i18n.getMessage('Show Widget Customizations'),
        ...
        var m = menu.slice();
        if (!$rootScope.user.can_debug_admin) m = basic_menu.slice();
        else if ($scope.page.internal) m.splice(3, 1);
        ...
        m[1] =
          (!w.option_schema &&
            w.options &&
            !w.options.widget_parameters &&
            !w.field_list) ||
          !$rootScope.user.can_debug_admin
            ? [i18n.getMessage('Instance Options')]
            : [i18n.getMessage('Instance Options'), editInstance];
        var p = '_debugContextMenu';
        if (p in w && Array.isArray(w[p])) return m.concat([null], w[p]);
        return m;
      };
      function logScope() {
      ...
      function openDesigner() {
        ...
        $window.open(
          spUtil.format('/{page}#/{portal}/editor/{pageId}/{instance}', ops),
          page
        );
      }
