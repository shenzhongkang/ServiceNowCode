  ...
  .factory(
    ...
    function (
      ...
      function alertConfirm(options) {
        ...
        if (options.title.length < 25) options.size = 'sm';
        var defer = $q.defer();
        open(options).then(
          function () {
            defer.resolve(true);
...
angular.module('sn.$sp').directive('spFocusIf', function focusIf($timeout) {
  function link($scope, $element, $attrs) {
    var dom = $element[0];
    if ($attrs.spFocusIf) $scope.$watch($attrs.spFocusIf, focus);
    else focus(true);
