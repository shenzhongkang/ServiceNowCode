...
angular
  .module('sn.$sp')
  .factory('spContextManager', function ($rootScope, $window, $interval, $q) {
    'use strict';
    ...
    function _removeKey(key) {
      if (_context[key]) delete _context[key];
    }
    function _setGlobalContext(rootScope) {
      return $q(function (resolve) {
        ...
        interval = $interval(function () {
          ...
          if (!scope.portal || !scope.page || !scope.user) return;
    ...
    function addContext(key, context) {
      ...
      if (_context[key] !== undefined)
        throw new Error(
          'Context for the key "' + key + '" is already registered'
        );
    ...
    function getContextForKey(key, returnPromise) {
      if (!returnPromise) return _clone(_context[key]);
      var interval;
      var cancelInterval = function () {
        $interval.cancel(interval);
        interval = undefined;
      ...
      return $q(function (resolve) {
        ...
        interval = $interval(function () {
          ...
          if (new Date().getTime() > expire) {
            cancelInterval();
            throw new Error(
              'Request to get context for the key "' + key + '" timed out'
            );
          }
        }, 10);
      });
