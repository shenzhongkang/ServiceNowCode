...
angular
  .module('sn.$sp')
  .directive('spSearchNav', function (i18n, $location, spFacetManager) {
    return {
      restrict: 'E',
      replace: true,
      scope: {
      ...
      controller: function ($scope) {
        ...
        function onLocationChangeSuccess(event, newUrl, oldUrl) {
          ...
          if (
            !pageChanged(newUrl, oldUrl) &&
            searchSourceChanged(newUrl, oldUrl)
          ) {
            var newUrlParams = newUrl.match(/t=.+/);
            if (!newUrlParams) {
              updateSelectedSearchSource(null);
            } else {
              updateSelectedSearchSource(
                newUrlParams[0].split('&')[0].substring(2)
              );
            }
          }
        }
