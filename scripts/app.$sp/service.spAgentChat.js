...
angular
  ...
  .factory(
    'spAgentChat',
    function ($q, $http, $window, $location, spContextManager) {
      'use strict';
      var CONTEXT_KEY = 'agent-chat';
      var STORAGE_KEY = 'sp.agent-chat';
      var DEFAULT_FRAME_URL =
        '/$sn-va-web-client-app.do?sysparm_nostack=true&sysparm_stack=no';
      ...
      function _initStateStorage() {
        ...
        Object.keys(state).forEach(function (key) {
          if (key !== _sessionId) delete state[key];
        });
      ...
      function getFrameUrl() {
        ...
        $http({
          ...
          function (response) {
            spContextManager
              .getContextForKey('global', true)
              .then(function (globalContext) {
                spContextManager
                  .getContextForKey(CONTEXT_KEY, true)
                  .then(function (acContext) {
                    var params = {};
                    ...
                    _appendParams(
                      spContextManager.getContextForKey('record') || {},
                      params
                    );
                    ...
                    deferred.resolve(
                      DEFAULT_FRAME_URL + _buildParamString(params)
                    );
                  });
              });
          },
          function (response) {
            console.error(
              'Unable to retrieve SP Agent Chat Context. Error: ' +
      ...
      function subscribe(event, handler) {
        ...
        if (Array.isArray(eventHandlers)) eventHandlers.push(handler);
      }
      function registerParam(key, value) {
        if (typeof value !== 'string')
          throw new Error(
            'Value for the "' +
              key +
              '" agent chat param must be of type "string"'
          );
        spContextManager
          .getContextForKey(CONTEXT_KEY, true)
