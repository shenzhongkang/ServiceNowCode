...
angular
  ...
  .factory(
    'spLabelHelper',
    function ($q, $http, i18n, spUtil, glideFormFieldFactory) {
      'use strict';
      ...
      function syncFieldAriaLabel(scope) {
        if (scope.labelElement) {
          var ariaLabel =
            scope.accessible && scope.field.hint
              ? undefined
              : getFieldAriaLabel(scope.field);
          if (ariaLabel) scope.labelElement.attr('aria-label', ariaLabel);
          else scope.labelElement.removeAttr('aria-label');
        }
      }
      function getFieldAriaLabel(field) {
        ...
        if (field.readonly)
          ariaLabel = i18n
            .getMessage('Read only - {0}')
            .withValues([ariaLabel]);
        if (field.type === 'reference')
          ariaLabel += ' ' + getReferenceLabelContents(field);
        return ariaLabel.trim() || undefined;
      }
      function getPriceLabelForCheckbox(field) {
        if (!field || !field._pricing) return;
        if (
          !(
            field._pricing.price_if_checked ||
            field._pricing.rec_price_if_checked
          )
        )
          return '';
        var label = ' [ ';
        if (field.value == 'true') {
          if (
            ...
            label += i18n
              .getMessage('has added {0} | has added {1}')
              .withValues([
                field._pricing.price_if_checked_display,
                field._pricing.rec_price_if_checked_display,
              ]);
          else if (
            field._pricing.price_if_checked ||
            field._pricing.rec_price_if_checked
          ...
          if (
            ...
            label += i18n
              .getMessage('will add {0} | will add {1}')
              .withValues([
                field._pricing.price_if_checked_display,
                field._pricing.rec_price_if_checked_display,
              ]);
      ...
      function preparePriceMap(field) {
        ...
        for (var i = 0; i < field.choices.length; i++) {
          ...
          var adjustedRecurringPrice =
            selectedRecurringPrice - choiceRecurringPrice;
          var adjustedPriceAbs = Math.abs(selectedPrice - choicePrice);
          var adjustedRecurringPriceAbs = Math.abs(
            selectedRecurringPrice - choiceRecurringPrice
          );
      ...
      function getPriceLabelArray(
        ...
        for (var i = 0; i < field.choices.length; i++) {
          ...
          if (
            adjustedPrice != 0 &&
            adjustedRecurringPrice != 0 &&
            recurringPriceFreq
          ) {
            formattedValues = [
              formattedPriceMap[key1] ? formattedPriceMap[key1] : '',
              formattedPriceMap[key2] ? formattedPriceMap[key2] : '',
            ];
            if (adjustedPrice > 0 && adjustedRecurringPrice > 0)
              message = i18n
                .getMessage('subtract {0} | subtract {1}')
                .withValues(formattedValues);
            else if (adjustedPrice > 0 && adjustedRecurringPrice <= 0)
              message = i18n
                .getMessage('subtract {0} | add {1}')
                .withValues(formattedValues);
            else if (adjustedPrice <= 0 && adjustedRecurringPrice > 0)
              message = i18n
                .getMessage('add {0} | subtract {1}')
                .withValues(formattedValues);
            else
              message = i18n
                .getMessage('add {0} | add {1}')
                .withValues(formattedValues);
            ...
            label +=
              adjustedPrice > 0
                ? i18n.getMessage('subtract {0}').withValues(formattedValues)
                : i18n.getMessage('add {0}').withValues(formattedValues);
      ...
      function getPriceLabelForChoices(field, recurringPriceFreq) {
        return $http
          .post(spUtil.getURL('format_prices'), preparePriceMap(field))
      ...
      function getPriceLabelsForChoiceFields(fields, recurringPriceFreq) {
        return $http
          ...
          .then(function (data) {
            fields.forEach(function (field) {
              if (field.type == 'choice' || field.type == 'multiple_choice') {
                var labelArray = getPriceLabelArray(
                  field,
                  recurringPriceFreq,
                  data
                );
