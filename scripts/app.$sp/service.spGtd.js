...
angular
  .module('sn.$sp')
  .factory('spGtd', function ($q, $http, $rootScope, spUtil, i18n) {
    'use strict';
    ...
    function isAdditonalParamsMatched(context, url) {
      ...
      for (var i = 0; i < additionalParams.length; i++) {
        ...
        var urlParm =
          param && param.name ? getUrlParameterWithName(url, param.name) : '';
        if ((urlParm && urlParm.value) !== param.value) return false;
      }
      return true;
    }
    function getToursForPage(options) {
      ...
      if (sessionStorage) {
        state =
          sessionStorage.getItem('guided_tour:tour.state') ||
          location.href.indexOf('gtd_preview_tour_id') >= 0;
        if (state) {
        ...
        _getTourData(options.portal.sys_id, options.page.sys_id, context).then(
          function (data) {
            ...
            data = data.filter(function (t) {
              if (!isAdditonalParamsMatched(t.context, url)) return false;
              ...
              if (filterByPageId) {
                ...
                if (t.options) {
                  try {
                    ...
                    if (
                      tourOptions.isMapped &&
                      tourOptions.actualContext === contt.value
                    ) {
                      return true;
                    }
                  } catch (e) {
    ...
    function _startTour(sysId, stepNum, cb) {
      _trigger(webAEventStream, {
        payload: sysId,
        event: guidedToursAPIEvents.TOUR_STARTED,
      });
      if (!top.NOW.guided_tours._tourService) return;
    ...
    function _endTour() {
      ...
      if (!_tourService) return;
      var sys_id =
        _tourService &&
        _tourService.currentTour &&
        _tourService.currentTour.sys_id;
      _trigger(webAEventStream, {
        payload: sys_id,
        event: guidedToursAPIEvents.TOUR_ENDED,
      });
    ...
    function _getAllTours(cb) {
      ...
      _trigger(webAEventStream, {
        payload: '',
        event: guidedToursAPIEvents.GET_ALL_TOURS,
      });
      var toursList = [];
      var filterFunc;
      getToursForPage({
        ...
        function (data) {
          ...
          try {
            ...
            if (filterFunc) toursList = toursList.filter(filterFunc);
    ...
    function _on(event, cbFunc) {
      if (CustomEvent) CustomEvent.on(event, cbFunc);
    }
    function _off(event, func) {
      if (CustomEvent)
        if (!func || typeof func !== 'function') CustomEvent.unAll(event);
        else CustomEvent.un(event, func);
    }
    ...
    function _insertPlayerScript() {
      ...
      script.src =
        '/scripts/app.guided_tours/guided_tours_player.js?v=' + g_builddate;
