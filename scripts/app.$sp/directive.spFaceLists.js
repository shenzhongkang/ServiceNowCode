...
angular.module('sn.$sp').directive('spFacetLists', function (spFacetManager) {
  return {
    ...
    controller: function ($scope) {
      ...
      spFacetManager.subscribe(
        'spSearchSources.toggle.data',
        'list',
      ...
      c.isCollapsed = function (searchSource, facetId) {
        return c.facetToogleMap[searchSource] !== undefined &&
          c.facetToogleMap[searchSource][facetId] !== undefined
          ? c.facetToogleMap[searchSource][menuId]
          : $scope.collapsed;
      };
      c.isSearchSourceCollapsed = function (searchSource) {
        return c.searchSourceToggleMap[searchSource] !== undefined
          ? c.searchSourceToggleMap[searchSource]
          : $scope.collapsedSearchsource;
      };
      function isEmpty(facetsObjectMap) {
        ...
        for (var searchSource in facetsObjectMap) {
          if (
            facetsObjectMap[searchSource] &&
            facetsObjectMap[searchSource].length > 0
          ) {
            return false;
          }
        }
      ...
      function updateFacet(queries) {
        ...
        keys.forEach(function (k) {
          ...
          var values = [];
          if (
            queryContent.charAt(0) === '[' &&
            queryContent.charAt(queryContent.length - 1) === ']'
          ) {
