...
angular
  .module('sn.$sp')
  .directive('spCatItem', function ($http, $rootScope, i18n, spModal, spUtil) {
    return {
      restrict: 'E',
      ...
      controller: function ($scope) {
        ...
        c.priceHasChanged = function (p, parms) {
          if (parms.force_update) return true;
          var changed = false;
        ...
        c.formatPrice = function (data) {
          ...
          t.recurring_price_subtotal_display =
            response.recurring_price_subtotal_display;
        };
        var g_form;
        $scope.$on('spModel.gForm.initialized', function (e, gFormInstance) {
          if (gFormInstance.getSysId() != c.getItemId()) return;
          g_form = gFormInstance;
          var scMandatoryAttachMessage = i18n.getMessage(
            'Attachment(s) are not added'
          );
          var scAttachmentDeletionMessage = i18n.getMessage(
            'Please wait, attachment deletion in progress'
          );
          g_form.$private.events.on('onSubmit', function () {
            if ($scope.item.attachment_action_in_progress) {
              var _response = spUtil.addErrorMessage(
                scAttachmentDeletionMessage
              );
              var handledMessage = _response !== false;
              if (!handledMessage) spModal.alert(scAttachmentDeletionMessage);
              return false;
            }
            if ($scope.item.mandatory_attachment) {
              if (
                !$scope.item.attachment_submitted &&
                !($scope.attachments && $scope.attachments.length > 0)
              ) {
                var _response = spUtil.addErrorMessage(
                  scMandatoryAttachMessage
                );
                var handledMessage = _response !== false;
                if (!handledMessage) spModal.alert(scMandatoryAttachMessage);
                return false;
              }
            }
            return true;
        ...
        function getCalculatedPrices() {
          ...
          $http
            .post(spUtil.getURL('update_price'), o, config)
            .success(c.formatPrice);
        }
