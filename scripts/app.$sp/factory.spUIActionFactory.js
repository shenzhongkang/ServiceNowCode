...
angular
  .module('sn.$sp')
  .factory('spUIActionFactory', function ($http, $rootScope) {
    'use strict';
    var _er;
    return {
    ...
    function GlideUIActions(uiActions, options) {
      ...
      this.getActionByName = function (name) {
        ...
        if (!foundAction) foundAction = this.getAction(name);
        return foundAction;
      };
    }
    function GlideUIAction(
      name,
      sysId,
      displayName,
      uiActionNotifier,
      ...
      this.execute = function (g_form) {
        ...
        if (attachmentGUID) formData._attachmentGUID = attachmentGUID;
    ...
    function executeDesktopUIAction(
      actionSysId,
      tableName,
      recordSysId,
      formData
