...
function formatDate(date, format) {
  if (format.indexOf('z') > 0) return formatDateServer(date, format);
  format = format + '';
  var result = '';
  var i_format = 0;
  var c = '';
...
Date.prototype.getWeek = function () {
  ...
  if (day < 4) {
    ...
    if (weekNum > 52) weekNum = this._checkNextYear(weekNum);
    return weekNum;
  }
  weekNum = Math.floor((dayNum + day - 1) / 7);
  if (weekNum < 1) weekNum = this._lastWeekOfYear();
  else if (weekNum > 52) weekNum = this._checkNextYear(weekNum);
  return weekNum;
};
Date.prototype._lastWeekOfYear = function () {
  ...
  var dayNum =
    Math.floor(
      (endOfYear.getTime() -
        newYear.getTime() -
        (endOfYear.getTimezoneOffset() - newYear.getTimezoneOffset()) * 60000) /
        86400000
    ) + 1;
  return day < 4
    ? Math.floor((dayNum + day - 1) / 7) + 1
    : Math.floor((dayNum + day - 1) / 7);
};
    ...
    if (token == 'yyyy' || token == 'yy' || token == 'y') {
      ...
      for (var i = 0; i < DAY_NAMES.length; i++) {
        ...
        if (
          ...
          if (week) {
            if (i == 0 || i == 7) date += 6;
            else if (i == 2 || i == 9) date += 1;
            else if (i == 3 || i == 10) date += 2;
            else if (i == 4 || i == 11) date += 3;
            else if (i == 5 || i == 12) date += 4;
            else if (i == 6 || i == 13) date += 5;
          }
