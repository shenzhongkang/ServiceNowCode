...
var GwtListEditTablename = Class.create(GwtListEditSelect, {
  addOptions: function () {
    ...
    this.editor.selectedOnly =
      this.editor.tableElement.getNamedAttribute('selected_only') == 'true';
    ga.addParam('sysparm_name', this.getName());
    ga.addParam('sysparm_noViews', this.getNoViews());
    ga.addParam(
      'sysparm_noSystemTables',
      this.editor.tableElement.getNamedAttribute('no_system_tables') == 'true'
    );
    ...
    if (this.editor.selectedOnly) {
      ...
      ga.addParam(
        'sysparm_forceSelected',
        this.editor.tableElement.getNamedAttribute('force_selected') == 'true'
      );
    }
  ...
  _getSelected: function () {
    ...
    var selectedOnlyOverride = this.editor.tableElement.getNamedAttribute(
      'selected_only_list_override'
    );
    if (
      (selected == null || selected.length == 0) &&
      selectedOnlyOverride != null
    ) {
      if (selectedOnlyOverride.indexOf('javascript:') == 0) {
        var script = selectedOnlyOverride.substring(
          11,
          selectedOnlyOverride.length
        );
        var response;
        try {
        ...
        if (typeof response == 'undefined') {
          ...
          return response == null || response.length == 0
            ? 'empty_c460fe18-371f-4315-a61b-c7bf21c82786'
            : response;
        }
      } else {
