...
var OpticsInspector = Class.create({
  CATEGORIES: {
  ...
  pushOpticsContext: function (category, name, sys_id, sourceTable) {
    if (category == 'sys_script_client') name = '"' + name + '"';
    var context = {
      category: category,
      name: name,
      sys_id: sys_id,
  ...
  popOpticsContext: function () {
    ...
    if (typeof g_form !== 'undefined' && g_form.actionStack) {
      ...
      if (g_form._pushAction) g_form._pushAction(context);
    }
    if (
      this.isInspecting() &&
      this.opticsContextStack.length > 0 &&
      context &&
      context.category !== 'container_action'
    )
      return this.opticsContextStack.pop();
    return null;
  },
  isInspecting: function (tableName, fieldName) {
    if (this.tableName == null && this.fieldName == null) return false;
    ...
    if (arguments.length == 2)
      return tableName == this.tableName && fieldName == this.fieldName;
    return false;
  },
  ...
  addWatchIcon: function (watchField) {
    ...
    if (!td) {
      ...
      if (
        fieldParts.length == 2 &&
        fieldParts[0].length > 0 &&
        fieldParts[1].length > 0
      ) {
    ...
    if (td) {
      if (isDoctype()) {
        ...
        icon =
          '<span class="label-icon icon-debug watch_icon" id="' +
          watchField +
          '.watch_icon"' +
        ...
        if (fieldParts.length === 2 && fieldParts[1].startsWith('IO:')) {
  ...
  clearWatchField: function (watchfield) {
    ...
    if (debuggerTools && debuggerTools.isDebugPanelVisible()) {
      ...
      if (wndw.updateFieldInfo) wndw.updateFieldInfo(null);
    } else {
  ...
  setWatchField: function (watchField) {
    ...
    if (debuggerTools && debuggerTools.isDebugPanelVisible()) {
      var wndw = debuggerTools.getJsDebugWindow();
      if (wndw.updateFieldInfo) wndw.updateFieldInfo(watchField);
    } else {
      debuggerTools = parent.parent.debuggerTools;
      if (debuggerTools && debuggerTools.isDebugPanelVisible()) {
        var wndw = debuggerTools.getJsDebugWindow();
  ...
  processClientMessage: function (notification) {
    var opticsContext =
      this.opticsContextStack[this.opticsContextStack.length - 1];
    if (!opticsContext) {
  ...
  processServerMessage: function (notification) {
    if (notification.getAttribute('processed') == 'true') return;
    var info = {
      type: 'SERVER',
      ...
      call_trace: this._getCallTrace(
        eval(notification.getAttribute('call_trace'))
      ),
    };
    this.process(info);
  },
  ...
  openScriptWindow: function (tablename, sysid) {
    if (tablename && sysid) {
      if (tablename == 'request_action') tablename = 'sys_ui_action';
  ...
  _log: function (msg) {
    ...
    if (debuggerTools && debuggerTools.isDebugPanelVisible()) {
      var wndw = debuggerTools.getJsDebugWindow();
      if (wndw.insertJsDebugMsg) wndw.insertJsDebugMsg(msg);
    } else {
  ...
  _getCallTrace: function (contextStack) {
    var trace = '';
    var arrows =
      '<span class="rtl-arrow"> &larr;</span><span class="lrt-arrow">&rarr; </span>';
    var space = arrows;
    for (i = 0, maxi = contextStack.length; i < maxi; i++) {
      var context = contextStack[i];
      if (i > 0) space = arrows + space;
  ...
  _getMessage: function (notification) {
    ...
    var legend_title =
      notif_type.indexOf('CLIENT') > -1
        ? 'Client-side activity'
        : 'Server-side activity';
    var msg =
      '<span class="expand-button" onclick="toggleCallTrace(this);">&nbsp;</span>';
    msg +=
      '<img class="infoIcon" height="16"  width="16" border="0" src="images/info-icon.png" title="' +
      legend_title +
      '" alt="' +
      legend_title +
      '">';
    msg +=
      ...
      '</span>';
    msg +=
      '<span class="log-category">' + this.CATEGORIES[notification['category']];
    if (notification['name'] && notification['name'].length > 0) {
      if (notification['sys_id'])
        msg +=
          ...
          notification['name'] +
          '</a></span>';
      else msg += '&nbsp;-&nbsp;' + notification['name'] + '</span>';
    } else msg += '</span>';
    msg += '<span class="log-value">';
    if ('request_action' === notification['category']) {
      msg +=
        'Value received from client is: <span class="value" title="Value">' +
  ...
  _getCategoryName: function (category) {
    ...
    if (name === 'undefined' || name === null) name = category;
    return name;
  },
...
function getClientScriptContextName(name, type) {
  var csname = null;
  if (type === 'submit') csname = g_event_handlers_onSubmit[name];
  else if (type === 'load') csname = g_event_handlers_onLoad[name];
  else if (type === 'change') csname = g_event_handlers_onChange[name];
  return csname;
}
CustomEvent.observe(
  OpticsInspector.PUT_CONTEXT,
  function (category, name, sys_id, sourceTable) {
    g_optics_inspect_handler.pushOpticsContext(
      category,
      name,
      sys_id,
      sourceTable
...
CustomEvent.observe(OpticsInspector.POP_CS_CONTEXT, function (name, type) {
  var csname = getClientScriptContextName(name, type);
  if (csname) g_optics_inspect_handler.popOpticsContext();
});
CustomEvent.observe(OpticsInspector.LOG_MESSAGE, function (notification) {
  if (
    g_optics_inspect_handler.isInspecting(
      notification['table'],
      notification['field']
    )
  ) {
