...
SN.formAnnotations = {
  ...
  toggle: function () {
    if (SN.formAnnotations.preference) SN.formAnnotations.hide();
    else SN.formAnnotations.show();
  ...
  init: function () {
    var selectors = [
      '.annotation-row[data-annotation-type="Info Box Blue"]',
    ...
    if (SN.formAnnotations.annotations.length) {
      ...
      if (!SN.formAnnotations.preference && SN.formAnnotations.infoPreference) {
        ...
        $j('#info_toggle_annotations')
          .show()
          .click(SN.formAnnotations.toggleFromInfoMsg);
      }
    } else {
      $annotationButton.prop('disabled', true);
      var $annotationTooltip = $annotationButton.closest('.annotation-tooltip');
      $annotationTooltip.attr(
        'title',
        $annotationTooltip.attr('data-title-disabled')
      );
    }
    if (SN.formAnnotations.preference) SN.formAnnotations.show();
  },
  ...
  addHiddenAnnotationMessage: function () {
    ...
    msg +=
      ' <span id="info_toggle_annotations" tabindex="0" class="icon-button icon-help sn-cloak" title="Toggle annotations on / off" style="display: inline-block;color:#678;cursor:pointer;font-size:1.4em"></span> ';
    msg += getMessage('to toggle them');
    msg +=
      ' - (<span style="text-decoration:underline;cursor:pointer;" onclick="SN.formAnnotations.setInfoPref()">';
