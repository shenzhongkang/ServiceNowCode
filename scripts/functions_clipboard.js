...
(function (exports) {
  ...
  exports.copyToClipboard = function (str, messageMethod) {
    if (document.execCommand && isCapableMessageMethod(messageMethod)) {
      ...
      try {
        ...
        result = false;
        if (window.jslog) jslog("Couldn't access clipboard " + e);
      } finally {
  ...
  function isCapableMessageMethod(messageMethod) {
    if (messageMethod == 'custom') return true;
  ...
  function legacyClipboardCopy(meintext) {
    prompt(
      'Because of a browser limitation the URL can not be placed directly in the clipboard. ' +
        'Please use Ctrl-C to copy the data and escape to dismiss this dialog',
      meintext
    );
  }
})(window.NOW.g_clipboard);
