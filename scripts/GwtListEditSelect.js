...
var GwtListEditSelect = Class.create(GwtListEditWindow, {
  createEditControls: function () {
    ...
    if (this.doctype)
      select =
        ...
        "' class='form-control list-edit-input' />";
    else select = "<select id = '" + GwtListEditWindow.inputID + "' />";
    this.setTitle(select);
    this.focusElement = GwtListEditWindow.getCellEditValue();
    this.addOptions();
    if (!this.staticOptions) return false;
  },
  addOptions: function () {
    ...
    ajax.addParam(
      'sysparm_target',
      this.editor.tableElement.getTable().getName()
    );
    ...
    if (this.editor.tableElement.choice != '3') {
      ...
      if (this.nullOverride)
        addOption(
          ...
          '',
          this.nullOverride,
          this._isSelected(value, '', this.nullOverride)
        );
  ...
  _checkDuplicateOptions: function (value, label) {
    if (!this.focusElement.length) return false;
  ...
  _isSelected: function (value, v, l) {
    if (l == value || v == value) return true;
    else return false;
  },
  save: function () {
