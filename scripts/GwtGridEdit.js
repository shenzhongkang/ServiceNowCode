...
var GwtGridEdit = Class.create({
  ...
  updateTable: function () {
    ...
    this.tableController.observe(
      'glide:list_v2.edit.saves_completed',
      this._handleSavesCompleted.bind(this)
    );
    this.tableController.observe(
      'glide:list_v2.edit.row_added',
      this._handleRowAdded.bind(this)
    );
  ...
  getAnchorAttribute: function (attribute) {
    return this.tableController.getAttributeByCell(
      attribute,
      this.getAnchorCell()
    );
  },
  ...
  keyDown: function (e) {
    ...
    switch (e.keyCode) {
      case Event.KEY_DOWN:
        e.stop();
        if (e.shiftKey) this.selectVerticalKey(e, 'down');
        else this.downArrow(e);
        break;
      case Event.KEY_UP:
        e.stop();
        if (e.shiftKey) this.selectVerticalKey(e, 'up');
        else this.upArrow(e);
        break;
      ...
      case Event.KEY_RETURN:
        ...
        if (this.tableController.isHierarchical()) e.stop();
        this.editCursor();
        break;
      case Event.KEY_ESC:
        this.clearCursor();
  ...
  editCursor: function () {
    ...
    if (cursor) GwtListEditor.forPage.edit(cursor);
  },
  editNextRow: function () {
    if (this.tableController.insertRow && this.rec >= this.cellSelector.maxRow)
      this.editOnInsert = true;
    else this.editOnInsert = false;
    this.downArrow();
    if (!this.editOnInsert) this.editCursor();
  },
  selectVerticalKey: function (evt, direction) {
    this.selectVertical(evt, direction, this.rec, this.col);
    if ('down' === direction) this._fireMove(this.cellSelector.rowTo, this.col);
    if ('up' === direction) this._fireMove(this.cellSelector.rowFrom, this.col);
  },
  downArrow: function () {
    this.clearSelected();
  ...
  moveRight: function () {
    ...
    var getIcons = function (_this) {
      return _this.tableController
        .getCellByNdx(_this.rec, 0)
        .getElementsByTagName('i');
    };
    if (
      this.col + 2 >= this._getMaxCol() ||
      hasClassName(this.getRowByPos(this.rec), 'list_row_detail')
    ) {
      var rec = this.tableController.getNextRowByPos(this.rec);
      if (!rec) {
          ...
          if (!icons[i].hasClassName('disabled')) {
            ...
            if (!icons[i].getAttribute('aria-label'))
              icons[i].setAttribute(
                'aria-label',
                icons[i].getAttribute('data-original-title') ||
                  icons[i].getAttribute('title')
              );
            icons[i].focus();
            return true;
          }
      ...
      if (
        !this.tableController.getPrevRowByPos(this.rec) &&
        this.iconIndex == null
      ) {
        this.col = 0;
        this._updateCursorCell();
  ...
  tabKey: function (e) {
    ...
    if (e.shiftKey) return this.moveLeft();
  ...
  setCursorElement: function (element) {
    ...
    if (hasClassName(row, 'list_row_detail')) this.col = 1;
    this._updateCursorCell();
  ...
  _makeVisible: function (e) {
    ...
    if (
      eVP.left > 44 &&
      eViewOffset.right < vp.width &&
      eVP.top > 24 &&
      eViewOffset.bottom < vp.height
    )
      return;
    var parent = getFormContentParent();
    if (isDoctype())
    ...
    if (e.getWidth() >= vp.width) parent.scrollLeft = eViewOffset.left;
    else if (eVP.left < 44) parent.scrollLeft = eViewOffset.left - 44;
    else if (eViewOffset.right > parent.scrollLeft + vp.width)
      parent.scrollLeft = eViewOffset.right - vp.width + relatedListOffset;
    if (isDoctype() && this.col == 2) parent.scrollLeft = 0;
    if (e.getHeight() >= vp.height) parent.scrollTop = eViewOffset.top;
    else if (eVP.top < vp.top + 24) parent.scrollTop = eViewOffset.top - 24;
  ...
  _headerOffsetTop: function (parent) {
    if (parent == document.body) return 0;
    var hdrDiv = $$('.section_header_div_no_scroll');
    var top = 0;
    if (hdrDiv && hdrDiv.length > 0) top = hdrDiv[0].getHeight();
    return top;
  },
  ...
  draw: function () {
    if (this.rec != 0 || this.col != 0) this.showCursor();
  },
  _resize: function () {
    if (GwtListEditor.forPage.cursor.isHidden()) return;
    this.showCursor();
  },
  ...
  _handleSavesCompleted: function (evt) {
    if (evt.memo.listId !== this.tableController.listID) return;
    this.draw();
  },
  ...
  getSelectedRows: function () {
    ...
    for (i = 0; i < records.length; i++) {
      ...
      if (row && result.indexOf(row) < 0) result.push(row);
    }
    return result;
  },
  getSelectedSysIds: function () {
    ...
    for (i = 0; i < records.length; i++) {
      ...
      if (sysId && result.indexOf(sysId) < 0) result.push(sysId);
    }
    return result;
  },
  getSelectedAttributes: function (sysIDs, attribute) {
    var listEd = GwtListEditor.forPage.getListEditor(
      this.tableController.listID
    );
