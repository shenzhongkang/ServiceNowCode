...
function popupClose(callback) {
  ...
  try {
    if (!popupCurrent.closed) popupCurrent.close();
  } catch (e) {}
  popupCurrent = null;
...
var g_IconBrowseHelpers = {
  ...
  getIcon: function () {
    var spanElem = jQuery('span.bookmark_image');
    if (spanElem.length) {
      var fontIcon = jQuery.grep(
        spanElem.prop('class').split(' '),
        function (v) {
...
function imageListPick(elementName, value) {
  ...
  if (element.nodeName == 'IMG' || element.nodeName == 'INPUT') {
    ...
    if (imgel != null) {
      if (value && value.length > 0) imgel.src = value;
      else imgel.src = 'images/s.gifx';
    }
  } else {
    element.className = '';
...
function getFormView(elementName, targetElement) {
  ...
  if (typeof g_form !== 'undefined') return g_form.getViewName();
  if (!targetElement || typeof GlideList2 === 'undefined') return '';
  var parentList = targetElement.split('.')[0];
  var list = GlideList2.get(parentList);
  return list ? list.getView() : '';
}
function reflistOpen(
  target,
  elementName,
  refTableName,
  dependent,
  ...
  var urlLengthLimit =
    (window.g_tiny_url && window.g_tiny_url.min_length) || urlMaxLength;
...
function reflistOpenUrl(
  ...
  var url;
  if (useQBE == 'true') url = new GlideURL(refTableName + '_search.do');
  else url = new GlideURL(refTableName + '_list.do');
  url.addParam('sysparm_target', target);
  var et = gel(targetElementID);
  if (et && !ignoreTargetValue) url.addParam('sysparm_target_value', et.value);
  var dspEl = gel('sys_display.' + targetElementID);
  if (dspEl && !et.value) url.addParam('sysparm_reference_value', dspEl.value);
  ...
  if (forceReference) url.addParam('sysparm_force_ref', 'true');
  var v = getDependentValue(target, dependent);
  if (v != null) url.addParam('sysparm_dependent', v);
...
function showPopupLiveFeedList(sysId, table) {
  ...
  if (box) box.close(0);
  var iframe_src =
    'show_live_feed.do?sysparm_stack=no&sysparm_sys_id=' +
    sysId +
    '&sysparm_table=' +
    table;
  box = new GlideBox({
...
function reflistPick(elementName, value, display) {
  ...
  if (element && element.onRefPick) element.onRefPick(element);
  if (
    element &&
    element.ac &&
    typeof element.ac.referenceSelect == 'function'
  ) {
    popupClose(function () {
  ...
  if (typeof elementActual.onchange == 'function') elementActual.onchange();
  ...
  if (element != null) {
    ...
    var fcs =
      "updateRelatedGivenNameAndValue('" + elementName + "','" + value + "');";
    setTimeout(fcs, 0);
  }
  popupClose();
  return false;
}
function picklistOpen(
  baseURL,
  width,
  modified,
  searchParam,
...
function getDependentValue(target, dependent) {
  if (!dependent || dependent == 'null') return null;
  ...
  if (el == null)
    return window.NOW.useHiddenDependent
      ? $('sys_display.' + target).getAttribute('data-dependent-value')
      : null;
  var selectValue = '';
  if (el.tagName == 'INPUT') var selectValue = el.value;
  else selectValue = el.options[el.selectedIndex].value;
  return selectValue;
}
function getRefQualURI(target, refQualElements) {
  if (!refQualElements || typeof g_form == 'undefined') return '';
...
function popupOpenStandard(url, name) {
  var width =
    document.documentElement.getAttribute('data-doctype') == 'true' ? 800 : 700;
  var height = 480;
  var features =
    'width=' +
    width +
    ',height=' +
    height +
    ',toolbar=no,status=no,directories=no,menubar=no,resizable=yes,scrollbars=1';
...
function popupOpenFocus(
  ...
  if (domain) url += '&sysparm_domain=' + domain.value;
  if (url.indexOf('sysparm_nameofstack') == -1) url += '&sysparm_stack=no';
...
function tearOff(table, sys_id, view, navigate, refKey) {
  ...
  if (refKey) url.addParam('sysparm_refkey', refKey);
  window.open(
    url.getURL(),
    '',
