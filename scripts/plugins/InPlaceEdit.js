...
Plugin.create('inPlaceEdit', {
  initialize: function (elem, options) {
    if (elem.retrieve('inPlaceEdit')) return;
    ...
    if (!this.options.turnClickEditingOff) {
      if (this.options.editOnSingleClick) mouseEvent = 'click';
      else mouseEvent = 'dblclick';
  ...
  beginEdit: function (event) {
    ...
    if (typeof event != 'undefined' && event) Event.stop(event);
  },
