...
var GwtListEditRecord = Class.create({
  initialize: function (sysId) {
    this.sysId = sysId;
    if (sysId == '-1') this.operation = 'default_values';
  ...
  isDeleted: function () {
    if ('delete_pending' === this.operation) return true;
    return 'delete' === this.operation;
  },
  clearModified: function () {
    for (var n in this.fields) this.fields[n].clearModified();
  },
...
var GwtListEditField = Class.create({
  ...
  setValue: function (v) {
    if (this.value != v) this.modified = true;
    this.value = v;
    this.valueSet = true;
  ...
  setDisplayValue: function (v) {
    if (this.displayValue != v) this.modified = true;
    this.displayValue = v;
  ...
  _convertNullsToBlank: function () {
    ...
    if (!this.renderValue) this.renderValue = '';
  },
