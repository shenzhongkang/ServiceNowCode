...
function onChange(elementName, fromSetTemplateValue) {
  ...
  var eChanged, eOriginal, eDisplay, isMappingField = ifFieldHasElementMappingPrefix(elementName);
  if (isMappingField) {
...
function onChangeLabelProcessByEl(elementNode, statusLabel, value) {
  ...
  if (typeof $j !== 'undefined') {
      ...
      if (tooltipButton && tooltipButton.length) {
          ...
          var fullTooltipLabel = (newTitle && baseTooltipMessage) ? newTitle + '. ' + baseTooltipMessage : (newTitle || baseTooltipMessage);
          if (fullTooltipLabel) {
              tooltipButton.attr('alt', fullTooltipLabel).attr('data-dynamic-title', fullTooltipLabel).show();
...
function callChangeHandler(handler, control, theWidget, oldVal, newVal, loading, template) {
  try {
      ...
      if (g_user.hasRole('client_script_admin')) {
          ...
          g_form.showFieldMsg(theWidget, "Script error encountered when changing this field - please contact your System Administrator", "error", false);
      }
