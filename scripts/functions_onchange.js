...
function onChange(elementName, fromSetTemplateValue) {
  ...
  if (niElement && niElement.type === 'checkbox') {
    if (vChanged === '') vChanged = 'false';
  }
  if (isMappingField && !isFilterField(eChanged)) {
    eChanged.changed = vDisplay != vOriginal;
  } else {
    if (!vChanged) eChanged.value = '';
    eChanged.changed = vOriginal != vChanged;
  }
  if (
    !elementName.startsWith('IO:') &&
    !elementName.startsWith('ni.VE') &&
    !elementName.startsWith('ni.QS')
  ) {
    onChangeLabelProcess(elementName);
  }
  setMandatoryExplained();
  onSelChangeAndClientScriptOnChange(
    elementName,
    eChanged,
    vOriginal,
    vChanged,
  ...
  if (
    ...
    g_form.triggerOnUserChangeValue
  )
...
function onChangeLabelProcessByEl(elementNode, statusLabel, value) {
  if (!elementNode || !statusLabel) return;
  ...
  if (value == undefined) value = elementNode.value;
  if (mandatory == 'true') {
    var currencySuffix = ';0.00';
    if (
      typeof value != 'undefined' &&
      (value == '' ||
        value.indexOf(currencySuffix, value.length - currencySuffix.length) !=
          -1)
    ) {
  ...
  if (statusLabel.className == newClassName) return;
  ...
  if (
    previousMandatory &&
    statusLabel.className.indexOf('required-marker') == -1 &&
    parentElement &&
    parentElement.className.indexOf('is-required') > -1
  )
    gel('element.' + elementNode.id).className =
      parentElement.className.replace('is-required', '');
  ...
  if (typeof $j !== 'undefined') {
    ...
    if (tooltipButton && tooltipButton.length) {
      ...
      var mandatoryTitle = getMessage(
        'Mandatory - must be populated before Submit'
      );
      if (baseTooltipMessage.indexOf(mandatoryTitle) !== -1) {
        baseTooltipMessage = baseTooltipMessage.replace(
          mandatoryTitle + '. ',
          ''
        );
        tooltipButton.attr('label-title', baseTooltipMessage);
      }
      var fullTooltipLabel =
      ...
      } else tooltipButton.hide();
    } else statusLabel.setAttribute('data-dynamic-title', newTitle);
  }
...
function onSelChangeAndClientScriptOnChange(
  elementName,
  eChanged,
  vOriginal,
  vChanged,
  ...
  if (typeof g_form != 'undefined') g_form.hideFieldMsg(fieldName, true);
  ...
  if (elementId) onSelChange(elementId, fromSetTemplateValue);
  callChangeHandlers(
    tableName,
    fieldName,
    eChanged,
...
function callChangeHandlers(
  ...
  for (var i = 0; i < g_event_handlers.length; i++) {
    ...
    if (!!handler.isUiPolicy !== isUiPolicy) continue;
...
function fireAllChangeHandlers() {
  for (var x = 0; x < g_event_handlers.length; x++) {
    ...
    var oldVal = 'unknown';
    if (original) oldVal = original.value;
    var newVal;
    if ($(theWidget).getAttribute('type') == 'radio') {
      ...
      var elems = $$('#' + $(theWidget).getAttribute('id')).each(function (el) {
        ...
        if (checkedValue != null && checkedValue.length > 0) newVal = el.value;
      });
    } else newVal = theWidget.value;
...
function callChangeHandler0(
  ...
  CustomEvent.fire(
    'glide_optics_inspect_put_cs_context',
    handler.handlerName,
    'change'
  );
  ...
  CustomEvent.fire(
    'glide_optics_inspect_pop_cs_context',
    handler.handlerName,
    'change'
  );
}
function multiKeyDown(me) {
  if ($(me.id).getAttribute('isquestionhtml') == 'true') return;
  var eOriginal = 'g_' + me.id.replace(/\./g, '_');
  var eOriginalSet =
    eval('typeof ' + eOriginal + " != 'undefined'") && eOriginal !== 'g_slush';
  if (eOriginalSet) return;
...
function multiModified(me, type, currentValue) {
  ...
  if (me.id && form) {
    ...
    if (type == undefined) type = 'htmlarea';
    if (currentValue == vOriginal) changeFlag = false;
    me.changed = changeFlag;
    onChangeLabelProcess(elementName, currentValue);
    if (
      type == 'tinymce' ||
      (type.baseURL && type.baseURL.indexOf('tinymce') > -1)
    ) {
      onSelChangeAndClientScriptOnChange(
        elementName,
        me,
...
function addOnChangeEvent(fields, tableName, callfunc) {
  for (var i = 0; i < fields.length; i++) {
    ...
    if (typeof field == 'string') {
      if (tableName) field = tableName + '.' + field;
      field = gel(field);
    }
    if (field && field.tagName) Event.observe(field, 'change', callfunc);
  }
}
function setColorSwatch(fieldName) {
  ...
  try {
    ...
    g_form.showErrorBox(
      fieldName,
      getMessage('Invalid color') + ':' + colorValue
    );
    $(fieldName).value = '';
