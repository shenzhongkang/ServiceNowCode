...
function runFilter0(name, listId) {
  ...
  if (!runFilterHandlers[name]) return;
...
function saveFilterRadioChange() {
  ...
  if (getGroupSaveOption() == grp) div.style.display = 'inline';
...
function getFilterVisibility() {
  ...
  var grp = getMessage('Group');
  if (vis == me) return vis;
  if (vis == eone) return 'GLOBAL';
  if (vis != grp) return me;
  var vis = '';
...
function getGroupSaveOption() {
  var rb = gel('MeRadio');
  if (rb && rb.checked) return rb.value;
