...
(function ($) {
  ...
  $(function () {
    ...
    function hideOpenPopovers() {
      $('.sn-popover-basic').each(function () {
        var $this = $(this);
        if ($this.attr('aria-describedby') !== undefined) $this.popover('hide');
      });
    }
    ...
    function debounce(fn, threshold, fireOnStart) {
      ...
      return function () {
        ...
        function delayed() {
          if (!fireOnStart) fn.apply(obj, args);
          timeout = null;
        }
        if (timeout) clearTimeout(timeout);
        else if (fireOnStart) fn.apply(obj, args);
    ...
    function closeOnBlur(e) {
      ...
      $('.sn-popover-basic').each(function () {
        ...
        if (!$popoverButton.hasClass('active')) return;
        if (e.target.closest('#tag_form')) return;
        if (
          eventTargetInElement($popoverButton) ||
          eventTargetInElement($popoverContent)
        )
          return;
        if (
          $popoverButton.data('auto-close') === false &&
          !$(e.target).is('.sn-popover-basic')
        )
          return;
