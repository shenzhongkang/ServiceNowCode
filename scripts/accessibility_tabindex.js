...
addLoadEvent(function () {
  $(document).on(
    'keydown',
    '*[tabindex]:not(.multiLinePill), .glide_ref_item_link',
    function (event) {
      ...
      if (element.click) {
        ...
        if ($parent.hasClass('open')) {
          ...
          if ($items.length) $items.eq(0).trigger('focus');
        }
      }
      event.stop();
    }
  ...
  if (typeof jQuery != 'undefined') {
    ...
    jQuery('[click-on-enter],[click-on-space]').on('keydown', function (event) {
      ...
      if (
        !(
          (keyCode === KEY_ENTER && $this.is('[click-on-enter]')) ||
          (keyCode === KEY_SPACE && $this.is('[click-on-space]'))
        )
      )
        return;
      setTimeout(function () {
